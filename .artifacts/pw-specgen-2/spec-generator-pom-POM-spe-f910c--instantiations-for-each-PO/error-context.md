...
```ts
  ...
  95  |     name: "FormPage", className: "FormPage", screenSignature: "sig:form", confidence: 0.8, sourcePlanId: "plan_1",
  96  |     methods: [{ name: "fillField", intent: "fill_form_field", parameters: ["fieldName", "value"] }]
  97  |   });
...
> 113 |   expect(result.pomStatus).toBe("promoted");
  ...
  115 |   expect(result.specContent).toContain("import { FormPage } from");
  116 |   expect(result.specContent).toContain("const categoryPage = new CategoryPage(page);");
  117 |   expect(result.specContent).toContain("const formPage = new FormPage(page);");
  118 | });
  119 | 

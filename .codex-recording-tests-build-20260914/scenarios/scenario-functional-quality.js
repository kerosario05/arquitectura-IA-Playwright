...
exports.extractPrerequisiteTargets = extractPrerequisiteTargets;
exports.extractRequirements = extractRequirements;
exports.buildRequirementManifest = buildRequirementManifest;
exports.buildCanonicalClaims = buildCanonicalClaims;
exports.isRequirementFunctionallyCoverable = isRequirementFunctionallyCoverable;
exports.computeFunctionalCoverage = computeFunctionalCoverage;
exports.evaluateFunctionalCoverageInvariant = evaluateFunctionalCoverageInvariant;
exports.validateStepRequirementRefs = validateStepRequirementRefs;
exports.normalizeInventedOrdinalSelection = normalizeInventedOrdinalSelection;
exports.buildRequirementAccounting = buildRequirementAccounting;
exports.applyFunctionalScenarioQuality = applyFunctionalScenarioQuality;
...
const norm = (s) => String(s ?? "")
    ...
    .replace(/\s+/g, " ")
    .trim();
const stripStepNumber = (s) => String(s).replace(/^\s*\d+[\.)]\s*/, "");
const isClickStep = (step) => /^clic\s+en/i.test(stripStepNumber(String(step ?? "")));
const isAssertionStep = (step) => /^(validar|verificar|comprobar|visualizar|esperar|confirmar|asegurar)\b/i.test(stripStepNumber(String(step ?? "")));
...
const titleHasVisibilityVerb = (title) => visibilityVerbRe.test(title);
const clickTargetsOf = (scenario) => (scenario.steps ?? [])
    .map((step) => String(step).match(/clic\s+en\s+"([^"]+)"/i)?.[1])
    .filter((t) => Boolean(t));
const assertionTargetsOf = (scenario) => (scenario.steps ?? [])
    .filter((step) => isAssertionStep(step))
    .map((step) => String(step).match(/"([^"]+)"/i)?.[1])
    .filter((t) => Boolean(t));
const isNegativeAssertion = (step) => {
    const s = String(step ?? "");
    return (/no\s+(?:se\s+)?(?:muestre|muestra|visualice|visualiza|vea|aparezca|encuentre|exhiba|deba|debe|estar|est[ée])\s+/i.test(s) ||
        /(?:ausencia|ocultamiento|no\s+visible|no\s+presente|sin\s+mostrar|enmascarad|no\s+se\s+exhiba)/i.test(s));
};
const scenarioText = (scenario) => [scenario.title ?? "", ...(scenario.steps ?? []), scenario.expectedResult ?? ""]
    .map((part) => String(part ?? ""))
    .join(" ");
const numberedSteps = (steps) => steps.map((step, index) => `${index + 1}. ${String(step).replace(/^\d+[\.)]\s*/, "").trim()}`);
...
function hasExplicitDependency(huText, ownLabel, siblingLabel) {
    ...
    while (idxOwn !== -1) {
        ...
        while (idxSibling !== -1) {
            if (Math.abs(idxSibling - idxOwn) <= 180) {
                ...
                if (dependencyConnector.test(windowText))
                    return true;
            }
 ...
 */
function enforceBranchPurity(scenarios, functionalBranches, huText) {
    const branchByLabel = new Map();
    for (const branch of functionalBranches) {
        if (!branch.sourceLabel)
            continue;
        const key = norm(branch.sourceLabel);
        if (!branchByLabel.has(key))
            branchByLabel.set(key, branch.branchId);
    }
    const kept = [];
    const rejected = [];
    let cleaned = 0;
    for (const scenario of scenarios) {
        ...
        for (const target of clickTargetsOf(scenario)) {
            ...
            if (siblingId !== undefined && siblingId !== ownBranchId) {
                if (hasExplicitDependency(huText, scenario.functionalBranch?.sourceLabel ?? "", target))
                    continue;
...
const negativeTitlePattern = /no\s+(?:se\s+)?(?:muestre|muestra|visualice|visualiza|vea|aparezca|exhiba|revele|exponga|exponer|mostrar|mostrarse)|ocult|restringid|enmascar|no\s+(?:debe|deber[íi]a)\s+(?:mostrar|ver|visualizar|exponer)/i;
const welcomePattern = /bienvenid|pantalla\s+(?:de\s+)?(?:inicio|inicial|principal|portada)|home|welcome|landing/i;
const entryDemandPattern = /no\s+(?:avanzar|navegar|continuar|ingresar|acceder|proseguir|pasar|seguir)\s+(?:sin|antes\s+de|hasta|antes\s+que|a\s+menos\s+que|previamente|que\s+completar)\s+(?:completar|hacer|realizar|el|la|lo|los|las|que|su\s+)?([a-zA-ZáéíóúüñÁÉÍÓÚÜÑ][a-zA-ZáéíóúüñÁÉÍÓÚÜÑ\s]{1,40})/i;
const stopWords = new Set([
    "que", "se", "de", "el", "la", "los", "las", "un", "una", "unos", "unas", "no", "sin", "y", "a", "al",
    "para", "por", "con", "en", "del", "lo", "al", "los", "las", "antes", "hasta", "cuando", "pueda",
]);
function extractEntryScreenLabel(huText) {
 ...
 */
function ensureRequirementStepAlignment(scenarios, huText) {
    const kept = [];
    const rejected = [];
    const generated = [];
    let mismatched = 0;
    for (const scenario of scenarios) {
        ...
        if (firstClick > 0) {
            const isWelcomeAssertion = (step) => isAssertionStep(step) && welcomePattern.test(String(step ?? ""));
    ...
    if (entryLabel) {
        ...
        const alreadyCoversPreEntry = kept.some((scenario) => {
            const steps = scenario.steps ?? [];
            const firstClick = steps.findIndex(isClickStep);
            const validatesEntryBeforeClick = firstClick === -1
    ...
    if (negativeTerm) {
        ...
        const alreadyCoversNegative = kept.some((scenario) => (scenario.steps ?? []).some(isNegativeAssertion) &&
...
const inactivityPattern = /inactiv|sin\s+(?:actividad|interacci[oó]n)|timeout|tiempo\s+de\s+espera|sesi[oó]n\s+(?:expira|termine|caduque)/i;
const restartPattern = /reinici|restart|apagad|p[eé]rdida\s+de\s+(?:energ[íi]a|corriente|conexi[oó]n)|power\s+(?:loss|failure)/i;
const environmentPattern = /entorno\s+restringid|environment\s+(?:restriction|capability|variable)|capacidad\s+del\s+entorno|restricci[oó]n\s+del\s+entorno|permisos\s+de\s+entorno|capacidad\s+ambiental/i;
const inactivitySecondsMatch = (huText) => {
 ...
 */
function generateNonUiRequirementScenarios(scenarios, huText) {
    const generated = [];
    const base = {
...
function objectiveStrength(scenario) {
 ...
 */
function dedupeByFunctionalObjective(scenarios) {
    const keyOf = (scenario) => {
    ...
    for (const scenario of scenarios) {
        ...
        if (objectiveStrength(scenario) > objectiveStrength(existing))
            byKey.set(key, scenario);
        removed++;
    }
...
function isTruncatedSource(sourceText) {
    const trimmed = String(sourceText ?? "").trim();
    if (!trimmed)
        return true;
    return truncatedMarker.test(trimmed);
}
const negativeRulePattern = /no\s+(?:se\s+)?(?:muestre|muestra|visualice|visualiza|vea|aparezca|exhiba|exponga|exponer|mostrar|mostrarse)\s+(?:la\s+|el\s+|los\s+|las\s+|ninguna?\s+)?([a-záéíóúüñ]{3,40}(?:\s+[a-záéíóúüñ]{2,40}){0,4})/i;
function extractNegativeTerm(huText) {
...
const visibilityPattern = /(?:se\s+(?:muestre|muestra|visualice|visualiza|vea|aparezca|encuentre)\s+|mostrar\s+|visualizar\s+|debe\s+estar\s+visible\s+|se\s+debe\s+(?:mostrar|visualizar)\s+)(?:la\s+|el\s+|los\s+|las\s+|un\s+|una\s+)?["""\u201C\u201D]?([a-záéíóúüñ0-9][\wáéíóúüñ0-9\s\-]{2,40})["""\u201C\u201D]?/gi;
/** "seleccionar X", "clic en X", "presionar X", "tocar X" → required functional action. */
const actionPattern = /\b(?:seleccionar|seleccione|selecciona|elegir|elija|hacer\s+clic\s+en|clic\s+en|clic\s+sobre|tocar|toque|presionar|presione|pulsar|pulse|oprimir)\s+(?:el\s+|la\s+|los\s+|las\s+|en\s+el\s+|en\s+la\s+)?["""\u201C\u201D]?([a-záéíóúüñ0-9][\wáéíóúüñ0-9\s\-]{1,40})["""\u201C\u201D]?/gi;
/** "redirige a X", "muestra la pantalla de X", "inicia el flujo de X" → expected destination/result. */
const destinationPattern = /\b(?:redirig\w*\s+(?:a|al|a\s+la)|muestra\s+la\s+pantalla\s+(?:de|del)|se\s+muestra\s+la\s+pantalla\s+(?:de|del)|lleva\s+(?:a|al|a\s+la)|permite\s+acceder\s+(?:a|al|a\s+la)|inicia\s+el\s+flujo\s+(?:de|del)|accede\s+(?:al|a\s+la)\s+m[oó]dulo\s+(?:de|del)|dirige\s+(?:a|al|a\s+la))\s+["""\u201C\u201D]?([a-záéíóúüñ0-9][\wáéíóúüñ0-9\s\-]{1,40})["""\u201C\u201D]?/gi;
...
const PREREQ_PREFIX = `(?:para\\s+(?:continuar|avanzar|ingresar|acceder|completar)|antes\\s+de\\s+continuar|primero|previamente|no\\s+debe\\s+permitirse\\s+(?:avanzar|continuar|proceder|ingresar|acceder)\\s+sin\\s+que)`;
const PREREQ_ACTION = `(?:seleccionar|seleccione|selecciona|elegir|elija|hacer\\s+clic\\s+en|haga\\s+clic\\s+en|clic\\s+en|clic\\s+sobre|tocar|toque|presionar|presione|pulsar|pulse|oprimir|ingresar\\s+a|acceder\\s+a)`;
const PREREQ_UI_NOUN = `(?:bot[oó]n|opci[oó]n|pantalla|p[aá]gina|vista|secci[oó]n|campo|men[uú]|elemento|control|enlace|link|tarjeta|recuadro|ventana|modal|í?t[eé]m|item)`;
const PREREQ_TARGET = `([a-záéíóúüñ0-9][\\wáéíóúüñ0-9\\s\\-]{1,40})`;
const PREREQUISITE_RE = new RegExp(`(?:\\b${PREREQ_PREFIX}\\b[^.\\n]{0,80}?\\b${PREREQ_ACTION}\\s+(?:el\\s+|la\\s+|los\\s+|las\\s+)?${PREREQ_UI_NOUN}\\s+[""\u201C\u201D]?${PREREQ_TARGET}[""\u201C\u201D]?|\\b${PREREQ_PREFIX}\\b[^.\\n]{0,80}?\\b${PREREQ_ACTION}\\s+(?:el\\s+|la\\s+|los\\s+|las\\s+)?[""\u201C\u201D]?${PREREQ_TARGET}[""\u201C\u201D]?)`, "gi");
/** Normalized action intent derived from the actual verb used in the clause. */
function prerequisiteActionIntent(matchText) {
    if (/ingresar\s+a|acceder\s+a/i.test(matchText))
        return "navigate";
    if (/hacer\s+clic|haga\s+clic|clic\s+en|clic\s+sobre|tocar|toque|presionar|presione|pulsar|pulse|oprimir/i.test(matchText))
        return "click";
    return "select";
}
...
function extractPrerequisiteTargets(huText) {
    const results = [];
...
const contentRestrictionPattern = /\b(?:solo\s+(?:se\s+)?(?:muestra|muestre|visualiza|visualice|aparece|debe\s+mostrar)|[uú]nicamente\s+(?:se\s+)?(?:muestra|muestre)|restring\w*\s+(?:el\s+acceso\s+a|la\s+visualizaci[oó]n\s+de|la\s+informaci[oó]n\s+de)|limita\s+(?:el\s+acceso\s+a|la\s+visualizaci[oó]n\s+de))\s+["""\u201C\u201D]?([a-záéíóúüñ0-9][\wáéíóúüñ0-9\s\-]{1,40})["""\u201C\u201D]?/gi;
/**
 ...
 */
const REQUIREMENT_NOISE_RE = /^(?:bot[oó]n|opci[oó]n|pantalla|p[aá]gina|vista|secci[oó]n|campo|men[uú]|elemento|control|enlace|link|tarjeta|fila|columna|panel|recuadro|ventana|modal|di[aá]logo|notificaci[oó]n|alerta|mensaje|texto|imagen|icono|t[ií]tulo|encabezado|header|footer|barra|pieza|componente)$/i;
/** Abstract business concepts ("tipo de gestión", "módulo de información") → not a concrete UI requirement. */
const ABSTRACT_REQUIREMENT_RE = /^(?:tipo\s+de|inicio\s+de|cierre\s+de|proceso\s+de|m[oó]dulo\s+de|gesti[oó]n\s+de|operaci[oó]n\s+de|transacci[oó]n\s+de|consulta\s+de|solicitud\s+de|registro\s+de|configuraci[oó]n\s+de|administraci[oó]n\s+de|generaci[oó]n\s+de|cancelaci[oó]n\s+de|actualizaci[oó]n\s+de|eliminaci[oó]n\s+de|creaci[oó]n\s+de)/i;
...
function startsWithNarrativeVerb(target) {
...
function cleanRequirementTarget(raw) {
    ...
    if (REQUIREMENT_NOISE_RE.test(lower))
        return null;
    if (ABSTRACT_REQUIREMENT_RE.test(lower))
        return null;
    return value;
}
...
function atomicRequirementTarget(raw) {
...
const clauseBoundaryRe = /\s+(?:y\s+poder|para\s+poder|con\s+el\s+fin\s+de|y\s+seleccionar|y\s+hacer\s+clic|y\s+elegir|y\s+presionar|luego|despu[eé]s|posteriormente)\b/i;
function truncateAtClauseBoundary(raw) {
    const value = String(raw ?? "").trim();
...
function isBranchTerm(raw, branchTermKeys) {
    const term = norm(raw);
    if (!term)
        return false;
    if (branchTermKeys.has(term))
        return true;
...
function isNegatedMatch(huText, match) {
...
function isTruncatedMatch(huText, match, atomic) {
    if (truncatedMarker.test(String(match[0])))
        return true;
 ...
 */
function extractRequirements(huText, functionalBranches) {
    const candidates = [];
    ...
    const seenContentRestrictionTerms = new Set();
    const seenDestinationTerms = new Set();
    const seenVisibilityTerms = new Set();
    const branchTermKeys = new Set();
    const add = (candidate) => {
        ...
        if (candidate.category === "action" || candidate.category === "prerequisite") {
            ...
            if (branchTermKeys.has(term))
                return; // the branch requirement already owns this action
            if (seenClickTerms.has(term))
                return; // already extracted as a click-oriented requirement
    ...
    for (const branch of functionalBranches) {
        ...
        add({
            ...
            facets: ["activation", ...(branch.expectedDestination ? ["destination"] : [])],
        });
    }
    ...
    for (const match of huText.matchAll(actionPattern)) {
        ...
        if (isNegatedMatch(huText, match))
            continue; // "no mostrar X" is a negative rule, not an action
        if (isBranchTerm(rawFull, branchTermKeys))
            continue; // the branch requirement owns it
        const atomic = cleanRequirementTarget(atomicRequirementTarget(rawFull));
        if (!atomic || isBranchTerm(atomic, branchTermKeys))
            continue;
        if (prereqTermKeys.has(norm(atomic)))
            continue; // explicit prerequisite owns the term
        if (startsWithNarrativeVerb(atomic))
            continue; // compound clause: "seleccionar visualizar ..." → skip
        actionCount++;
        add({
    ...
    for (const prereq of prerequisiteTargets) {
        if (isBranchTerm(prereq.actionTarget, branchTermKeys))
            continue;
        prerequisiteCount++;
        add({
    ...
    for (const match of huText.matchAll(contentRestrictionPattern)) {
        ...
        if (seenContentRestrictionTerms.has(tKey))
            continue;
    ...
    for (const match of huText.matchAll(destinationPattern)) {
        ...
        if (!term)
            continue;
        const tKey = norm(term);
        if (seenDestinationTerms.has(tKey))
            continue;
    ...
    for (const match of huText.matchAll(visibilityPattern)) {
        if (isNegatedMatch(huText, match))
            continue; // "no mostrar X" → negative rule
        const term = cleanRequirementTarget(truncateAtClauseBoundary(atomicRequirementTarget(String(match[1] ?? ""))));
        if (!term)
            continue;
        if (startsWithNarrativeVerb(term))
            continue; // compound clause: "mostrar seleccionar ..." → skip
        const tKey = norm(term);
        if (seenVisibilityTerms.has(tKey))
            continue;
...
function buildRequirementManifest(huText, functionalBranches = []) {
...
function claimTypeForFacet(facet) {
    if (facet === "activation" || facet === "action")
        return "action";
    if (facet === "destination")
        return "semantic_destination_assertion";
    if (facet === "visibility")
        return "visibility_assertion";
    return "technical_route_step";
}
function targetKindForFacet(facet) {
    if (facet === "destination")
...
function buildCanonicalClaims(requirements) {
    return requirements.flatMap((requirement) => {
        ...
        const facets = requirement.facets?.length
            ? requirement.facets
            : requirement.category === "visibility" ? ["visibility"]
                : requirement.category === "action" || requirement.category === "prerequisite" ? ["action"]
                    : requirement.category === "branch" ? ["activation", "destination"]
                        : requirement.category === "destination" ? ["destination"]
                            : ["technical"];
        ...
        return facets.map((facet) => ({
            ...
            ...(requirement.associatedBranchId
                ? { scope: "branch", scopeId: requirement.associatedBranchId }
                : { scope: "global" }),
        }));
    });
}
 ...
 */
const EVALUABLE_CATEGORIES = new Set([
    "branch",
    "visibility",
    "action",
    "destination",
...
function isRequirementFunctionallyCoverable(requirement) {
...
function withinOneEdit(a, b) {
    ...
    if (m !== n) {
        ...
        let common = 0;
        while (common < short.length && short[common] === long[common])
            common++;
    ...
    if (diffs === 1) {
        let common = 0;
        while (common < m && a[common] === b[common])
            common++;
    ...
    if (diffs === 2) {
        const isTransposition = firstDiff >= 0 &&
            firstDiff + 1 < m &&
...
function visibilityTokenNear(reqToken, assertionToken) {
    if (reqToken === assertionToken)
        return true;
    if (reqToken.length < 4 || assertionToken.length < 4)
        return false;
    if (withinOneEdit(reqToken, assertionToken))
        return true;
    ...
    while (common < limit && reqToken[common] === assertionToken[common])
        common++;
...
function visibilityCoveredByAssertions(assertions, key) {
    const reqTokens = significantTokens(key);
    if (reqTokens.length === 0)
        return false;
    return assertions.some((assertion) => {
        const asTokens = significantTokens(assertion);
        if (asTokens.length === 0)
            return false;
        return reqTokens.every((reqToken) => asTokens.some((asToken) => visibilityTokenNear(reqToken, asToken)));
    });
}
/**
 ...
 */
function scenarioCoversRequirement(scenario, requirement) {
    const trace = (result, reason, refFound, refIndex) => {
    ...
    const refFor = (id) => refs.filter((ref) => ref.requirementId === id && Number.isInteger(ref.stepIndex) && ref.stepIndex >= 0 && ref.stepIndex < (scenario.steps ?? []).length);
    const requirementRefs = refFor(requirementId);
    if (requirementRefs.length === 0)
        return trace(false, "ref_missing_or_invalid", false);
    if (requirement.associatedBranchId && scenario.functionalBranch?.branchId !== requirement.associatedBranchId)
        return trace(false, "branch_mismatch", true, requirementRefs[0].stepIndex);
    for (const prerequisiteId of requirement.prerequisiteRequirementIds ?? []) {
        ...
        if (prerequisiteRefs.length === 0)
            return trace(false, "prerequisite_ref_missing", true, requirementRefs[0].stepIndex);
        if (!prerequisiteRefs.some((prerequisite) => requirementRefs.some((dependent) => prerequisite.stepIndex < dependent.stepIndex)))
            return trace(false, "prerequisite_order_invalid", true, requirementRefs[0].stepIndex);
    }
    ...
    const matches = (list, term) => list.some((entry) => entry === term || entry.includes(term) || term.includes(entry));
    switch (requirement.category) {
        ...
        case "visibility":
            if (matches(assertions, key))
                return true;
            if (visibilityCoveredByAssertions(assertions, key))
                return true;
            if (steps.some(isAssertionStep) &&
 ...
 * this stage.
 */
function computeFunctionalCoverage(requirements) {
...
function evaluateFunctionalCoverageInvariant(requirements) {
    ...
    const evaluable = requirements.filter(isRequirementFunctionallyCoverable);
...
function validateStepRequirementRefs(scenario, requirements) {
    ...
    const refs = scenario.stepRequirementRefs.filter((ref) => Number.isInteger(ref.stepIndex) && ref.stepIndex >= 0 && ref.stepIndex < scenario.steps.length && known.has(ref.requirementId));
    const covered = new Set(refs.flatMap((ref) => [ref.requirementId, ...(ref.prerequisiteRequirementIds ?? [])]));
    const required = new Set(scenario.requirementDependencies?.flatMap((dependency) => dependency.prerequisiteRequirementIds ?? []) ?? []);
...
const DYNAMIC_SELECTION_RE = /^(?:uno\s+de\s+(?:los|las)|una\s+de\s+(?:los|las)|un\s+|una\s+|unos\s+|unas\s+|alg[uú]n\s+|alguna\s+|algunos\s+|algunas\s+|cualquier\s+|cualquiera\s+|otro\s+|otra\s+|otros\s+|otras\s+|varios\s+|varias\s+|cada\s+|el\s+primer\s+|la\s+primera\s+|los\s+primeros\s+|las\s+primeras\s+|primer\s+|primera\s+|primeros\s+|primeras\s+)/i;
 ...
 *  labels. */
const SENSITIVE_ACTION_RE = /\b(?:solicitar|solicita|solicite|pagar|pague|paga|transferir|transfer|contratar|contrata|confirma|confirmar|autorizar|autoriza|aprobar|aprueba|firmar|firma|enviar|env[ií]a|aceptar|acepta|debitar|debita|eliminar|elimina|cancelar|cancela|sensibl\w*)\b/i;
...
function isDynamicSelectionAction(requirement) {
...
const SELECTION_ORDINAL_RE = /^Seleccionar\s+(?:el\s+|la\s+)?(primer|primera|[uú]ltimo|[uú]ltima)\s+(.+)$/i;
 ...
 */
function normalizeInventedOrdinalSelection(step, huText) {
    ...
    if (!nounPhrase || !firstWord)
        return raw;
    // Declared by HU/source requirement? e.g. "seleccionar el primer préstamo".
    const declared = new RegExp(`(?:primer|primera|ultimo|ultima)\\s+${escapeRegExp(norm(firstWord))}`, "i").test(norm(huText));
    if (declared)
        return raw;
...
function isSensitiveAction(requirement) {
    const text = norm(`${requirement.keyTerm ?? ""} ${requirement.sourceText ?? ""}`);
    return SENSITIVE_ACTION_RE.test(text);
}
function buildRequirementAccounting(scenarios, functionalBranches, huText, sourceIssueKey) {
    ...
    const requirementsByBranchId = new Map(requirements
    ...
    for (const scenario of scenarios) {
        ...
        if (!requirement?.prerequisiteRequirementIds?.length || !requirement.requirementId)
            continue;
 ...
 */
function applyFunctionalScenarioQuality(scenarios, functionalBranches, huText) {
    const rejected = [];

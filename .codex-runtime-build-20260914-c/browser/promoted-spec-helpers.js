...
exports.scanVisibleElements = scanVisibleElements;
exports.waitForPromotedSpecStepReady = waitForPromotedSpecStepReady;
exports.waitForListReadiness = waitForListReadiness;
exports.capturePageDiagnostics = capturePageDiagnostics;
exports.buildListReadinessErrorMessage = buildListReadinessErrorMessage;
...
async function scanVisibleElements(page, selector) {
    try {
...
function containsAny(texts, keywords) {
    const normalizedTexts = texts.map(normalizeText);
    return keywords.some(kw => normalizedTexts.some(t => t.includes(normalizeText(kw))));
}
async function waitForPromotedSpecStepReady(page, options = {}) {
    const { previousUrl, timeoutMs = 15000, pollMs = 300, stabilizationMs = 500, requireUrlChange = previousUrl !== undefined, expectEntityList = false } = options;
    ...
    if (requireUrlChange && !urlChanged) {
        const currentUrl = page.url();
        throw new Error(`Page did not transition after action. URL unchanged: "${currentUrl}". ` +
            `This may indicate the click target did not trigger navigation.`);
    }
}
async function waitForListReadiness(page, options = {}) {
    const { timeoutMs = 10000, pollMs = 500, minCards = 1, loadingTimeoutMs = 8000 } = options;
    const startTime = Date.now();
...
async function checkListReadiness(page, _options) {
    const texts = await scanPageTexts(page);
...
function buildListReadinessErrorMessage(target, diagnostics, tokens, candidates) {
    const lines = [];

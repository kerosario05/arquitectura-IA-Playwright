...
exports.tokenizeForSemanticMatch = tokenizeForSemanticMatch;
exports.scoreSemanticTextMatch = scoreSemanticTextMatch;
exports.collectVisibleClickableElements = collectVisibleClickableElements;
exports.findBestSemanticMatch = findBestSemanticMatch;
exports.findSemanticTargetMatch = findSemanticTargetMatch;
 ...
 */
function tokenizeForSemanticMatch(text) {
 ...
 */
function isSensitiveTarget(text) {
    const normalized = (0, target_resolver_1.normalizeText)(text);
    return SENSITIVE_PATTERNS.some(pattern => normalized === pattern ||
 ...
 */
function scoreSemanticTextMatch(target, candidate, options) {
 ...
 */
async function collectVisibleClickableElements(page, options) {
    ...
    try {
        ...
        for (let i = 0; i < buttonCount; i++) {
            ...
            try {
                ...
                candidates.push({
                    ...
                    normalizedText: (0, target_resolver_1.normalizeText)(text.trim() || ariaLabel || ""),
                    type: "button",
                    role: "button",
                    tagName: "button",
                    score: 0,
        ...
        for (let i = 0; i < headingCount; i++) {
            ...
            try {
                ...
                candidates.push({
                    text: text.trim(),
                    normalizedText: (0, target_resolver_1.normalizeText)(text.trim()),
                    type: "heading",
                    role: "heading",
                    tagName: tagName,
                    score: 0,
        ...
        for (let i = 0; i < Math.min(cardCount, 20); i++) {
            ...
            try {
                ...
                candidates.push({
                    text: trimmedText,
                    normalizedText: (0, target_resolver_1.normalizeText)(trimmedText),
                    type: "card",
                    role: "listitem",
                    tagName: "div",
                    score: 0,
 ...
 */
function findBestSemanticMatch(target, candidates, options) {
 ...
 */
async function findSemanticTargetMatch(page, target, options) {
    const timeoutMs = options?.timeoutMs ?? 3000;

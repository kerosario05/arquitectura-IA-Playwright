...
exports.detectTransientScreen = detectTransientScreen;
...
function extractVisibleTexts(snapshot) {
    const texts = [];
...
function hasMatchingKeyword(texts, keywords) {
    const matched = [];
...
function hasFunctionalLanding(texts) {
    const normalizedTexts = texts.map(normalizeText);
    return FUNCTIONAL_LANDING_KEYWORDS.some((keyword) => normalizedTexts.some((text) => text.includes(normalizeText(keyword))));
}
function hasLoadedProducts(texts) {
    const normalizedTexts = texts.map(normalizeText);
    return LOADED_PRODUCT_INDICATORS.some((indicator) => normalizedTexts.some((text) => text.includes(normalizeText(indicator))));
}
function hasProductListContainer(texts) {
    const normalizedTexts = texts.map(normalizeText);
    return PRODUCT_LIST_CONTAINER_HINTS.some((hint) => normalizedTexts.some((text) => text.includes(normalizeText(hint))));
}
function hasVisibleProductCards(snapshot) {
    ...
    for (const el of elements) {
        ...
        const isProductCard = LOADED_PRODUCT_INDICATORS.some(indicator => normalized.includes(normalizeText(indicator)));
        if (isProductCard) {
...
function hasActionableElements(snapshot) {
    if (!snapshot.elements)
        return false;
    const buttons = snapshot.elements.filter((el) => el.type === "button" || el.tagName === "button").length;
    const links = snapshot.elements.filter((el) => el.type === "link" || el.tagName === "a").length;
    return buttons >= 3 || links >= 3;
}
function hasDisabledZeroButton(texts) {
    const normalizedTexts = texts.map(normalizeText);
    return ZERO_PRODUCT_INDICATORS.some((indicator) => normalizedTexts.some((text) => text.includes(normalizeText(indicator))));
}
function hasStableEmptyState(snapshot, zeroProductMatches, loadingMatches, dataLoadingMatches, redirectMatches, successMatches, urlMatches) {
...
function detectTransientScreen(snapshot) {
    ...
    const urlMatches = TRANSIENT_URL_PATTERNS.filter((pattern) => urlNormalized.includes(normalizeText(pattern)));

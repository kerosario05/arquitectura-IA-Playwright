...
exports.generateConsolidatedEvidenceDocx = generateConsolidatedEvidenceDocx;
exports.generateEvidenceDocx = generateEvidenceDocx;
...
const GENERATOR_VERSION = (0, node_crypto_1.createHash)("sha1")
 ...
 */
async function generateConsolidatedEvidenceDocx(scenarios, templatePath, outputPath) {
 ...
 */
async function generateEvidenceDocx(record, templatePath, outputPath) {
 ...
 */
async function generateConsolidatedFromTemplate(scenarios, templatePath, outputPath) {
 ...
 */
async function copyTemplateToOutput(templatePath, outputPath) {
 ...
 */
async function tryWordComGeneration(scenarios, outputPath) {
    let jsonTempPath;
    try {
        // Build scenario data
        const scenariosData = [];
        for (const sc of scenarios) {
            ...
            if (sc.detailEvidence?.required && !sc.detailEvidence?.captured) {
                ...
                console.log(`[evidence-docx] scenario has invalid detailEvidence required=true captured=false target=${sc.detailEvidence.target || "unknown"}`);
            }
            // Get functional step screenshots (exclude validation steps, deduplicate by path)
            const seenFuncPaths = new Set();
            const functionalScreenshots = sc.steps
                .filter(step => {
                if (!step.screenshotPath || !isImageFile(step.screenshotPath))
                    return false;
                if (!fs.existsSync(step.screenshotPath))
                    return false;
                if (/^\s*validar\b/i.test(step.stepText))
                    return false;
            ...
            if (images.length === 0) {
                const seenAllPaths = new Set();
                const allScreenshots = sc.steps
                    .filter(step => {
                    if (!step.screenshotPath || !isImageFile(step.screenshotPath) || !fs.existsSync(step.screenshotPath))
                        return false;
                    const resolvedPath = path.resolve(step.screenshotPath);
                    if (seenAllPaths.has(resolvedPath))
                        return false;
            ...
            console.log(`[evidence-docx] imagesForDocx scenario=${sc.scenarioId} count=${images.length} finalIncluded=${preparedImages.finalIncluded} finalIsLast=${preparedImages.finalIsLast} finalImagePath=${preparedImages.finalImagePath ?? "none"}`);
            scenariosData.push({
        ...
        for (const detail of finalImageLogValidation.details) {
            console.log(`[evidence:docx] final image log validation scenario=${detail.scenarioId} finalIncluded=${detail.finalIncluded} finalIsLast=${detail.finalIsLast} finalLogged=${detail.finalLogged} finalImagePath=${detail.finalImagePath ?? "none"}`);
        }
...
function prepareScenarioDocxInput(scenario) {
    ...
    if (scenario.detailEvidence?.required && !scenario.detailEvidence?.captured) {
        scenarioStatus = "Fallido";
        console.log(`[evidence-docx] scenario has invalid detailEvidence required=true captured=false target=${scenario.detailEvidence.target || "unknown"}`);
    }
    const seenFuncPaths = new Set();
        ...
        if (lastDetailIdx > 0) {
            ...
            if (lastIdx !== undefined && lastIdx !== null) {
                for (let i = lastDetailIdx - 1; i >= 0; i--) {
                    ...
                    if (imgIdx === lastIdx - 1 && removed.length >= 0) {
                        removed.push(i);
                        continue;
                    }
                    break;
    ...
    if (scenario.detailEvidence?.screenshotPath) {
        ...
        console.log(`[evidence-docx] detailEvidence found scenario=${scenario.scenarioId} path=${detailPath} required=${scenario.detailEvidence.required} captured=${scenario.detailEvidence.captured}`);
        if (isImageFile(detailPath) && fs.existsSync(detailPath)) {
    ...
    console.log(`[evidence-docx] imagesForDocx scenario=${scenario.scenarioId} count=${finalizedImages.images.length} finalIncluded=${finalizedImages.finalIncluded} finalIsLast=${finalizedImages.finalIsLast} finalImagePath=${finalizedImages.finalImagePath ?? "none"}`);
    return {
...
function finalizeScenarioImagesForDocx(scenario, imageCandidates) {
    const dedupedImages = [];
    ...
    if (finalImagePath) {
        const finalImageEntry = images.find(image => path.resolve(image.path) === finalImagePath) ??
            {
                path: finalImagePath,
...
function prependInitialScreenImage(scenario, images) {
    const initial = scenario.initialScreenEvidence;
    if (!initial?.path || !isImageFile(initial.path) || !fs.existsSync(initial.path))
        return images;
    const initialPath = path.resolve(initial.path);
    if (images.some((image) => path.resolve(image.path) === initialPath))
        return images;
    return [{
            path: initialPath,
...
function validateFinalImageInsertionLogs(stdout, scenarios) {
    const details = scenarios
        ...
        const insertedRegex = new RegExp(`\\[docx-word\\] IMAGE_INSERTED scenario=${escapedScenarioId} index=${expectedIndex} final=(True|true)\\b`, "i");
        const verifiedRegex = new RegExp(`\\[docx-word\\] FINAL_IMAGE_VERIFIED scenario=${escapedScenarioId} .*inserted=(True|true)`, "i");
...
function buildWordComScript(args) {
    const escapeSingleQuote = (s) => s.replace(/'/g, "''");
    return [
 ...
 */
async function jsZipFallbackGeneration(scenarios, outputPath) {
    const zip = await jszip_1.default.loadAsync(await fs.promises.readFile(outputPath));
 ...
 */
function deduplicateEmbeddedImagesPerScenario(documentXml, relsXml, scenarios) {
    ...
    const dedupedXmlByOrig = new Map();
    for (let bi = 0; bi < blocks.length; bi++) {
        ...
        for (const para of block.paragraphs) {
            const dedupedXml = para.xml.replace(containerRegex, (fullMatch, ...groups) => {
                const rid = groups.slice(0, -2).find((g) => typeof g === 'string' && g.length > 0) || '';
                beforeCount++;
...
async function normalizeWordComDocxOutput(outputPath, scenarios) {
    const zip = await jszip_1.default.loadAsync(await fs.promises.readFile(outputPath));
    const documentXmlFile = zip.file("word/document.xml");
    if (!documentXmlFile) {
        return;
    ...
    documentXml = documentXml.replace(/<w:t>(Estado:\s+)(Exitoso|Fallido)\s+(?:Exitoso|Fallido)<\/w:t>/g, "<w:t>$1$2</w:t>");
    // Fix "Estado: Fallido Exitoso" — separate adjacent runs case
    documentXml = documentXml.replace(/(<w:t>Estado:\s*(?:Exitoso|Fallido))<\/w:t>[\s\S]*?<w:t>\s*(?:Exitoso|Fallido)\s*<\/w:t>/g, "$1</w:t>");
    documentXml = documentXml.replace(/<w:p\b[\s\S]*?<w:br\b[^>]*w:type="page"[^>]*\/>[\s\S]*?<w:t>-<\/w:t>[\s\S]*?<\/w:p>/, "");
    ...
    console.log(`[evidence:docx] image extent summary fixed=${normalization.fixed} zeroRemaining=${validation.zeroExtents}`);
    console.log(`[evidence:docx] validation mediaFiles=${validation.mediaFiles} embeds=${validation.embeds} zeroExtents=${validation.zeroExtents} titlesFound=${validation.titlesFound}`);
    for (const finalCheck of validation.finalImageChecks) {
        console.log(`[evidence:docx] final image validation scenario=${finalCheck.scenarioId} finalIncluded=${finalCheck.finalIncluded} finalIsLast=${finalCheck.finalIsLast} finalEmbedFound=${finalCheck.finalEmbedFound} finalImagePath=${finalCheck.finalImagePath ?? "none"}`);
    }
    ...
    const failedFinalCheck = validation.finalImageChecks.find(check => !(check.finalIncluded && check.finalIsLast && check.finalEmbedFound));
    if (failedFinalCheck) {
...
async function normalizeZeroImageExtents(documentXml, zip, relsMap) {
    ...
    while ((match = containerRegex.exec(documentXml)) !== null) {
        ...
        updatedMatch = updatedMatch.replace(/<wp:extent\b[^>]*\bcx="0"[^>]*\bcy="0"[^>]*\/>/g, `<wp:extent cx="${cx}" cy="${cy}"/>`);
        updatedMatch = updatedMatch.replace(/<a:ext\b([^>]*)\bcx="0"([^>]*)\bcy="0"([^>]*)\/>/g, `<a:ext$1cx="${cx}"$2cy="${cy}"$3/>`);
...
function normalizeWordMediaPath(target) {
...
function scaleImageToEvidenceWidth(widthPx, heightPx) {
...
async function validateNormalizedDocx(outputPath, scenarios) {
    const zip = await jszip_1.default.loadAsync(await fs.promises.readFile(outputPath));
    ...
    const textContent = decodeXmlEntities(Array.from(documentXml.matchAll(/<w:t[^>]*>([\s\S]*?)<\/w:t>/g))
        .map(match => match[1])
        .join(" ")).replace(/\s+/g, " ");
    ...
    const finalImageChecks = await Promise.all(scenarios
...
async function buildDocxMediaHashIndex(zip, relsMap) {
    const hashes = new Set();
    const mediaPaths = new Set(Array.from(relsMap.values()).map(target => normalizeWordMediaPath(target)).filter(Boolean));
    for (const mediaPath of mediaPaths) {
        ...
        hashes.add((0, node_crypto_1.createHash)("sha1").update(buffer).digest("hex"));
    }
    return hashes;
}
async function hashFileIfExists(filePath) {
    try {
        ...
        return (0, node_crypto_1.createHash)("sha1").update(buffer).digest("hex");
    }
    catch {
        return null;
    }
...
function buildSimplifiedScenarioBlocks(scenarios, startRid = 100) {
    const blocks = [];
    const images = [];
    let imageCounter = 1;
  ...
  <w:tr>
    ...
    <w:tc>
      ...
      <w:p><w:r><w:t>${escapeXml(sc.status || "Exitoso")}</w:t></w:r></w:p>
        ...
        const seenBlockPaths = new Set();
        const functionalScreenshots = sc.steps.filter(step => {
            if (!step.screenshotPath || !isImageFile(step.screenshotPath))
                return false;
            if (!fs.existsSync(step.screenshotPath))
                return false;
        ...
        const screenshotsToUse = functionalScreenshots.length > 0
            ...
            : (() => {
                const seenFallbackPaths = new Set();
                return sc.steps.filter(step => {
                    if (!step.screenshotPath || !isImageFile(step.screenshotPath) || !fs.existsSync(step.screenshotPath))
                        return false;
                    const resolvedPath = path.resolve(step.screenshotPath);
                    if (seenFallbackPaths.has(resolvedPath))
                        return false;
 ...
 */
async function embedImages(zip, images) {
    // Update rels
    let relsXml;
...
function ensureImageNamespaces(docXml) {
...
function replacePlaceholderInXml(xml, placeholder, replacement) {

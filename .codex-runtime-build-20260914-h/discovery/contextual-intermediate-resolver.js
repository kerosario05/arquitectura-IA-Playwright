...
exports.resolveAmbiguousIntermediateTarget = resolveAmbiguousIntermediateTarget;
...
function isOrdinalSelectionTarget(target) {
    const normalized = normalizeText(target);
    return (/\bprimera?\s+/i.test(normalized) ||
...
function isBackNavigation(target) {
    return BACK_NAVIGATION_PATTERNS.some(pattern => pattern.test(target));
}
function classifyCandidateType(element, target, nextTarget) {
...
function calculateCandidateScore(candidate, input) {
    let score = 0;
    const reasons = [];
    ...
    if (input.previousTarget && input.routeProfile) {
        ...
        const routeMatches = input.routeProfile.routes?.some(route => {
            if (!route.from || !route.intermediates)
                return false;
            ...
            if (Array.isArray(route.intermediates)) {
                intermediateMatch = route.intermediates.some(int => typeof int === "string" && normalizeText(int).includes(normalizedTarget));
            }
            else if (typeof route.intermediates === "object") {
                // Object format: check all values
                for (const key of Object.keys(route.intermediates)) {
                    const values = route.intermediates[key];
                    if (Array.isArray(values)) {
                        intermediateMatch = values.some((v) => typeof v === "string" && normalizeText(v).includes(normalizedTarget));
...
function matchRouteProfile(target, routeProfile) {
    if (!routeProfile)
        return undefined;
    const normalizedTarget = normalizeText(target);
    // Check intermediates
 ...
 */
function evaluateIntermediateAlreadySatisfied(input, classifiedCandidates) {
    const normalizedTarget = normalizeText(input.target);
    const nextIsOrdinal = input.nextTarget ? isOrdinalSelectionTarget(input.nextTarget) : false;
    const targetIsShort = isShortTarget(input.target);
    ...
    const visibleItems = classifiedCandidates.filter(c => c.element.visible &&
    ...
    if (input.routeProfile && input.nextTarget) {
        ...
        for (const item of visibleItems) {
            ...
            const containsDomainTerm = nextDomainTerms.some(term => item.normalizedText.includes(normalizeText(term)));
...
function resolveAmbiguousIntermediateTarget(input) {
    const normalizedTarget = normalizeText(input.target);
    const targetIsShort = isShortTarget(input.target);
    const targetIsGeneric = isGenericTarget(input.target);
    const nextIsOrdinal = input.nextTarget ? isOrdinalSelectionTarget(input.nextTarget) : false;
    ...
    const classifiedCandidates = input.candidates.map(element => {

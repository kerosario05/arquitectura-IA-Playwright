...
function identityFromContractStep(step) {
    ...
    if (technicalTargetRefs.length === 0 && !targetIdentity && !nonEmptyString(step.valueKey))
        return undefined;
    return {
        valueKey: nonEmptyString(step.valueKey),
        technicalTargetRefs,
        entityScope: nonEmptyString(step.entityScope),
        ...
        semanticType: nonEmptyString(step.semanticType),
...
function identityFromRecordingForValueKey(plan, valueKey, target) {
    ...
    for (const recordingPath of recordingCandidates()) {
        try {
            ...
            if (technicalTargetRefs.length === 0)
                continue;
            return {
                valueKey,
                technicalTargetRefs: Array.from(new Set(technicalTargetRefs)),
        ...
        try {
            ...
            if (identity?.valueKey) {
                ...
                if (recordingIdentity)
                    return { ...identity, ...recordingIdentity };
                return identity;
            }
        }
        catch {

...
async function capturePromotedActionSurfaceSnapshot(page, target) {
    ...
    return page.evaluate((expectedTarget) => {
        ...
        const visibleControls = Array.from(document.querySelectorAll("button, a, [role=button], [role=link], [role=dialog], h1, h2, h3, h4, h5, h6"))
            .filter(visible)
            .map((element) => `${element.tagName.toLowerCase()}|${normalize(element.textContent || "")}`)
...
class PromotedSpecRuntime {
            ...
            if (options.skipNativeTargetResolution) {
                ...
                if (nativeClickSucceeded) {
                    ...
                    if (resolved && resolved.locator) {
                        ...
                        try {
                            await withTimeout(resolved.locator.click({ timeout: this.config.actionTimeoutMs, noWaitAfter: true }), this.config.actionTimeoutMs, "native click");
                        }
                        catch (clickError) {
                            if (!isRetryableTargetClickError(clickError))
                                throw clickError;
                            const freshResolved = await resolvePromotedClickableLocator(this.page, options.target, {
                                containerLocator: containerSelector,
                                targetIdentity,
                                timeoutMs: this.config.actionTimeoutMs,
                                actionKind: options.actionIntent,
                            ...
                            matchedLocatorStrategy = `${freshResolved.strategy}:bounded_reresolution`;
                            await withTimeout(freshResolved.locator.click({ timeout: this.config.actionTimeoutMs, noWaitAfter: true }), this.config.actionTimeoutMs, "native click after bounded re-resolution");
                        }
                        nativeClickSucceeded = true;
                        clickPath = "native_runtime";
    ...
    async selectPromotedItem(options) {
        ...
        if (targetIdentity
            && runtimeValue
            && parsedTargetRefs.semanticRole === "selection") {
            const previousUrl = this.page.url();

...
exports.decodeJsStringLiteralBody = decodeJsStringLiteralBody;
exports.semanticallyEqualText = semanticallyEqualText;
...
function decodeMojibakeLevel(value) {
    const bytes = [];
    for (const char of value) {
        const code = char.codePointAt(0) ?? -1;
        if (code >= 0 && code <= 0xff)
            bytes.push(code);
        else if (CP1252_HIGH_BYTE[code] !== undefined)
            bytes.push(CP1252_HIGH_BYTE[code]);
        else
            return null;
    }
    try {
...
function normalizeSemanticText(value) {
    ...
    for (let iteration = 0; iteration < MAX_MOJIBAKE_ITERATIONS; iteration += 1) {
        ...
        if (!decoded || decoded === current || nonAsciiCount(decoded) >= nonAsciiCount(current))
            break;
        current = decoded;
    }
...
function decodeJsStringLiteralBody(body) {
    let result = "";
...
function semanticallyEqualText(left, right) {

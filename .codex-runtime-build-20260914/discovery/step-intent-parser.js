...
exports.normalizeParsedTarget = normalizeParsedTarget;
exports.parseStepIntent = parseStepIntent;
exports.extractLeadingRowScope = extractLeadingRowScope;
exports.parseSingleIntent = parseSingleIntent;
exports.splitByIntentBoundaries = splitByIntentBoundaries;
exports.isAssertionLike = isAssertionLike;
exports.cleanActionTarget = cleanActionTarget;
exports.classifyStepSet = classifyStepSet;
...
function normalizeParsedTarget(input) {
    ...
    const valueSource = input.valueSource === "literal" || input.valueSource === "test_data" || input.valueSource === "unknown"
        ? input.valueSource
        : "unknown";
    return {
        ...input,
        target,
        valueSource,
        valueKey: typeof input.valueKey === "string" ? input.valueKey : undefined,
        value: typeof input.value === "string" ? input.value : undefined,
    };
}
function parseStepIntent(stepText, _context) {
    const intents = [];
    ...
    const withAssertionValueKey = (intent) => {
        if (intent.type !== "assertion" || intent.expectedValueKey)
            return intent;
    ...
    const parsedCanonicalAssertion = (0, canonical_scenario_1.parseCanonicalAssertion)(trimmed);
    const canonicalAssertion = parsedCanonicalAssertion
        ? {
            ...parsedCanonicalAssertion,
            ...(parsedCanonicalAssertion.oracleType ? {} : scope?.rowRelation === "added" ? { oracleType: "structural_row_count", prerequisite: "row_added" } : {}),
        }
        : undefined;
    ...
    for (const fragment of fragments) {
        ...
        for (let intentIndex = 0; intentIndex < fragmentIntents.length; intentIndex += 1) {
            ...
            if (intent.type !== "action_select" && intent.type !== "action_fill")
                continue;
            ...
            if (!activationTarget)
                continue;
            if (!intent.selectionField && intent.type === "action_select")
                intent.selectionField = activationTarget;
            if (!intent.actionTarget || normalizeText(intent.actionTarget) === "valor")
                intent.actionTarget = activationTarget;
            if (!intent.associatedField && activation.associatedField)
                intent.associatedField = activation.associatedField;
        }
        intents.push(...fragmentIntents);
    }
    return intents;
}
function tryParseConditionalAction(text, normalized) {
    const match = text.match(/^(?:si|en\s+caso\s+de\s+que)\s+(.+?)\s*[,;]\s*(.+)$/i);
    if (!match)
        return null;
    const conditionText = match[1].trim();
    const actionText = match[2].trim();
    const conditionType = /\b(?:visible|visible\b|se\s+muestra|aparece|est[aá]\s+visible|est[aá]\s+disponible)\b/i.test(conditionText)
        ? "visibility"
        : /\b(?:existe|exist[ae]|est[aáé]\s+presente|disponible)\b/i.test(conditionText)
            ? "presence"
            : "visibility";
    if (!/\b(?:visible|se\s+muestra|aparece|existe|disponible|presente)\b/i.test(conditionText))
        return null;
    ...
    if (!actionMatch)
        return null;
    const operation = /^(?:seleccionar|escoger|elegir)\b/i.test(actionText) ? "select" : "click";
    ...
    if (!conditionTarget || !actionTarget)
        return null;
    return {
...
function extractLeadingRowScope(text) {
    ...
    const indexedPattern = new RegExp(`^en\\s+(?:la\\s+|el\\s+)?(${indexed})\\s+${unit}(?:\\s+de\\s+([^,]+?))?\\s*,\\s*`, "i");
...
function parseSingleIntent(text) {
    ...
    if (dynamicItemResult)
        return dynamicItemResult;
...
function splitByIntentBoundaries(text) {
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        if (afterPos < text.length && isLetter(text[afterPos]))
            continue;
...
function tryParseSetupRoute(text, normalized) {
    const navVerbs = [
        "abrir", "acceder", "ingresar", "entrar", "navegar", "ir", "cargar",
        "open", "access", "enter", "launch", "navigate", "go", "load"
    ];
    const targetKeywords = /\b(url|portal|aplicaci[oó]n|sistema|sitio|web|p[aá]gina|home|inicio|iniciar\s+s[eé]sion|application|app|site|page|homepage|website)\b/i;
    const verbPattern = new RegExp(`^(?:${navVerbs.join("|")})(?:\\s+(?:a|al|the|el|la|los|las))?\\s+`, "i");
...
function tryParseOptionalDirective(text, normalized) {
...
function tryParsePreconditionContext(text, normalized) {
...
function tryParseNavigationPath(text, normalized) {
    const navVerbs = [
...
function tryParseNavigationPathEnhanced(text, normalized) {
    if (/^.+@.+\..+/i.test(text) || /https?:\/\/|www\./i.test(text))
        return null;
...
function tryParseOptionalAction(text, normalized) {
...
function tryParseClickAction(text, normalized) {
    const clickVerbs = [
        "clic en", "hacer clic en", "click en", "click on", "click",
        "presionar", "tocar"
    ];
    const verbPattern = new RegExp(`^(?:${clickVerbs.join("|")})\\s+`, "i");
    const verbMatch = text.match(verbPattern);
    if (!verbMatch)
        return null;
    const afterVerb = text.slice(verbMatch[0].length).trim();
    const activationText = afterVerb.replace(/\s+(?:y|e)\s+(?=(?:seleccionar|escoger|elegir|ingresar|digitar|escribir|completar|llenar|type|enter|fill)\b).*$/i, "").replace(/\s+(?:y|e)\s*$/i, "").trim();
    ...
    // embedded here.
    const selectorInField = activationText.match(/^(?:el|la|un|una)?\s*(?:selector|control)\s+["']([^"']+)["']\s+(?:del|de|en)\s+(?:el|la)?\s*campo\s+["']([^"']+)["']$/i);
    if (selectorInField) {
    ...
    const associatedControl = activationText.match(/^(?:el|la|un|una)?\s*(?:campo|control|selector)\s+(?:de|del|para)\s+(.+?)\s+asociad[oa]\s+(?:a|con)\s+["']([^"']+)["']$/i);
    ...
    const associatedPattern = text.match(/^(?:clic en|hacer clic en|click en|click on|click|presionar|tocar)\s+['""]([^'""]+)['""]\s+(?:asociado|associat\w*|relacionado|related)\s+(?:a|al|con|to|with|the)?\s*(?:\w+\s+)*\s*['""]([^'""]+)['""]/i);
    ...
    const relationalPattern = text.match(/^(?:clic en|hacer clic en|click en|click on|click|presionar|tocar)\s+(?:el|la|los|las)?\s*(producto|card|opci[oó]n|elemento|secci[oó]n|categor[ií]a|[ií]tem)\s*(?:visible\s+)?(?:relacionad[oa]|asociad[oa]|vinculad[oa])\s*(?:con|a)\s*['""]([^'""]+)['""](?:\s+(?:dentro\s+de|en)\s+(?:la\s+)?(categor[ií]a\s+seleccionada|secci[oó]n\s+actual|contexto\s+actual|[^'""]+))?/i);
    if (relationalPattern) {
        const rawRole = relationalPattern[1].toLowerCase().replace(/[óo]/g, "o").replace(/[ií]/g, "i");
        let semanticRole = "unknown";
        if (rawRole.includes("product"))
            semanticRole = "product";
        else if (rawRole.includes("card"))
            semanticRole = "card";
        else if (rawRole.includes("opcion"))
            semanticRole = "option";
        else if (rawRole.includes("categoria"))
            semanticRole = "category";
        else if (rawRole.includes("seccion"))
            semanticRole = "section";
        else if (rawRole.includes("elemento") || rawRole.includes("item"))
            semanticRole = "item";
...
function tryParseSelectAction(text, normalized) {
    const selectVerbs = [
        "seleccionar", "escoger", "elegir"
    ];
    const verbPattern = new RegExp(`^(?:${selectVerbs.join("|")})\\s+`, "i");
    const verbMatch = text.match(verbPattern);
    if (!verbMatch)
        return null;
    const afterVerb = text.slice(verbMatch[0].length).trim();
    const standaloneRuntimeValue = afterVerb.match(/^(?:el|la|un|una)?\s*(?:valor|opci[oó]n)\s+\[([^\]\r\n]+)\]$/i);
...
function tryParseDynamicItemSelection(text, normalized) {
...
function tryParseLoginSetup(text, normalized) {
...
function tryParseCompositeAction(text, normalized) {
...
function tryParseFillAction(text, normalized) {
    const fillVerbs = [
        "ingresar", "digitar", "escribir", "completar", "llenar",
        "type", "enter", "fill"
    ];
    const verbPattern = new RegExp(`^(?:${fillVerbs.join("|")})\\s+`, "i");
    const verbMatch = text.match(verbPattern);
    if (!verbMatch)
        return null;
    const verb = verbMatch[0].trim().toLowerCase();
    const standaloneRuntimeValue = text.slice(verbMatch[0].length).trim().match(/^(?:el|la|un|una)?\s*(?:valor)\s+\[([^\]\r\n]+)\]$/i);
    if (standaloneRuntimeValue) {
        return {
            type: "action_fill",
            originalText: text,
    ...
    const bracketDataToField = text.match(/^(?:ingresar|digitar|escribir|completar|llenar|type|enter|fill)\s+(?:(?:el\s+)?valor(?:\s+secreto)?\s+)?\[([^\]\r\n]+)\]\s+en\s+(?:el\s+)?campo\s+(?:["']([^"']+)["']|([^\.\r\n]+?))\.?$/i);
    ...
    const testDataToField = text.match(/^(?:escribir|ingresar|digitar|type|enter)\s+el\s+valor\s+del\s+dato\s+['""]([^'""]+)['""]\s+en\s+(?:el\s+)?campo\s+['""]([^'""]+)['""]/i);
    if (testDataToField) {
        return {
            type: "action_fill",
            originalText: text,
    ...
    const englishDataToField = text.match(/^(?:type|enter)\s+the\s+value\s+of\s+(?:test\s+)?data\s+['""]([^'""]+)['""]\s+(?:into|in)\s+(?:the\s+)?field\s+['""]([^'""]+)['""]/i);
    ...
    const campoConDato = text.match(/^(?:completar|llenar|fill)\s+(?:el\s+)?campo\s+['""]([^'""]+)['""]\s+con\s+(?:el\s+)?(?:valor\s+del\s+)?dato\s+['""]([^'""]+)['""]/i);
    if (campoConDato) {
        return {
            type: "action_fill",
            originalText: text,
    ...
    const fillFieldWithData = text.match(/^fill\s+field\s+['""]([^'""]+)['""]\s+with\s+(?:test\s+)?data\s+['""]([^'""]+)['""]/i);
    ...
    const literalToField = text.match(/^(?:escribir|ingresar|digitar|type|enter)\s+['""]([^'""]+)['""]\s+en\s+(?:el\s+)?campo\s+['""]([^'""]+)['""]/i);
    if (literalToField) {
        return {
            type: "action_fill",
            originalText: text,
    ...
    const englishLiteralToField = text.match(/^(?:type|enter)\s+['""]([^'""]+)['""]\s+(?:into|in)\s+(?:the\s+)?field\s+['""]([^'""]+)['""]/i);
    ...
    const campoConLiteral = text.match(/^(?:completar|llenar|fill)\s+(?:el\s+)?campo\s+['""]([^'""]+)['""]\s+con\s+['""]([^'""]+)['""]/i);
    if (campoConLiteral) {
        return {
            type: "action_fill",
            originalText: text,
...
function tryParseAssertion(text, normalized) {
...
function isAssertionLike(text) {
    return ASSERTION_PREFIX_PATTERN.test(normalizeText(text));
}
function cleanActionTarget(target) {
    return target
        .replace(/\u00a0/g, " ")
        .replace(/^[\s>.:;\-–—]+/, "")
        .replace(/[\s>.:;\-–—]+$/, "")
...
function splitMixedActionNavigation(text) {
    ...
    if (!actionPart || !navigationPart)
        return [text];
    if (!NAVIGATION_PREFIX_PATTERN.test(normalizeText(navigationPart)))
        return [text];
    return [actionPart, navigationPart];
}
function protectSplitSensitiveTokens(text) {
...
function restoreProtectedTokens(text, tokenMap) {
    let restored = text;
...
function tryParseStandaloneAction(text, normalized) {
...
function classifyStepSet(steps) {
    const result = {
        setupIntents: [],
        actionIntents: [],
        assertionIntents: []
    ...
    for (const intent of steps) {
        ...
        else if (intent.type === "action_click" ||

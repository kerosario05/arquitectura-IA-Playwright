...
exports.loadSkillMarkdown = loadSkillMarkdown;
exports.getAllSkillIds = getAllSkillIds;
exports.writeSelectedSkillFiles = writeSelectedSkillFiles;
const promises_1 = require("node:fs/promises");
const node_path_1 = __importDefault(require("node:path"));
const SKILLS_DIR = node_path_1.default.resolve(__dirname, "skills");
...
function loadSkillDefinition(skillId) {
...
function loadSkillMarkdown(skillId) {
    return [
        `# Skill: ${skillId}`,
        "",
...
function getSkillFrontMatter(skillId) {
    const def = BUILTIN_SKILL_DEFINITIONS[skillId];
    return `## ${def.purpose}`;
}
function getAllSkillIds() {
    return Object.keys(BUILTIN_SKILL_DEFINITIONS);
}
async function writeSelectedSkillFiles(handoffDir, skillId, definition) {
    const mdPath = node_path_1.default.join(handoffDir, "selected-skill.md");
    const jsonPath = node_path_1.default.join(handoffDir, "selected-skill.json");
    ...
    await (0, promises_1.writeFile)(mdPath, mdContent, "utf-8");
    await (0, promises_1.writeFile)(jsonPath, JSON.stringify({ selectedSkill: skillId, definition }, null, 2), "utf-8");

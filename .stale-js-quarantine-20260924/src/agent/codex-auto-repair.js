...
exports.buildCompactPrompt = buildCompactPrompt;
exports.buildCodexPrompt = buildCodexPrompt;
exports.runCodexAutoRepair = runCodexAutoRepair;
...
function buildDeterministicRouteDecision(topCandidate, pack) {
    if (!pack)
        return undefined;
    if (topCandidate.score < 0.70)
        return undefined;
    const goodRelations = ["exact_match", "near_match", "parent_category", "broader_candidate", "related_action"];
    if (!goodRelations.includes(topCandidate.relation ?? ""))
        return undefined;
    if (topCandidate.actionability !== "clickable")
        return undefined;
    const candidateExists = pack.topVisibleCandidates.some((c) => c.id === topCandidate.candidateId);
    if (!candidateExists)
        return undefined;
    const hasFailedForCandidate = pack.actionHistorySummary.some((a) => (a.target === topCandidate.candidateId || a.target === topCandidate.text) && a.status === "failed");
    if (hasFailedForCandidate)
        return undefined;
    return {
        decision: {
...
function buildTargetFromCandidate(candidateId, candidate) {
...
function buildAgentResponseFromRouteDecision(decision, candidateDetails) {
        ...
        case "needs_more_context": {
            return {
                response: {
                    ...
                    plans: [],
                    proposedObjects: [],
                    unresolvedQuestions: decision.unresolvedQuestions.map((q) => ({ type: "missing_data", message: q })),
                    rationale: [decision.rationale]
                },
                errors
            };
...
function buildVerbosePrompt(input) {
...
function buildCompactRouteRecoveryPrompt(input) {
    const absDir = node_path_1.default.resolve(input.handoffDir);
    const packPath = input.routeRecoveryPackPath ? node_path_1.default.resolve(input.routeRecoveryPackPath) : node_path_1.default.join(absDir, "route-recovery-pack.json");
...
async function tryDeterministicFallback(topCandidate, pack) {
    const result = buildDeterministicRouteDecision(topCandidate, pack);
    if (!result)
        return undefined;
    return result;
}
function canUseDeterministicFallback(decisionContent, decisionErrors) {
    if (!decisionContent) {
        return true;
    }
...
async function runCodexAutoRepair(input) {
    ...
    const runnerResult = await (0, codex_cli_runner_1.runCodexCli)({
    ...
    if (runnerResult.timedOut) {
        return {
            ...
            error: (0, codex_cli_runner_1.formatCodexTimeoutError)({
                command: input.codexCommand,
                extraArgs: input.codexExtraArgs,
                prompt,
                cwd: input.projectRoot,
    ...
    if (runnerResult.exitCode !== 0) {
        ...
        return {
            ...
            error: (0, codex_cli_runner_1.formatCodexCliError)(runnerResult, runnerInput)
        };
    }
    ...
    if (isCompactRecovery) {
        try {
            ...
            let decisionContent;
            try {
                decisionContent = await (0, promises_1.readFile)(decisionPath, "utf-8");
            }
            catch {
            ...
            let routeDecision;
            let decisionErrors = [];
            let finalAgentResponseBuiltBy = "deterministic_fallback";
            if (decisionContent) {
                // Validate Codex-written decision
                const parsedDecision = JSON.parse(decisionContent);
                const validation = (0, route_recovery_decision_validator_1.validateRouteRecoveryDecision)(parsedDecision, pack);
            ...
            const finalValidation = (0, agent_response_validator_1.validateAgentHandoffResponse)(agentResponse, { promptMode: "compact-route-recovery" });
            const finalErrors = [
            ...
            await (0, promises_1.writeFile)(input.responsePath, JSON.stringify(agentResponse, null, 2), "utf-8");
    ...
    try {
        try {
            await (await Promise.resolve().then(() => __importStar(require("node:fs/promises")))).access(input.responsePath);
        }
        catch {
            return {
                success: false,
        ...
        const responseContent = await (0, promises_1.readFile)(input.responsePath, "utf-8");
        ...
        if (normalizedResponse !== parsedResponse) {
            await (0, promises_1.writeFile)(input.responsePath, JSON.stringify(normalizedResponse, null, 2), "utf-8");
        }
        const requestContent = await (0, promises_1.readFile)(input.requestPath, "utf-8");
        ...
        const validation = (0, agent_response_validator_1.validateAgentHandoffResponse)(response, {

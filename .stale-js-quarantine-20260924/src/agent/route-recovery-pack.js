...
exports.deriveSemanticGoal = deriveSemanticGoal;
exports.buildRouteRecoveryPack = buildRouteRecoveryPack;
exports.computeRouteRecoveryPackStats = computeRouteRecoveryPackStats;
...
function deriveSemanticGoal(scenario, pendingSteps, finalAssertions, failedTarget) {
    const intentParts = [];
    let targetConcept;
    let sensitive = false;
    const allSteps = [
    ...
    for (const step of allSteps) {
        ...
        if (!intentParts.includes(action))
            intentParts.push(action);
        if (!targetConcept && step.target)
            targetConcept = step.target;
...
function computeSemanticRelation(candidateText, failedTarget) {
    if (!failedTarget)
        return { relation: "unrelated", score: 0 };
    const ct = normalizeText(candidateText);
    if (!ct)
        return { relation: "unrelated", score: 0 };
    const ft = normalizeText(failedTarget);
    if (ct === ft)
        return { relation: "exact_match", score: 1.0 };
    if (ct.includes(ft) || ft.includes(ct))
        return { relation: "near_match", score: 0.85 };
    const cTokens = tokenize(candidateText);
    const fTokens = tokenize(failedTarget);
    if (cTokens.length === 0 || fTokens.length === 0)
        return { relation: "unrelated", score: 0 };
    const overlap = cTokens.filter((t) => fTokens.includes(t)).length;
    if (overlap === 0)
        return { relation: "unrelated", score: 0 };
    // All tokens of the shorter set match → parent/child relation
    if (overlap >= Math.min(cTokens.length, fTokens.length))
        return { relation: "parent_category", score: 0.8 };
    if (overlap >= Math.min(cTokens.length, fTokens.length) * 0.5)
        return { relation: "near_match", score: 0.7 };
    return { relation: "broader_candidate", score: 0.5 };
}
function classifyActionability(candidate) {
    if (candidate.disabled)
        return "static";
    const role = (candidate.role ?? "").toLowerCase();
    const type = (candidate.type ?? "").toLowerCase();
    const tag = (candidate.tagName ?? "").toLowerCase();
    if (["button", "link", "menuitem", "tab", "checkbox", "radio"].includes(role) || type === "clickable" || ["a", "button"].includes(tag))
        return "clickable";
    if (["textbox", "input", "combobox"].includes(role) || type === "input" || ["input", "textarea", "select"].includes(tag))
        return "fillable";
    if (["listbox", "option", "select"].includes(role) || type === "select")
        return "selectable";
    if (["heading", "paragraph", "cell", "gridcell"].includes(role) || type === "text")
        return "readable";
    return "static";
}
function isActionable(actionability) {
    return actionability === "clickable" || actionability === "fillable" || actionability === "selectable";
}
// --- Candidate scoring ---
function scoreCandidate(candidate, failedTarget, semanticGoal, pendingStepTexts) {
    ...
    if (failedTarget)
        score += computeTokenOverlapScore(failedTarget, text) * 0.4;
    // Semantic overlap with targetConcept
    if (semanticGoal?.targetConcept)
        score += computeTokenOverlapScore(semanticGoal.targetConcept, text) * 0.2;
    // Semantic overlap with intent
    if (semanticGoal?.intent)
        score += computeTokenOverlapScore(semanticGoal.intent, text) * 0.1;
    // Semantic relation boost
    if (relation === "exact_match")
        score += 0.3;
    else if (relation === "parent_category")
        score += 0.25;
    else if (relation === "near_match")
        score += 0.2;
    else if (relation === "broader_candidate")
        score += 0.1;
    // Actionability bonus
    if (actionability === "clickable")
        score += 0.2;
    else if (actionability === "fillable")
        score += 0.15;
    else if (actionability === "selectable")
        score += 0.1;
    else
        score -= 0.15; // static/readable penalty
    ...
    if (candidate.nearbyText && pendingStepTexts) {
        ...
        if (maxStepOverlap > 0.3)
            score += maxStepOverlap * 0.15;
    }
    return { score: Math.round(Math.max(score, -0.5) * 100) / 100, actionability, semanticRelation: relation };
}
function scoreObject(obj, failedTarget, semanticGoal) {
    const text = [obj.key, obj.name].filter(Boolean).join(" ");
    let score = 0;
    if (failedTarget)
        score += computeTokenOverlapScore(failedTarget, text) * 0.5;
    if (semanticGoal?.targetConcept)
        score += computeTokenOverlapScore(semanticGoal.targetConcept, text) * 0.3;
    if (obj.confidence)
        score += obj.confidence * 0.1;
    return Math.round(score * 100) / 100;
}
function scoreRoute(route, failedTarget, semanticGoal) {
    ...
    if (semanticGoal?.targetConcept)
        score += computeTokenOverlapScore(semanticGoal.targetConcept, text) * 0.3;
    if (semanticGoal?.intent)
        score += computeTokenOverlapScore(semanticGoal.intent, text) * 0.2;
    return Math.round(score * 100) / 100;
}
function scorePlan(plan, failedTarget, semanticGoal) {
...
function buildRouteRecoveryPack(input) {
    const budget = { ...codex_auto_repair_types_1.DEFAULT_PLANNING_BUDGET, ...(input.budget ?? {}) };
    ...
    function stepTarget(s) {
        if (typeof s.target === "object" && s.target !== null) {
            return s.target.name ?? s.target.value ?? undefined;
        }
        return undefined;
    }
    ...
    const isAssertAction = (a) => a === "assertVisible" || a === "assertText" || a === "assertUrl";
    const semanticGoal = deriveSemanticGoal(input.scenario, allSteps.map((s) => ({ action: s.action, target: stepTarget(s) })), allSteps.filter((s) => isAssertAction(s.action)).map((s) => ({ target: stepTarget(s) })), input.failedTarget);
    ...
    const pendingStepTexts = pendingSteps.map((s) => s.target ?? s.action).filter(Boolean);
...
function computeRouteRecoveryPackStats(pack) {
    return {

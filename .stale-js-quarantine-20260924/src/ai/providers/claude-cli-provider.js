...
class ClaudeCliProvider {
    ...
    buildPromptContent(systemMessage, userMessage, jsonSchema) {
        const parts = [];
        if (systemMessage)
            parts.push("SYSTEM:\n" + systemMessage);
        if (userMessage)
    ...
    spawnClaude(prompt, cwd, timeoutMs) {
    ...
    async completeJson(request) {
        ...
        try {
            ...
            if (result.exitCode !== 0) {
                ...
                throw new ai_provider_types_1.AiProviderError("claude_cli_execution_failed", `Claude CLI exited with code ${result.exitCode}`, {
                    provider: this.providerName,
                    purpose,
            ...
            if (!stdoutTrimmed) {
                ...
                throw new ai_provider_types_1.AiProviderError("claude_cli_empty_output", "Claude CLI returned empty stdout", {
                    provider: this.providerName,
                    purpose,
                    exitCode: result.exitCode,
            ...
            if (!extractionResult.success) {
                ...
                throw new ai_provider_types_1.AiProviderError("ai_provider_output_missing", "Claude CLI did not return valid JSON", {
                    provider: this.providerName,
                    purpose,
                    artifactDir,
                    exitCode: result.exitCode,
    ...
    extractJsonFromSources(stdout) {
        ...
        if (stdoutTrimmed) {
            try {
                ...
                if (typeof parsed === "object" && parsed !== null && !Array.isArray(parsed)) {
                    return { success: true, raw: stdoutTrimmed, parsed: parsed, attempts };
                }
            }
            catch {
        ...
        if (fencedJsonMatch) {
            try {
                ...
                if (typeof parsed === "object" && parsed !== null && !Array.isArray(parsed)) {
                    return { success: true, raw: fencedJsonMatch[1], parsed: parsed, attempts };
                }
            }
            catch {
        ...
        if (fencedGenericMatch) {
            try {
                ...
                if (typeof parsed === "object" && parsed !== null && !Array.isArray(parsed)) {
                    return { success: true, raw: fencedGenericMatch[1], parsed: parsed, attempts };
                }
            }
            catch {
                // Invalid JSON
    ...
    async saveDebugArtifacts(stdout, stderr, messages, purpose) {
        try {

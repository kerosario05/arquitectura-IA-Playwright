...
class CopilotCliProvider {
    ...
    buildCopilotPromptContent(systemMessage, userMessage, jsonSchema) {
        const parts = [];
        if (systemMessage)
            parts.push("SYSTEM:\n" + systemMessage);
        if (userMessage)
    ...
    spawnCopilot(fullPrompt, promptPath, tempDir, timeoutMs) {
        ...
        const env = { ...process.env, COPILOT_MODEL: this.model };
        let spawnCmd = command;
        let spawnArgs = args;
        let spawnOptions = { env, cwd: tempDir };
    ...
    async completeJson(request) {
        ...
        try {
            ...
            if (result.exitCode !== 0) {
                ...
                throw new ai_provider_types_1.AiProviderError("copilot_cli_execution_failed", `Copilot CLI exited with code ${result.exitCode}`, {
                    provider: this.providerName,
                    purpose,
            ...
            if (!stdoutTrimmed) {
                ...
                throw new ai_provider_types_1.AiProviderError("copilot_cli_empty_output", "Copilot CLI returned empty stdout", {
                    provider: this.providerName,
                    purpose,
                    exitCode: result.exitCode,
            ...
            const extractionResult = await this.extractJsonFromSources(result.stdout, result.stderr, "", purpose);
            if (!extractionResult.success) {
                ...
                throw new ai_provider_types_1.AiProviderError("ai_provider_output_missing", this.getOutputMissingErrorMessage(purpose), {
                    provider: this.providerName,
                    purpose,
                    artifactDir,
                    exitCode: result.exitCode,
            ...
            if (purpose === "scenario_generation") {
                ...
                if (!shapeValidation.valid) {
                    ...
                    console.log(`[scenarios:ai] invalidShape receivedKeys=${receivedKeys} ` +
                    ...
                    throw new ai_provider_types_1.AiProviderError("ai_provider_invalid_json", `AI provider returned JSON but scenario shape is invalid: ${shapeValidation.reason}`, {
                        provider: this.providerName,
                        purpose,
                        artifactDir,
                        exitCode: result.exitCode,
    ...
    async extractJsonFromSources(stdout, stderr, outputPath, purpose) {
        const attempts = [];
    ...
    async saveDebugArtifacts(stdout, stderr, outputPath, messages, purpose) {
        try {
            ...
            await fs.writeFile(path.join(artifactDir, "messages.json"), JSON.stringify(sanitizedMessages, null, 2), "utf-8");
    ...
    async writeInputFiles(tempDir, outputPath, promptPath, systemMessage, userMessage, purpose) {
    ...
    buildFileOutputPrompt(outputPath, promptPath, systemMessage, userMessage, purpose) {
        if (purpose === "repair") {
    ...
    buildGenericFileOutputPrompt(outputPath, promptPath, systemMessage, userMessage) {
        const lines = [
            "TASK: Write exactly ONE file at the absolute path below with valid JSON.",
            "",
            `OUTPUT FILE (absolute path): ${outputPath}`,
        ...
        lines.push("", "INSTRUCTION: Analyze the SYSTEM CONTEXT and USER REQUEST above, then generate the appropriate JSON response.", `Write the JSON object to: ${outputPath}`, "Do NOT write any other files. Do NOT write explanations.", "", "IMPORTANT: The file must contain ONLY valid JSON. No text before or after the JSON object.");
        return lines.join("\n");
    }
    ...
    getOutputMissingErrorMessage(purpose) {
        switch (purpose) {
            case "repair":
    ...
    getInvalidJsonErrorMessage(purpose) {
    ...
    tryExtractJsonFromStdout(stdout) {
        if (!stdout?.trim())
            return null;
        ...
        try {
            ...
            if (typeof parsed === "object" && parsed !== null && !Array.isArray(parsed)) {
                return { raw: trimmed, parsed: parsed };
            }
        }
        catch {
        ...
        if (fencedMatch) {
            try {
                ...
                if (typeof parsed === "object" && parsed !== null && !Array.isArray(parsed)) {
                    return { raw: fencedMatch[1], parsed: parsed };
                }
            }
            catch {
                // Invalid JSON in fenced block
        ...
        if (braceMatch) {
            try {
                ...
                if (typeof parsed === "object" && parsed !== null && !Array.isArray(parsed)) {
                    return { raw: braceMatch[0], parsed: parsed };
                }
            }
            catch {
                // Invalid JSON

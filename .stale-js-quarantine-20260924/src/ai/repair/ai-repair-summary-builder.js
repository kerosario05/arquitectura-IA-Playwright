...
exports.buildAiRepairCaseSummary = buildAiRepairCaseSummary;
exports.buildAiRepairBatchSummary = buildAiRepairBatchSummary;
exports.formatAiRepairConsoleOutput = formatAiRepairConsoleOutput;
exports.formatAiRepairBatchConsoleOutput = formatAiRepairBatchConsoleOutput;
const ai_repair_metrics_1 = require("./ai-repair-metrics");
function buildAiRepairCaseSummary(steps, appSlug) {
    ...
    for (const step of stepsWithAiRepair) {
        ...
        if (diag.providerName)
            summary.providerName = diag.providerName;
        if (diag.model)
            summary.model = diag.model;
        // Count decision statuses
        const status = diag.decisionStatus;
        if (status === "repaired_plan")
            summary.decisionCounts.repaired_plan++;
        else if (status === "no_safe_action")
            summary.decisionCounts.no_safe_action++;
        else if (status === "needs_more_context")
            summary.decisionCounts.needs_more_context++;
        else if (status === "invalid_response")
            summary.decisionCounts.invalid_response++;
        else if (status === "provider_error")
            summary.decisionCounts.provider_error++;
        else if (status === "provider_disabled")
            summary.decisionCounts.provider_disabled++;
        // Count validation statuses
        const validation = diag.validationStatus;
        if (validation === "valid")
            summary.validationCounts.valid++;
        else if (validation === "blocked")
            summary.validationCounts.blocked++;
        else if (validation === "invalid")
            summary.validationCounts.invalid++;
        else if (validation === "error")
            summary.validationCounts.error++;
        // Count repair types
        const repairType = diag.repairType ?? "unknown";
        if (repairType === "target_resolution")
            summary.repairTypeCounts.target_resolution++;
        else if (repairType === "route_recovery")
            summary.repairTypeCounts.route_recovery++;
        else if (repairType === "assertion_resolution")
            summary.repairTypeCounts.assertion_resolution++;
        else if (repairType === "pom_method_missing")
            summary.repairTypeCounts.pom_method_missing++;
            ...
            summary.repairTypeCounts.missing_intermediate_step++;
        else
            summary.repairTypeCounts.unknown++;
...
function buildAiRepairBatchSummary(caseSummaries) {
    const batch = (0, ai_repair_metrics_1.createEmptyBatchSummary)();
    ...
    batch.providerErrors = casesWithRepair.reduce((sum, c) => sum + c.summary.decisionCounts.provider_error, 0);
    batch.invalidResponses = casesWithRepair.reduce((sum, c) => sum + c.summary.decisionCounts.invalid_response, 0);
...
function formatAiRepairConsoleOutput(summary) {
...
function formatAiRepairBatchConsoleOutput(batch) {

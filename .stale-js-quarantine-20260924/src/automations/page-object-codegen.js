...
exports.sanitizeMethodName = sanitizeMethodName;
exports.buildPageObjectClassSource = buildPageObjectClassSource;
exports.generatePageObjectCandidateFiles = generatePageObjectCandidateFiles;
...
const INTENT_LOCATOR_TEMPLATES = {
    start_session: [
...
function buildLocatorStub(method) {
...
function buildMethodStub(method) {
...
function buildPageObjectClassSource(candidate) {
    const className = sanitizeClassName(candidate.className);
    const warnings = [];
    const filteredMethods = candidate.methods.filter((m) => {
        if (m.status === "active" && m.available)
            return false;
        const warning = (0, pom_ownership_1.getMisplacedMethodWarning)(className, m.name, m.intent);
        if (warning) {
    ...
    const needsPromotedSpecHelpers = filteredMethods.some((m) => ["open_product_information", "select_category", "select_operation", "start_session"].includes(m.intent) ||
    ...
    lines.push("");
    const classSpecific = pom_ownership_1.CLASS_SPECIFIC_METHODS[className];
...
async function generatePageObjectCandidateFiles(options) {
    const appProfile = {
    ...
    const registry = await (0, page_object_registry_1.loadPageObjectRegistry)(appProfile, options.outputRoot);
    const appPaths = (0, app_profile_1.buildAppAutomationPaths)(appProfile, undefined, options.outputRoot);
    ...
    for (const candidate of filtered) {
        ...
        const candidateFilePath = node_path_1.default.join(pagesDir, fileName);
        try {
            ...
            const misplacedMethods = candidate.methods.filter((m) => {
                if (m.status === "active" && m.available)
                    return false;
                return (0, pom_ownership_1.getMisplacedMethodWarning)(candidate.className, m.name, m.intent) !== null;
            });
            for (const m of misplacedMethods) {
                const preferredOwner = (0, pom_ownership_1.getPreferredOwnerForIntent)(m.intent);
                if (!existingClassNames.has(preferredOwner)) {
                    result.warnings.push(`Method '${m.name}' (intent: ${m.intent}) filtered from ${candidate.className} and no owner candidate '${preferredOwner}' exists. ` +
            ...
            await promises_1.default.writeFile(candidateFilePath, source, "utf-8");
            candidate.filePath = candidateFilePath.replace(/\\/g, "/");
            const now = new Date().toISOString();
            candidate.codegenMetadata = {
                generatedAt: now,
    ...
    if (!options.dryRun && (result.generated > 0 || result.errors.length > 0)) {
        await (0, page_object_registry_1.savePageObjectRegistry)(registry, appProfile, options.outputRoot);
    }
    return result;
}

...
function isOptionalOrInformational(step) {
...
function evaluatePromotionGate(input) {
    ...
    const policy = input.promotionPolicy ?? automation_promotion_types_1.DEFAULT_PROMOTION_POLICY;
    if (!plan) {
    ...
    const eligibleFromPartialDiscovery = discovery.status === "discovered_partial"
        && plan.status === "validated"
        && validation.valid
        && blockingSteps.length === 0;
    if (discovery.status !== "discovered_passed"
    ...
    if (discovery.failedReason) {
        ...
        if (plan.status === "validated" && validation.valid) {
            ...
            const hasUnresolvedBlocking = discovery.steps.some((s) => !isOptionalOrInformational(s) && isBlockingStep(s));
    ...
    if (input.observableOracles && input.observableOracles.length > 0) {
        const unresolvedRequired = input.observableOracles.filter((oracle) => oracle.type === "unsupported_or_unresolved" && oracle.backed === false);
        if (unresolvedRequired.length > 0) {
            ...
            reasons.push(`Required observable oracle unresolved (${unresolvedRequired.length}): ` +
    ...
    if (plan.metadata?.authFlowRequired && input.specContent) {
        ...
        if (!hasAuthFlowImport || !hasAuthFlowCall) {
            reasons.push(`Plan requires AuthFlow (authGate detected during discovery at step ${plan.metadata.authFlowInsertionAfterStepIndex ?? "unknown"}), ` +
    ...
    const evidenceJsonPath = discovery.evidenceJsonPath
        ?? (discovery.evidenceDir ? (0, node_path_1.join)(discovery.evidenceDir, "evidence.json") : undefined);
    if (evidenceJsonPath && (0, node_fs_1.existsSync)(evidenceJsonPath)) {
        try {
            const evidenceJsonContent = (0, node_fs_1.readFileSync)(evidenceJsonPath, "utf-8");
            ...
            if (detailEvidence?.required === true) {
                ...
                console.log(`[promotion-gate] evidence-gate check evidenceJsonPath=${evidenceJsonPath} ` +
                ...
                if (!isDetailEvidence) {
                    ...
                    reasons.push(`Evidence gate failed: Detail screenshot was required but not captured. ` +
                        `Target="${detailEvidence.target ?? "unknown"}". ` +
                        `Reason: ${detailEvidence.reason ?? "missing_detail_screenshot"}`);
                    pomStatus = "needs_manual_review";
                }
                else if (!detailActuallyOpened) {
                    reasons.push(`Evidence gate failed: Detail screen did not open. ` +
                        `Target="${detailEvidence.target ?? "unknown"}". ` +
                    ...
                    console.log(`[promotion-gate] evidence-gate passed detailScreenshot captured and validated ` +
                        `path=${detailEvidence.screenshotPath}`);
                }
            }
        }
        catch (err) {
            warnings.push(`Evidence gate check failed to read evidence.json: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    else if (evidenceJsonPath) {
        console.log(`[promotion-gate] evidence-gate skipped (evidence.json not found) path=${evidenceJsonPath}`);
    }
    return {

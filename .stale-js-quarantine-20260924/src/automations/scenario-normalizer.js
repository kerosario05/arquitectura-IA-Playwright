...
exports.stripStepNumbering = stripStepNumbering;
exports.buildCanonicalLabelRegistry = buildCanonicalLabelRegistry;
exports.buildCanonicalLabelMap = buildCanonicalLabelMap;
exports.canonicalizeText = canonicalizeText;
exports.normalizeScenarioEntryStepsOrder = normalizeScenarioEntryStepsOrder;
exports.buildCanonicalEntrySteps = buildCanonicalEntrySteps;
exports.normalizeEntrySteps = normalizeEntrySteps;
exports.normalizeScenario = normalizeScenario;
exports.normalizeVirtualCase = normalizeVirtualCase;
exports.filterUnsupportedClickTargets = filterUnsupportedClickTargets;
exports.ensureDetailScenarioHasItemSelection = ensureDetailScenarioHasItemSelection;
exports.convertUnsupportedPreOrdinalClicks = convertUnsupportedPreOrdinalClicks;
exports.validateVirtualCases = validateVirtualCases;
exports.applyFinalCanonicalization = applyFinalCanonicalization;
const app_auto_resolver_1 = require("./app-auto-resolver");
Object.defineProperty(exports, "loadAppConfig", { enumerable: true, get: function () { return app_auto_resolver_1.loadAppConfig; } });
...
function stripStepNumbering(step) {
...
function shouldReplaceCanonicalEntry(current, next) {
    if (!current)
        return true;
    if (next.priority !== current.priority)
        return next.priority < current.priority;
    if (next.visual !== current.visual)
        return next.visual && !current.visual;
    return next.canonical.length > current.canonical.length;
}
function registerCanonicalLabel(registry, canonical, source) {
...
function buildCanonicalLabelRegistry(routeProfile, appConfig, options = {}) {
    ...
    if (routeProfile?.entry) {
        for (const e of routeProfile.entry) {
            if (e.visibleLabel)
                registerCanonicalLabel(registry, e.visibleLabel, "entry");
        }
    }
    if (routeProfile?.aliases) {
        for (const key of Object.keys(routeProfile.aliases)) {
            const val = routeProfile.aliases[key];
            if (typeof val === "string")
                registerCanonicalLabel(registry, val, "alias");
            else if (Array.isArray(val)) {
                for (const v of val) {
                    if (typeof v === "string")
                        registerCanonicalLabel(registry, v, "alias");
                }
            }
        }
    }
    if (routeProfile?.visibleControls) {
        for (const vc of routeProfile.visibleControls) {
            if (typeof vc === "string")
                registerCanonicalLabel(registry, vc, "visibleControl");
        }
    }
    if (includeDomainTerms && routeProfile?.domainTerms) {
        for (const key of Object.keys(routeProfile.domainTerms)) {
            const val = routeProfile.domainTerms[key];
            if (typeof val === "string")
                registerCanonicalLabel(registry, val, "domainTerm");
            else if (Array.isArray(val)) {
                for (const v of val) {
                    if (typeof v === "string")
                        registerCanonicalLabel(registry, v, "domainTerm");
                }
            }
        }
    }
    const appRouteProfile = (0, app_auto_resolver_1.getRouteProfileFromConfig)(appConfig);
    if (appRouteProfile) {
    ...
    if (appConfig?.routeProfiles && typeof appConfig.routeProfiles === "object") {
        const routeProfiles = appConfig.routeProfiles;
        const activeName = appConfig.activeRouteProfileName;
        if (activeName && routeProfiles[activeName]) {
            collectLabelsFromRouteProfile(routeProfiles[activeName], registry, { includeDomainTerms });
        }
        if (!activeName) {
            for (const rpName of Object.keys(routeProfiles)) {
                collectLabelsFromRouteProfile(routeProfiles[rpName], registry, { includeDomainTerms });
            }
        }
    }
    if (!visualOnly) {
...
function buildCanonicalLabelMap(routeProfile, appConfig, options = {}) {
...
function canonicalizeText(text, labelMap) {
    if (!text || labelMap.size === 0)
        return text;
...
function entryStepToCanonicalText(entryStep) {
...
function normalizeScenarioEntryStepsOrder(steps, entrySteps) {
    ...
    let alreadyFirst = 0;
    let deduped = 0;
    const placed = new Array(canonicalTexts.length).fill(false);
    const nonEntrySteps = [];
    ...
    for (let ci = 0; ci < placed.length; ci++) {
        if (placed[ci]) {
            const origIdx = steps.findIndex((s) => normalizeForComparison(stripStepNumbering(s)) === normalizedCanonical[ci]);
            if (origIdx !== ci)
                moved++;
        }
    }
...
function buildCanonicalEntrySteps(routeProfile, entrySteps) {
...
function normalizeEntrySteps(steps, routeProfile, entrySteps) {
    if (steps.length === 0)
        return { steps: [], deduped: 0 };
    ...
    const result = [...canonicalEntrySteps];
...
function normalizeScenario(scenario, routeProfile, appConfig, entrySteps) {
...
function normalizeVirtualCase(vc, routeProfile, appConfig, entrySteps) {
    const labelMap = buildCanonicalLabelMap(routeProfile, appConfig);
    let canonicalizedCount = 0;
    ...
    const { steps: normalizedSteps, deduped } = normalizeEntrySteps(canonicalizedSteps, routeProfile, entrySteps);
    for (let i = 0; i < canonicalizedSteps.length; i++) {
        if (canonicalizedSteps[i] !== vc.steps[i])
            canonicalizedCount++;
    }
...
function computeProfileContextStrength(entryCount, visibleControlsCount, aliasCount, domainTermCount, entryStepsCount) {
    const total = entryCount + visibleControlsCount + aliasCount + domainTermCount + entryStepsCount;
    if (total === 0)
        return "none";
    if (visibleControlsCount >= 4 || (visibleControlsCount >= 2 && aliasCount + domainTermCount >= 2))
        return "high";
    if (visibleControlsCount >= 1 || entryCount >= 1 || entryStepsCount >= 1)
        return "medium";
    return "low";
}
function filterUnsupportedClickTargets(steps, routeProfile, entrySteps) {
    if (!routeProfile) {
...
function hasSensitiveExecutableAction(steps) {
...
function hasPassiveSensitiveAssertion(steps) {
    return steps.some((step) => {
        const stripped = stripStepNumbering(step);
 ...
 */
function findRecoverableParentCategory(steps, routeProfile, entrySteps) {
...
function hasDetailAssertions(steps, expectedResult) {
...
function hasItemSelection(steps) {
    return steps.some((step) => {
        const stripped = stripStepNumbering(step);
        return SELECTION_PATTERNS.some((p) => p.test(stripped));
    });
}
function hasSensitiveAction(steps) {
    return steps.some((step) => {
        const stripped = stripStepNumbering(step).toLowerCase();
        return SENSITIVE_ACTION_PATTERNS.some((p) => p.test(stripped));
    });
}
function findDominantDomainTerm(routeProfile) {
    if (!routeProfile?.domainTerms)
        return null;
 ...
 */
function findMatchingTargetPath(assertionTargets, routeProfile) {
 ...
 */
function expandTargetPathToSteps(targetPath, routeProfile, entrySteps) {
    const expandedSteps = [];
    // Add entry steps
    if (entrySteps) {
        for (const es of entrySteps) {
...
function ensureDetailScenarioHasItemSelection(steps, expectedResult, routeProfile, entrySteps) {
    ...
    if (assertionTargets.length > 0) {
        ...
        if (match) {
            console.log(`[detail-route-expansion] found targetPath match for "${match.matchedLabel}" ` +
            ...
            console.log(`[detail-route-expansion] expanded targetPath to ${newSteps.length} steps ` +
            ...
            console.log(`[detail-route-expansion] no targetPath match found for assertion targets: ${assertionTargets.slice(0, 3).join(", ")}${assertionTargets.length > 3 ? "..." : ""}`);
        }
    }
    ...
    const allowedClickTargets = new Set();
    const addTarget = (t) => { const n = normalizeForComparison(t); if (n.length > 0)
        allowedClickTargets.add(n); };
    if (routeProfile?.entry)
        for (const e of routeProfile.entry)
            if (e.visibleLabel)
                addTarget(e.visibleLabel);
    ...
    if (routeProfile?.entry)
        for (const e of routeProfile.entry)
            if (e.visibleLabel)
                entryLabels.add(normalizeForComparison(e.visibleLabel));
    if (entrySteps)
        for (const es of entrySteps)
            entryLabels.add(normalizeForComparison(es.target));
...
function isOrdinalSelectionStep(step) {
    const stripped = stripStepNumbering(step);
 ...
 */
function isLikelyContextualTarget(target) {
    const t = target.trim();
    if (t.length > 20)
        return false;
    ...
    if (wordCount === 1 && afterNfd.length <= 7 && !/\b(de|del|para|por|en|con|sin|al)\b/i.test(lower))
        return true;
    // Two words, short total (<= 10 chars), no prepositions → likely flat qualifier
    if (wordCount === 2 && afterNfd.length <= 10 && !/\b(de|del|para|por|en|con|sin|al)\b/i.test(lower))
        return true;
    return false;
}
function convertUnsupportedPreOrdinalClicks(steps, routeProfile, entrySteps) {
    if (steps.length < 2)
        return { steps, converted: 0, diagnostics: [] };
    ...
    for (let i = 0; i < resultSteps.length - 1; i++) {
        ...
        if (!clickMatch)
            continue;
        if (!isOrdinalSelectionStep(nextStripped))
            continue;
        const target = clickMatch[1];
        const normalizedTarget = normalizeForComparison(target);
        ...
        if (!isLikelyContextualTarget(target))
            continue;
        ...
        const alreadyHasAssertion = resultSteps.some((s) => stripStepNumbering(s) === stripStepNumbering(validationStep));
...
function validateVirtualCases(cases, routeProfile, appConfig) {
    const issues = [];
 ...
 */
function applyFinalCanonicalization(cases, routeProfile, appConfig) {
    const labelMap = buildCanonicalLabelMap(routeProfile, appConfig);
    const labelRegistry = buildCanonicalLabelRegistry(routeProfile, appConfig);
    const diagnostics = [];
    let totalCanonicalized = 0;
    const findMatchedSource = (normalizedText) => {

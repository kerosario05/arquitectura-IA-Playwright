...
exports.buildSpecExecutionContract = buildSpecExecutionContract;
exports.validateSpecExecutionContract = validateSpecExecutionContract;
exports.extractAuthFlowAggregateBindings = extractAuthFlowAggregateBindings;
exports.computeTraceFidelity = computeTraceFidelity;
exports.computeExecutionContractMetrics = computeExecutionContractMetrics;
...
function oracleRequiresPolarity(type) {
    return type === "navigation_transition" || type === "url_state";
}
const CONTRACT_VERSION = "1.0";
function mapPlanAction(action) {
    switch (action) {
...
function buildSpecStepTarget(target) {
    if (!target || target === "APP_BASE_URL")
        return undefined;
    return {
...
function buildAuthGateMechanism(oracle) {
    if (oracle.type !== "auth_gate")
        return undefined;
    const details = oracle.details ?? {};
...
function buildNavigationTransitionMechanism(oracle) {
    if (oracle.type !== "navigation_transition")
        return undefined;
    const details = oracle.details ?? {};
...
function buildOracleMechanism(oracle) {
    return buildAuthGateMechanism(oracle) ?? buildNavigationTransitionMechanism(oracle);
}
function requiresOracleMechanism(oracleType) {
    return oracleType === "auth_gate" || oracleType === "navigation_transition";
}
function findSourceActionStepIndex(oracle, planSteps) {
    const details = oracle.details ?? {};
    if (typeof details.sourceActionStepIndex === "number")
        return details.sourceActionStepIndex;
...
function resolveStepOracle(step, observableOracles, planSteps) {
    const evidenceRefs = [];
    ...
    const composites = observableOracles.filter((oracle) => isCompositeObservableState(oracle)
        && oracle.backed
...
function resolveImplementationDescriptor(step, registry) {
    const runtimeMethodByAction = {
        click: "clickPromotedTarget",
        fill: "fillPromotedField",
        select: "selectPromotedItem",
    };
    const runtimeMethod = runtimeMethodByAction[step.action];
    ...
    const runtimeImplementation = runtimeMethod && runtimeTarget
        ? { kind: "runtime", runtimeMethod }
        : undefined;
    if (!registry || registry.pageObjects.length === 0)
    ...
    if (runtimeImplementation && step.action === "fill")
        return runtimeImplementation;
    const semanticIntent = (0, pom_classification_1.deriveSemanticMethodIntent)(step, "unknown", []);
    const resolved = (0, page_object_registry_1.findMethodBySemanticIntent)(registry, semanticIntent, step);
    if (resolved) {
        ...
        if (resolved.method.intent !== semanticIntent)
            return undefined;
        ...
        if (step.action === "click" && stepTarget && (!targetBinding || (0, semantic_text_normalization_1.normalizeSemanticText)(targetBinding) !== (0, semantic_text_normalization_1.normalizeSemanticText)(stepTarget))) {
            return undefined;
        }
    ...
    const owner = (0, pom_ownership_1.getPreferredOwnerForIntent)(semanticIntent);
    const methodName = pom_ownership_1.METHOD_INTENT_NAME_MAP[semanticIntent];
    if (owner && owner !== "GenericPage" && methodName) {
        ...
        if (method) {
            if (method.intent !== semanticIntent)
                return undefined;
...
function resolveAuthContext(plan, sourceScenario) {
    ...
    if (!authRequired && !gateDetected)
        return undefined;
    const scenarioSteps = sourceScenario?.steps ?? [];
    const authValuePlanSteps = plan.steps.filter((step) => typeof step.valueKey === "string" && step.valueKey.startsWith("auth."));
    const authValueScenarioIndexes = new Set();
    for (const planStep of authValuePlanSteps) {
        const compatible = scenarioSteps.find((scenarioStep) => scenarioStep.index === planStep.index
            || (scenarioStep.action && planStep.description && (0, semantic_text_normalization_1.normalizeSemanticText)(scenarioStep.action) === (0, semantic_text_normalization_1.normalizeSemanticText)(planStep.description)));
        if (compatible)
            authValueScenarioIndexes.add(compatible.index);
    }
    ...
    if (authSubmitPlanStep) {
        const compatible = scenarioSteps.find((scenarioStep) => scenarioStep.index === authSubmitPlanStep.index
            || (scenarioStep.action && authSubmitPlanStep.description && (0, semantic_text_normalization_1.normalizeSemanticText)(scenarioStep.action) === (0, semantic_text_normalization_1.normalizeSemanticText)(authSubmitPlanStep.description)));
        if (compatible)
            authValueScenarioIndexes.add(compatible.index);
    }
...
function classifyScenarioAction(action, canonicalAssertion, conditionalAction) {
    if (conditionalAction)
        return conditionalAction.operation;
    if (canonicalAssertion)
        return "assertState";
    const lower = (action ?? "").trim().toLowerCase();
    if (!lower)
        return "noop";
    if (/^(noop|screenshot)$/.test(lower))
        return "noop";
    if (/^(navigate|navegar|ir\s+a|visitar|abrir\s+url)\b/.test(lower))
        return "navigate";
    if (/^(login|iniciar\s+sesi[oó]n|loguear|autenticar)\b/.test(lower))
        return "login";
    if (/^(click|clic|tocar|pulsar|presionar|tap|abrir|acceder)\b/.test(lower))
        return "click";
    if (/^(fill|ingresar|escribir|completar|llenar|digitar|type)\b/.test(lower))
        return "fill";
    if (/^(select|elegir|seleccionar)\b/.test(lower))
        return "select";
    if (/^(check|marcar|tildar|uncheck|desmarcar)\b/.test(lower))
        return "click";
    if (/^(wait|esperar)\b/.test(lower))
        return "wait";
    if (/^(asserturl|validar\s+url)\b/.test(lower))
        return "assertUrl";
    if (/^(asserttext)\b/.test(lower))
        return "assertText";
    if (/^(assert|assertvisible|validar|validate|verificar|verify|comprobar|confirmar|expect|mostrar|muestra|visible|se\s+muestr|aparezc|existe|existan)\b/.test(lower))
        return "assertVisible";
    return "noop";
}
function extractSelectSemanticTarget(text) {
...
function buildScenarioStepTarget(scenarioStep, operation) {
...
function resolveScenarioStepOracle(scenarioStep, observableOracles, planSteps) {
...
function findCompatiblePlanStep(scenarioStep, operation, planSteps) {
    ...
    const byIndex = planSteps.find((step) => step.index === scenarioStep.index && mapPlanAction(step.action) === operation);
    if (byIndex) {
        const indexedTarget = getStepTargetValue(byIndex);
        if (!normalizedScenarioTarget || normalizeOracleMatch(indexedTarget) === normalizedScenarioTarget)
            return byIndex;
    ...
    const isPlanOperationCompatible = (step) => mapPlanAction(step.action) === operation
        || (operation === "select"
            && step.action === "click"
            && (/\bselect\b/i.test(step.description ?? "") || step.target?.toString().includes("selection_keyboard_typeahead") === true));
    const planTargetMatchesScenario = (step) => {
    ...
    if (leadingNavigateOffset > 0) {
        const shifted = planSteps.find((step) => step.index === scenarioStep.index + leadingNavigateOffset
    ...
    if (scenarioStep.valueKey) {
        const byValueKey = planSteps.filter((step) => isPlanOperationCompatible(step) && step.valueKey === scenarioStep.valueKey);
        if (byValueKey.length === 1)
            return byValueKey[0];
    }
    // 2. Same operation + same normalized business target.
    if (normalizedScenarioTarget) {
        const byTarget = planSteps.filter((step) => {
            if (!isPlanOperationCompatible(step))
                return false;
    ...
    const byDescription = planSteps.filter((step) => {
        if (!step.description || normalizeOracleMatch(step.description) !== normalizedScenarioAction)
            return false;
...
function resolveResolvedExecutionTarget(scenarioStep, operation, observableOracles) {
    if (operation !== "select")
        return undefined;
    for (const oracle of observableOracles) {
        ...
        const sourceIdx = typeof oracle.details?.sourceActionStepIndex === "number"
            ? oracle.details.sourceActionStepIndex
            : oracle.stepIndex;
        if (sourceIdx !== scenarioStep.index)
            continue;
...
function hasCanonicalRequiredness(sourceScenario, scenarioStepIndex, scenarioStep) {
    ...
    const directText = (0, semantic_text_normalization_1.normalizeSemanticText)(`${scenarioStep?.action ?? ""} ${scenarioStep?.description ?? ""} ${scenarioStep?.expected ?? ""}`);
    const canonicalTextMatch = canonicalRequirements.some((requirement) => {
        if (requirement.coverability === "nonAutomatable")
            return false;
        const requirementText = (0, semantic_text_normalization_1.normalizeSemanticText)(requirement.description);
        return requirementText.length > 0 && (directText.includes(requirementText) || requirementText.includes(directText));
    });
    if (refs.length === 0)
        return canonicalTextMatch;
    const claims = sourceScenario?.stepClaims ?? [];
    return refs.every((ref) => {
        const claim = claims.find((candidate) => (candidate.stepIndex === ref.stepIndex || candidate.stepIndex === scenarioStepIndex || candidate.stepIndex === scenarioStepIndex - 1)
...
function buildSpecExecutionContract(plan, sourceScenario, options = {}) {
    ...
    const steps = requiredScenarioSteps.map((scenarioStep, index) => {
        ...
        const sourceHasAssertionAuthority = Boolean(sourceScenario
        ...
        const discoveredStatus = sourceScenario?.stepStatuses?.find((item) => item.index === scenarioStep.index)?.status;
        ...
        console.log(`[execution-contract-step] contractStepIndex=${index} scenarioStepIndex=${scenarioStep.index} operation=${operation} scenarioTarget="${scenarioTargetText}" implementationSource=${implementationSource} planMatched=${Boolean(planStep)} required=${required} executionStatus=${executionStatus}`);
...
function validateSpecExecutionContract(contract) {
    const errors = [];
    ...
    if (aggregate) {
        ...
        for (const scenarioStepIndex of aggregate.coveredScenarioStepIndices) {
            ...
            if (!step)
                errors.push(`auth_aggregate_unknown_step:${scenarioStepIndex}`);
            else if (step.required === false)
                errors.push(`auth_aggregate_non_required_step:${scenarioStepIndex}`);
        }
    }
...
function isTechnicalEvidenceMetadata(value) {
    ...
    if (!normalized)
        return false;
    if (TECHNICAL_EVIDENCE_KEYS.has(normalized))
        return true;
    if (TECHNICAL_EVIDENCE_KEYS.has(normalized.split(":")[0]))
        return true;
...
function extractRuntimeTargetValue(body) {
    const stringForm = /\btarget\s*:\s*(['"])((?:\\.|(?!\1)[^])*)\1/.exec(body);
    if (stringForm)
        return (0, semantic_text_normalization_1.decodeJsStringLiteralBody)(stringForm[2]);
    const objectForm = /\btarget\s*:\s*\{[^{}]*?\bvalue\s*:\s*(['"])((?:\\.|(?!\1)[^])*)\1/.exec(body);
    if (objectForm)
        return (0, semantic_text_normalization_1.decodeJsStringLiteralBody)(objectForm[2]);
    return undefined;
}
function extractBalancedBraces(body, startIdx) {
    ...
    while (i < body.length) {
        ...
        if (ch === "{")
            ...
            depth -= 1;
            if (depth === 0)
                return body.slice(startIdx + 1, i);
        }
        i += 1;
    }
    return undefined;
...
function extractActionCallbackBody(callBody) {
    ...
    while ((keyMatch = actionKey.exec(callBody)) !== null) {
        ...
        while (i < callBody.length && /\s/.test(callBody[i]))
            i += 1;
...
function extractQualifiedMethodCalls(body) {
    const calls = [];
        ...
        if (ch === '"' || ch === "'") {
            ...
            if (j < body.length && inCall && depth - callDepth >= 0) {
                if (value.trim().length > 0)
                    literals.push((0, semantic_text_normalization_1.decodeJsStringLiteralBody)(value));
            }
...
function extractDirectNavigationCalls(specContent) {
    const calls = [];
...
function isBootstrapNavigation(navigation, traceContext) {
    if (traceContext.hasContractNavigateStep)
        return false;
    if (traceContext.firstFunctionalStepPosition === undefined)
        return false;
    return navigation.position < traceContext.firstFunctionalStepPosition;
}
function extractAuthFlowCompletionCalls(specContent) {
    const calls = [];
...
function extractAuthFlowAggregateBindings(specContent) {
...
function getContractTargetValue(step) {
    return step.target?.value ?? step.target?.name ?? step.target?.role;
}
function computeTraceFidelity(specContent, contract) {
    const errors = [];
    ...
    const callsByStepIndex = new Map();
    ...
    const validAggregateCall = aggregate
        ? authCompletions.filter((call) => call.bindingId === aggregate.bindingId
        ...
        : [];
    const aggregateCoverage = validAggregateCall.length === 1 ? declaredAggregateCoverage : new Set();
    let implemented = 0;
    let lastPosition = -1;
    if (aggregate) {
        ...
        if (validAggregateCall.length === 1)
            implemented += aggregate.coveredScenarioStepIndices.length;
    }
    ...
    for (const step of contract.steps) {
        ...
        if (expectedTarget) {
            const targetPreserved = calls.some((call) => call.target && normalizeOracleMatch(call.target) === normalizeOracleMatch(expectedTarget));
        ...
        if (step.operation === "select" && step.resolvedExecutionTarget) {
            ...
            let candidateResolvedTarget;
            let callbackFound = false;
            let rawBodyLength = 0;
            let candidateLiterals = [];
            for (const call of calls) {
                ...
                if (literals.some((lit) => (0, semantic_text_normalization_1.semanticallyEqualText)(lit, step.resolvedExecutionTarget))) {
        ...
        if (step.operation.startsWith("assert") && step.oracle) {
            const oraclePreserved = calls.some((call) => call.method === "expectPromotedVisible" || call.method === "expectPromotedState");
            ...
            if (expectedMethod) {
                ...
                for (const call of calls) {
                    ...
                    for (const qc of extractQualifiedMethodCalls(callbackBody)) {
                        ...
                        if (qc.method === expectedMethod)
                            methodImplemented = true;
                    }
                }
                if (!methodImplemented) {
                    errors.push(`callback_implementation_mismatch:stepIndex=${step.scenarioStepIndex}:expectedOwner=${impl.owner}:expectedMethod=${impl.method}:observedCalls=${[...new Set(observed)].join(",") || "none"}`);
                }
            }
        }
        for (const call of calls) {
    ...
    const firstFunctionalStepPosition = runtimeCalls.length > 0
        ...
        : undefined;
    const bootstrapNavigations = directNavigations.filter((navigation) => isBootstrapNavigation(navigation, { firstFunctionalStepPosition, hasContractNavigateStep }));
...
function computeExecutionContractMetrics(contract) {
    return {

...
exports.rewritePromotedRuntimeImport = rewritePromotedRuntimeImport;
exports.rewritePromotedAuthFlowHelperImport = rewritePromotedAuthFlowHelperImport;
exports.validatePromotedSpecInternalImports = validatePromotedSpecInternalImports;
exports.buildSemanticCoverageDiagnostics = buildSemanticCoverageDiagnostics;
exports.normalizeMojibakeUtf8 = normalizeMojibakeUtf8;
exports.materializePromotedOracleDescriptors = materializePromotedOracleDescriptors;
exports.materializeAuthFlowAggregateBinding = materializeAuthFlowAggregateBinding;
exports.buildPromotedOracleImplementations = buildPromotedOracleImplementations;
exports.structuralValidation = structuralValidation;
exports.repairMissingExpectImport = repairMissingExpectImport;
exports.summarizePlaywrightDiscoveryError = summarizePlaywrightDiscoveryError;
exports.defaultRunTypeScriptValidation = defaultRunTypeScriptValidation;
exports.defaultRunPlaywrightDiscovery = defaultRunPlaywrightDiscovery;
exports.runHybridSpecGeneration = runHybridSpecGeneration;
...
const ai_config_resolver_1 = require("../ai/ai-config-resolver");
const promoted_spec_runtime_1 = require("./runtime/promoted-spec-runtime");
...
function rewritePromotedRuntimeImport(specContent, specFilePath) {
    const sourceRoot = node_path_1.default.resolve(process.cwd(), "src");
    const runtimeModulePath = node_path_1.default.resolve(sourceRoot, "automations/runtime/promoted-spec-runtime");
    const runtimeImportPath = node_path_1.default.relative(node_path_1.default.dirname(specFilePath), runtimeModulePath).replace(/\\/g, "/");
    const rewrittenRuntime = specContent.replace(/import\s+\{\s*createPromotedSpecRuntime\s*\}\s+from\s+['"][^'"]+['"];?/, `import { createPromotedSpecRuntime } from '${runtimeImportPath}';`);
    return rewrittenRuntime.replace(/(from\s+['"])([^'"]+)(['"])/g, (fullImport, prefix, importPath, suffix) => {
        const normalizedImportPath = importPath.replace(/\\/g, "/");
        const srcMarker = normalizedImportPath.lastIndexOf("/src/");
        if (srcMarker < 0)
            return fullImport;
        const targetPath = node_path_1.default.join(sourceRoot, normalizedImportPath.slice(srcMarker + "/src/".length));
        const relativeImportPath = node_path_1.default.relative(node_path_1.default.dirname(specFilePath), targetPath)
            .replace(/\\/g, "/")
            .replace(/\.(ts|tsx|js)$/i, "");
        return `${prefix}${relativeImportPath}${suffix}`;
    });
}
function rewritePromotedAuthFlowHelperImport(specContent, specFilePath, appSlug) {
    const helperModulePath = node_path_1.default.resolve(process.cwd(), "automations/apps", appSlug, "flows/auth.flow.helpers");
    const relativeImportPath = node_path_1.default.relative(node_path_1.default.dirname(specFilePath), helperModulePath)
        .replace(/\\/g, "/");
    const importPath = relativeImportPath.startsWith(".")
        ? relativeImportPath
        : `./${relativeImportPath}`;
    return specContent.replace(/import\s+\{\s*resolvePromotedSpecAuthDataFromEnv\s*\}\s+from\s+['"][^'"]+['"];?/, `import { resolvePromotedSpecAuthDataFromEnv } from '${importPath}';`);
}
async function validatePromotedSpecInternalImports(specContent, specFilePath) {
    const imports = Array.from(specContent.matchAll(/from\s+['"]([^'"]+)['"]/g))
        .map((match) => match[1])
        .filter((importPath) => Boolean(importPath))
        .filter((importPath) => /(^|[\\/])src[\\/]/.test(importPath.replace(/\\/g, "/")));
    const unresolved = [];
    for (const importPath of imports) {
        const basePath = node_path_1.default.resolve(node_path_1.default.dirname(specFilePath), importPath);
        const candidates = [basePath, `${basePath}.ts`, `${basePath}.tsx`, `${basePath}.js`, node_path_1.default.join(basePath, "index.ts")];
...
function parseRepairMaxAttempts() {
...
function getFailedSpecValidationNames(validation) {
    const failed = [];
    if (validation.structure === "failed")
        failed.push("structuralValidation");
    if (validation.traceFidelity === "failed")
        failed.push("traceFidelityValidation");
    if (validation.semanticCoverage === "failed")
        failed.push("semanticCoverage");
    if (validation.typescript === "failed")
        failed.push("typescriptValidation");
    if (validation.playwrightDiscovery === "failed")
        failed.push("playwrightDiscovery");
    if (validation.functionalExecution === "failed")
        failed.push("functionalExecution");
    return failed;
}
const SPEC_VALIDATION_GATES = ["structuralValidation", "traceFidelityValidation", "semanticCoverage", "typescriptValidation", "playwrightDiscovery", "functionalExecution"];
const SPEC_VALIDATION_KEY_BY_GATE = {
...
function getPassedGatesAttempt1(diagnostics) {
    const failed = new Set(diagnostics.failedGatesAttempt1);
    return SPEC_VALIDATION_GATES.filter((gate) => !failed.has(gate));
}
function buildSpecRepairConstraints(diagnostics, repairContext, specInputMode = "legacy") {
...
function parseStepIndexesFromErrors(errors) {
...
function buildSpecRepairContext(input) {
    ...
    const prioritizedErrors = isExecutionContract
        ...
        : [
            ...input.diagnostics.errors.filter((error) => error.startsWith("missing_assertion_oracle_context:")
    ...
    const technicalTargetErrors = input.diagnostics.errors.filter((error) => error.startsWith("technical_metadata_used_as_runtime_target:"));
    const technicalTargetFailure = isExecutionContract
        ...
        : (() => {
            ...
            const oracleTypes = Array.from(new Set(input.observableOracles
    ...
    const relevantObservableOracles = isExecutionContract
        ...
        : (() => {
            ...
            const byMissingRequirement = input.observableOracles.filter((oracle) => input.missingRequirements.some((requirement) => normalizeText(requirement.text).includes(normalizeText(oracle.requirement))
                || normalizeText(oracle.requirement).includes(normalizeText(requirement.text))));
            if (byMissingRequirement.length > 0)
                return byMissingRequirement;
            const byMissingAssertion = input.observableOracles.filter((oracle) => exactErrors.some((error) => error.startsWith("missing_assertion_coverage:")
                && normalizeText(error).includes(normalizeText(oracle.requirement))));
            if (byMissingAssertion.length > 0)
                return byMissingAssertion;
...
function buildSemanticCoverageDiagnostics(input) {
    ...
    for (const requirement of input.requiredAssertions) {
        ...
        const linkedOracle = input.observableOracles.find((oracle) => normalizeText(oracle.requirement).includes(normalizedRequirement)
            || normalizedRequirement.includes(normalizeText(oracle.requirement)));
        const referencedBySemanticError = input.semanticErrors.some((error) => normalizeText(error).includes(normalizedRequirement));
        if (referencedBySemanticError) {
            const scenarioStep = input.scenarioSteps.find((step) => normalizeText(step.action).includes(normalizedRequirement)
...
function summarizeRepairGateError(failedGate, exactErrors) {
    const byGate = (() => {
        if (failedGate === "semanticCoverage") {
            return exactErrors.find((error) => error.startsWith("missing_assertion_oracle_context:")
                || error.startsWith("missing_assertion_coverage:")
                || error.startsWith("assertion_without_observed_evidence:")
        ...
        if (failedGate === "playwrightDiscovery") {
            return exactErrors.find((error) => error.startsWith("playwright_discovery_error:")
                || error.startsWith("playwright_discovery_failed:"));
        }
...
function shouldAttemptSpecRepair(diagnostics, repairState, specInputMode) {
    if (repairState.attempt >= repairState.maxAttempts)
        return false;
    if (diagnostics.mode !== "ai_hybrid")
        return false;
    if (specInputMode !== "execution_contract" && specInputMode !== "legacy")
        return false;
    if (getFailedSpecValidationNames(diagnostics.validation).length === 0)
        return false;
...
function mergeRepairDiagnostics(previous, repaired, repairAttempt) {
...
function finalizeSpecAttemptMetrics(diagnostics, repairState, passed) {
...
function decodeSingleMojibakeLevel(value) {
    const bytes = [];
...
function normalizeMojibakeUtf8(value) {
    ...
    for (let i = 0; i < MAX_MOJIBAKE_ITERATIONS; i += 1) {
        ...
        if (decoded === null || decoded === current)
            break;
        if (countNonAscii(decoded) >= countNonAscii(current))
            break;
        current = decoded;
        iterations += 1;
    }
...
function readAssignedStringLiteral(content, variable) {
    const assignment = new RegExp(`process\\.env\\.${variable}\\s*=\\s*(['"])((?:\\\\.|(?!\\1)[^])*)\\1\\s*;`).exec(content);
    return assignment ? (0, semantic_text_normalization_1.decodeJsStringLiteralBody)(assignment[2]) : undefined;
}
function normalizeLiteralMatch(value) {
    return (0, semantic_text_normalization_1.normalizeSemanticText)(value).replace(/\s+/g, " ").trim();
}
...
function summarizeSkillState(skillState) {
...
function buildResponseArtifact(providerResponse, diagnostics) {
    return {
        ...(providerResponse ?? {}),
        _finalSpec: diagnostics.finalSpec
    };
}
function buildSpecGenerationSystemPrompt() {
    return [
...
function buildSpecGenerationUserPrompt(input) {
    ...
    sections.push(JSON.stringify({
        promotedRuntimeMethodsAllowlist: promoted_spec_runtime_1.PROMOTED_SPEC_RUNTIME_PUBLIC_METHODS,
    }, null, 2));
...
const PROMOTED_SPEC_RUNTIME_API_DESCRIPTOR = {
...
function buildContractStepBindings(contract) {
    const lines = [];
    let steps = 0;
    for (const step of contract.steps) {
        ...
        if (runtimeTarget)
            lines.push(`runtimeTarget=${runtimeTarget}`);
        if (step.valueKey)
            lines.push(`valueKey=${step.valueKey}`);
        if (step.entityScope)
            lines.push(`entityScope=${step.entityScope}`);
        if (step.rowRelation)
            lines.push(`rowRelation=${step.rowRelation}`);
        if (step.selectionField)
            lines.push(`selectionField=${step.selectionField}`);
        if (step.technicalTargetRef)
            lines.push(`technicalTargetRef=${step.technicalTargetRef}`);
        if (step.technicalTargetRefs?.length)
            lines.push(`technicalTargetRefs=${step.technicalTargetRefs.join(" || ")}`);
        if (step.resolvedExecutionTarget)
            lines.push(`resolvedExecutionTarget=${step.resolvedExecutionTarget}`);
        ...
        if (step.oracle) {
            ...
            if (expectedUrl)
                lines.push(`expectedUrl=${expectedUrl}`);
            if (step.oracle.polarity)
                lines.push(`polarity=${step.oracle.polarity}`);
...
function materializePromotedOracleDescriptors(specContent, implementations) {
    let materialized = specContent;
    for (const implementation of implementations) {
        if (implementation.implementationKind !== "url_state")
            continue;
        ...
        while ((match = callRegex.exec(materialized)) !== null) {
            ...
            if (!stepIndex || Number(stepIndex[1]) !== implementation.scenarioStepIndex)
                continue;
...
function materializeAuthFlowAggregateBinding(specContent, binding) {
    if (!binding)
        return specContent;
    ...
    if (/\bcontractBinding\s*:/.test(rawBody))
        return specContent;
...
function buildSpecGenerationUserPromptFromContract(input) {
    ...
    console.log(`[spec-prompt-blocks] skill=${input.skill?.content.trim().length ?? 0} ` +
...
function getSpecGenerationInputMode() {
    const mode = process.env.AI_SPEC_INPUT_MODE?.trim().toLowerCase();
    if (mode === "legacy")
        return "legacy";
    return "execution_contract";
}
function getScenarioId(plan, preferred) {
    if (preferred && preferred.trim())
        return preferred.trim();
    if (plan.scenario.externalId?.trim())
        return plan.scenario.externalId.trim();
    return `C${plan.scenario.caseId ?? ""}`;
}
function getSectionSlug(preferred) {
...
function extractRequiredAssertionsFromPlan(plan) {
    const assertions = [];
    for (const step of plan.steps) {
        if (step.action !== "assertText" && step.action !== "assertVisible" && step.action !== "assertUrl")
            continue;
...
function dedupeRequirements(values) {
    const seen = new Set();
    const deduped = [];
...
function normalizeObservableOracleType(value) {
    switch ((value ?? "").trim()) {
...
function normalizeObservableOracles(input) {
    ...
    for (let i = 0; i < input.length; i += 1) {
        ...
        if (!oracle || typeof oracle.requirement !== "string")
            continue;
...
function buildRequiredAssertions(plan, sourceScenario) {
...
function buildScenarioRequiredRequirements(input) {
    ...
    const add = (requirement) => {
        ...
        console.log(`[spec-requirement] id=${requirement.id} source=${requirement.source} type=${requirement.type} ` +
    ...
    for (const assertion of context.observedAssertions) {
        const authorized = requirements.some((requirement) => normalizeText(requirement.text) === normalizeText(assertion)
    ...
    console.log(`[spec-contract-requirements] total=${requirements.length} required=${requiredCount} backed=${backedCount} ` +
        `unresolved=${unresolvedCount}`);
    return requirements;
}
function buildSourceScenarioContext(plan, sourceScenario) {
...
function extractExecutableStepIndexes(plan) {
    return plan.steps
        .filter((s) => s.action !== "noop")
        .map((s) => s.index);
}
function buildAvailablePageObjects(registry, appPaths) {
    if (!registry)
        return [];
    return registry.pageObjects
        ...
        importPath: node_path_1.default.relative(node_path_1.default.dirname(appPaths.specPath ?? appPaths.caseDir ?? process.cwd()), po.filePath).replace(/\\/g, "/").replace(/\.(ts|tsx|js)$/i, ""),
        methods: po.methods
...
function toRelativeImportPath(fromFilePath, targetPathWithoutExt) {
    const raw = node_path_1.default.relative(node_path_1.default.dirname(fromFilePath), targetPathWithoutExt).replace(/\\/g, "/");
    if (raw.startsWith("."))
        return raw;
    return `./${raw}`;
}
function resolvePromotedRuntimeModulePath() {
    return node_path_1.default.resolve(process.cwd(), "src", "automations", "runtime", "promoted-spec-runtime.ts");
}
function resolveAuthGateDetectorModulePath() {
    return node_path_1.default.resolve(process.cwd(), "src", "discovery", "auth-gate-detector.ts");
}
function resolvePageScannerModulePath() {
    return node_path_1.default.resolve(process.cwd(), "src", "explorer", "page-scanner.ts");
}
...
function countCallArguments(raw) {
    const trimmed = raw.trim();
    if (!trimmed)
        return 0;
    return splitTopLevelArguments(trimmed).length;
}
function extractAuthFlowMethodSignaturesFromFile(content) {
 ...
 */
const AUTH_COMPLETION_ACTION_RE = /(iniciar sesi|autenticarse|loguearse|completar (la )?identificaci|ingresar (el )?(c[oó]digo )?otp|acceder al [aá]rea autenticad|completar (la )?autenticaci|complete (the )?(login|authentication)|do (the )?login|perform (the )?login)/i;
function extractStepTargetText(step) {
...
function resolveAuthOutcomeMode(input) {
    const explicitCompletionStep = (input.plan.steps ?? []).find((step) => step.action === "login"
    ...
    if (explicitCompletionStep) {
        if (input.authContractMode === "gate_observation") {
            console.log(`[spec-auth-invariant] override contract=gate_observation -> complete_authentication stepIndex=${explicitCompletionStep.index} authority=validated_auth_completion`);
        }
        return {
...
function buildPromotedOracleImplementations(observableOracles, authFlowContext) {
...
function buildObservedEvidenceCorpus(sourceScenario, plan) {
...
function hasUnexpectedKeys(obj, allowed) {
    return Object.keys(obj).filter((k) => !allowed.includes(k));
}
function detectLegacyContractKeys(obj) {
    return LEGACY_SPEC_KEYS.filter((key) => key in obj);
}
function validateSingleSpecShape(payload, allowScenarioId) {
    const errors = [];
    ...
    if (unexpected.length > 0)
        errors.push(`additionalProperties are not allowed: ${unexpected.join(", ")}`);
    if (typeof obj.specContent !== "string" || obj.specContent.trim().length === 0)
        errors.push("specContent must be a non-empty string");
    if (!Array.isArray(obj.coveredStepIndexes) || obj.coveredStepIndexes.some((n) => typeof n !== "number"))
        errors.push("coveredStepIndexes must be number[]");
    if (!Array.isArray(obj.coveredAssertions))
        errors.push("coveredAssertions must be array");
    if (!Array.isArray(obj.usedPageObjects))
        errors.push("usedPageObjects must be array");
    if (!Array.isArray(obj.declaredIdentifiers) || obj.declaredIdentifiers.some((v) => typeof v !== "string"))
        errors.push("declaredIdentifiers must be string[]");
    if (!Array.isArray(obj.unresolvedRequirements) || obj.unresolvedRequirements.some((v) => typeof v !== "string"))
        errors.push("unresolvedRequirements must be string[]");
    if (!Array.isArray(obj.warnings) || obj.warnings.some((v) => typeof v !== "string"))
    ...
    if (Array.isArray(obj.coveredAssertions)) {
        for (const item of obj.coveredAssertions) {
            ...
            const cast = item;
            if (typeof cast.requirement !== "string" || cast.requirement.trim().length === 0)
                errors.push("coveredAssertions.requirement must be non-empty string");
            if (typeof cast.implementation !== "string" || cast.implementation.trim().length === 0)
                errors.push("coveredAssertions.implementation must be non-empty string");
            const extra = hasUnexpectedKeys(cast, ["requirement", "implementation"]);
            if (extra.length > 0)
                errors.push(`coveredAssertions additionalProperties not allowed: ${extra.join(", ")}`);
        }
    }
    if (Array.isArray(obj.usedPageObjects)) {
        for (const item of obj.usedPageObjects) {
            ...
            const cast = item;
            if (typeof cast.className !== "string" || cast.className.trim().length === 0)
                errors.push("usedPageObjects.className must be non-empty string");
            if (!Array.isArray(cast.methods) || cast.methods.some((v) => typeof v !== "string"))
                errors.push("usedPageObjects.methods must be string[]");
            const extra = hasUnexpectedKeys(cast, ["className", "methods"]);
            if (extra.length > 0)
                errors.push(`usedPageObjects additionalProperties not allowed: ${extra.join(", ")}`);
        }
    }
    ...
    if (errors.length > 0)
        return { valid: false, errors, legacyContract: legacyKeys.length > 0 };
    return { valid: true, errors: [], value: obj, legacyContract: false };
}
function parseSpecGenerationResponse(payload, scenarioId, batchEnabled) {
    if (!batchEnabled) {
    ...
    if (extras.length > 0)
        return { valid: false, errors: [`batch additionalProperties not allowed: ${extras.join(", ")}`], legacyContract: false };
    if (!Array.isArray(root.specs))
        return { valid: false, errors: ["batch specs must be array"], legacyContract: false };
    const seen = new Set();
    let selected;
    const errors = [];
    ...
    for (const spec of root.specs) {
        ...
        if (entryScenarioId === scenarioId)
            selected = shape.value;
    }
    if (!selected)
        errors.push(`missing scenarioId in batch response: ${scenarioId}`);
    return errors.length > 0
        ? { valid: false, errors, legacyContract }
        : { valid: true, errors: [], value: selected, legacyContract: false };
}
function extractDeclaredIdentifiersFromSpec(specContent) {
    const ids = new Set();
...
function parseImports(specContent) {
    const imports = [];
...
async function resolveImportFilePath(specPath, importPath) {
    if (!importPath.startsWith("."))
        return undefined;
    const resolvedBase = node_path_1.default.resolve(node_path_1.default.dirname(specPath), importPath);
    const candidates = [
        resolvedBase,
        `${resolvedBase}.ts`,
        `${resolvedBase}.tsx`,
        `${resolvedBase}.js`,
        node_path_1.default.join(resolvedBase, "index.ts")
    ];
...
function extractRuntimeStepCalls(specContent) {
    const calls = [];
...
const TECHNICAL_EVIDENCE_KEY_SET = new Set(TECHNICAL_EVIDENCE_KEYS);
const TECHNICAL_EVIDENCE_KEY_SET_LOWER = new Set(TECHNICAL_EVIDENCE_KEYS.map((key) => key.toLowerCase()));
function normalizeTechnicalEvidenceTarget(value) {
    return value.trim().toLowerCase().replace(/^["']|["']$/g, "");
}
function isTechnicalEvidenceMetadata(value) {
    const normalized = normalizeTechnicalEvidenceTarget(value);
    if (!normalized)
        return false;
    if (TECHNICAL_EVIDENCE_KEY_SET_LOWER.has(normalized))
        return true;
    if (TECHNICAL_EVIDENCE_KEY_SET_LOWER.has(normalized.split(":")[0]))
        return true;
    for (const key of TECHNICAL_EVIDENCE_KEYS) {
        const lowerKey = key.toLowerCase();
        if (normalized === lowerKey || normalized.startsWith(`${lowerKey}:`) || normalized.endsWith(`:${lowerKey}`))
            return true;
    }
    return false;
}
function extractRuntimeTargetCalls(specContent) {
    const calls = [];
...
function hasRuntimeStepCall(specContent, stepIndex) {
    return extractRuntimeStepCalls(specContent).some((call) => call.stepIndex === stepIndex);
}
function hasRuntimeStepCallForMethod(specContent, stepIndex, method) {
    return extractRuntimeStepCalls(specContent).some((call) => call.stepIndex === stepIndex && call.method === method);
}
function validateExpectPromotedVisibleContracts(specContent) {
    const errors = [];
    const callRegex = /promotedRuntime\.expectPromotedVisible\s*\(\s*\{([\s\S]*?)\}\s*\)/gm;
    const assertionFunctionPattern = /\bassertion\s*:\s*(?:(?:async\s*)?(?:\([^)]*\)|[A-Za-z_]\w*)\s*=>|(?:async\s+)?function\b)/m;
    let match;
    let callIndex = 0;
...
function findRuntimeStatementByStepIndex(specContent, stepIndex, method) {
    const methodPattern = method ? escapeRegex(method) : "[A-Za-z_]\\w*";
    const regex = new RegExp(`await\\s+promotedRuntime\\.${methodPattern}\\s*\\(\\s*\\{[\\s\\S]*?stepIndex\\s*:\\s*${stepIndex}\\b[\\s\\S]*?\\}\\s*\\)\\s*;?`, "g");
...
function reorderGateStartOnlyLoginStepCall(specContent, loginStepIndexes, clickStepIndexes) {
    if (loginStepIndexes.length === 0 || clickStepIndexes.length === 0)
        return specContent;
    const loginStatement = loginStepIndexes
        ...
        .find((statement) => Boolean(statement));
    if (!loginStatement)
        return specContent;
...
function ensureNamedImport(specContent, symbol, importPath) {
    if (new RegExp(`\\b${escapeRegex(symbol)}\\b`).test(specContent)) {
        ...
        if (importRegex.test(specContent))
            return specContent;
    }
    ...
    if (firstImportEnd < 0)
        return `${importLine}${specContent}`;
    return `${specContent.slice(0, firstImportEnd + 1)}${importLine}${specContent.slice(firstImportEnd + 1)}`;
}
function normalizeGateStartOnlyAuthAssertion(specContent, loginStepIndex) {
...
function buildPromotedClickAction(step, target) {
...
function ensureGateStartOnlyClickSteps(specContent, loginStepIndexes, clickSteps) {
...
function filterUsedPageObjectsByInvocation(specContent, usedPageObjects) {
    const filtered = [];
    for (const item of usedPageObjects) {
        const instances = extractConstructedInstanceNames(specContent, item.className);
        const invokedMethods = item.methods.filter((method) => instances.some((instance) => extractMethodCallArgumentCounts(specContent, instance, method).length > 0));
...
function buildGateStartOnlyCanonicalSpec(params) {
    const runtimeImport = toRelativeImportPath(params.specPath, node_path_1.default.resolve(process.cwd(), "src", "automations", "runtime", "promoted-spec-runtime"));
    const scanImport = toRelativeImportPath(params.specPath, node_path_1.default.resolve(process.cwd(), "src", "explorer", "page-scanner"));
    const detectImport = toRelativeImportPath(params.specPath, node_path_1.default.resolve(process.cwd(), "src", "discovery", "auth-gate-detector"));
...
function extractPromotedRuntimeMethodCalls(specContent) {
    const calls = [];
...
function extractConstructedInstanceNames(specContent, className) {
    const names = [];
...
function extractMethodCallArgumentCounts(specContent, variableName, methodName) {
    const counts = [];
...
function extractAssertionTextLiterals(specContent) {
    const literals = [];
    const patterns = [
        /getByText\(\s*(?:\/([^/\r\n]+)\/[dgimsuy]*|["'`]([^"'`\r\n]+)["'`])\s*\)/g,
...
function isAuthFlowRequirement(requirement) {
...
async function validateImportContracts(input) {
    ...
    const trackedSymbols = new Set(input.availablePageObjects.map((po) => po.className));
    if (input.authFlowContext)
        trackedSymbols.add(input.authFlowContext.className);
    const usedMethodsByClass = new Map();
    for (const used of input.response.usedPageObjects) {
        usedMethodsByClass.set(used.className, new Set(used.methods));
    }
    const classInstanceNames = new Map();
    ...
    if (input.authFlowContext) {
        classInstanceNames.set(input.authFlowContext.className, extractConstructedInstanceNames(input.specContent, input.authFlowContext.className));
    }
    for (const imp of imports) {
        ...
        for (const symbol of imp.symbols) {
            if (symbol === "test" || symbol === "expect" || symbol === "createPromotedSpecRuntime")
                continue;
            ...
            if (!resolved)
                continue;
            const fileContent = await promises_1.default.readFile(resolved, "utf-8").catch(() => "");
            ...
            if (available) {
                ...
                const usedMethods = usedMethodsByClass.get(symbol) ?? new Set();
...
function normalizeGateOnlyCredentialBindings(specContent, candidateSpecPath, appSlug, authFlowContext) {
    if (authFlowContext?.authOutcomeMode !== "gate_start_only")
        return specContent;
    if (!/\bAPP_USERNAME\b|\bAPP_PASSWORD\b|fillUsername\s*\(|fillPassword\s*\(|submitLogin\s*\(/.test(specContent)) {
        return specContent;
    }
    const helperPath = node_path_1.default.resolve(process.cwd(), "automations/apps", appSlug, "flows", "auth.flow.helpers.ts");
    ...
    const formPagePath = node_path_1.default.resolve(process.cwd(), "automations/apps", appSlug, "pages", "form.page.ts");
...
function materializeBackedNavigationAssertions(specContent, response, observableOracles) {
    const backedNavigation = observableOracles.filter((oracle) => oracle.backed
    ...
    if (backedNavigation.length === 0)
        return { specContent, response };
    let next = ensurePlaywrightExpectImport(specContent);
    const addedAssertions = [...response.coveredAssertions];
    for (const oracle of backedNavigation) {
        const stepIndex = oracle.stepIndex;
        if (extractRuntimeStepCalls(next).some((call) => call.method === "expectPromotedVisible" && call.stepIndex === stepIndex))
            continue;
...
function isValidAuthAggregateBinding(specContent, expected) {
    if (!expected)
        return false;
    const bindings = (0, spec_execution_contract_1.extractAuthFlowAggregateBindings)(specContent);
...
function structuralValidation(input) {
    ...
    if (input.executionContract) {
        for (const contractStep of input.executionContract.steps) {
            ...
            const invoked = instanceNames.some((instanceName) => extractMethodCallArgumentCounts(input.specContent, instanceName, implementation.method).length > 0);
            if (!invoked) {
                semanticErrors.push(`page_object_method_semantic_mismatch:step=${contractStep.scenarioStepIndex}`
    ...
    if (!content.includes("createPromotedSpecRuntime("))
        errors.push("missing_createPromotedSpecRuntime");
    if (!content.includes("finally"))
        errors.push("missing_finally_block");
    if (!content.includes("finishEvidence("))
        errors.push("missing_finishEvidence_call");
    if (content.includes("page.waitForTimeout("))
        errors.push("forbidden_page_waitForTimeout_detected");
    ...
    for (const [variable, expectedValue] of expectedMetadata) {
        ...
        const semanticEqual = actualValue !== undefined && (0, semantic_text_normalization_1.semanticallyEqualText)(actualValue, expectedValue);
        if (!semanticEqual)
            errors.push(`missing_metadata_line:${variable}`);
    ...
    const authAssertionCoveredByFlow = (requirement) => Boolean((input.authFlowContext?.required || input.authFlowContext?.gateDetected)
    ...
    const availableByClass = new Map();
    for (const po of input.availablePageObjects) {
    ...
    for (const used of input.response.usedPageObjects) {
        ...
        for (const method of used.methods) {
            if (!methods.has(method))
                errors.push(`unknown_page_object_method:${used.className}.${method}`);
        }
    }
    const imports = [...content.matchAll(/import\s+\{\s*([A-Za-z_]\w*)\s*\}\s+from\s+['"][^'"]*pages\/[^'"]+['"]/g)];
    for (const imp of imports) {
        const className = imp[1];
        if (!availableByClass.has(className))
            errors.push(`imported_unknown_page_object:${className}`);
    ...
    for (const id of input.response.declaredIdentifiers) {
        if (!declared.has(id))
            errors.push(`undeclared_identifier:${id}`);
    }
    if (input.response.unresolvedRequirements.length > 0) {
        for (const requirement of input.response.unresolvedRequirements) {
            ...
            if (input.authFlowContext?.required
                ...
                && (isValidAuthAggregateBinding(content, input.authFlowContext.aggregateBinding)
                    || (input.authFlowContext.authOutcomeMode === "gate_start_only"
                        && runtimeStepCalls.some((call) => call.method === "expectPromotedVisible"
    ...
    if (input.authFlowContext && (input.authFlowContext.required || input.authFlowContext.gateDetected)) {
        ...
        if (authMode === "complete_authentication") {
            if (!content.includes("new AuthFlow(page)"))
                errors.push("missing_auth_flow_instance");
            if (!content.includes("authFlow.ensureAuthenticated("))
                errors.push("missing_auth_flow_ensure_authenticated");
        ...
        if (input.authFlowContext.authOutcomeMode === "gate_start_only"
    ...
    if (input.mode === "ai_hybrid") {
        ...
        for (const stepIndex of input.executableStepIndexes) {
            if (!covered.has(stepIndex))
                semanticErrors.push(`missing_step_coverage:${stepIndex}`);
            ...
            const implemented = (isNavigateStep && /await\s+page\.goto\(/.test(content))
                || loginViaAuthFlow
    ...
    if (input.requiredAssertions.length > 0) {
        if (input.response.coveredAssertions.length === 0) {
            ...
            const oracleByRequirement = new Map(input.observableOracles.map((oracle) => [normalizeLiteralMatch(oracle.requirement), oracle]));
                ...
                if (!covered) {
                    ...
                    if (linkedOracle) {
                        const resolvedTarget = typeof linkedOracle.details?.resolvedObservableTarget === "string"
                            ...
                            : linkedOracle.target;
                        semanticErrors.push(`missing_assertion_oracle_context:requirement=${required}|oracleType=${linkedOracle.type}|historicalRequirement=${linkedOracle.requirement}|resolvedTarget=${resolvedTarget ?? "unknown"}|backed=${linkedOracle.backed}`);
                    }
                }
            }
            if (input.mode === "ai_hybrid") {
    ...
    if (input.sourceExpectedResultPresent && input.requiredAssertions.length === 0) {
        const authCoversExpected = Boolean(input.authFlowContext
...
function resolvePlaywrightLaunchContext(input) {
...
function buildPlaywrightCommandEnv(launchContext) {
    if (!launchContext || launchContext.headless === null)
        return undefined;
    const envPatch = { HEADLESS: launchContext.headless ? "true" : "false" };
    if (launchContext.headless) {
        envPatch.PWDEBUG = "";
    }
    return envPatch;
}
async function runNodeCommand(command, args, envPatch) {
    try {
...
function parseImportClauses(specContent) {
    const clauses = [];
...
function stripImportStatements(specContent) {
    return specContent.replace(/import\s+[\s\S]*?\s+from\s+['"][^'"]+['"]\s*;?\s*/g, "");
}
function repairMissingExpectImport(specContent) {
    const contentWithoutImports = stripImportStatements(specContent);
    if (!/\bexpect\s*\(/.test(contentWithoutImports))
        return specContent;
    if (/\b(?:const|let|var|function|class)\s+expect\b/.test(contentWithoutImports)
    ...
    const namedBindings = compatibleImport[1]
        ...
        .filter(Boolean);
    if (namedBindings.some((binding) => /^expect(?:\s+as\s+\w+)?$/.test(binding)))
        return specContent;
    const replacement = compatibleImport[0].replace(compatibleImport[1], `${compatibleImport[1].replace(/\s*$/, "")}, expect `);
    return specContent.slice(0, compatibleImport.index)
        + replacement
        + specContent.slice(compatibleImport.index + compatibleImport[0].length);
}
function summarizePlaywrightDiscoveryError(stdout, stderr) {
    const lines = `${stdout}\n${stderr}`
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter((line) => line.length > 0);
    const useful = lines.find((line) => line !== "Listing tests:"
...
function buildTypeValidationSource(specContent) {
    ...
    const declarations = new Set([
        ...
        ...promoted_spec_runtime_1.PROMOTED_SPEC_RUNTIME_PUBLIC_METHODS.map((method) => `  ${method}(...args: any[]): Promise<unknown>;`),
        "}",
    ...
    for (const imp of imports) {
        for (const named of imp.namedBindings) {
            if (named === "test" || named === "expect" || named === "createPromotedSpecRuntime")
                continue;
...
async function defaultRunTypeScriptValidation(specPath) {
    try {
        ...
        const validationPath = node_path_1.default.join(node_path_1.default.dirname(specPath), `.spec-validation-${process.pid}-${Date.now()}.ts`);
        await promises_1.default.writeFile(validationPath, buildTypeValidationSource(specContent), "utf-8");
        try {
            const tscPath = node_path_1.default.resolve(process.cwd(), "node_modules", "typescript", "bin", "tsc");
            ...
            await promises_1.default.rm(validationPath, { force: true }).catch(() => undefined);
        }
    }
    catch (error) {
        return {
...
async function defaultRunPlaywrightDiscovery(specPath, launchContext) {
    const cliPath = node_path_1.default.resolve(process.cwd(), "node_modules", "@playwright", "test", "cli.js");
    const sourcePath = node_path_1.default.resolve(specPath);
    const sourceDir = node_path_1.default.dirname(sourcePath);
    const isCandidate = node_path_1.default.basename(sourceDir).toLowerCase() === "spec-generation";
    ...
    if (isCandidate) {
        const physicalCaseDir = node_path_1.default.dirname(sourceDir);
        const candidateText = await promises_1.default.readFile(sourcePath, "utf8");
        const rebased = candidateText.replace(/(from\s*["'])(\.{1,2}[\\/][^"']+)(["'])/g, (full, prefix, importPath, suffix) => {
            const candidateResolved = node_path_1.default.resolve(sourceDir, importPath);
            const physicalResolved = node_path_1.default.resolve(physicalCaseDir, importPath);
            const target = (0, node_fs_1.existsSync)(candidateResolved) || (0, node_fs_1.existsSync)(`${candidateResolved}.ts`)
                ? candidateResolved
                : physicalResolved;
            const rebasedPath = node_path_1.default.relative(physicalCaseDir, target).replace(/\\/g, "/").replace(/\.ts$/i, "");
            return `${prefix}${rebasedPath.startsWith(".") ? rebasedPath : `./${rebasedPath}`}${suffix}`;
        });
        validationPath = node_path_1.default.join(physicalCaseDir, `.candidate-validation-${process.pid}-${Date.now()}.spec.ts`);
        await promises_1.default.writeFile(validationPath, rebased, "utf8");
    }
    try {
        return await runNodeCommand(process.execPath, [cliPath, "test", validationPath.replace(/\\/g, "/"), "--list", "--config", "playwright.config.ts"], buildPlaywrightCommandEnv(launchContext));
    }
    finally {
        if (validationPath !== sourcePath)
            await promises_1.default.rm(validationPath, { force: true }).catch(() => undefined);
    }
}
function parseFunctionalExecutionOutput(output) {
...
async function defaultRunFunctionalExecution(specPath, launchContext) {
    const cliPath = node_path_1.default.resolve(process.cwd(), "node_modules", "@playwright", "test", "cli.js");
...
async function runHybridSpecGenerationInternal(input, deps, repairState) {
    const now = deps?.now ?? (() => Date.now());
    const startedAt = now();
    let lastUserPromptContent;
    let specInputMode = "legacy";
    ...
    const executionContract = (0, spec_execution_contract_1.buildSpecExecutionContract)(input.plan, input.sourceScenario, {
        appSlug: input.appProfile.appSlug,
        sectionSlug,
        pageObjectRegistry: input.pageObjectRegistry
    });
    const contractValidation = (0, spec_execution_contract_1.validateSpecExecutionContract)(executionContract);
    const contractMetrics = (0, spec_execution_contract_1.computeExecutionContractMetrics)(executionContract);
    ...
    const requiredRequirements = [
        ...sourceRequirements,
        ...unresolvedContractAssertions.filter((candidate) => !sourceRequirements.some((existing) => existing.scenarioStepIndex === candidate.scenarioStepIndex
            && existing.assertionLike
            && existing.backed === false)),
    ];
    const requiredAssertions = dedupeRequirements(requiredRequirements
        .filter((requirement) => requirement.required && requirement.backed && requirement.assertionLike)
        .map((requirement) => requirement.text));
    const unresolvedRequiredRequirements = requiredRequirements.filter((requirement) => requirement.required && !requirement.backed && requirement.assertionLike);
    ...
    const authFlowContext = (await buildAuthFlowRuntimeContext(input.appPaths, sourceScenario.auth, input.plan, authContractMode, executionContract.auth?.aggregate)) ?? null;
    if (authFlowContext) {
        console.log(`[spec-auth-outcome] mode=${authFlowContext.authOutcomeMode === "gate_start_only" ? "gate_observation" : "flow_execution"} source=${authFlowContext.authOutcomeSource} reason=${authFlowContext.authOutcomeReason} stepIndex=${authFlowContext.authOutcomeStepIndex ?? "-"}`);
    }
    ...
    const artifactsDir = input.appPaths.caseDir
        ? node_path_1.default.join(input.appPaths.caseDir, "spec-generation")
        : node_path_1.default.join(process.cwd(), ".artifacts", "spec-generation", `${input.appProfile.appSlug}-${scenarioId}`);
    const candidateSpecPath = node_path_1.default.join(artifactsDir, "candidate.spec.ts");
    ...
    if (!contractValid) {
        ...
        diagnostics.errors.push(...contractValidation.errors, ...unresolvedRequiredRequirements.map((requirement) => `promotion_oracle_gate:unresolved_required_requirement:${requirement.text}:oracleType=${requirement.oracleType ?? "unresolved"}:source=${requirement.source}`));
        ...
        await promises_1.default.writeFile(node_path_1.default.join(artifactsDir, "request-summary.json"), JSON.stringify(sanitizeForArtifact({
            appSlug: input.appProfile.appSlug,
            sectionSlug,
            scenarioId,
        ...
        await promises_1.default.writeFile(candidateSpecPath, sanitizeText(rewritePromotedRuntimeImport(input.deterministicDraft, candidateSpecPath)), "utf-8");
        await promises_1.default.writeFile(node_path_1.default.join(artifactsDir, "validation.json"), JSON.stringify(sanitizeForArtifact(diagnostics), null, 2), "utf-8");
    ...
    let providerForRepair = input.provider;
    ...
    if (aiEnabled) {
        try {
            skillState = await (0, spec_generation_skill_loader_1.loadSpecGenerationSkill)();
            diagnostics.skill = summarizeSkillState(skillState);
            let provider = input.provider;
            if (!provider) {
                const aiConfig = (0, ai_config_resolver_1.resolveSpecGenerationAiConfig)();
                ...
                provider = deps?.createProvider ? await deps.createProvider() : await (0, ai_provider_factory_1.createSpecGenerationAiProvider)();
            }
            else {
            ...
            if (!provider) {
                throw new ai_provider_types_1.AiProviderError("ai_provider_config_missing", "Spec generation provider is unavailable.");
            }
            ...
            let promotedRuntimeImport;
            try {
                await promises_1.default.access(promotedRuntimeModulePath);
            ...
            try {
                await promises_1.default.access(authGateDetectorModulePath);
                await promises_1.default.access(pageScannerModulePath);
            ...
            console.log(`[spec-prompt-size] systemChars=${systemPromptContent.length} skillChars=${promptSkillChars} contractChars=${promptContractChars} ` +
            ...
            providerResponse = (completion.parsedJson ?? {});
            ...
            if (!parsed.valid || !parsed.value) {
                ...
                if (parsed.legacyContract || !allowDeterministicFallback) {
                    ...
                    await promises_1.default.writeFile(node_path_1.default.join(artifactsDir, "response.json"), JSON.stringify(sanitizeForArtifact(buildResponseArtifact(providerResponse, diagnostics)), null, 2), "utf-8");
                    const candidatePath = node_path_1.default.join(artifactsDir, "candidate.spec.ts");
                    await promises_1.default.writeFile(candidatePath, sanitizeText(rewritePromotedRuntimeImport(candidate.specContent, candidatePath)), "utf-8");
                    await promises_1.default.writeFile(node_path_1.default.join(artifactsDir, "validation.json"), JSON.stringify(sanitizeForArtifact(diagnostics), null, 2), "utf-8");
            ...
            const code = error instanceof ai_provider_types_1.AiProviderError ? error.code : "ai_provider_http_error";
            ...
            if (skillFailure || !allowDeterministicFallback) {
                ...
                await promises_1.default.writeFile(node_path_1.default.join(artifactsDir, "response.json"), JSON.stringify(sanitizeForArtifact(buildResponseArtifact({ error: message, code }, diagnostics)), null, 2), "utf-8");
                const candidatePath = node_path_1.default.join(artifactsDir, "candidate.spec.ts");
                await promises_1.default.writeFile(candidatePath, sanitizeText(rewritePromotedRuntimeImport(candidate.specContent, candidatePath)), "utf-8");
                await promises_1.default.writeFile(node_path_1.default.join(artifactsDir, "validation.json"), JSON.stringify(sanitizeForArtifact(diagnostics), null, 2), "utf-8");
                return { promotionAllowed: false, specContent: candidate.specContent, diagnostics, artifactsDir, executionContract };
    ...
    executableSpecContent = materializeAuthFlowAggregateBinding(executableSpecContent, authFlowContext?.aggregateBinding);
    executableSpecContent = materializePromotedOracleDescriptors(executableSpecContent, promotedOracleImplementations);
    executableSpecContent = normalizeGateOnlyCredentialBindings(executableSpecContent, input.appPaths.specPath, input.appProfile.appSlug, authFlowContext ?? undefined);
    const materializedAssertions = materializeBackedNavigationAssertions(executableSpecContent, candidate, sourceScenario.observableOracles);
    ...
    await promises_1.default.writeFile(candidateSpecPath, executableSpecContent, "utf-8");
    let lastSemanticCoverageDiag;
    const evaluateSpecCandidate = async (specContent, response, options) => {
        const validation = {
        ...
        if (specInputMode === "execution_contract") {
            ...
            const aggregateCovered = isValidAuthAggregateBinding(specContent, authFlowContext?.aggregateBinding)
                ...
                : new Set();
            const missingSteps = requiredContractSteps.filter((step) => !aggregateCovered.has(step.scenarioStepIndex) && !implementedStepIndices.has(step.scenarioStepIndex));
            const retainedSemanticErrors = structure.semanticErrors.filter((error) => !error.startsWith("missing_step_coverage:")
        ...
        if (specInputMode === "execution_contract") {
            const requiredExecutionContract = {
                ...executionContract,
                steps: executionContract.steps.filter((step) => step.required !== false)
            };
            const traceResult = (0, spec_execution_contract_1.computeTraceFidelity)(specContent, requiredExecutionContract);
        ...
        await promises_1.default.writeFile(validationPath, persistedCandidate, "utf-8");
        const diskCandidate = await promises_1.default.readFile(validationPath, "utf-8").catch(() => "");
        ...
        const resolvedImportAbsolutePath = node_path_1.default.resolve(node_path_1.default.dirname(validationPath), `${runtimeImport}.ts`);
        const runtimeImportResolves = node_path_1.default.resolve(resolvedImportAbsolutePath) === node_path_1.default.resolve(runtimeModuleAbsolutePath)
            && await promises_1.default.access(resolvedImportAbsolutePath).then(() => true).catch(() => false);
                ...
                if (functionalExecutionEnabled && discoveryOk) {
                    ...
                    if (!functionalResult.ok) {
                        ...
                        const combinedLines = `${functionalResult.stdout}\n${functionalResult.stderr}`
                            ...
                            .map((line) => line.trim())
                            .filter((line) => line.length > 0);
                        const functionalErrorLine = combinedLines.find((line) => /\b(error|failed|timeout|expect|assert|locator|timed out)\b/i.test(line))
                            ?? combinedLines[0];
    ...
    if (!passed
        ...
        ].filter(([, status]) => status === "failed").map(([name]) => name);
    ...
    await promises_1.default.writeFile(candidateSpecPath, sanitizeText(rewritePromotedRuntimeImport(executableSpecContent, candidateSpecPath)), "utf-8");
...
async function runHybridSpecGeneration(input, deps) {
    return runHybridSpecGenerationInternal(input, deps, {
        attempt: 0,

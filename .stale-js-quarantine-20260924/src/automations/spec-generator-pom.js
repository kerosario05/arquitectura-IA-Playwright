...
exports.generatePOMSpecFromPlan = generatePOMSpecFromPlan;
...
const auth_flow_helpers_1 = require("../discovery/auth-flow-helpers");
 ...
 */
function buildPreviousStepsParam(steps, currentIndex, currentStepSemanticIntent) {
    ...
    for (let i = 0; i < currentIndex; i++) {
        const step = steps[i];
        let intent = step.semanticIntent;
        if (!intent) {
            const stepMetadata = step.actionIntent;
            if (stepMetadata) {
                intent = stepMetadata;
            }
            else {
        ...
        if (SAFE_INTENTS.has(intent) && !UNSAFE_INTENTS.has(intent)) {
            const targetValue = getTargetValue(step.target) || step.description || '';
            const isSensitive = step.sensitive || false;
            previousSteps.push(`{ stepIndex: ${step.index}, actionIntent: '${intent}', target: '${escapeSpecString(targetValue)}', sensitive: ${isSensitive} }`);
        }
    }
 ...
 * NOTE: return_to_list is NOT safe to replay - it consumes context (detail page), doesn't produce it
 */
function buildPreviousStepReplaysParam(steps, currentIndex, pageObjectRegistry, options) {
    const SAFE_INTENTS = new Set([
        'start_session', 'open_home', 'open_module', 'open_product_information',
        'select_category', 'select_product', 'select_visible_item_by_ordinal',
        'select_first_visible_item', 'select_first_visible_product', 'select_first_visible_card',
    ...
    for (let i = 0; i < currentIndex; i++) {
        ...
        let intent = step.semanticIntent;
        if (!intent) {
            const locatorStrategy = step.locatorStrategy;
            const ordinalDiag = step.recoveryMetadata?.ordinalSelectionDiagnostics;
        ...
        replayArrayItems.push(`{ stepIndex: ${step.index}, actionIntent: '${intent}', target: '${escapeSpecString(targetValue)}', sensitive: ${isSensitive}, replay: ${funcName} }`);
    }
    return {
        replayFunctions,
 ...
 */
function findLastSelectionStep(steps, currentIndex) {
...
function deriveReplayIntent(step) {
    let intent = step.semanticIntent;
    if (intent)
        return intent;
    const locatorStrategy = step.locatorStrategy;
...
function buildReplayBodyForStep(step) {
 ...
 */
function buildLastSelectionReplayParam(steps, currentIndex) {
...
function inferExpectedEffect(step, semanticIntent) {
...
function buildPageObjectImport(className, filePath, specPath) {
    const importPath = node_path_1.default.relative(node_path_1.default.dirname(specPath), filePath).replace(/\\/g, "/").replace(/\.ts$/, "");
    return `import { ${className} } from '${importPath}';`;
}
function normalizePageObjectFilePath(filePath, _appDir) {
    if (!filePath.includes(".candidate"))
        return filePath;
    return filePath.replace(/\.candidate\.ts$/, ".ts");
}
function isSelectionLikeStep(step) {
    ...
    for (const kw of selectionKeywords) {
        if (normalizedTarget.includes(kw))
            return true;
    }
...
function isModuleNavigationStep(step) {
    const targetValue = getTargetValue(step.target);
    const normalizedTarget = targetValue.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    const action = step.action.toLowerCase();
    if (action === "navigate" || action === "login")
        return false;
    if (action !== "click" && action !== "select")
        return false;
    if (isHomeRouteTarget(targetValue))
        return false;
...
function isHomeRouteTarget(target) {
...
function deriveSelectionIntent(step) {
    const recoveryMeta = step.recoveryMetadata;
    const semanticRole = recoveryMeta?.semanticRole;
    if (semanticRole === "category")
        return "select_category";
    if (["product", "card", "item", "entity"].includes(semanticRole))
        return "select_product";
    const targetValue = getTargetValue(step.target);
    const normalizedTarget = targetValue.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    ...
    if (/\b(primer[ao]?\s+(producto|item|registro|card|tarjeta|fila)|primera?\s+(tarjeta|card|fila)|first\s+visible\s+(item|product|card|row))\b/.test(normalizedTarget)) {
        if (/\b(fila|row)\b/.test(normalizedTarget))
            return "select_first_visible_row";
        if (/\b(tarjeta|card)\b/.test(normalizedTarget))
            return "select_first_visible_card";
        if (/\b(producto|product)\b/.test(normalizedTarget))
...
function deriveSelectionIntentWithOrdinalSupport(step) {
    const recoveryMeta = step.recoveryMetadata;
...
function findAuthGateStepIndex(steps) {
    for (let i = 0; i < steps.length; i++) {
        ...
        const targetValue = getTargetValue(step.target);
        if ((0, auth_flow_helpers_1.isLikelyAuthGate)(targetValue)) {
            return i;
        }
    }
    return -1;
 ...
 */
function validateContextChain(plan) {
    const errors = [];
    const producedContexts = new Set();
    for (let i = 0; i < plan.steps.length; i++) {
        const step = plan.steps[i];
        const semanticIntent = step.semanticIntent;
        const contextMetadata = step.contextMetadata;
        if (!semanticIntent)
            continue;
        // Track context produced by this step
        const producedContext = (0, pom_ownership_1.getContextProducedByIntent)(semanticIntent);
        if (producedContext) {
        ...
        if (contextMetadata?.isContextDependent || (0, pom_ownership_1.isContextDependentIntent)(semanticIntent)) {
            ...
            if (!hasRequiredContext && requiredContexts.length > 0) {
                const targetValue = getTargetValue(step.target);
                errors.push(`Context dependency violation at step ${step.index}: Action '${semanticIntent}' on target '${targetValue}' ` +
...
function generatePOMSpecFromPlan(plan, automationId, appProfile, appPaths, pageObjectRegistry, policy, inlineDebugMode, authFlowOptions, metadata) {
    const escapedTitle = escapeSpecString(plan.scenario.title);
    const usedPageObjects = [];
    const missingPageObjects = [];
    ...
    const actionLines = [];
    const preambleLines = [];
    const dataHelperLines = [];
    const requiredDataUsed = new Set();
    const importedClasses = new Set();
    const instantiatedVars = new Map();
    const declaredValueVars = new Set();
    const dataVarMap = (0, promoted_data_1.buildDataKeyVariableMap)(plan.steps
    ...
    function ensurePageObject(className, filePath) {
        const key = className;
    ...
    function ensureDataValue(step) {
        ...
        if (!declaredValueVars.has(varName)) {
            ...
            dataHelperLines.push(`const ${varName} = requirePromotedData(dataContext, '${escapeSpecString(step.valueKey)}', { fieldName: '${escapeSpecString(fieldName)}', stepIndex: ${step.index} });`);
        }
        return varName;
    }
    ...
    const preAuthActionLines = [];
    const postAuthActionLines = [];
    // First pass: collect all unique replay functions globally
    const allReplayFunctions = new Map();
    for (let stepIndex = 0; stepIndex < plan.steps.length; stepIndex++) {
        ...
        if (stepDesc.startsWith("authflow handled") || stepDesc.includes("step consumed by authflow"))
            continue;
        if (step.recoveryMetadata?.recoveredBy === "auth_flow")
            continue;
        if (step.recoveryMetadata?.recoveredBy === "contextual_intermediate_already_satisfied")
            continue;
    ...
    for (const step of plan.steps) {
        ...
        if (moduleNav) {
            ...
            if (isOrdinalSelection) {
                ...
                const hasExplicitSelectionDiagnostics = selectionDiagnostics?.selectionLike ||
                ...
                if (hasExplicitSelectionDiagnostics) {
                    ...
                    semanticIntent = (0, pom_classification_1.deriveMethodIntentFromStepWithContext)(step, plan.steps);
                }
            }
        ...
        let resolved = pageObjectRegistry
            ? (0, page_object_registry_1.findMethodBySemanticIntent)(pageObjectRegistry, semanticIntent, step)
            : undefined;
        let fallbackUsed = false;
        let fallbackInfo;
        if (!resolved && pageObjectRegistry) {
            const targetValue = getTargetValue(step.target);
            const expectedOwner = (0, pom_classification_1.deriveExpectedOwnerForStep)(step, plan.steps);
            if (!resolved && semanticIntent === "select_product") {
                const productListPO = pageObjectRegistry.pageObjects.find((po) => po.className === "ProductListPage" && po.status === "active") ?? pageObjectRegistry.pageObjects.find((po) => po.className === "ProductListPage" && po.status === "candidate");
                if (productListPO) {
                    const selectMethod = productListPO.methods.find((m) => m.name === "selectProduct" && m.status === "active" && m.available) ?? productListPO.methods.find((m) => m.name === "selectProduct" && m.status === "candidate" && m.available);
                    if (selectMethod) {
            ...
            if (!resolved && semanticIntent === "click_primary_action") {
                const detailPO = pageObjectRegistry.pageObjects.find((po) => po.className === "ProductDetailPage" && po.status === "active") ?? pageObjectRegistry.pageObjects.find((po) => po.className === "ProductDetailPage" && po.status === "candidate");
                if (detailPO) {
                    const clickMethod = detailPO.methods.find((m) => m.name === "clickPrimaryAction" && m.status === "active" && m.available) ?? detailPO.methods.find((m) => m.name === "clickPrimaryAction" && m.status === "candidate" && m.available);
                    if (clickMethod) {
            ...
            if (!resolved && semanticIntent === "start_session") {
                const homePO = pageObjectRegistry.pageObjects.find((po) => po.className === "HomePage" && po.status === "active") ?? pageObjectRegistry.pageObjects.find((po) => po.className === "HomePage" && po.status === "candidate");
                if (homePO) {
                    const startMethod = homePO.methods.find((m) => m.name === "start" && m.status === "active" && m.available) ?? homePO.methods.find((m) => m.name === "start" && m.status === "candidate" && m.available);
                    if (startMethod) {
            ...
            if (method.parameters.length > 0) {
                ...
                if (semanticIntent === "select_visible_item_by_ordinal") {
                    ...
                    const runtimeTarget = recoveryMeta?.selectedCandidateText
                        ? step.target?.metadata?.originalTarget || step.description || targetValue
                        : targetValue;
                    const args = [`"${ordinal}"`];
                    if (domainTerm) {
                    ...
                    const args = method.parameters.map((parameter, index) => {
                        ...
                        if (semanticIntent === "submit_login" && method.name === "loginWithCredentials") {
                            if (parameter.toLowerCase().includes("user"))
                                return "usuario_valido";
                            if (parameter.toLowerCase().includes("pass"))
                    ...
                    if (step.action === "fill") {
                        ...
                        const expectedOwnerPage = (0, pom_classification_1.deriveExpectedOwnerForStep)(step, plan.steps);
        ...
        if (!actionLine && resolved === undefined) {
            ...
            const expectedMethodName = pom_ownership_1.METHOD_INTENT_NAME_MAP[semanticIntent] || semanticIntent;
            const missingInfo = {
                target: targetValue,
                stepIndex: stepIndex,
            ...
            missingMethods.push(`step=${stepIndex} target="${targetValue}" ` +
    ...
    if (usedAuthFlow && authFlowOptions) {
        ...
        const authFlowAbsolutePath = node_path_1.default.resolve(process.cwd(), "automations/apps", appProfile.appSlug, "flows/auth.flow.ts");
        ...
        const authInstantiation = (0, auth_flow_helpers_1.buildAuthFlowInstantiation)();
    ...
    const lines = [];
    const promotedManifestPath = escapeSpecString(buildPortablePathFromCwd(node_path_1.default.join(appPaths.caseDir ?? node_path_1.default.dirname(appPaths.specPath ?? ""), "promoted-data.json")));
    ...
    if (usesPromotedRuntime) {
        const runtimeImportPath = escapeSpecString(buildPortablePathFromSpec(appPaths.specPath ?? "", node_path_1.default.resolve(process.cwd(), "src/automations/runtime/promoted-spec-runtime.ts")));
    ...
    if (requiredDataUsed.size > 0) {
        const envImportPath = escapeSpecString(buildPortablePathFromSpec(appPaths.specPath ?? "", node_path_1.default.resolve(process.cwd(), "src/config/env.ts")));
        const dataImportPath = escapeSpecString(buildPortablePathFromSpec(appPaths.specPath ?? "", node_path_1.default.resolve(process.cwd(), "src/data/index.ts")));
        const appProfileImportPath = escapeSpecString(buildPortablePathFromSpec(appPaths.specPath ?? "", node_path_1.default.resolve(process.cwd(), "src/automations/app-profile.ts")));
        const promotedDataImportPath = escapeSpecString(buildPortablePathFromSpec(appPaths.specPath ?? "", node_path_1.default.resolve(process.cwd(), "src/data/promoted-data.ts")));
    ...
    if (usedAuthFlow) {
        const authFlowHelpersPath = buildPortablePathFromSpec(appPaths.specPath ?? "", node_path_1.default.resolve(process.cwd(), "automations/apps", appProfile.appSlug, "flows", "auth.flow.helpers.ts"));
...
function validateSpecQuality(actionLines, plan, specContent) {
    const errors = [];
    ...
    for (const step of plan.steps) {
        ...
        if (stepDesc.startsWith("authflow handled") || stepDesc.includes("step consumed by authflow")) {
            const targetValue = getTargetValue(step.target);
            if (!targetValue || commonNavWords.has(targetValue.toLowerCase()))
                continue;
...
function validatePreAuthSteps(plan, specContent, usedAuthFlow) {
    const errors = [];
    if (!usedAuthFlow)
        return errors;
    let authGateIdx = -1;
...
function validateSelectionMapping(plan, actionLines) {
    const errors = [];
    for (const step of plan.steps) {
        if (!isSelectionLikeStep(step))
            continue;
        if (isSubmitLikeStep(step))
            continue;
        const stepDesc = (step.description ?? "").toLowerCase();
        if (stepDesc.startsWith("authflow handled") || stepDesc.includes("step consumed by authflow"))
            continue;
        ...
        if (!targetValue)
            continue;
        const hasSelectionCall = actionLines.some(l => /\.select(Option|Item|Card|Entity|Product|Recipient|ByCondition)\(/.test(l));
        const hasSelectIntentInPromotedTarget = actionLines.some(l => l.includes(`clickPromotedTarget`) &&
        ...
        const hasPrimaryActionOnSelection = actionLines.some(l => l.includes(`clickPrimaryAction`) && l.toLowerCase().includes(targetValue.toLowerCase()));
...
function validateNavigationDegradation(plan, actionLines) {
    const errors = [];
    for (const step of plan.steps) {
        if (step.action === "navigate" || step.action === "login")
            continue;
        if (!isModuleNavigationStep(step))
            continue;
        const targetValue = getTargetValue(step.target);
        if (!targetValue || isHomeRouteTarget(targetValue))
            continue;
        const degradedToOpenHome = actionLines.some(l => /\.open\(\)/.test(l) && !l.includes("// [target:"));
...
function findCandidateMethod(registry, intent, _description) {
    for (const po of registry.pageObjects) {
        const candidateMethod = po.methods.find((m) => m.intent === intent && m.available === false && m.status === "candidate");
        if (candidateMethod) {
...
function buildInlineLocator(step) {
    ...
    if (t.strategy === "role")
        return `page.getByRole('${value}', { name: '${name}' })`;
    if (t.strategy === "text")
        return `page.getByText('${value}')`;
    if (t.strategy === "label")
        return `page.getByLabel('${value}')`;
    if (t.strategy === "placeholder")
        return `page.getByPlaceholder('${value}')`;
    if (t.strategy === "testId")
        return `page.getByTestId('${value}')`;
    return `page.locator('${value}')`;
}
function buildInlineAction(step, locator) {

...
function parseArgs(argv) {
    const args = { statusFilter: "all", json: false, allApps: false };
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if ((token === "--project-id" || token === "--suite-id" || token === "--section-id" || token === "--status" || token === "--app") &&
...
async function main() {
    ...
    const result = await (0, case_automation_status_1.getCaseAutomationStatus)(rawCases, undefined, {

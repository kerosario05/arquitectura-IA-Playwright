...
const case_start_workflow_1 = require("../cases/case-start-workflow");
...
async function main() {
    ...
    if (args.headed)
        console.log("[cases:start] Headed mode enabled.");
    if (args.continueOnFailure)
        console.log("[cases:start] Continue on failure enabled.");
    if (args.noReport)
        console.log("[cases:start] TestRail reporting disabled.");
    if (args.dryRun)
        console.log("[cases:start] Dry-run mode enabled.");
    if (args.autoPromote)
        console.log("[cases:start] Auto-promote enabled.");
    if (args.autoHandoff)
        console.log("[cases:start] Auto-handoff enabled.");
    if (args.autoRepair)
        console.log("[cases:start] Auto-repair enabled.");
    if (args.reuseExisting)
        console.log("[cases:start] Automation reuse enabled.");
    const input = {
        caseId: args.caseId,
        projectId,
        suiteId,
    ...
    const result = await (0, case_start_workflow_1.startCaseAutomationWorkflow)(input);
    if (args.json) {

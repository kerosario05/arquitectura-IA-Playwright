...
exports.resolveRuntimeEntriesForCase = exports.loadRuntimeContextFromPath = void 0;
exports.parseBatchArgs = parseBatchArgs;
exports.selectCases = selectCases;
exports.resolveTestRailSelectionIds = resolveTestRailSelectionIds;
exports.executeBatch = executeBatch;
exports.resolveBatchAppProfile = resolveBatchAppProfile;
...
const case_automation_status_1 = require("../cases/case-automation-status");
const case_execution_queue_1 = require("../runner/case-execution-queue");
const app_profile_1 = require("../automations/app-profile");
const ai_repair_summary_builder_1 = require("../ai/repair/ai-repair-summary-builder");
const json_utils_1 = require("../utils/json-utils");
var runtime_context_1 = require("./runtime-context");
Object.defineProperty(exports, "loadRuntimeContextFromPath", { enumerable: true, get: function () { return runtime_context_1.loadRuntimeContextFromPath; } });
Object.defineProperty(exports, "resolveRuntimeEntriesForCase", { enumerable: true, get: function () { return runtime_context_1.resolveRuntimeEntriesForCase; } });
const runtime_context_2 = require("./runtime-context");
function parseBatchArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if (token === "--testrail-project-id" || token === "--testrail-suite-id" || token === "--testrail-section-id") {
            ...
            if (token === "--testrail-project-id")
                args.testRailProjectId = parsed;
            if (token === "--testrail-suite-id")
                args.testRailSuiteId = parsed;
            if (token === "--testrail-section-id")
                args.testRailSectionId = parsed;
            i += 1;
            continue;
        }
        ...
        if (token === "--all") {
            if (modeSet)
                throw new Error("Mode already set. Choose one of --all, --not-automated, --case-ids, --from/--to.");
            args.mode = "all";
            modeSet = true;
            continue;
        }
...
async function selectCases(client, args) {
    ...
    const statusResult = await (0, case_automation_status_1.getCaseAutomationStatus)(rawCases);
...
function resolveTestRailSelectionIds(args) {
    return {
        projectId: args.testRailProjectId?.toString() ?? env_1.config.integrations.testRail?.projectId,
        suiteId: args.testRailSuiteId?.toString() ?? env_1.config.integrations.testRail?.suiteId,
        sectionId: args.testRailSectionId?.toString() ?? env_1.config.integrations.testRail?.sectionId,
    };
}
async function executeBatch(args, entries, appProfile) {
    const batchId = new Date().toISOString().replace(/[:.]/g, "-");
    const batchDir = node_path_1.default.resolve(`./.artifacts/discovery/batch/${batchId}`);
    await promises_1.default.mkdir(node_path_1.default.join(batchDir, "cases"), { recursive: true });
    const selected = entries.filter((e) => e.selected);
    if (args.dryRun || selected.length === 0) {
        const startTime = Date.now();
        // Generate empty AI Repair batch summary for dry-run
        const aiRepairBatchSummary = (0, ai_repair_summary_builder_1.buildAiRepairBatchSummary)([]);
        console.log("");
        console.log((0, ai_repair_summary_builder_1.formatAiRepairBatchConsoleOutput)(aiRepairBatchSummary));
        const result = {
            batchId,
    ...
    const sharedClient = new testrail_client_1.TestRailClient(testRailRuntimeConfig);
    const runtimeContext = await (0, runtime_context_2.loadRuntimeContextFromPath)(process.env.DISCOVERY_RUNTIME_CONTEXT);
    async function runSingleCase(entry, _ctx) {
        const caseStartTime = Date.now();
        console.log(`[discovery:batch] Running case C${entry.caseId} - ${entry.title}`);
        const caseOutputDir = node_path_1.default.join(batchDir, "cases", `case-${entry.caseId}`);
        const workflowOptions = {
            caseId: entry.caseId,
            runtimeEntries: (0, runtime_context_2.resolveRuntimeEntriesForCase)(runtimeContext, entry.caseId),
            headed: args.headed,
        ...
        const resultEntry = {
            ...
            pendingAssertionForensics: cr.partialDiagnostics?.pendingForensics,
            pendingAssertionCount: cr.partialDiagnostics?.pendingAssertions?.length ?? 0,
            notConsumedReasons: (cr.partialDiagnostics?.pendingForensics ?? [])
                .map((f) => f.notConsumedReason)
                .filter(Boolean),
    ...
    const queueResult = await (0, case_execution_queue_1.runCaseExecutionQueue)(selected, async (entry, ctx) => {
    ...
    const aiRepairCaseSummaries = allExecutedResults
    ...
    const aiRepairBatchSummary = (0, ai_repair_summary_builder_1.buildAiRepairBatchSummary)(aiRepairCaseSummaries);
    // Save AI Repair batch summary artifact
    const aiRepairBatchSummaryPath = node_path_1.default.join(batchDir, "ai-repair-batch-summary.json");
    await (0, json_utils_1.writeJsonSafe)(aiRepairBatchSummaryPath, aiRepairBatchSummary);
...
async function resolveBatchAppProfile(args) {
    const ids = resolveTestRailSelectionIds(args);
    const resolvedApp = await (0, app_profile_1.resolveAppProfile)({
        cliAppSlug: args.app,
        envAppSlug: process.env.APP_SLUG,
        testRailProjectId: ids.projectId,
        testRailBaseUrl: env_1.config.integrations.testRail?.url,
        testRailEmail: env_1.config.integrations.testRail?.email,
        testRailApiKey: env_1.config.integrations.testRail?.apiKey,
        baseUrl: env_1.config.app.baseUrl,
        appName: env_1.config.app.name
    });
    const ensured = await (0, app_profile_1.ensureAppStructure)(resolvedApp.baseDir);
    console.log(`[discovery:batch] App profile resolved: appSlug=${resolvedApp.profile.appSlug} source=${resolvedApp.profile.source}`);
    (0, app_profile_1.logAppProfile)(resolvedApp.profile, resolvedApp.baseDir, ensured.length > 0 ? ensured : undefined);
...
async function writeBatchArtifacts(batchDir, result) {
    const jsonPath = process.env.DISCOVERY_BATCH_RESULT_PATH ?? node_path_1.default.join(batchDir, "batch-result.json");
    await promises_1.default.mkdir(node_path_1.default.dirname(jsonPath), { recursive: true });
    await promises_1.default.writeFile(jsonPath, JSON.stringify(result, null, 2), "utf-8");
    const mdLines = [];
    ...
    const mdPath = node_path_1.default.join(batchDir, "batch-summary.md");
    await promises_1.default.writeFile(mdPath, mdLines.join("\n"), "utf-8");
}
...
async function main() {
    ...
    if (args.limit)
        console.log(`[discovery:batch] Limit: ${args.limit}`);
    ...
    if (args.dryRun)
        console.log("[discovery:batch] DRY RUN - no discovery will be executed");
    if (args.autoRepair) {
        ...
        if (args.compactAgentPrompt) {
            ...
            if (args.agentPromptBudgetSeconds)
                console.log(`[discovery:batch] Agent prompt budget: ${args.agentPromptBudgetSeconds}s`);
            if (args.agentMaxCandidates)
                console.log(`[discovery:batch] Agent max candidates: ${args.agentMaxCandidates}`);
            if (args.agentMaxProposedActions)
                console.log(`[discovery:batch] Agent max proposed actions: ${args.agentMaxProposedActions}`);
            if (args.agentMaxAttempts)
                console.log(`[discovery:batch] Agent max attempts: ${args.agentMaxAttempts}`);
        }
    }
    const testRailRuntimeConfig = (0, env_1.requireTestRailConfig)(env_1.config);
    ...
    printSummary(result);
    const batchDir = node_path_1.default.resolve(`./.artifacts/discovery/batch/${result.batchId}`);
    console.log("");
    console.log(`Batch result: ${node_path_1.default.join(batchDir, "batch-result.json")}`);
    console.log(`Batch summary: ${node_path_1.default.join(batchDir, "batch-summary.md")}`);

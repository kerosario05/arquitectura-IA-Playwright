...
const testrail_case_publisher_1 = require("../server/services/testrail-case-publisher");
...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if (token === "--active-sprint") {
            args.activeSprint = true;
            continue;
        }
        if (token === "--headed") {
            args.headed = true;
        ...
        if (token === "--sprint-id") {
            ...
            if (!Number.isFinite(n) || n <= 0)
                throw new Error(`Invalid --sprint-id: ${nextValue}`);
            args.sprintId = n;
            i += 1;
            continue;
        }
        if (token === "--status") {
            args.status = nextValue.trim();
            i += 1;
            continue;
        }
        if (token === "--max-results") {
            const n = Number(nextValue);
            if (!Number.isFinite(n) || n <= 0)
                throw new Error(`Invalid --max-results: ${nextValue}`);
            args.maxResults = n;
            i += 1;
            continue;
        }
...
function mapPromotionStatusToTestRail(promotionStatus, caseStatus) {
    if (promotionStatus === "promoted")
        return TR_PASSED;
    if (promotionStatus === "spec_failed")
        return TR_FAILED;
    if (caseStatus === "exploration_failed")
        return TR_FAILED;
    if (promotionStatus === "not_promoted")
        return TR_BLOCKED;
    return TR_FAILED;
}
function buildDiscoveryComment(promotionStatus, caseStatus, outputDir, durationMs) {
    const parts = [];
    parts.push(`Discovery: ${caseStatus}`);
    if (promotionStatus !== "not_promoted")
        parts.push(`Promotion: ${promotionStatus}`);
    parts.push(`Duración: ${Math.round(durationMs / 1000)}s`);
    parts.push(`Evidencia: ${outputDir}`);
    return parts.join(" | ");
}
async function syncScenarioToTestRail(client, scenario, projectId, suiteId, sectionId) {
    const stepsSeparated = scenario.steps.map((s) => ({
        content: s.action,
        expected: s.expected ?? ""
    }));
    // Sanitize refs: never send undefined/null/empty
    const safeRef = (0, testrail_case_publisher_1.sanitizeTestRailRef)(scenario.externalId) || "UNKNOWN";
...
async function main() {
    ...
    if (!projectId)
        throw new Error("TESTRAIL_PROJECT_ID no está configurado en .env");
    if (!sectionId)
        throw new Error("TESTRAIL_SECTION_ID no está configurado en .env");
    const jiraClient = new jira_client_1.JiraClient(jiraConfig);
    const testRailClient = new testrail_client_1.TestRailClient(testRailConfig);
    ...
    if (!sprintId)
        throw new Error("No se pudo determinar el sprint.");
    ...
    const scenarioCaseMap = new Map(); // jiraKey → testRailCaseId
    for (const scenario of scenarios) {
        try {
            const caseId = await syncScenarioToTestRail(testRailClient, scenario, projectId, suiteId, sectionId);
    ...
    for (const scenario of scenarios) {
        ...
        let comment = "";
        try {
            const workflowResult = await (0, case_discovery_workflow_1.runCaseDiscoveryWorkflow)({
                scenario,
                headed: args.headed,
            ...
            testRailStatusId = mapPromotionStatusToTestRail(workflowResult.promotionStatus, workflowResult.caseResult.status);
            discoveryStatus = testRailStatusId === TR_PASSED ? "passed" : "failed";
            comment = buildDiscoveryComment(workflowResult.promotionStatus, workflowResult.caseResult.status, workflowResult.outputDir, workflowResult.durationMs);
    ...
    const testRailResults = storyResults.map((r) => ({
        runId: testRailRun.id,
        caseId: r.testRailCaseId,
        statusId: r.testRailStatusId,
        comment: r.comment,
        elapsed: (0, testrail_reporter_1.formatDuration)(r.durationMs)
    }));

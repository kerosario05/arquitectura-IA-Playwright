    ...
    for (let i = 0; i < argv.length; i += 1) {
        ...
        if (token === "--project-key") {
            if (!nextValue || nextValue.startsWith("--"))
                throw new Error("Missing value for --project-key");
...
async function printProjectSprints(client, project, showClosed) {
    let boards;
    try {
...
async function main() {
    ...
    for (const project of projects) {
        ...
        const { activeSprintId, activeSprintName } = await printProjectSprints(client, project, args.showClosed);
        if (activeSprintId) {
            suggestions.push(`  # Sprint activo: "${activeSprintName}" (id: ${activeSprintId})\n` +

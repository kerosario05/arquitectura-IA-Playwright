...
const registry_promoter_1 = require("../registry/registry-promoter");
const automation_promoter_1 = require("../automations/automation-promoter");
...
function printReport(report) {
    ...
    if (report.registryWritten) {
        ...
        if (report.backupPath)
            console.log(`Registry backup: ${report.backupPath}`);
    }
    else {
...
async function main() {
    ...
    let report = await (0, registry_promoter_1.buildRegistryPromotionReport)(args);
    if (args.approve) {
        if (args.promoteObjects) {
            report = await (0, registry_promoter_1.applyRegistryPromotion)(report);
        }
        if (args.promoteAutomation && report.planToPromote) {
            const promotedAutomation = await (0, automation_promoter_1.promoteDiscoveryAutomation)({

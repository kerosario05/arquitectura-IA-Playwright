...
function parseArgs(argv) {
    const args = { showSections: true };
    for (let i = 0; i < argv.length; i += 1) {
        const token = argv[i];
        const nextValue = argv[i + 1];
        if (token === "--no-sections") {
            args.showSections = false;
            continue;
        }
        if (token === "--project-id") {
...
function suiteModLabel(mode) {
    if (mode === 1)
        return "suite única";
    if (mode === 2)
        return "suite única + baselines";
    return "múltiples suites";
}
function buildSectionTree(sections, parentId = null, indent = "    ") {
...
async function printProject(client, project, showSections, currentProjectId, currentSuiteId, currentSectionId) {
...
async function main() {
    ...
    const client = new testrail_client_1.TestRailClient(trConfig);
    const currentProjectId = env_1.config.integrations.testRail?.projectId;
    const currentSuiteId = env_1.config.integrations.testRail?.suiteId;
    const currentSectionId = env_1.config.integrations.testRail?.sectionId;
    ...
    if (args.projectId) {
        ...
        if (projects.length === 0)
            throw new Error(`No se encontró proyecto con id: ${args.projectId}`);
    }
    if (projects.length === 0) {

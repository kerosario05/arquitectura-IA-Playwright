...
exports.parseRawTestData = parseRawTestData;
exports.normalizeAppProfile = normalizeAppProfile;
exports.parseMissingInputBehavior = parseMissingInputBehavior;
exports.parseAutoGenerateTestData = parseAutoGenerateTestData;
exports.parseAutoGenerateSensitiveData = parseAutoGenerateSensitiveData;
exports.parseTestDataProfile = parseTestDataProfile;
exports.parseAutoSelectSafeDefaults = parseAutoSelectSafeDefaults;
exports.parseAutoAcceptSafeCheckboxes = parseAutoAcceptSafeCheckboxes;
exports.requireTestRailConfig = requireTestRailConfig;
exports.requireJiraConfig = requireJiraConfig;
const dotenv_loader_1 = require("./dotenv-loader");
const browser_profile_1 = require("../browser/browser-profile");
(0, dotenv_loader_1.loadDotenvWithPowerShellSupport)();
const allowedBrowsers = ["chromium", "firefox", "webkit"];
const allowedLoginModes = ["password", "no_login", "manual"];
const allowedMissingInputBehaviors = ["fail", "prompt", "skip", "auto_generate"];
const allowedTestDataProfiles = ["demo", "qa", "staging", "production_like"];
...
function parseBrowser(value) {
    if (!allowedBrowsers.includes(value)) {
        throw new Error(`Invalid BROWSER value: ${value}. Allowed values: ${allowedBrowsers.join(", ")}`);
    }
    return value;
}
function parseLoginMode(value) {
    if (!allowedLoginModes.includes(value)) {
        throw new Error(`Invalid APP_LOGIN_MODE value: ${value}. Allowed values: ${allowedLoginModes.join(", ")}`);
    }
    return value;
}
...
function parseOptionalPositiveNumber(rawValue, name) {
    if (!rawValue || !rawValue.trim()) {
        return undefined;
    }
    return parseNumber(rawValue, name);
}
function parseOptionalThreshold(rawValue, name) {
    if (!rawValue || !rawValue.trim()) {
        return undefined;
    }
...
function parseAgentProvider(rawValue) {
    if (!rawValue || !rawValue.trim()) {
        return undefined;
    }
...
function parseExpectedResultMode(rawValue) {
    if (!rawValue || !rawValue.trim()) {
...
function parseExtraLoginFields(rawValue) {
    if (!rawValue || !rawValue.trim()) {
        return undefined;
    }
    let parsed;
...
function parseFlatJsonObject(rawValue, envName) {
    let parsed;
    try {
        parsed = JSON.parse(rawValue);
    }
...
function parseTestData(rawValue) {
    if (!rawValue || !rawValue.trim()) {
        return {};
    }
...
function parseRawTestData(rawValue) {
    if (!rawValue || !rawValue.trim()) {
        return {};
    }
    return parseFlatJsonObject(rawValue, "APP_TEST_DATA_JSON");
}
function parseTestDataAliases(rawValue) {
    if (!rawValue || !rawValue.trim()) {
        return {};
    }
...
function normalizeAppProfile(rawValue) {
    if (!rawValue || !rawValue.trim())
        return "default";
    return rawValue
        .trim()
...
function parseMissingInputBehavior(rawValue) {
    const trimmed = rawValue?.trim();
    const value = (!trimmed ? "fail" : trimmed).toLowerCase();
    if (!allowedMissingInputBehaviors.includes(value)) {
        throw new Error(`Invalid MISSING_INPUT_BEHAVIOR value: ${rawValue}. Allowed values: ${allowedMissingInputBehaviors.join(", ")}`);
    }
    return value;
}
function parseAutoGenerateTestData(rawValue) {
    const trimmed = rawValue?.trim();
    if (!trimmed)
        return false;
...
function parseAutoGenerateSensitiveData(rawValue) {
    const trimmed = rawValue?.trim();
    if (!trimmed)
        return false;
    const normalized = trimmed.toLowerCase();
...
function parseTestDataProfile(rawValue) {
    const trimmed = rawValue?.trim();
    if (!trimmed)
        return "qa";
    const normalized = trimmed.toLowerCase();
    if (!allowedTestDataProfiles.includes(normalized)) {
        throw new Error(`Invalid APP_TEST_DATA_PROFILE value: ${rawValue}. Allowed values: ${allowedTestDataProfiles.join(", ")}`);
    }
    return normalized;
}
function parseAutoSelectSafeDefaults(rawValue) {
    const trimmed = rawValue?.trim();
    if (!trimmed)
        return false;
    const normalized = trimmed.toLowerCase();
...
function parseAutoAcceptSafeCheckboxes(rawValue) {
    const trimmed = rawValue?.trim();
    if (!trimmed)
        return false;
    const normalized = trimmed.toLowerCase();
...
const promotedSpecTimeoutMs = parseOptionalPositiveNumber(process.env.PROMOTED_SPEC_TIMEOUT_MS, "PROMOTED_SPEC_TIMEOUT_MS") ?? 120000;
...
exports.config = {
    ...
    execution: {
        ...
        evidenceDir,
        defaultTimeoutMs,
        qaBrowserProfilePath: (0, browser_profile_1.resolveQaBrowserProfilePath)(process.env.QA_BROWSER_PROFILE_PATH),
    ...
    integrations: {
        ...
        ai: {
            ...
            autoApproveDiscoveredObjects: (process.env.AUTO_APPROVE_DISCOVERED_OBJECTS ?? "false").toLowerCase() === "true",
            discoveryEnabled: (process.env.AI_DISCOVERY_ENABLED ?? "false").toLowerCase() === "true",
            discoveryConfidenceThreshold: parseOptionalThreshold(process.env.AI_DISCOVERY_CONFIDENCE_THRESHOLD, "AI_DISCOVERY_CONFIDENCE_THRESHOLD") ?? 0.85,
            discoveryRequireApprovalThreshold: parseOptionalThreshold(process.env.AI_DISCOVERY_REQUIRE_APPROVAL_THRESHOLD, "AI_DISCOVERY_REQUIRE_APPROVAL_THRESHOLD") ?? 0.7,
            discoveryMaxAttempts: parseOptionalPositiveNumber(process.env.AI_DISCOVERY_MAX_ATTEMPTS, "AI_DISCOVERY_MAX_ATTEMPTS") ?? 3,
            ...
            routeCompletion: {
                ...
                minConfidence: parseOptionalThreshold(process.env.AI_ROUTE_COMPLETION_MIN_CONFIDENCE, "AI_ROUTE_COMPLETION_MIN_CONFIDENCE") ?? 0.75,
                maxInsertedSteps: parseOptionalPositiveNumber(process.env.AI_ROUTE_COMPLETION_MAX_INSERTED_STEPS, "AI_ROUTE_COMPLETION_MAX_INSERTED_STEPS") ?? 1,
            ...
            routeProfileLearning: {
                ...
                autoApproveThreshold: parseOptionalThreshold(process.env.ROUTE_PROFILE_LEARNING_AUTO_APPROVE_THRESHOLD, "ROUTE_PROFILE_LEARNING_AUTO_APPROVE_THRESHOLD") ?? 0.90,
                autoApply: (process.env.ROUTE_PROFILE_LEARNING_AUTO_APPLY ?? "false").toLowerCase() === "true",
                minOccurrences: parseOptionalPositiveNumber(process.env.ROUTE_PROFILE_LEARNING_MIN_OCCURRENCES, "ROUTE_PROFILE_LEARNING_MIN_OCCURRENCES") ?? 1,
        ...
        agent: {
            ...
            autoRepairPromptMode: process.env.CODEX_AUTO_REPAIR_PROMPT_MODE?.trim().toLowerCase() || "compact"
        },
        codex: {
        ...
        android: {
            ...
            systemPort: parseOptionalPositiveNumber(process.env.APPIUM_ANDROID_SYSTEM_PORT, "APPIUM_ANDROID_SYSTEM_PORT"),
            headless: (process.env.ANDROID_EMULATOR_HEADLESS ?? "true").toLowerCase() === "true",
            bootTimeoutMs: parseOptionalPositiveNumber(process.env.ANDROID_EMULATOR_BOOT_TIMEOUT_MS, "ANDROID_EMULATOR_BOOT_TIMEOUT_MS") ?? 120000,
            appiumPort: parseOptionalPositiveNumber(process.env.APPIUM_SERVER_PORT, "APPIUM_SERVER_PORT") ?? 4723,
...
function normalizeTestRailUrl(url) {
    return url.replace(/\/+$/, "");
}
function requireTestRailConfig(fullConfig) {
...
function requireJiraConfig(fullConfig) {

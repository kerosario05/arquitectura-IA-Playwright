...
exports.inferSyntheticSemanticType = inferSyntheticSemanticType;
exports.generateSemanticSyntheticValue = generateSemanticSyntheticValue;
exports.generateSyntheticValueFromCapability = generateSyntheticValueFromCapability;
exports.resolveSyntheticValue = resolveSyntheticValue;
function normalizeSemanticText(value) {
    return value.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, " ").trim();
}
/** Infers only a generation hint; it never changes the declared input key. */
function inferSyntheticSemanticType(input) {
    const text = normalizeSemanticText([input.key, input.label ?? ""].join(" "));
    if (input.fieldCapability?.kind === "email" || /email|correo|mail/.test(text))
        return "email";
    if (input.fieldCapability?.kind === "tel" || /phone|telefono|celular|mobile|tel/.test(text))
        return "phone";
    if (/uuid|guid/.test(text))
        return "uuid";
    if (/postal|zip|codigo postal|codigo_postal/.test(text))
        return "postal_code";
    if (/company|business|empresa|razon social|legal name|trade name|company name/.test(text))
        return "company_name";
    if (/address|direccion|domicilio/.test(text))
        return "address";
    if (/city|ciudad|provincia/.test(text))
        return "city";
    if (/last name|apellido|surname/.test(text))
        return "last_name";
    if (/first name|nombre|name|nombre esperado/.test(text))
        return "name";
    if (/job title|cargo|puesto|ocupacion|occupation|position/.test(text))
        return "job_title";
    if (/username|usuario|user name/.test(text))
        return "username";
    if (/document|identif|cedula|rnc|identifier|documento/.test(text))
        return "identifier";
    if (/money|monto|amount|income|ingreso|salary|salario|importe/.test(text))
        return "money";
    if (/percent|porcentaje/.test(text))
        return "decimal";
    if (/quantity|cantidad|count|numero|number|integer/.test(text) || input.fieldCapability?.kind === "number")
        return "integer";
    if (/datetime|timestamp|fecha y hora/.test(text) || input.fieldCapability?.kind === "datetime")
        return "datetime";
    if (/date|fecha/.test(text) || input.fieldCapability?.kind === "date")
        return "date";
    if (/description|descripcion|detalle|comentario/.test(text))
        return "description";
    if (/code|codigo|reference|referencia/.test(text))
        return "code";
    if (input.fieldCapability?.kind === "text")
        return "generic_text";
    return "unknown";
}
function semanticWords(seed) {
...
function applyTextConstraints(value, constraints) {
    if (!validLength(value, constraints))
        return unresolved("semantic_length_constraints_not_satisfied");
    if (constraints?.pattern !== undefined) {
        try {
            if (!new RegExp(constraints.pattern).test(value))
                return unresolved("semantic_pattern_not_satisfied");
        }
        catch {
...
function generateSemanticSyntheticValue(input) {
    ...
    const relatedName = Object.entries(requirement.relatedValues ?? {}).find(([key, value]) => /name|nombre/i.test(key) && typeof value === "string")?.[1];
    switch (semantic) {
        case "email": {
...
function generateSyntheticValueFromCapability(input) {
    ...
    switch (capability.kind) {
        ...
        case "radio": {
            ...
            if (options.length === 0)
                return unresolved("missing_allowed_values");
        ...
        case "date": {
            if (constraints)
                return unresolved("date_constraints_not_supported");
        ...
        case "datetime": {
            if (constraints)
                return unresolved("datetime_constraints_not_supported");
...
function validLength(value, constraints) {
...
function resolveText(seed, constraints) {
    if (constraints?.pattern !== undefined)
        return unresolved("pattern_not_safely_satisfied");
...
function resolveNumber(seed, constraints) {
    const min = constraints?.min ?? 0;
    const max = constraints?.max ?? (min + 999);
    if (!Number.isFinite(min) || !Number.isFinite(max) || min > max)
        return unresolved("invalid_number_constraints");
    const value = min === max ? min : min + ((seed / 0xffffffff) * (max - min));
    return { status: "generated", value, strategy: "synthetic:number" };
}
function resolveEmail(seed, constraints) {
    const value = `synthetic${seed.toString(36)}@example.test`;
    if (!validLength(value, constraints))
        return unresolved("email_length_constraints_not_satisfied");
    if (constraints?.pattern !== undefined) {
        try {
            if (!new RegExp(constraints.pattern).test(value))
                return unresolved("email_pattern_not_satisfied");
        }
        catch {
            return unresolved("invalid_pattern_constraint");
        }
...
function resolveTel(seed, constraints) {
    ...
    let value = "";
    for (let index = 0; index < length; index += 1)
        value += ((seed + index * 7) % 10).toString();
    if (constraints?.pattern !== undefined) {
        try {
            if (!new RegExp(constraints.pattern).test(value))
                return unresolved("tel_pattern_not_satisfied");
        }
        catch {
            return unresolved("invalid_pattern_constraint");
        }
...
function resolveSyntheticValue(input) {
    const requirement = input.requirement;
    if (requirement.valuePolicy !== "safe_synthetic")
        return unresolved("value_policy_not_synthetic");
    if (requirement.sensitive === true)
        return unresolved("sensitive_requirement");
    const capability = requirement.fieldCapability;
    if (!capability)
        return unresolved("missing_field_capability");
    if (capability.kind === "password")
        return unresolved("password_not_generated");

...
exports.initAdaptiveRoute = initAdaptiveRoute;
exports.buildAdaptiveSuggestionPrompt = buildAdaptiveSuggestionPrompt;
exports.validateAdaptiveSuggestion = validateAdaptiveSuggestion;
exports.executeAdaptiveSuggestion = executeAdaptiveSuggestion;
exports.runAdaptiveRouteDiscovery = runAdaptiveRouteDiscovery;
exports.chainNowReachesTarget = chainNowReachesTarget;
...
function initAdaptiveRoute(targetScreen, currentChain, requiredChain) {
    return {
        targetScreen,
 ...
 */
function buildAdaptiveSuggestionPrompt(state) {
 ...
 */
function validateAdaptiveSuggestion(suggestion, state, pageVisibleTexts) {
    if (!suggestion.targetLabel)
        return { accepted: false, reason: "no_target_label" };
    ...
    if (pageVisibleTexts.length > 0) {
        ...
        if (!found)
            return { accepted: false, reason: "target_not_visible" };
    }
 ...
 */
async function executeAdaptiveSuggestion(page, suggestion) {
    try {
        if (!suggestion.targetLabel)
            return { success: false, actionText: "", error: "no target" };
        const locator = page.getByRole(suggestion.targetType, { name: suggestion.targetLabel }).first();
 ...
 */
async function runAdaptiveRouteDiscovery(page, initialState, snapshotTexts, aiProvider) {
    ...
    while (state.stepsTaken < state.maxSteps) {
        ...
        if (!suggestion && aiProvider) {
            try {
                ...
                suggestion = response.parsedJson;
...
function chainNowReachesTarget(state, newStepType) {
    ...
    return state.requiredChain.some(pattern => {
        let pi = 0;
        for (const t of state.currentChain) {

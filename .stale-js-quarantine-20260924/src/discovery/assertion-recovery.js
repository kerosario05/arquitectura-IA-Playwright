...
exports.attemptAssertionRecovery = attemptAssertionRecovery;
exports.classifyAssertionImportance = classifyAssertionImportance;
exports.detectConditionalAssertionRisk = detectConditionalAssertionRisk;
...
function toPlural(text) {
    if (text.endsWith("ión"))
        return text.slice(0, -3) + "iones";
    if (text.endsWith("dad"))
        return text.slice(0, -3) + "dades";
    if (text.endsWith("tad"))
        return text.slice(0, -3) + "tades";
    if (text.endsWith("tud"))
        return text.slice(0, -3) + "tudes";
    if (text.endsWith("z"))
...
function toSingular(text) {
    if (text.endsWith("iones"))
        return text.slice(0, -5) + "ión";
    if (text.endsWith("dades"))
        return text.slice(0, -5) + "dad";
    if (text.endsWith("tades"))
        return text.slice(0, -5) + "tad";
    if (text.endsWith("tudes"))
        return text.slice(0, -5) + "tud";
    if (text.endsWith("ces"))
        return text.slice(0, -3) + "z";
    if (text.endsWith("s"))
        return text.slice(0, -1);
    return text;
}
function getPluralSingularVariants(text) {
    const variants = [];
...
function getAliasesFromRouteProfile(assertionText, routeProfile, appConfig) {
    const aliases = [];
    const normalizedAssertion = normalizeForComparison(assertionText);
    const collectAliases = (rp) => {
        if (!rp?.aliases || typeof rp.aliases !== "object")
            return;
        const aliasesObj = rp.aliases;
    ...
    collectAliases(routeProfile);
    if (appConfig?.routeProfile && typeof appConfig.routeProfile === "object") {
        collectAliases(appConfig.routeProfile);
    }
    return aliases;
}
function getVisibleControlsFromRouteProfile(routeProfile, appConfig) {
    const controls = [];
    const collectControls = (rp) => {
    ...
    collectControls(routeProfile);
    if (appConfig?.routeProfile && typeof appConfig.routeProfile === "object") {
        collectControls(appConfig.routeProfile);
    }
    return controls;
}
function getDomainTermsFromRouteProfile(routeProfile, appConfig) {
    const terms = [];
    const collectTerms = (rp) => {
        if (!rp?.domainTerms || typeof rp.domainTerms !== "object")
            return;
        const termsObj = rp.domainTerms;
    ...
    collectTerms(routeProfile);
    if (appConfig?.routeProfile && typeof appConfig.routeProfile === "object") {
        collectTerms(appConfig.routeProfile);
    }
    return terms;
}
function findTextInSnapshot(snapshot, targetText) {
    const normalizedTarget = normalizeForComparison(targetText);
    for (const el of snapshot.elements) {
        const texts = [el.text, el.label, el.name, el.placeholder, el.value].filter(Boolean);
    ...
    const snapshotBlob = normalizeForComparison(`${snapshot.title} ${snapshot.elements.map((e) => `${e.text ?? ""} ${e.label ?? ""} ${e.name ?? ""}`).join(" ")}`);
...
function attemptAssertionRecovery(snapshot, failedAssertionText, options = {}) {
    ...
    for (const control of visibleControls) {
        ...
        if (normalizedAssertion.includes(normalizedControl) ||
    ...
    for (const term of domainTerms) {
        ...
        const normalizedAssertion = normalizeForComparison(failedAssertionText);
        if (normalizedAssertion.includes(normalizedTerm) ||
...
function trySemanticEquivalents(snapshot, assertionText) {
    const normalizedAssertion = normalizeForComparison(assertionText);
    const semanticMap = [
        {
...
function tryConditionalVariants(snapshot, assertionText) {
    const normalizedAssertion = normalizeForComparison(assertionText);
...
function classifyAssertionImportance(assertionText, options = {}) {
    ...
    if (rp) {
        const allLabels = [];
        if (rp.visibleControls && Array.isArray(rp.visibleControls)) {
            allLabels.push(...rp.visibleControls);
        }
        if (rp.aliases && typeof rp.aliases === "object") {
            for (const value of Object.values(rp.aliases)) {
                if (typeof value === "string")
...
function detectConditionalAssertionRisk(assertionText, options = {}) {

...
exports.resolveStructuralRowAssertion = resolveStructuralRowAssertion;
exports.resolveEntityWithinContainerAssertion = resolveEntityWithinContainerAssertion;
exports.resolveRowScopedAssertion = resolveRowScopedAssertion;
exports.classifyAssertion = classifyAssertion;
exports.buildConcreteAssertionsFromExpected = buildConcreteAssertionsFromExpected;
exports.resolveAssertionTargets = resolveAssertionTargets;
const target_resolver_1 = require("./target-resolver");
const catalog_list_resolver_1 = require("./catalog-list-resolver");
const feedback_message_resolver_1 = require("./feedback-message-resolver");
const compound_form_fields_resolver_1 = require("./compound-form-fields-resolver");
/**
 * Back/return button aliases for semantic matching
 * These allow "Volver al menú principal" to match "Volver al menú" or "Volver"
 */
const BACK_RETURN_ALIASES = {
 ...
 */
function isBackReturnAssertion(assertionText) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
 ...
 */
function getBackReturnAliases(assertionText) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
    const aliases = [assertionText]; // Always include original
...
async function resolveStructuralRowAssertion(page, input) {
    const base = {
        assertionText: input.target,
        normalizedAssertion: (0, target_resolver_1.normalizeText)(input.target),
        classification: "structural_assertion",
        closestCandidates: [],
        visibleTexts: [],
        assertionType: "field",
    };
    const target = (0, target_resolver_1.normalizeText)(input.target);
    const requestedRow = input.rowScope
        ?? (/\b(?:segunda|second)\b/.test(target) ? 2 : undefined);
    ...
    for (let index = 0; index < containerCount; index += 1) {
        const rows = containers.nth(index).locator("tbody tr, [role='row']");
        const rowCount = await rows.count().catch(() => 0);
        let dataRows = 0;
        for (let rowIndex = 0; rowIndex < rowCount; rowIndex += 1) {
            if (await rows.nth(rowIndex).locator("td, th, [role='gridcell']").count().catch(() => 0) > 0)
                dataRows += 1;
        }
        observedRows = Math.max(observedRows, dataRows);
    }
    const requiredRows = requestedRow ?? (input.rowRelation === "added" ? 2 : 1);
    if (observedRows >= requiredRows) {
        return {
            ...base,
            status: "passed",
            matchedText: `rows>=${requiredRows}`,
            confidence: 1,
            reason: "structural_row_count",
            structuralSignals: [`row_count_${observedRows}`, `required_row_count_${requiredRows}`],
            assertionDiagnostics: { oracleType: "structural_row_count", observedRows, requiredRows, globalTextFallbackUsed: false },
        };
    }
    return {
        ...base,
        ...
        confidence: 0,
        reason: "structural_row_not_observed",
        structuralSignals: [`row_count_${observedRows}`, `required_row_count_${requiredRows}`],
        assertionDiagnostics: { oracleType: "structural_row_count", observedRows, requiredRows, globalTextFallbackUsed: false },
    };
}
/** Evaluate a two-value containment assertion from one semantic DOM container. */
async function resolveEntityWithinContainerAssertion(page, input, options = {}) {
    const base = {
        assertionText: input.target,
        normalizedAssertion: (0, target_resolver_1.normalizeText)(input.target),
        classification: "structural_assertion",
    ...
    const entityValue = options.entityValue?.trim();
    const containerValue = options.containerValue?.trim();
    const entitySource = options.entitySource ?? "unknown";
    const containerSource = options.containerSource ?? "unknown";
    const synthetic = options.entityVerified === false || options.containerVerified === false
        || /synthetic|generated|suggested|demo|default|autogenerated/i.test(`${entitySource} ${containerSource}`);
    if (!entityValue || !containerValue || synthetic) {
        return { ...base, status: "needs_assertion_resolution", confidence: 0, reason: "ORACLE_AUTHORITY_MISSING", assertionDiagnostics: { entitySource, containerSource, syntheticUnverified: synthetic } };
    }
    const containers = page.locator("section, [role='region'], [role='group'], article, form, fieldset, table, [role='row'], [data-testid]");
    const count = await containers.count().catch(() => 0);
    for (let index = 0; index < count; index += 1) {
        const container = containers.nth(index);
        if (!(await container.isVisible().catch(() => false)))
            continue;
        const text = normalizeAssertionCellValue(await container.innerText().catch(() => ""));
        if (text.includes(normalizeAssertionCellValue(entityValue)) && text.includes(normalizeAssertionCellValue(containerValue))) {
            return { ...base, status: "passed", matchedText: text, confidence: 1, reason: "entity_within_container", structuralSignals: ["entity_and_container_same_dom_container"], assertionDiagnostics: { entitySource, containerSource, containmentVerified: true } };
        }
    }
    return { ...base, status: "failed", confidence: 0, reason: "entity_within_container_not_observed", assertionDiagnostics: { entitySource, containerSource, containmentVerified: false } };
}
function normalizeAssertionCellValue(value) {
    return (0, target_resolver_1.normalizeText)(value).replace(/\s+/g, " ").trim();
}
function sameSemanticDate(left, right) {
    const iso = /^(\d{4})[-/.](\d{1,2})[-/.](\d{1,2})$/;
    const dmy = /^(\d{1,2})[-/.](\d{1,2})[-/.](\d{4})$/;
    const parse = (value) => {
        const normalized = normalizeAssertionCellValue(value);
        const isoMatch = normalized.match(iso);
        if (isoMatch)
            return `${isoMatch[1]}-${isoMatch[2].padStart(2, "0")}-${isoMatch[3].padStart(2, "0")}`;
        const dmyMatch = normalized.match(dmy);
        if (dmyMatch)
...
function assertionFieldLabel(target) {
    const withoutExpected = target.replace(/\[[^\]\r\n]+\]/g, "").trim();
    const match = withoutExpected.match(/(?:que\s+)?(?:el|la|los|las)?\s*(.+?)\s+(?:sea|est[ée]|es|corresponda|coincida)\b/i);
    return match?.[1]?.replace(/^(?:valor|campo)\s+/i, "").trim() || undefined;
}
function rowHeaderMatches(fieldLabel, headerText) {
    const fieldTokens = (0, target_resolver_1.normalizeText)(fieldLabel).split(/\s+/).filter((token) => !["el", "la", "los", "las", "de", "del", "en", "campo", "valor", "autocompletado", "autocompletada"].includes(token));
    const headerTokens = (0, target_resolver_1.normalizeText)(headerText).split(/\s+/).filter(Boolean);
    return fieldTokens.length > 0 && fieldTokens.every((token) => headerTokens.some((candidate) => candidate === token || candidate.includes(token) || token.includes(candidate)));
}
/**
 * Evaluates an exact assertion at a declared row/column intersection. This is
 * intentionally page-aware so it can inspect the same dynamic editors used by
 * the grid interaction engine; it never falls back to global text visibility.
 */
async function resolveRowScopedAssertion(page, input, options = {}) {
    const base = {
        assertionText: input.target,
        normalizedAssertion: (0, target_resolver_1.normalizeText)(input.target),
        classification: "structural_assertion",
    ...
    if (input.rowScope === undefined || !input.expectedValueKey) {
        return { ...base, status: "needs_assertion_resolution", confidence: 0, reason: "row_scope_or_expected_value_missing" };
    }
    const expectedValue = options.expectedValue?.trim();
    const source = options.expectedValueSource ?? "unknown";
    const synthetic = /synthetic|generated|suggested|demo|default|autogenerated/i.test(source);
    if (!expectedValue || synthetic) {
        return {
            ...base,
            status: "needs_assertion_resolution",
            confidence: 0,
            reason: "ORACLE_AUTHORITY_MISSING",
            assertionDiagnostics: { expectedValueKey: input.expectedValueKey, expectedValueSource: source, syntheticUnverified: synthetic },
        };
    }
    const fieldLabel = assertionFieldLabel(input.target);
    if (!fieldLabel)
        return { ...base, status: "needs_assertion_resolution", confidence: 0, reason: "row_assertion_subject_missing" };
    const containers = page.locator("table, [role='grid']");
    const containerCount = await containers.count().catch(() => 0);
    for (let containerIndex = 0; containerIndex < containerCount; containerIndex += 1) {
        const container = containers.nth(containerIndex);
        const headers = container.locator("thead th, thead td, [role='columnheader']");
        const headerCount = await headers.count().catch(() => 0);
        const effectiveHeaders = headerCount > 0 ? headers : container.locator("tr, [role='row']").first().locator("th, td, [role='columnheader']");
        const effectiveHeaderCount = await effectiveHeaders.count().catch(() => 0);
        let columnIndex = -1;
        for (let index = 0; index < effectiveHeaderCount; index += 1) {
            if (rowHeaderMatches(fieldLabel, await effectiveHeaders.nth(index).innerText().catch(() => ""))) {
                columnIndex = index;
                break;
            }
        }
        ...
        const row = dataRows[input.rowScope - 1];
        if (!row)
            continue;
        const cells = row.locator("td, th, [role='gridcell']");
        const cellCount = await cells.count().catch(() => 0);
        const cellIndex = columnIndex + Math.max(0, cellCount - effectiveHeaderCount);
        ...
        const actual = editorCount > 0
            ? await editors.first().inputValue().catch(() => cell.innerText().catch(() => ""))
            : await cell.innerText().catch(() => "");
        const exact = normalizeAssertionCellValue(actual) === normalizeAssertionCellValue(expectedValue);
        const dateEquivalent = !exact && sameSemanticDate(actual, expectedValue);
        return {
            ...base,
            status: exact || dateEquivalent ? "passed" : "failed",
            matchedText: actual,
            confidence: exact || dateEquivalent ? 1 : 0,
            reason: exact ? "row_scoped_exact_match" : dateEquivalent ? "row_scoped_date_normalized_match" : "row_scoped_value_mismatch",
            assertionDiagnostics: { rowScope: input.rowScope, columnIndex, cellResolved: true, expectedValueKey: input.expectedValueKey, expectedValueSource: source },
        };
    }
    return { ...base, status: "failed", confidence: 0, reason: "row_scoped_cell_not_found", assertionDiagnostics: { rowScope: input.rowScope, fieldLabel } };
}
...
const DESCRIPTOR_TYPE_KEYWORDS = {
...
function tokenizeForConsumption(text) {
    return (0, target_resolver_1.normalizeText)(text)
        .split(/\s+/)
        .filter(Boolean)
...
function extractQuotedOrDelimitedFieldGroups(text) {
    const groups = [];
    ...
    if (groups.length > 0)
        return groups;
    const normalized = (0, target_resolver_1.normalizeText)(text);
...
function detectAssertionConsumption(snapshot, assertionText, executedActions, currentContext) {
    const normalizedAssertion = (0, target_resolver_1.normalizeText)(assertionText);
    ...
    const visibleBlob = (0, target_resolver_1.normalizeText)(`${snapshot.title} ${snapshot.elements.map((e) => `${e.text ?? ""} ${e.label ?? ""} ${e.name ?? ""} ${e.placeholder ?? ""}`).join(" ")}`);
    ...
    if (currentContext === "detail" && /\b(add to cart|agregar al carrito|accion principal|primary action|boton|button)\b/i.test((0, target_resolver_1.normalizeText)(assertionText))) {
    ...
    if (currentContext === "confirmation" && /\b(close|cerrar|ok|aceptar|accept)\b/i.test((0, target_resolver_1.normalizeText)(assertionText))) {
        return {
            consumed: true,
            decision: "satisfied_by_structural_evidence",
    ...
    for (const action of executedActions) {
        if (action.status !== "found")
            continue;
        const actionText = (0, target_resolver_1.normalizeText)(`${action.action} ${action.target}`);
...
function uniqueVisibleTexts(snapshot) {
...
function buildClosestCandidates(snapshot, assertionText) {
    const candidates = [];
    for (const element of snapshot.elements) {
        const texts = [element.text, element.label, element.name, element.placeholder].filter(Boolean);
        for (const text of texts) {
            const score = (0, target_resolver_1.computeTokenScore)(assertionText, text);
            if (score > 0) {
...
function isTextVisible(snapshot, assertionText) {
    const normalizedAssertion = (0, target_resolver_1.normalizeText)(assertionText);
    ...
    for (const visibleText of uniqueVisibleTexts(snapshot)) {
        const normalizedVisible = (0, target_resolver_1.normalizeText)(visibleText);
        // Try matching against each alias
        for (const textToMatch of textsToMatch) {
            const normalizedToMatch = (0, target_resolver_1.normalizeText)(textToMatch);
            let confidence = 0;
            let matchReason;
            if (normalizedVisible === normalizedToMatch) {
                ...
                confidence = Math.max(0.85, (0, target_resolver_1.computeTokenScore)(textToMatch, visibleText));
                matchReason = textToMatch === assertionText ? "contains_match" : "alias_contains_match";
            }
            else {
                confidence = (0, target_resolver_1.computeTokenScore)(textToMatch, visibleText);
...
function resolveStructuralAssertion(snapshot, assertionText) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
...
function extractDescriptorTypes(assertionText) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
    const types = [];
    for (const [type, keywords] of Object.entries(DESCRIPTOR_TYPE_KEYWORDS)) {
        if (keywords.some((keyword) => normalized.includes((0, target_resolver_1.normalizeText)(keyword)))) {
            types.push(type);
        }
    }
    return types;
}
function extractSubject(assertionText) {
    let text = (0, target_resolver_1.normalizeText)(assertionText);
    ...
    text = text
        ...
        .replace(/\s+/g, " ")
        .trim();
    const descriptorTokens = Object.values(DESCRIPTOR_TYPE_KEYWORDS).flat().map((t) => (0, target_resolver_1.normalizeText)(t));
...
function tokenizeStrongSubject(subject) {
    return (0, target_resolver_1.normalizeText)(subject)
        .split(/\s+/)
        .filter(Boolean)
        .filter((token) => token.length > 2 && !WEAK_TOKENS.has(token));
}
function matchSubjectSignals(snapshot, subject) {
    ...
    for (const text of visible) {
        const normalizedVisible = (0, target_resolver_1.normalizeText)(text);
...
function classifyAssertion(assertionText, context) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
    ...
    const compoundFormFields = (0, compound_form_fields_resolver_1.parseCompoundFormFieldsAssertion)(assertionText);
    if (compoundFormFields) {
        return "compound_form_fields";
    }
    if ((0, feedback_message_resolver_1.isFeedbackAssertion)(assertionText)) {
        return "feedback_message";
    }
    if ((0, catalog_list_resolver_1.isCatalogListAssertion)(assertionText)) {
...
function detectPrecondition(assertionText) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
...
function isCartAssertion(assertionText) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
    if (/\bcarrito\b|\bcart\b/.test(normalized))
        return true;
    return (/\btotal\b/.test(normalized) && /\bcarrito\b|\bcart\b/.test(normalized));
}
function isProductDetailAssertion(assertionText) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
...
function inferRequiredContext(assertionText) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
    if (/\bcarrito\b|\bcart\b|\bcheckout\b|\bsubtotal\b|\btotal\b/.test(normalized))
        return "cart";
    if (/\bmodal\b|\bdialog\b|\bform\b|\bformulario\b|\bcampo\b|\bfield\b/.test(normalized))
        return "form";
    if (/\bconfirm\w*\b|\bsuccess\b|\bexito\b|\bfinaliz\w*\b|\bcompletad\w*\b/.test(normalized))
        return "confirmation";
    if (/\bdetalle\b|\bdetail\b|\bdescripcion\b|\bdescription\b|\bimagen\b|\bimage\b|\bprecio\b|\bprice\b/.test(normalized))
        return "detail";
    if (/\bfiltro\b|\bfilter\b|\bcategoria\b|\bcategory\b|\bbusqueda\b|\bsearch\b|\bresultad\w*\b/.test(normalized))
        return "filtered_list";
    if (/\blistado\b|\bcatalog\w*\b|\bproductos?\b|\bitems?\b|\bcards?\b/.test(normalized))
        return "catalog";
    return "unknown";
}
function inferAssertionType(assertionText) {
    const normalized = (0, target_resolver_1.normalizeText)(assertionText);
...
function inferCurrentContext(snapshot, executedActions) {
    const visible = uniqueVisibleTexts(snapshot).map((t) => (0, target_resolver_1.normalizeText)(t));
    ...
    const actionsText = executedActions.map((a) => (0, target_resolver_1.normalizeText)(`${a.action} ${a.target}`)).join(" ");
    if (hasSuccessSignal && /\bsubmit\b|\benviar\b|\bconfirm\b|\bfinalizar\b|\bcomprar\b|\bpagar\b/.test(actionsText))
        return "confirmation";
    if ((hasDialog && hasInputs) || (hasInputs && snapshot.summary.buttons > 0))
        return "form";
    if (hasCartPageSignal || /\bopen\b.*\bcart\b|\bver\b.*\bcarrito\b|\bir al carrito\b|\bcheckout\b/.test(actionsText))
        return "cart";
    if ((hasHeading || hasImage || hasMoney || hasAddToCart) && /\bview\b|\bdetalle\b|\bselect\b|\bseleccionar\b|\bproduct\b|\bitem\b/.test(actionsText))
        return "detail";
    if ((hasCards || hasRows) && /\bfilter\b|\bfiltro\b|\bcategoria\b|\bcategory\b|\bbusqueda\b|\bsearch\b/.test(actionsText))
        return "filtered_list";
    if (hasCards || hasRows)
        return "catalog";
    if (hasAuthSignal)
...
function isContextSatisfied(required, current) {
    if (required === "unknown")
        return true;
    if (required === current)
        return true;
    if (required === "catalog" && (current === "catalog" || current === "filtered_list"))
        return true;
    return false;
}
function resolveProductDetailStructural(snapshot, assertionText) {
    const visible = uniqueVisibleTexts(snapshot).map((t) => (0, target_resolver_1.normalizeText)(t));
    ...
    const descriptionVisible = snapshot.elements.some((el) => {
        const txt = (0, target_resolver_1.normalizeText)(`${el.text ?? ""} ${el.label ?? ""}`);
...
function isAbstractExpected(text) {
    const normalized = (0, target_resolver_1.normalizeText)(text);
...
function buildConcreteAssertionsFromExpected(expectedTexts, existingAssertions, _options) {
    const assertions = [];
    const expectedResultConsumption = [];
    const nonExecutableCriteria = [];
    for (const text of expectedTexts) {
        ...
        if (quotedTexts.length > 0) {
            ...
            for (const quoted of quotedTexts) {
                if (!existingAssertions?.some((a) => (0, target_resolver_1.normalizeText)(a) === (0, target_resolver_1.normalizeText)(quoted))) {
        ...
        if (classification === "literal_observable" || classification === "structural_assertion") {
            if (!existingAssertions?.some((a) => (0, target_resolver_1.normalizeText)(a) === (0, target_resolver_1.normalizeText)(trimmed))) {
...
function resolveAssertionTargets(snapshot, assertionTargets, options) {
    ...
    function extractAssertionTarget(assertion) {
        const patterns = [
        ...
        for (const pattern of patterns) {
            ...
            if (match) {
                ...
                let kind = "text";
                if (assertion.toLowerCase().includes("botón"))
                    kind = "button";
                else if (assertion.toLowerCase().includes("opción"))
                    kind = "option";
                else if (assertion.toLowerCase().includes("control"))
                    kind = "control";
    ...
    for (const assertion of assertionTargets) {
        const structuredOracleType = assertion.canonicalAssertion?.oracleType
            ?? (assertion.rowScope !== undefined || assertion.expectedValueKey ? "row_scoped_value" : undefined);
        const isPassiveVisibility = !structuredOracleType && /validar que|assertion.*visible/i.test(assertion.action || assertion.target);
        if (structuredOracleType) {
            console.log(`[assertion] classified target="${assertion.target}" type=${structuredOracleType} source=canonical`);
        }
        else if (isPassiveVisibility) {
            const { target, kind } = extractAssertionTarget(assertion.target);
            console.log(`[assertion] classified target="${target}" type=passive_visibility kind="${kind}"`);
        }
    }
    ...
    return assertionTargets.map((assertion, index) => {
        const structuredOracleType = assertion.canonicalAssertion?.oracleType
            ?? (assertion.rowScope !== undefined || assertion.expectedValueKey ? "row_scoped_value" : undefined);
        if (assertion.rowScope !== undefined || assertion.expectedValueKey || structuredOracleType === "structural_row_count" || structuredOracleType === "entity_within_container") {
            return {
                assertionText: assertion.target,
                normalizedAssertion: (0, target_resolver_1.normalizeText)(assertion.target),
                classification: "structural_assertion",
                status: "needs_assertion_resolution",
                confidence: 0,
                reason: "row_scoped_requires_runtime_observation",
                closestCandidates: [],
                visibleTexts,
                assertionDiagnostics: {
                    rowScope: assertion.rowScope,
                    expectedValueKey: assertion.expectedValueKey,
                    oracleType: structuredOracleType,
                    globalTextFallbackUsed: false,
                },
            };
        }
                ...
                for (const el of snapshot.elements || []) {
                    ...
                    for (const text of texts) {
                        ...
                        if (normalizedText === normalizedAlias) {
                            console.log(`[assertion] aliasResolved target="${extractedTarget}" alias="${aliasForTarget}" source=app_config confidence=0.95`);
        ...
        if (isPassiveVisibility) {
            if (bestMatch) {
                console.log(`[assertion] passed target="${extractedTarget}" matched="${bestMatch.text}" source="${bestMatch.source}"`);
                return {
                    assertionText: assertion.target,
                    normalizedAssertion: (0, target_resolver_1.normalizeText)(assertion.target),
                ...
                console.log(`[assertion] failed target="${extractedTarget}" reason=ambiguous_assertion_target candidates=${candidates.length}`);
                return {
                    assertionText: assertion.target,
                    normalizedAssertion: (0, target_resolver_1.normalizeText)(assertion.target),
                    classification: "passive_visibility",
                ...
                // Task 5: Not found
                console.log(`[assertion] failed target="${extractedTarget}" reason=assertion_not_found`);
                return {
                    assertionText: assertion.target,
                    normalizedAssertion: (0, target_resolver_1.normalizeText)(assertion.target),
                    classification: "passive_visibility",
        ...
        const normalized = (0, target_resolver_1.normalizeText)(assertion.target);
        ...
        if (classification === "catalog_list_assertion") {
            const catalogResult = (0, catalog_list_resolver_1.resolveCatalogListAssertion)(snapshot, assertion.target);
            const passed = !shouldDeferForContext && catalogResult.passed;
            const assertionDiagnostics = {
        ...
        if (isCartAssertion(assertion.target)) {
            const hasCartNavigationIntent = /\bcart\b|\bcarrito\b|\bcheckout\b/.test(executedActions.map((a) => (0, target_resolver_1.normalizeText)(`${a.action} ${a.target}`)).join(" "));
        ...
        if (precondition.detected) {
            const relevantActions = executedActions.filter(a => a.action.toLowerCase().includes("add") ||
        ...
        const equivalentPrev = prevResults.find(r => r.status === "passed" &&
            (0, target_resolver_1.normalizeText)(r.assertionText) === normalized);
        if (equivalentPrev) {
            return {
                assertionText: assertion.target,
                normalizedAssertion: (0, target_resolver_1.normalizeText)(assertion.target),
        ...
        if (classification === "feedback_message") {
            const feedbackResult = (0, feedback_message_resolver_1.resolveFeedbackMessageAssertion)(snapshot, assertion.target);
        ...
        if (classification === "compound_form_fields") {
            const compoundResult = (0, compound_form_fields_resolver_1.resolveCompoundFormFieldsAssertion)(snapshot, assertion.target);

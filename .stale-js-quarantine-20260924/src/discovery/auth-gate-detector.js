...
exports.detectAuthGate = detectAuthGate;
    ...
    if (snapshot.elements) {
        for (const el of snapshot.elements) {
            ...
            if (el.label)
                texts.push(el.label);
            if (el.value)
                texts.push(String(el.value));
        }
    }
    if (snapshot.summary) {
        const summary = snapshot.summary;
        if (summary.buttons !== undefined)
            texts.push(`buttons:${summary.buttons}`);
        if (summary.inputs !== undefined)
            texts.push(`inputs:${summary.inputs}`);
    }
    return texts;
}
function extractClassNames(snapshot) {
    const classes = [];
    if (snapshot.elements) {
        for (const el of snapshot.elements) {
            if (el.className) {
...
function countInputs(snapshot) {
    if (snapshot.summary?.inputs !== undefined) {
        return snapshot.summary.inputs;
    }
    if (snapshot.elements) {
        return snapshot.elements.filter((el) => el.tagName === "input").length;
    }
    return 0;
}
...
function hasMatchingClass(classes, indicators) {
    const normalizedClasses = classes.map(normalizeText);
    return indicators.some((indicator) => normalizedClasses.some((cls) => cls.includes(normalizeText(indicator))));
}
function hasContinueButton(texts) {
    const normalizedTexts = texts.map(normalizeText);
    return CONTINUE_BUTTON_KEYWORDS.some((keyword) => normalizedTexts.some((text) => text.includes(normalizeText(keyword))));
}
function countNumericButtons(snapshot) {
    if (!snapshot.elements)
        return 0;
    let count = 0;
    for (const el of snapshot.elements) {
...
function detectAuthGate(snapshot) {
    ...
    if (credentialsMatches.length > 0) {
        const strongMatches = credentialsMatches.filter(k => STRONG_CREDENTIAL_KEYWORDS.some(sk => normalizeText(k).includes(normalizeText(sk))));
    ...
    if (inputMatches.length > 0) {
        ...
        if (hasInputIndicators && (inputConfidence > confidence || stage === "identification_type_selection")) {
            ...
            if (hasVirtualKeyboard)
                evidence.push("virtual keyboard detected");
            if (continueButtonPresent)
                evidence.push("continue button present");
        }
    }
    ...
    if (otpMatches.length > 0 || hasOtpInputs) {
        ...
        if (stage !== "phone_confirmation" || otpConfidence > confidence + 0.2) {
            ...
            if (hasOtpInputs)
                evidence.push("otp-input elements detected");
            if (hasVirtualKeyboard)
                evidence.push("virtual keyboard detected");
        }
    }
    ...
    const detected = confidence >= 0.5 && stage !== "authenticated_landing";
    return {
        detected,
        gateType,
        stage,

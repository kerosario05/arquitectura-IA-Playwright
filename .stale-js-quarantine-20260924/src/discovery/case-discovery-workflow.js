...
exports.resolvePromotionOutcome = resolvePromotionOutcome;
exports.reconcileWorkflowAssertionsBeforeFinalStatus = reconcileWorkflowAssertionsBeforeFinalStatus;
exports.attachObservableOracleRequirementRefs = attachObservableOracleRequirementRefs;
exports.resolveCanonicalRequirementById = resolveCanonicalRequirementById;
exports.resolveObservableOraclePolarity = resolveObservableOraclePolarity;
exports.buildPromotionSourceScenario = buildPromotionSourceScenario;
exports.isAssertionLikeStep = isAssertionLikeStep;
exports.inferAssertionImportance = inferAssertionImportance;
exports.buildRuntimeEvidenceTrace = buildRuntimeEvidenceTrace;
exports.collectLocalPendingAssertionDiagnostics = collectLocalPendingAssertionDiagnostics;
exports.printCaseDiscoverySummary = printCaseDiscoverySummary;
exports.toScenarioDataOverrides = toScenarioDataOverrides;
exports.buildDiscoveryBrowserContextOptions = buildDiscoveryBrowserContextOptions;
exports.runCaseDiscoveryWorkflow = runCaseDiscoveryWorkflow;
const node_path_1 = __importDefault(require("node:path"));
const promises_1 = require("node:fs/promises");
const test_1 = require("@playwright/test");
const ai_explorer_1 = require("../ai/ai-explorer");
...
const promotion_gate_1 = require("../automations/promotion-gate");
...
const segment_route_recovery_1 = require("../agent/segment-route-recovery");
...
function attachCanonicalScenarioLineage(scenario) {
    const rawCase = scenario.raw;
    if (!rawCase || rawCase.id !== scenario.caseId || typeof rawCase.title !== "string") {
        return scenario;
    }
    const canonicalScenario = (0, testrail_canonical_adapter_1.canonicalizeTestRailCase)(rawCase);
    const canonicalInputRequirements = (0, testrail_canonical_adapter_1.extractCanonicalInputRequirements)(rawCase);
    const runtimeTransformation = (0, testrail_runtime_transformer_1.transformTestRailCaseForRuntime)(rawCase);
    ...
    const stepRequirementRefs = canonicalScenario.steps.flatMap((step) => (step.requirementRefs ?? []).map((requirementId) => ({ stepIndex: step.order, requirementId })));
    return {
        ...scenario,
        steps,
...
function resolvePromotionOutcome(input) {
...
function scoreSemanticEquivalence(expected, observed) {
    ...
    if (normalizedExpected === normalizedObserved)
        return 1;
    if (normalizedExpected.includes(normalizedObserved) || normalizedObserved.includes(normalizedExpected))
        return 0.86;
...
function isClickAction(action) {
    ...
    if (normalized === "click" || normalized === "select" || normalized === "tap")
        return true;
...
function collectSemanticObservedTargets(caseResult) {
    const fromRuntime = (caseResult.runtimeEvidenceTrace?.clickActions ?? [])
        ...
        source: click.transitionDetected || click.postClickUiChange ? "runtime_transition" : "runtime_snapshot",
        evidence: [
    ...
    const fromSteps = caseResult.steps
        ...
        return {
            ...
            source: step.authGateDiagnostics?.detected ? "runtime_auth_gate" : "runtime_snapshot",
            evidence: [
...
function extractQuotedAssertionTarget(text) {
    if (!text)
        return undefined;
    const match = text.match(/["'“”‘’]([^"'“”‘’]+)["'“”‘’]/);
    return match ? match[1].trim() : undefined;
}
function isAssertionAction(text) {
    return /^(validar|verificar|assert|comprobar|confirmar|mostrar|visualizar|se\s+muestr)/i.test((text ?? "").trim());
}
function findCausalTransition(transitionEvidence, assertionStepIndex, caseResult) {
    ...
    if (caseResult?.steps.some((step) => step.index > candidate.stepIndex
...
function isStructuralStateAssertion(text) {
...
function findAssertionStepIndexAfter(scenarioSteps, actionStepIndex) {
...
function buildExpectedObservableOracles(expectedLines, observedAssertions, caseResult, authGateEvidence, authMetadata, semanticReconciliations, scenarioSteps, canonicalRequirements, expectedResultRequirementRefs, recordingOutcomePolarity) {
    ...
    const resolveScenarioAssertionPolarity = (_requirement, stepIndex) => {
        const scenarioStep = typeof stepIndex === "number"
            ? scenarioSteps?.find((step) => step.index === stepIndex)
            : undefined;
        if (scenarioStep?.polarity)
            return scenarioStep.polarity;
        return resolveObservableOraclePolarity(scenarioStep?.requirementRefs, canonicalRequirements);
    };
    const authAssertionCandidates = assertionCandidates.filter((candidate) => /(auth|autentic|identific|otp|flujo)/.test(candidate.combined));
    const navigationAssertionCandidates = assertionCandidates.filter((candidate) => /(redirig|dirig|modul|naveg|ruta|acceso|transicion|ingres)/.test(candidate.combined));
    ...
    return expectedLines.map((line, index) => {
        ...
        const semanticEquivalent = Boolean(bestSemanticTarget
        ...
        let type = "unsupported_or_unresolved";
        let backed = false;
        const evidence = [];
        let details;
        let stepIndex;
        ...
        if (authSignal && (authGateEvidence?.authGateDiagnostics?.detected || authMetadata?.authGateDetectedDuringDiscovery === true)) {
            ...
            backed = true;
            const navigationAssertion = assertionCandidates.find((candidate) => isStructuralStateAssertion(candidate.requirement)
                && (normalizeOracleText(candidate.requirement).includes(normalized)
                    || normalized.includes(normalizeOracleText(candidate.requirement)))) ?? navigationAssertionCandidates[0];
            ...
            polarity = resolveObservableOraclePolarity(navigationAssertion?.requirementRefs, canonicalRequirements) ?? resolveScenarioAssertionPolarity(requirement, assertionIndex) ?? recordingOutcomePolarity;
            target = causalTransition.resolvedTarget ?? causalTransition.target;
            evidence.push(`transition_observed:${causalTransition.transitionDetected === true}`, `click_target:${causalTransition.target}`, ...(causalTransition.resolvedTarget ? [`resolved_target:${causalTransition.resolvedTarget}`] : []), `action_type:${causalTransition.actionType}`);
            ...
            if (polarity)
                evidence.push(`assertion_polarity:${polarity}`);
        }
            ...
            console.log(`[semantic-reconciliation] expected="${line}" observed="${bestSemanticTarget.candidate.observedTarget}" equivalent=true confidence=${bestSemanticTarget.confidence.toFixed(2)} source=${bestSemanticTarget.candidate.source}`);
        }
...
function reconcileWorkflowAssertionsBeforeFinalStatus(scenario, steps) {
    const provisionalResult = {
        steps,
        runtimeEvidenceTrace: buildRuntimeEvidenceTrace({ steps }),
    };
    ...
    const pendingAssertions = steps.filter((step) => isAssertionLikeStep(step)
    ...
    for (const pending of pendingAssertions) {
        ...
        const alreadyBound = oracles.some((oracle) => oracle.backed === true
            && typeof oracle.stepIndex === "number"
            && oracle.stepIndex === pending.index);
        if (!target || alreadyBound)
            continue;
        const supportingOracle = sourceOracles.find((oracle) => {
            if (oracle.backed !== true || !["navigation_transition", "auth_gate"].includes(oracle.type))
                return false;
    ...
    return (0, case_discovery_1.reconcilePendingAssertionsWithBackedOracles)(steps, oracles);
}
function attachObservableOracleRequirementRefs(oracles, sourceSteps) {
...
function resolveCanonicalRequirementById(requirements, requirementId) {
    if (!requirements || !requirementId)
        return undefined;
    return requirements.find((requirement) => requirement.requirementId === requirementId);
}
function resolveObservableOraclePolarity(requirementRefs, requirements) {
    if (!requirementRefs || requirementRefs.length === 0 || !requirements)
        return undefined;
    const polarities = requirementRefs.map((ref) => resolveCanonicalRequirementById(requirements, ref)?.polarity);
    if (polarities.some((polarity) => polarity === undefined))
        return undefined;
    const distinct = new Set(polarities);
    return distinct.size === 1 ? polarities[0] : undefined;
}
function buildPromotionSourceScenario(scenario, caseResult) {
    const canonicalRequirements = scenario.canonicalRequirements;
    const expectedResultRequirementRefs = scenario.expectedResultRequirementRefs;
    const recordingExecutionContract = scenario.recordingExecutionContract;
    const recordingNegativeOracle = scenario.negativeOracle;
    ...
    const satisfiedByPreviousAssertionSteps = caseResult.steps
        .filter((step) => isAssertionLikeStep(step))
        .filter((step) => step.status === "satisfied_by_previous_assertion"
    ...
    const recordingOutcomePolarity = recordingExecutionContract
        && !recordingNegativeOracle
        && transitionEvidence.length > 0
        ? "positive"
        : undefined;
    const resolveScenarioStepPolarity = (stepIndex, _requirement) => {
        const scenarioStep = scenario.steps.find((candidate) => candidate.index === stepIndex);
        const declaredPolarity = scenarioStep?.polarity;
        if (declaredPolarity)
            return declaredPolarity;
        const requirementRefs = Array.isArray(scenarioStep?.requirementRefs)
            ? scenarioStep.requirementRefs
            : undefined;
        return resolveObservableOraclePolarity(requirementRefs, canonicalRequirements);
    };
    const satisfiedReconciledOracles = satisfiedByPreviousAssertionSteps
        .map((step) => {
        ...
        if (authSignal && gateDetected) {
            console.log(`[observable-oracle-reconciled] type=auth_gate backed=true step=${step.index} text="${requirement.slice(0, 120)}"`);
            return {
        ...
        if ((navigationSignal || isStructuralStateAssertion(requirement)) && transitionEvidence.length > 0) {
            ...
            const expectedUrl = typeof causalTransition.afterUrl === "string" && causalTransition.afterUrl.trim()
                ? causalTransition.afterUrl.trim()
                : undefined;
            console.log(`[observable-oracle-reconciled] type=navigation_transition backed=true step=${step.index} sourceActionStepIndex=${causalTransition.stepIndex} target="${causalTransition.resolvedTarget ?? causalTransition.target}"${expectedUrl ? ` expectedUrl=${expectedUrl}` : ""}`);
            return {
        ...
        console.log(`[observable-oracle-reconciled] type=unsupported_or_unresolved backed=false step=${step.index} text="${requirement.slice(0, 120)}"`);
        return {
    ...
    const scenarioSteps = scenario.steps.map((step) => ({
        ...
        conditionalAction: step.conditionalAction,
    ...
    const seenControlledAdvanceAssertions = new Set();
    const controlledAdvanceAssertions = scenario.steps.flatMap((step) => {
        ...
        return refs.flatMap((ref) => ((canonicalRequirementById.get(ref)?.assertionIntents ?? []).flatMap((intent) => {
            const key = `${ref}:${intent}`;
            if (seenControlledAdvanceAssertions.has(key))
                return [];
    ...
    const controlledAdvanceOracles = caseResult.controlledAdvanceProbe && controlledAdvanceAssertions.length > 0
        ? (0, controlled_advance_probe_1.buildControlledAdvanceProbeOracles)({ result: caseResult.controlledAdvanceProbe, assertions: controlledAdvanceAssertions })
        : [];
    const semanticReconciliations = [];
    const observableOraclesRaw = [
        ...
        ...buildExpectedObservableOracles(expectedLines, observedAssertions, caseResult, authGateEvidence, authMetadata, semanticReconciliations, scenarioSteps, canonicalRequirements, expectedResultRequirementRefs, recordingOutcomePolarity),
    ...
    const filteredObservableOracles = observableOraclesRaw.filter((oracle) => {
        const provisional = oracle.type === "unsupported_or_unresolved"
            && oracle.backed === false
            && typeof oracle.stepIndex === "number"
            && backedStepIndexes.has(oracle.stepIndex);
        if (provisional) {
    ...
    return {
        ...
        stepRequirementRefs: scenario.stepRequirementRefs?.map((ref) => ({
            stepIndex: ref.stepIndex,
            requirementId: ref.requirementId,
            facet: ref.facet,
        })),
        stepClaims: scenario.stepClaims?.map((claim) => ({
...
function getDefaultOutputDir(id) {
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    const label = typeof id === "number" ? `case-${id}` : `story-${id}`;
    return node_path_1.default.resolve(`./.artifacts/discovery/${label}/${stamp}`);
}
function isAssertionLikeStep(step) {
    const actionText = (step.action ?? "").toLowerCase();
    if (step.assertionStatus || step.assertionClassification)
        return true;
    if (["asserttext", "assertvisible", "assertexists"].includes(actionText.replace(/\s+/g, "")))
        return true;
...
function inferAssertionImportance(assertionText, scenarioTitle) {
...
function inferAssertionTypeFromText(assertionText) {
...
function normalizeForensics(value) {
    return value
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
...
function buildRuntimeEvidenceTrace(caseResult) {
    const clickActions = caseResult.steps
        ...
        actionType: s.action,
        ownerContext: s.assertionDiagnostics?.assertionContextDiagnostics?.currentContext,
        locatorStrategy: s.locatorStrategy,
        controlIdentity: s.controlIdentity,
        success: true,
        transitionDetected: s.status !== "click_no_transition",
        postClickUiChange: s.assertionDiagnostics?.postClickUiChangeReason,
        beforeContext: s.assertionDiagnostics?.assertionContextDiagnostics?.previousContext,
        afterContext: s.assertionDiagnostics?.assertionContextDiagnostics?.currentContext,
    ...
    const fillActions = caseResult.steps
        ...
        valueKey: s.assertionDiagnostics?.valueKey,
        source: s.assertionDiagnostics?.valueSource,
        locatorStrategy: s.locatorStrategy,
        success: true,
        activeContainerType: s.assertionDiagnostics?.activeContainer?.type,
        controlIdentity: s.controlIdentity
    }));
    ...
    const toContextType = (step) => {
        const contextDiag = step.assertionDiagnostics?.assertionContextDiagnostics;
        const current = normalizeForensics(String(contextDiag?.currentContext ?? ""));
        if (/filtered|search/.test(current))
            return "filtered_list";
        if (/catalog/.test(current))
            return "catalog";
        if (/list/.test(current))
            return "list";
        if (/detail/.test(current))
            return "detail";
        if (/cart|checkout|summary/.test(current))
            return "cart";
        if (/form|modal|dialog/.test(current))
            return "form";
        if (/confirm|success/.test(current))
            return "confirmation";
        const signals = step.structuralSignals ?? [];
        if (signals.some((s) => /catalog|list|items_visible|cards_visible/.test(s)))
            return "list";
        if (signals.some((s) => /detail|product_detail/.test(s)))
            return "detail";
        if (signals.some((s) => /cart|summary/.test(s)))
            return "cart";
        if (signals.some((s) => /form|modal/.test(s)))
            return "form";
        if (signals.some((s) => /confirm|success/.test(s)))
            return "confirmation";
        return undefined;
    };
    const toEvidenceType = (step) => {
        const signals = step.structuralSignals ?? [];
        const text = normalizeForensics(`${step.targetText ?? ""} ${step.matchedText ?? ""} ${step.error ?? ""}`);
        if (signals.some((s) => /list|catalog|cards|items/.test(s)) || /\b(list|catalog|cards|items|productos)\b/.test(text))
            return "items_visible";
        if (signals.some((s) => /detail|product_detail/.test(s)) || /\b(detail|detalle|descripcion|description|price|precio)\b/.test(text))
            return "item_detail_visible";
        if (signals.some((s) => /form|modal|dialog/.test(s)) || /\b(form|modal|dialog)\b/.test(text))
            return "form_visible";
        if (signals.some((s) => /summary|cart|total/.test(s)) || /\b(summary|resumen|total|cart|carrito)\b/.test(text))
            return "summary_visible";
        if (signals.some((s) => /success|confirmation/.test(s)) || /\b(thank you|success|confirmacion|confirmation)\b/.test(text))
            return "success_message_visible";
        return undefined;
    };
    ...
    const feedbackEvidence = caseResult.steps
        ...
        if (!looksLikeFeedback)
            return undefined;
        const source = /\b(alert)\b/.test(normalizedMessage) ? "alert"
...
function buildForensicsRecommendation(step) {
    const diag = (step.assertionDiagnostics ?? {});
    const contextDiag = diag.assertionContextDiagnostics;
...
function reconcileDiscoveryStatusAfterLocalClosure(caseResult, localPending) {
    ...
    const canTreatEarlyCompletionAsStale = localPending.shouldSkipAutoRepair
    ...
    const consumedAssertions = new Set((localPending.localClosureDiagnostics?.consumed ?? []).map((a) => a.trim().toLowerCase()));
    const blockingFailures = caseResult.steps.filter((s) => {
        if (!blockingStatuses.has(s.status))
            return false;
    ...
    const noBlockers = afterPendingAssertionCount === 0
        && pendingActionsCount === 0
        && functionalRequiredCount === 0
        && unresolvedPreconditionsCount === 0
        && blockingFailuresCount === 0
        && !hasBlockingFailedReason;
    const reconciledConsumedAssertions = new Set((localPending.localClosureDiagnostics?.consumed ?? []).map((a) => a.trim().toLowerCase()));
    const reconciledSteps = caseResult.steps.map((step) => {
        ...
        if (!(step.status === "needs_assertion_resolution" || step.status === "not_found"))
            return updatedStep;
        const key = (step.targetText ?? step.action).trim().toLowerCase();
        if (!reconciledConsumedAssertions.has(key))
            return updatedStep;
        return {
            ...updatedStep,
    ...
    if (noBlockers) {
        const reconciledPlan = caseResult.candidatePlan
            ? { ...caseResult.candidatePlan, status: "validated" }
...
async function loadLatestSnapshot(dir) {
    try {
        ...
        if (!last)
            return {};
        const snapshotPath = node_path_1.default.join(dir, last);
        return { snapshotPath, snapshot: JSON.parse(await (0, promises_1.readFile)(snapshotPath, "utf-8")) };
    }
    catch {
        return {};
    }
}
function collectLocalPendingAssertionDiagnostics(caseResult, runtimeEvidenceTrace, narrativeRequirements) {
    const normalize = (value) => value
        ...
        .replace(/\s+/g, " ")
        .trim();
    const tokenSet = (value) => new Set(normalize(value)
        .split(" ")
        .filter((t) => t.length > 1 && !["de", "la", "el", "y", "the", "of", "to", "en", "for", "and"].includes(t)));
    const coversByTokens = (needle, haystack) => {
    ...
    const hasTransitionBackedClick = (runtimeEvidenceTrace?.clickActions ?? []).some((click) => click.success && (click.transitionDetected === true || Boolean(click.postClickUiChange)));
    const transitionBackedTarget = (runtimeEvidenceTrace?.clickActions ?? [])
        ...
        ?.target ?? "";
    const planMetadata = caseResult.candidatePlan?.metadata;
    const hasDetectedAuthGate = caseResult.steps.some((step) => step.authGateDiagnostics?.detected) ||
    ...
    const hasNavigationNarrativeSignal = (text) => /(redirig|dirig|modul|naveg|ruta|acceso|transicion|ingres|transition)/.test(normalize(text));
    const hasAuthNarrativeSignal = (text) => /(auth|autentic|identific|otp|flujo|sesion|session)/.test(normalize(text));
    const hasStructuralEvidenceFor = (assertionText, inferredType) => {
        ...
        return structuralEvidence.some((e) => {
            ...
            if (inferredType === "list")
                return e.evidenceType === "items_visible" || /catalog|list|cards|items|productos/.test(normalized);
            if (inferredType === "detail")
                return e.evidenceType === "item_detail_visible" || /detalle|detail|producto|price|precio|descripcion/.test(normalized);
            if (inferredType === "cart")
                return e.evidenceType === "summary_visible";
            if (inferredType === "form")
                return e.evidenceType === "form_visible";
            if (inferredType === "confirmation")
                return e.evidenceType === "success_message_visible";
            return textMatch || signals.includes(normalize(inferredType));
        });
    };
    const hasContextEvidence = (assertionText, contextType) => {
        const normalized = normalize(assertionText);
        return structuralEvidence.some((e) => {
    ...
    const wasActionExecutedForAssertion = (assertionText) => {
        const normalized = normalize(assertionText);
        return executedSuccessfulActions.some((executed) => {
            const e = normalize(executed);
            if (coversByTokens(normalized, e) || coversByTokens(e, normalized))
                return true;
    ...
    const hasFeedbackEvidenceFor = (assertionText) => {
        const normalized = normalize(assertionText)
    ...
    const consumedByLocalEvidence = (assertionText) => {
        ...
        if (hasTransitionBackedClick
            ...
            && ((hasNavigationNarrativeSignal(assertionText) && (bestSemanticMatch?.confidence ?? 0) >= 0.35)
                || (Boolean(navNarrativeLine) && scoreSemanticEquivalence(assertionText, navNarrativeLine) >= 0.5))) {
        ...
        if (hasDetectedAuthGate
            && (hasAuthNarrativeSignal(assertionText) || Boolean(authNarrativeLine))
            && (hasAuthNarrativeSignal(assertionText)
                || (Boolean(authNarrativeLine) && scoreSemanticEquivalence(assertionText, authNarrativeLine) >= 0.5))) {
        ...
        if (wasActionExecutedForAssertion(assertionText))
            return true;
        if (hasFeedbackEvidenceFor(assertionText))
            return true;
        if (hasStructuralEvidenceFor(assertionText, inferredType))
            return true;
        ...
        if (/\b(carrito|cart|resumen|summary|producto|item|fila|precio|total|subtotal)\b/.test(normalized)) {
            if (hasContextEvidence(assertionText, "cart"))
                return true;
        }
        ...
        if (/\b(formulario|form|modal|dialog|campo|required|requerido|visible)\b/.test(normalized)) {
            if (hasContextEvidence(assertionText, "form"))
                return true;
        }
        ...
        if (/\b(confirmacion|confirmation|cerrad|close|ok|aceptar|finalizar|continuar|home|menu|cart|navegacion)\b/.test(normalized)) {
            if (hasContextEvidence(assertionText, "confirmation"))
                return true;
        }
        return false;
    };
    const pendingSteps = caseResult.steps.filter((step) => step.status === "needs_assertion_resolution"
    ...
    const buildForensics = (step) => {
        if (!(step.status === "needs_assertion_resolution" || (step.status === "not_found" && isAssertionLikeStep(step))))
            return undefined;
        const diag = (step.assertionDiagnostics ?? {});
        const consumptionDiag = diag.assertionConsumptionDiagnostics;
        const contextDiag = diag.assertionContextDiagnostics;
        ...
        if (inferredType === "field")
            expectedConsumption.push("satisfied_by_fill_action");
        if (inferredType === "action")
            expectedConsumption.push("satisfied_by_action_executed");
        if (inferredType === "form")
            expectedConsumption.push("satisfied_by_form_field_presence");
        if (["list", "detail", "cart", "confirmation"].includes(inferredType))
            expectedConsumption.push("satisfied_by_structural_evidence");
        if (/\b(product added|agregado|success|exito|confirmation|confirmacion|thank you|guardado)\b/.test(normalizedAssertion))
            expectedConsumption.push("satisfied_by_feedback_message");
        if (inferredType === "confirmation")
            expectedConsumption.push("satisfied_by_confirmation_closed");
        ...
        let notConsumedReason = "concrete_evidence_missing";
        if (requiredContext !== "unknown" && requiredContext !== currentContext && contextDiag?.decision === "deferred_until_context")
            notConsumedReason = "wrong_context";
        else if ((structuralMatch || feedbackMatch || evidenceAvailable) && !consumptionDiag?.decision)
            notConsumedReason = "evidence_not_passed_to_resolver";
        else if (inferredType === "field" && /"|,|\by\b|\band\b/.test(assertionText) && !matchingFill)
            notConsumedReason = "compound_field_not_split";
        else if (!evidenceAvailable && expectedConsumption.length > 0)
            notConsumedReason = "missing_history";
        else if (!feedbackMatch && step.closestCandidates && step.closestCandidates.length > 0 && step.closestCandidates[0].score < 0.5)
            notConsumedReason = "normalization_mismatch";
        else if (step.assertionClassification === "ambiguous_assertion")
            notConsumedReason = "ambiguous_assertion";
        return {
    ...
    for (const step of pendingSteps) {
        const diag = (step.assertionDiagnostics ?? {});
        const decision = diag.assertionContextDiagnostics?.decision;
    ...
    for (const pending of pendingFromEarlyCompletion) {
        ...
        if (/\b(country|city|credit card|month|year|name|amount|card number|date)\b/.test(normalizedPending)
        ...
        if (/\b(add to cart|place order|purchase|thank you|confirmacion|confirmation|cerrar|close|home|cart)\b/.test(normalizedPending)
    ...
    const consumedAssertions = [];
    const allLocalBySteps = pendingSteps.length > 0 && pendingSteps.every((step) => {
        const diag = (step.assertionDiagnostics ?? {});
        const decision = diag.assertionContextDiagnostics?.decision;
        const consumptionDecision = diag.assertionConsumptionDiagnostics?.decision;
        ...
        if (decision === "deferred_until_context" || decision === "structurally_satisfied")
            return true;
        const reason = typeof step.error === "string" ? step.error : "";
    ...
    const partialReason = deferredCount === pendingSteps.length
...
function printCaseDiscoverySummary(result, workflowResult) {
    console.log("");
    ...
    for (const step of result.steps) {
        ...
        if (step.status === "found")
            ...
            icon = "✗";
        else if (step.status === "click_no_transition")
            icon = "⚠";
        else if (step.status === "optional_confirmation_detail_missing")
            icon = "⚠";
        else if (step.status === "satisfied_by_previous_assertion")
            icon = "✓";
        else if (step.status === "precondition_unresolved")
            icon = "⚠";
        else if (step.status === "skipped_semantic_descriptor")
            icon = "○";
        else if (step.status === "satisfied_by_children")
            icon = "✓";
...
function toScenarioDataOverrides(runtimeEntries) {
    const overrides = {};
...
function buildDiscoveryBrowserContextOptions(config) {
    return { ignoreHTTPSErrors: config.app?.ignoreHTTPSErrors === true };
}
async function runCaseDiscoveryWorkflow(options) {
    const startTime = Date.now();
    let activeConfig = options.config ?? env_1.config;
    const outputId = options.scenario?.externalId ?? options.caseId ?? "unknown";
    const outputDir = options.outputDir ? node_path_1.default.resolve(options.outputDir) : getDefaultOutputDir(outputId);
    const evidenceDir = node_path_1.default.join(outputDir, "evidence");
    const pendingObjectsPath = node_path_1.default.join(outputDir, "discovered-objects.pending.json");
    const pendingPlansPath = node_path_1.default.join(outputDir, "discovered-plans.pending.json");
    ...
    try {
        const { loadPromotedAppConfigSync: loadCfg } = await Promise.resolve().then(() => __importStar(require("../automations/app-profile")));
        const cfg = loadCfg({ appSlug: workflowAppSlug });
        const projectConfig = await (0, project_reader_1.getProjectConfigurationBySlug)(workflowAppSlug);
        ...
        const configured = projectBaseUrl
            ?? (!hasProjectWebConfig && typeof cfg?.baseUrl === "string" ? cfg.baseUrl.trim() : undefined);
        const requested = activeConfig.app.baseUrl?.trim();
        if (configured) {
        ...
        if (e instanceof Error && e.message.includes("FAIL CLOSED"))
            throw e;
    ...
    if (options.scenario) {
        ...
        if (!options.testRailClient) {
            const testRailRuntimeConfig = (0, env_1.requireTestRailConfig)(activeConfig);
            client = new testrail_client_1.TestRailClient(testRailRuntimeConfig);
        }
        else {
    ...
    if (sectionProfile) {
        scenario.sectionId = sectionProfile.sectionId;
        scenario.sectionName = sectionProfile.sectionName;
    }
    const browserType = { chromium: test_1.chromium, firefox: test_1.firefox, webkit: test_1.webkit }[activeConfig.execution.browser];
    ...
    try {
        ...
        const runtimeEntryByKey = new Map(runtimeEntries.map((entry) => [entry.key.trim().toLowerCase(), entry]));
        const runtimeValue = (...keys) => {
        ...
        const loginConfig = runtimeUsername || runtimePassword
            ...
            : activeConfig;
        const loginStrategy = (0, login_strategy_factory_1.getLoginStrategy)(loginConfig.app.loginMode);
        ...
        const evidenceRecorder = await (async () => {
            try {
                const { initDiscoveryEvidence } = await Promise.resolve().then(() => __importStar(require("../evidence/discovery-evidence")));
        ...
        try {
            caseResult = await (0, case_discovery_1.runCaseDiscovery)({
                page,
                scenario,
                evidenceDir,
                pendingObjectsPath,
                ...
                aiAssistedDiscovery: {
                    explorer: (0, ai_explorer_1.createAIExplorer)({
            ...
            try {
                const { finalizeDiscoveryEvidence } = await Promise.resolve().then(() => __importStar(require("../evidence/discovery-evidence")));
                ...
                if (evidenceRecorder && caseResult?.steps?.length) {
                    ...
                    let isDetail = false;
                    if (lastAction === "assert" || lastAction === "assertVisible") {
                        kind = "assertionEvidence";
                    }
                    else if (lastAction === "click" && /Continuar|Confirmar|Cancelar|Enviar|Volver|Generar|Imprimir/i.test(lastTarget)) {
                        kind = "formEvidence";
                    }
                    else if (lastAction === "click" && /seleccionar\s+(?:el|la)\s+primer/i.test(lastTarget)) {
                        kind = "selectionEvidence";
                    }
                    else if (lastAction === "click" && /detalle|consultar\s+detalle/i.test(lastTarget)) {
                        kind = "detailEvidence";
                        isDetail = true;
                    }
                ...
                if (caseResult) {
                    console.log(`[discovery:workflow] status after evidence finalize: ${caseResult.status} ` +
        ...
        const agentCfg = (0, agent_1.resolveAgentAutoRepairConfig)(activeConfig);
        ...
        let diagnosticsBuildError;
        let runtimeEvidenceTrace = {
            clickActions: [],
            fillActions: [],
            formEvidence: [],
        ...
        try {
            ...
            const narrativeRequirements = [
                scenario.title,
                ...(typeof scenario.expectedResult === "string" ? [scenario.expectedResult] : []),
        ...
        if (localPending.shouldSkipAutoRepair) {
            ...
            const autoRepairDecisionDiagnostics = {
                attempted: true,
                skipped: true,
        ...
        const shouldAttemptRepair = agentCfg.enabled
        ...
        if (shouldAttemptRepair) {
            ...
            const dataContext = (0, data_context_1.buildDataContext)(activeConfig);
            const autoRepairStartDiagnostics = {
                attempted: true,
                skipped: false,
                evaluatedPendingAssertions: localPending.pendingAssertions,
                localClosureAttempted: true,
            ...
            for (let attempt = 1; attempt <= agentCfg.maxAttempts; attempt += 1) {
                ...
                const segmentResult = await (0, segment_route_recovery_1.runSegmentedRouteRecovery)({
                    page,
                    outputDir,
                    evidenceDir,
                    fullConfig: activeConfig,
        ...
        const promotionPolicy = {
            ...
            autoApproveConfidenceThreshold: options.autoPomThreshold ?? automation_promotion_types_1.DEFAULT_PROMOTION_POLICY.autoApproveConfidenceThreshold,
        ...
        const promotionResultPath = node_path_1.default.join(outputDir, "promotion-result.json");
        ...
        if (options.contextOnly) {
            if (!caseResult.candidatePlan || !sourceScenario.steps?.length) {
                return { ...caseResult, status: "discovery_failed", failedReason: "insufficient_context", outputDir };
            }
            await (0, promote_plan_1.materializePromotionContext)({ plan: caseResult.candidatePlan, sourceScenario, appProfile: options.appProfile, sectionSlug: sectionProfile?.sectionSlug, outputRoot: node_path_1.default.resolve(".") });
            return {
                caseResult,
        ...
        if (!options.autoPromote) {
            ...
            const promotedEntry = await (0, promote_plan_1.promoteExecutionPlan)({
            ...
            specGeneration = (() => {
                const raw = promotedEntry.metadata?.specGeneration;
                const validation = raw?.validation;
                const finalSpec = raw?.finalSpec;
                if (!raw || !validation || !finalSpec)
                    return undefined;
                return {
                    ...
                    failedGatesAttempt1: Array.isArray(raw.failedGatesAttempt1)
                        ? raw.failedGatesAttempt1.filter((value) => typeof value === "string")
                        : undefined,
                    oracleTypes: Array.isArray(raw.oracleTypes)
                        ? raw.oracleTypes.filter((value) => typeof value === "string")
                        : undefined,
                    ...
                    errors: Array.isArray(raw.errors) ? raw.errors.filter((value) => typeof value === "string") : [],
        ...
        await (0, promises_1.writeFile)(promotionResultPath, JSON.stringify(promotionReport, null, 2), "utf-8");
    }
    finally {
        if (session) {
            await session.close();

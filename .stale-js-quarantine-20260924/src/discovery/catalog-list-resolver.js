...
exports.resolveCatalogListAssertion = resolveCatalogListAssertion;
exports.isCatalogListAssertion = isCatalogListAssertion;
...
function isCatalogListAssertion(assertionText) {
...
function extractCategoryFromAssertion(assertionText) {
...
function collectContainerItems(snapshot) {
...
function analyzeItemForEvidence(elements) {
    const evidence = {
        hasTitle: false,
        hasPrice: false,
        hasImage: false,
...
function analyzeFlatElements(elements) {
    const items = [];
    ...
    const maxCount = Math.max(priceElements.length, imageElements.length, titleElements.length, 0);
    if (maxCount === 0)
        return [];
...
function resolveCatalogListAssertion(snapshot, assertionText) {
    ...
    const passingItems = items.filter((item) => {
        if (requiresTitle && !item.hasTitle)
            return false;
        if (requiresPrice && !item.hasPrice)
            return false;
        if (requiresImage && !item.hasImage)
            return false;
        return true;
    });
    ...
    for (const item of passingItems.slice(0, 5)) {
        const parts = [];
        if (item.hasTitle)
            parts.push(`item has title text${item.titleText ? `: "${item.titleText.slice(0, 40)}"` : ""}`);
        if (item.hasPrice)
            parts.push(`item has price pattern${item.priceText ? `: "${item.priceText}"` : ""}`);
        if (item.hasImage)
            parts.push(`item has visible image${item.imageAlt ? `: "${item.imageAlt.slice(0, 40)}"` : ""}`);
        if (parts.length > 0) {

...
exports.resolveAdvanceCandidate = resolveAdvanceCandidate;
exports.resolveAdvanceControl = resolveAdvanceControl;
exports.compareControlledAdvanceSnapshots = compareControlledAdvanceSnapshots;
exports.runControlledAdvanceProbe = runControlledAdvanceProbe;
exports.buildControlledAdvanceProbeOracles = buildControlledAdvanceProbeOracles;
...
function isStructurallyEligibleAdvance(control) {
    if (!control.visible)
        return false;
...
function eligibleAdvanceControls(controls) {
 ...
 */
function resolveAdvanceCandidate(controls, preferredAction) {
...
function resolveAdvanceControl(controls, preferredAction) {
    const allEligible = eligibleAdvanceControls(controls);
    const normalizedPreferredAction = preferredAction?.trim().toLowerCase();
    const preferred = normalizedPreferredAction
        ? allEligible.filter((control) => control.accessibleName?.trim().toLowerCase() === normalizedPreferredAction)
...
function controlForIdentity(snapshot, identity) {
    if (!identity)
        return undefined;
    const matches = snapshot.controls.filter((control) => control.identity === identity);
    return matches.length === 1 ? matches[0] : undefined;
}
function observationControlIdentity(control) {
    return (0, control_identity_1.buildRuntimeControlIdentity)({
...
function subjectControl(snapshot, identities, structuredIdentities) {
    ...
    const matches = snapshot.controls.filter((control) => {
        ...
        return structural.some((identity) => (0, control_identity_1.matchControlIdentity)(identity, candidate) === "match");
    });
    return matches.length === 1 ? matches[0] : undefined;
}
function countOtherRequiredControls(snapshot, subject) {
    let invalid = 0;
    let empty = 0;
    for (const control of snapshot.controls) {
        if (!control.required || control.identity === subject?.identity)
            continue;
...
function observeSubjectMutation(before, after, identities, structuredIdentities) {
    ...
    if (!beforeSubject || !afterSubject)
        return { found: false, changed: false, invalidAfter: false, identity: undefined };
...
function compareControlledAdvanceSnapshots(input) {
    ...
    const applicationEvents = input.networkEvents.filter((event) => ["fetch", "xhr"].includes(String(event.resourceType ?? "").toLowerCase()));
    const validationTransport = applicationEvents.some((event) => String(event.method ?? "").toUpperCase() !== "GET"
    ...
    const advanceCausality = subject.found && subject.changed
    ...
    const blockedObserved = input.attemptObserved
        ...
        && formPreserved
        && validationObserved;
    const functionalDefectAssertion13 = Boolean(input.subjectInputApplied
...
async function clickAdvanceCandidate(page, candidate) {
 ...
 */
async function runControlledAdvanceProbe(input) {
    const empty = {
        intent: input.intent,
    ...
    const controls = await input.page.evaluate(function collectAdvanceControls() {
        ...
        for (const element of Array.from(document.querySelectorAll("button, input, [role=button]"))) {
            ...
            const actionPriority = opaqueBackground ? "primary" : "secondary";
    ...
    if (!resolution.candidate)
        return { ...empty, resolutionState: resolution.state, evidence: [`advance_candidate_${resolution.state}`] };
    const candidate = resolution.candidate;
    const candidateEnabled = !candidate.disabled;
    const before = await (0, assertion_observation_1.captureAssertionObservationSnapshot)(input.page).catch(() => undefined);
    if (!before)
        return { ...empty, candidateFound: true, candidateEnabled, attemptPossible: candidateEnabled, resolutionState: resolution.state, candidateResolutionSource: candidate.resolutionSource, advanceActionIdentity: candidate.identity, evidence: ["before_snapshot_unavailable"] };
    if (!candidateEnabled) {
    ...
    const after = await (0, assertion_observation_1.captureAssertionObservationSnapshot)(input.page).catch(() => undefined);
...
function buildControlledAdvanceProbeOracles(input) {
    const { result } = input;

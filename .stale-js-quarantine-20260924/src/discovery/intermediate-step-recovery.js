...
exports.recoverWithParentIntermediate = recoverWithParentIntermediate;
exports.recoverMissingIntermediateForFinalTarget = recoverMissingIntermediateForFinalTarget;
 ...
 */
function calculateTokenOverlapScore(targetTokens, candidateText) {
 ...
 */
async function recoverWithParentIntermediate(page, detailTarget, currentSnapshot, evidenceDir) {
    ...
    for (const element of allElements) {
        ...
        try {
            ...
            if (!effectiveText || effectiveText.trim().length < 2)
                continue;
            ...
            if (GIANT_CONTAINER_TAGS.has(tagName))
                continue;
            ...
            if (primaryText.length > 50) {
                ...
                if (inner && inner.trim().length > 3 && inner.trim().length < 50)
                    primaryText = inner.trim();
                else
            ...
            const scanned = { text: primaryText, tagName, className: className.substring(0, 100), cursor, role, ariaLabel: "", bbox, isVisible, isClickableLike };
            ...
            if (isEntryLike) {
                rejectedElements.push({ text: primaryText, reason: "entry_or_navigation_control" });
                continue;
            }
    ...
    const seen = new Set();
    const uniqueParents = parentCandidates.filter(c => { const k = normalizeToken(c.element.text); if (seen.has(k))
...
async function recoverMissingIntermediateForFinalTarget(page, detailTarget, currentSnapshot, routeProfile, evidenceDir) {
    const urlBefore = page.url();
    console.log(`[intermediate-recovery] started detailTarget="${detailTarget}" ` +
    ...
    const targetTokens = extractSignificantTokens(detailTarget);
    console.log(`[intermediate-recovery] targetTokens=[${targetTokens.join(", ")}] ` +
        ...
        try {
            ...
            if (score > 0.15) {
                ...
                if (matchedTokens.length >= 2)
                    reason = "multi_token_match";
                if (matchedTokens.some(t => t.length > 5))
                    reason = "contains_key_token";
                ...
                console.log(`[intermediate-recovery] candidate text="${primaryText}" tag=${tagName} ` +
    ...
    console.log(`[intermediate-recovery] scan totalElements=${scanStats.totalElements} ` +
    ...
    if (evidenceDir) {
        try {
            ...
            debugPath = (0, node_path_1.join)(evidenceDir, debugFilename);
            await (0, promises_1.mkdir)((0, node_path_1.dirname)(debugPath), { recursive: true }).catch(() => { });
            const debugPayload = {
                currentUrl: urlBefore,
                detailTarget,
                targetTokens,
            ...
            await (0, promises_1.writeFile)(debugPath, JSON.stringify(debugPayload, null, 2));
    ...
    console.log(`[intermediate-recovery] selected text="${selected.text}" ` +
    ...
    try {
        ...
        for (let retry = 0; retry < 5; retry++) {
            ...
            if (detailTargetNowVisible)
                break;
            await page.waitForTimeout(500);
        }
        console.log(`[intermediate-recovery] detailTargetVisible=${detailTargetNowVisible} ` +

...
exports.resolveLoginForm = resolveLoginForm;
...
async function resolveLoginForm(page, snapshot, valueKeys) {
    ...
    const userSnapshotEl = snapshot.elements.find((el) => isEditableField(el) && el.visible &&
    ...
    // --- Find password field ---
    // Try snapshot hints
    const passwordSnapshotEl = snapshot.elements.find((el) => isEditableField(el) && el.visible && el.inputType === "password");
    ...
    const submitSnapshotEl = snapshot.elements.find((el) => isSubmitButton(el) && el.visible);
    if (submitSnapshotEl) {
...
async function resolveLoginFieldByName(page, el) {
    const candidates = [
...
async function resolveLoginButton(page, el) {
    const candidates = [
        { label: `input[name="${el.name}"]`, fn: () => page.locator(`input[name="${el.name}"]`) },
        { label: `input[id="${el.id}"]`, fn: () => page.locator(`input[id="${el.id}"]`) },
...
function isSubmitButton(el) {
    ...
    if (el.tagName?.toLowerCase() === "button")
        return true;
    if (el.inputType === "submit")
        return true;
    if (el.role === "button" || el.role === "submit")
        return true;
    return false;
}

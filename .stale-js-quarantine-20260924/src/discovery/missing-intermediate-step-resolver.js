...
exports.resolveMissingIntermediateStep = resolveMissingIntermediateStep;
...
function isSubmitLike(text, routeProfile) {
...
function isBlockedLabel(text, routeProfile) {
    const normalized = normalizeText(text);
...
function isCandidateSafe(candidate, routeProfile) {
...
function findIntermediateFromRouteProfile(input) {
 ...
 * before the target.
 */
function findIntermediateByForwardMatch(input) {
 ...
 */
function findIntermediateByAlias(input) {
    const { routeProfile, candidates, currentTarget } = input;
    if (!routeProfile?.aliases || !currentTarget)
        return null;
    const aliasResult = (0, target_alias_resolver_1.resolveTargetWithAliases)(currentTarget, routeProfile);
    if (!aliasResult.resolved || !aliasResult.resolvedTarget)
        return null;
...
function computeSemanticRelationScore(candidate, context) {
...
function findIntermediateFromGenericHeuristics(input) {
    ...
    const stepLooksLikeSelection = /seleccionar|select|elegir|choose|primero|first|visible|listado|list|producto|product|opción|option/i.test(currentStepText);
...
function resolveMissingIntermediateStep(input) {

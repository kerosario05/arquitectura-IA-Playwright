...
exports.detectListingScreen = detectListingScreen;
exports.detectDetailAssertionsPending = detectDetailAssertionsPending;
exports.inferMissingSelection = inferMissingSelection;
exports.buildInsertedStepMetadata = buildInsertedStepMetadata;
/**
 * Task 1: Detect listing screen
 * Generic detection of repetitive clickeable/selectable elements
 */
function detectListingScreen(snapshot, currentTarget) {
    const candidates = [];
    let source = "none";
    let reason = "";
    // Detect cards (generic: repeated role=article, class patterns, structure)
    const cardElements = snapshot.elements.filter(el => el.role === "article" ||
        el.type === "card" ||
    ...
    if (candidates.length === 0) {
        const rowElements = snapshot.elements.filter(el => el.tagName === "tr" || (el.tagName === "li" && el.role !== "button"));
    ...
    if (candidates.length === 0) {
        const clickeableWithText = snapshot.elements.filter(el => (el.role === "button" || el.tagName === "a" || el.tagName === "button") &&
            el.text &&
            el.text.length > 0 &&
            el.text.length < 100 &&
            el.visible !== false);
    ...
    console.log(`[list-detection] detected=${detected} itemCount=${candidates.length} source="${source}" currentTarget="${currentTarget}"`);
    return {
        detected,
 ...
 */
function detectDetailAssertionsPending(pendingAssertions, currentSnapshot, navigationTarget) {
    const targets = pendingAssertions.slice(0, 5); // Focus on next few assertions
    const seemsLikeDetailAssertion = [];
    ...
    const visibleNow = targets.some(target => {
        const normalized = target.toLowerCase();
        return currentSnapshot.elements.some(el => (el.text ?? "").toLowerCase().includes(normalized.substring(0, 20)) ||
    ...
    console.log(`[detail-assertions] pending count=${pendingCount} targets="${targets.join(", ")}" visibleNow=${visibleNow} strongDetail=${likelyDetailCount} navTarget="${normalizedNavTarget}"`);
    return {
        pendingCount,
        targets,
        visibleNow,
        seemsLikeDetailAssertion,
        reason: likelyDetailCount > 0
 ...
 */
function inferMissingSelection(context) {
    ...
    if (!listingDetection.detected) {
        console.log(`[route-completion] blocked reason=missing_intermediate_selection_unresolved no_listing_detected`);
        return {
            detected: false,
            shouldInsert: false,
            confidence: "low",
    ...
    if (detailDetection.visibleNow || detailDetection.pendingCount === 0) {
        console.log(`[route-completion] blocked reason=assertions_already_visible detail_detected`);
        return {
            detected: false,
            shouldInsert: false,
            confidence: "low",
    ...
    const selectableItems = listingDetection.candidates.filter(el => el.visible !== false && (el.role === "button" || el.tagName === "a" || el.tagName === "li"));
    if (selectableItems.length === 0) {
        console.log(`[route-completion] blocked reason=missing_intermediate_selection_unresolved no_selectable_items`);
        return {
            detected: false,
            shouldInsert: false,
            confidence: "low",
    ...
    if (currentTarget.toLowerCase().includes("producto"))
        domainItemTerm = "producto";
    else if (currentTarget.toLowerCase().includes("tarjeta"))
        domainItemTerm = "tarjeta";
    else if (currentTarget.toLowerCase().includes("cuenta"))
        domainItemTerm = "cuenta";
    ...
    const confidence = selectableItems.length >= 2 && detailDetection.seemsLikeDetailAssertion.some(v => v) ? "high" : "medium";
    console.log(`[route-completion] insertedStep type=missing_intermediate_selection text="${proposedStep}" reason=detail_assertions_after_listing confidence=${confidence}`);
    return {
        detected: true,
 ...
 */
function buildInsertedStepMetadata(proposal) {
    return {

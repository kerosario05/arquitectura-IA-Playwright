...
exports.detectPostClickUiChange = detectPostClickUiChange;
...
function detectModalDialogElements(snapshot) {
    const evidence = [];
    for (const element of snapshot.elements) {
        const role = element.role?.toLowerCase() || "";
        const tag = element.tagName?.toLowerCase() || "";
        const ariaModal = element.ariaModal;
...
function detectFormElements(snapshot) {
    const evidence = [];
    let formCount = 0;
    let inputCount = 0;
    for (const element of snapshot.elements) {
...
function detectNextTargetsVisible(snapshot, nextTargets) {
    const evidence = [];
...
function detectNewUiElements(beforeSnapshot, afterSnapshot) {
    const evidence = [];
    const beforeTexts = new Set(beforeSnapshot.elements.map(e => (e.text || e.label || e.name || "").toLowerCase().trim()));
    const afterTexts = new Set(afterSnapshot.elements.map(e => (e.text || e.label || e.name || "").toLowerCase().trim()));
...
async function detectPostClickUiChange(options) {

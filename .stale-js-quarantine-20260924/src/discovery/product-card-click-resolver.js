...
exports.isProductCardTarget = isProductCardTarget;
exports.findProductCardClickCandidates = findProductCardClickCandidates;
exports.tryProductCardClickStrategies = tryProductCardClickStrategies;
 ...
 */
function isProductCardTarget(target, semanticRole, locatorStrategy) {
    // Check semantic role
 ...
 */
async function waitForCandidateScanStability(page, target) {
    ...
    try {
        ...
        const currentUrl = page.url();
        console.log(`[product-card-click] preCandidateWait completed targetVisible=${targetVisible} ` +
        ...
        if (!targetVisible) {
            console.log(`[product-card-click] warning: target not visible after ${elapsedMs}ms ` +
                `retries=10 url=${currentUrl}`);
        }
        if (elementCount < 100) {
            console.log(`[product-card-click] warning: few elements detected count=${elementCount} ` +
 ...
 */
async function findProductCardClickCandidates(page, initialLocator, target, snapshot) {
    const candidates = [];
    ...
    if (!waitResult.targetVisible && waitResult.elementCount < 100) {
        console.log(`[product-card-click] earlyExit targetNotVisible=true fewElements=${waitResult.elementCount} ` +
            `reason=page_not_fully_loaded noFallback=true`);
        console.log(`[product-card-click] skipping exact_text fallback (will timeout) ` +
    ...
    try {
        ...
        for (let i = 0; i < maxScan; i++) {
            ...
            try {
                ...
                if (cursor !== "pointer" && !className.includes("cursor-pointer")) {
                    console.log(`[product-card-click] visualCardReject reason=no_pointer_cursor ` +
                ...
                if (!looksLikeCard || isGiantContainer) {
                    console.log(`[product-card-click] visualCardReject reason=${isGiantContainer ? "giant_container" : "not_card_like"} ` +
        ...
        console.log(`[product-card-click] scan textMatches=${textMatches} pointerMatches=${pointerMatches} ` +
        ...
        if (visualCardCandidates.length > 0) {
            ...
            console.log(`[product-card-debug] visualCardFound=true target="${target}" ` +
        ...
        const spaClickablesLocator = page.locator(`[routerLink]:has-text("${target}"), ` +
            `[ng-reflect-router-link]:has-text("${target}"), ` +
            `[data-testid]:has-text("${target}"), ` +
            `[data-cy]:has-text("${target}"), ` +
            `[data-qa]:has-text("${target}")`).first();
        ...
        const clickableAncestorLocator = page.locator(`button:has-text("${target}"), ` +
            `a[href]:has-text("${target}"), ` +
            `[role="button"]:has-text("${target}"), ` +
            `[onclick]:has-text("${target}")`).first();
        ...
        if (visualCardCandidates.length > 0) {
            ...
            if (innerButtonVisible) {
                ...
                if (buttonTextNormalized.includes(normalizedTarget) || normalizedTarget.includes(buttonTextNormalized)) {
                    ...
                    console.log(`[product-card-click] excluded candidate strategy=inner_button ` +
    ...
    if (candidates.length === 1 && candidates[0].strategy === "exact_text") {
        console.log(`[product-card-click] warning=no_visual_card_candidates target="${target}" ` +
            `fallbackOnly=exact_text`);
    }
    console.log(`[product-card-click] target="${target}" candidates=${candidates.length}`);
    candidates.forEach((c, idx) => {
        console.log(`[product-card-click] candidate[${idx}] strategy=${c.strategy} ` +
 ...
 */
async function writeProductCardDebug(page, target, candidates, attemptedStrategies, evidenceDir) {
    if (!evidenceDir)
        return;
    try {
        const debugData = {
        ...
        const sanitizedTarget = target
            ...
            .replace(/[^a-z0-9]/g, "-")
            .substring(0, 50);
        const debugPath = (0, node_path_1.join)(evidenceDir, `product-card-debug-${sanitizedTarget}.json`);
        await (0, promises_1.mkdir)(evidenceDir, { recursive: true });
        await (0, promises_1.writeFile)(debugPath, JSON.stringify(debugData, null, 2), "utf-8");
 ...
 */
async function tryProductCardClickStrategies(page, candidates, target, checkDetailOpened, evidenceDir) {
    ...
    for (let i = 0; i < candidates.length; i++) {
        ...
        console.log(`[product-card-click] attempt=${i + 1}/${candidates.length} ` +
        ...
        try {
            ...
            if (candidate.strategy === "card_center" && candidate.bbox) {
                ...
                console.log(`[product-card-click] position-click x=${centerX.toFixed(1)} y=${centerY.toFixed(1)}`);
            ...
            while (Date.now() - detailPollStart < DETAIL_TIMEOUT_MS) {
                ...
                const currentSnapshot = await (0, page_scanner_1.scanCurrentPage)(page).catch(() => null);
                // Detect loading
                const hasLoading = currentSnapshot ? (() => {
                    const texts = [];
                    if (currentSnapshot.elements) {
                        for (const el of currentSnapshot.elements) {
                            if (el.text)
                                texts.push(el.text.toLowerCase());
                        }
                    }
                ...
                if (dc) {
                    detailReady = true;
                    detailReadyReason = "checkDetailOpened";
                    break;
                }
                // Alert/dialog signal
                const hasAlert = currentSnapshot && currentSnapshot.elements?.some((e) => e.visible && (e.role === "alert" || e.role === "alertdialog" || e.tagName === "dialog"));
                const buttonCount = currentSnapshot ? currentSnapshot.elements?.filter((e) => (e.role === "button" || e.tagName === "button") && e.visible).length || 0 : 0;
                if (hasAlert && buttonCount >= 1) {
                    detailReady = true;
                    detailReadyReason = "alert_with_buttons";
                    break;
                }
                // Button increase as detail signal
                if (buttonCount >= 3) {
                    detailReady = true;
                    detailReadyReason = "detail_buttons_visible";
                    break;
                }
            ...
            console.log(`[product-card-click] attempt=${i + 1} strategy=${candidate.strategy} ` +
            ...
            if (detailReady) {
                console.log(`[product-card-click] success=true strategy=${candidate.strategy} ` +
    ...
    console.log(`[product-card-click] failed target="${target}" ` +

...
exports.discoverProductCatalog = discoverProductCatalog;
const page_scanner_1 = require("../explorer/page-scanner");
const target_normalization_1 = require("../scenarios/target-normalization");
 ...
 */
async function captureProductDetailSignals(page) {
    await (0, page_readiness_1.waitForPageReady)(page, { domContentLoadedTimeoutMs: 5000, stabilizationMs: 500 });
    ...
    for (const el of snapshot.elements) {
        ...
        const isButton = el.type === "button" || el.tagName?.toLowerCase() === "button" || el.role?.toLowerCase() === "button";
        if (isButton) {
    ...
    console.log(`[catalog-discovery:detail] captured sections=${detailSections.length} buttons=${actionButtons.length} hasDetailPage=${hasDetailPage}`);
    return {
        detailSections,
        actionButtons,
        hasDetailPage,
 ...
 */
async function tryBackNavigation(page, entrySteps) {
    try {
        ...
        for (const pattern of backButtonPatterns) {
            try {
                ...
                if (count > 0) {
                    ...
                    await (0, page_readiness_1.waitForPageReady)(page, { domContentLoadedTimeoutMs: 3000, stabilizationMs: 500 });
        ...
        const exitedCatalog = urlAfterBack.includes("/success") ||
 ...
 */
async function detectProductCards(snapshot, context) {
    const detectedCards = [];
    const seen = new Set();
    ...
    if (context.pageContext === "catalog_root" || context.navigationCandidateCount > 0) {
        console.log(`[catalog-discovery:cards] skipping card detection: pageContext="${context.pageContext}" navCandidates=${context.navigationCandidateCount}`);
        return [];
    }
    ...
    const containers = snapshot.elements.filter((el) => {
        ...
        if (tag === "article")
            return true;
        if (role === "listitem" || role === "list-item")
            return true;
        if (typeof className === "string") {
            ...
            for (const pattern of cardPatterns) {
                if (normalizedClass.includes(pattern))
                    return true;
            }
        }
        return false;
    ...
    for (let i = 0; i < containers.length; i++) {
        ...
        let clickableAncestorStrategy = "none";
        let clickableAncestorTag = "unknown";
        ...
        if (isHeadingElement) {
            ...
            if (labelElement) {
                ...
                if (ancestorResult.clickableAncestor) {
                    ...
                    console.log(`[catalog-discovery:cards] found clickable ancestor for "${productLabel}" ` +
        ...
        const normalized = (0, target_normalization_1.normalizeTarget)(productLabel);
        ...
        if (isHeadingElement && !isClickable && cardTextPreview.length < 50) {
            console.log(`[catalog-discovery:cards] REJECTED[${i}] "${productLabel}" - reason: heading_element_no_clickable_no_content`);
            rejectedCards.push({
                label: productLabel,
        ...
        const hasMinimumSignals = bulletCount >= 1 || cardTextPreview.length >= 30 || (isClickable && hasImageOrIcon);
        if (!hasMinimumSignals) {
            console.log(`[catalog-discovery:cards] REJECTED[${i}] "${productLabel}" - reason: insufficient_product_signals (bullets=${bulletCount} textLen=${cardTextPreview.length} clickable=${isClickable} hasImage=${hasImageOrIcon})`);
            rejectedCards.push({
                label: productLabel,
        ...
        if (cardTextPreview.length > 0 && cardTextPreview.trim() === productLabel.trim()) {
            console.log(`[catalog-discovery:cards] REJECTED[${i}] "${productLabel}" - reason: text_preview_identical_to_label`);
            rejectedCards.push({
                label: productLabel,
        ...
        console.log(`[catalog-discovery:cards] ACCEPTED[${i}] "${productLabel}" bullets=${bulletCount} hasImage=${hasImageOrIcon} ` +
    ...
    if (detectedCards.length === 1) {
        ...
        if (!hasStrongSignals) {
            console.log(`[catalog-discovery:cards] rejected single card without strong signals: "${singleCard.productLabel}"`);
            rejectedCards.push({
            ...
            if (rejectedCards.length > 0) {
                ...
                for (const rejected of rejectedCards) {
                    console.log(`  - "${rejected.label}" reason=${rejected.reason} bullets=${rejected.bulletCount} hasImage=${rejected.hasImage}`);
                }
            }
            return [];
        }
    ...
    if (rejectedCards.length > 0) {
        ...
        for (const rejected of rejectedCards) {
            console.log(`  - "${rejected.label}" reason=${rejected.reason} bullets=${rejected.bulletCount} hasImage=${rejected.hasImage} textPreview="${rejected.textPreview.substring(0, 50)}"`);
        }
    }
 ...
 */
async function probeProductCardsForDetail(page, detectedCards, entrySteps, exhaustiveMode = false) {
    const probedCards = [];
    console.log(`[catalog-discovery:probe] starting detail probing for ${detectedCards.length} cards (exhaustiveMode=${exhaustiveMode})`);
    for (const card of detectedCards) {
        let detailProbeAttempted = false;
        let detailProbeResult = "click_failed";
        let detailSignals;
        ...
        try {
            ...
            console.log(`[catalog-discovery:probe] attempting click on "${card.productLabel}" ` +
            ...
            if (!clicked && card.locatorStrategy) {
                try {
                    ...
                    switch (strategy.type) {
                        case "role":
                            if (strategy.role) {
                                locator = page.getByRole(strategy.role, { name: strategy.text });
                            }
                            else {
            ...
            console.log(`[catalog-discovery:probe] after click on "${card.productLabel}" urlChanged=${urlChanged} ` +
            ...
            if (detailSignals.hasDetailPage) {
                ...
                console.log(`[catalog-discovery:probe] detail detected for "${card.productLabel}" ` +
                    `sections=${detailSignals.detailSections?.length || 0} buttons=${detailSignals.actionButtons?.length || 0}`);
            }
            else {
                detailProbeResult = "no_detail";
                console.log(`[catalog-discovery:probe] no detail detected for "${card.productLabel}" - card is not clickable to detail`);
            }
    ...
    console.log(`[catalog-discovery:probe] completed probing: validated_detail=${validatedDetailCount} ` +
 ...
 */
async function scrollAndRescanForCards(page, initialCards, context) {
    ...
    try {
        ...
        for (let i = 1; i <= scrollSteps; i++) {
            ...
            await (0, page_readiness_1.waitForPageReady)(page, { domContentLoadedTimeoutMs: 3000, stabilizationMs: 300 });
            // Re-scan for cards
            const snapshot = await (0, page_scanner_1.scanCurrentPage)(page);
        ...
        console.log(`[catalog-discovery:scroll] completed: initial=${initialCards.length} total=${allCards.length} new=${allCards.length - initialCards.length}`);
    }
    catch (err) {
 ...
 */
async function exploreMultilevelCatalog(page, currentPath, depth, maxDepth, entrySteps, catalogMode) {
    if (depth >= maxDepth) {
    ...
    console.log(`[catalog-discovery:detail] captured sections=${detailSignals.detailSections?.length || 0} ` +
    ...
    if (detailSignals.hasDetailPage && currentPath.length > 0) {
        ...
        const product = {
            label: productLabel,
            normalizedLabel: (0, target_normalization_1.normalizeTarget)(productLabel),
            confidence: "high",
        ...
        console.log(`[catalog-discovery] captured product with detailSignals sections=${detailSignals.detailSections?.length || 0} ` +
    ...
    console.log(`[catalog-discovery] depth=${depth} candidates categories=${candidates.filter((c) => c.type === "category").length} ` +
    ...
    if (navCandidates.length > 0) {
        ...
        for (const candidate of navCandidates) {
            try {
                ...
                const branchProducts = await exploreMultilevelCatalog(page, [...currentPath, candidate.label], depth + 1, maxDepth, entrySteps, catalogMode);
        ...
        for (const candidate of productCandidates) {
            try {
                ...
                if (productDetailSignals.hasDetailPage) {
                    ...
                    console.log(`[catalog-discovery] captured detailSignals sections=${productDetailSignals.detailSections?.length || 0} ` +
    ...
    if (navCandidates.length === 0 &&
        ...
        console.log(`[catalog-discovery] no navigable candidates, checking for product cards pageContext="${pageContext}"`);
        ...
        if (detectedCards.length > 0) {
            ...
            console.log(`[catalog-discovery] persisting ${probedCards.length} product cards with detail probe results`);
            const cardProducts = probedCards.map((card) => {
                // Determine presentation type and validation status based on probe results
                let presentationType = "product_card";
                let validationStatus = "validated_card";
                ...
                console.log(`[catalog-discovery:persist] "${card.productLabel}" ` +
 ...
 */
function detectClickableEvidence(el) {
    const tag = el.tagName?.toLowerCase();
    const role = el.role?.toLowerCase();
    ...
    const ariaAttrs = el.ariaExpanded || el.ariaControls || el.ariaHasPopup;
    if (ariaAttrs) {
        evidence.hasInteractiveAria = true;
        hasAnyEvidence = true;
    }
    // Check onclick attribute
    const hasOnClick = el.onclick !== undefined;
    if (hasOnClick) {
        evidence.hasInteractiveAria = true;
        hasAnyEvidence = true;
    }
    // Check tabindex (indicates focusable/interactive element)
    const tabindex = el.tabIndex || el.tabindex;
 ...
 */
function findClickableAncestor(labelElement, snapshot, productLabel) {
    ...
    for (const ancestor of potentialAncestors) {
        ...
        else if (ancestor.onclick !== undefined)
            strategy = "onclick";
        else if (ancestor.tabIndex !== undefined && ancestor.tabIndex >= 0)
            strategy = "tabindex";
        console.log(`[catalog-discovery:ancestor] found clickable ancestor for "${productLabel}" tag=${tag} role=${role} strategy=${strategy}`);
        return {
 ...
 */
function buildLocatorStrategy(el, label, hasInternalClickable) {
    const tag = el.tagName?.toLowerCase();
    const role = el.role?.toLowerCase();
    const testId = el.testId || el["data-testid"];
    // Priority 1: testId
    if (testId) {
        return {
            type: "testId",
...
async function clickCandidate(page, candidate) {
    ...
    console.log(`[catalog-discovery:click] attempting "${candidate.label}" strategy=${strategy.type} role=${strategy.role || "none"}`);
 ...
 */
function extractLabelFromClickable(clickable, snapshot) {
    ...
    const ariaLabel = clickable.ariaLabel || clickable["aria-label"];
 ...
 */
async function detectCatalogCandidates(snapshot, context) {
    const candidates = [];
    const seen = new Set();
    const rejected = [];
    ...
    for (const clickable of clickableElements) {
        ...
        if (controlClassification && !controlClassification.safeNavigation) {
            ...
            console.log(`[catalog-discovery:control] rejected action control "${label}" type=${controlClassification.controlType}`);
            continue;
        }
        ...
        const normalized = (0, target_normalization_1.normalizeTarget)(label);
        if (seen.has(normalized))
            continue;
        seen.add(normalized);
        ...
        console.log(`[catalog-discovery:candidate] "${label}" type=${type} strategy=${locatorStrategy.type} ` +
 ...
 */
function extractLabelFromContainer(container, snapshot) {
    const children = getContainerChildren(container, snapshot);
 ...
 */
function inferPageContext(snapshot, candidates, currentPath, depth) {
 ...
 */
async function discoverProductCatalog(page, options) {
    const { appSlug, routeProfile, catalogMode = "representative", maxProductsPerCategory = 2, maxDepth = parseInt(process.env.AI_CATALOG_DISCOVERY_MAX_DEPTH || "3", 10), } = options;
    ...
    for (const product of products) {
        if (product.discoveryPath && product.discoveryPath.length > 1) {
            ...
            if (pathDepth >= 2)
                product.category = product.discoveryPath[pathDepth - 2];
            if (pathDepth >= 3)
                product.subcategory = product.discoveryPath[pathDepth - 3];
        }
    }
    // Group by category
    const productGroups = groupProductsByCategory(products);
    // Select representative or all products
    const selectedProducts = catalogMode === "representative"
    ...
    console.log(`[catalog-discovery] discovered total=${products.length} categories=${categories.length} selected=${selectedProducts.length}`);
    return {
        appSlug,
        catalogUrl: page.url(),
 ...
 */
function buildEmptyResult(appSlug, catalogUrl) {
    return {
        appSlug,
        catalogUrl,
        capturedAt: new Date().toISOString(),
 ...
 */
function buildEntryStepsFromRouteProfile(routeProfile) {
    const steps = [];
 ...
 */
async function navigateToCatalogSection(page, entrySteps) {
 ...
 */
function isProductContainer(el) {
    const tag = el.tagName?.toLowerCase();
    const role = el.role?.toLowerCase();
    const className = el.className || "";
    if (tag === "article")
 ...
 */
function getContainerChildren(container, snapshot) {
 ...
 */
function buildLocatorString(el) {
    const testId = el.testId || el["data-testid"];
    if (testId)
        return `[data-testid="${testId}"]`;
    const domId = el.domId || el.id;
    if (domId)
        return `#${domId}`;
    if (el.role && el.text) {
 ...
 */
function buildGroupKey(label) {
    return (0, target_normalization_1.normalizeTarget)(label).replace(/[^a-z0-9]/g, "_");
}
/**
 ...
 */
function selectRepresentativeProducts(productGroups, maxPerCategory) {
    const selected = [];

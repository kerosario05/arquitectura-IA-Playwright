...
exports.persistDiscoveredProducts = persistDiscoveredProducts;
exports.loadAppConfig = loadAppConfig;
exports.getRouteProfile = getRouteProfile;
 ...
 */
async function persistDiscoveredProducts(appSlug, discoveryResult, automationRoot) {
    ...
    const appConfigPath = node_path_1.default.join(automationRoot, "automations", "apps", appSlug, "app.config.json");
    console.log(`[catalog-persistence] loading app config from ${appConfigPath}`);
    // Read existing config
    let appConfig;
    try {
        const content = await promises_1.default.readFile(appConfigPath, "utf-8");
    ...
    const existingRuntimeProducts = Object.values(existingTargetPaths).filter((tp) => tp.source === "runtime_discovery");
    ...
    const isSuspiciousRun = discoveryResult.representativeProducts.length === 1 &&
    ...
    if (isSuspiciousRun && existingRuntimeCount > discoveryResult.representativeProducts.length) {
        console.log(`[catalog-persistence] suspicious discovery result detected - keeping previous entries. ` +
    ...
    await promises_1.default.writeFile(tempPath, JSON.stringify(appConfig, null, 2) + "\n", "utf-8");
    await promises_1.default.rename(tempPath, appConfigPath);
    console.log(`[catalog-persistence] persisted ${Object.keys(discoveredTargetPaths).length} products to ${appConfigPath}`);
}
/**
 * Build base intermediates from route profile entry
 */
function buildBaseIntermediates(routeProfile) {
 ...
 */
function buildTargetPathsFromProducts(products, baseIntermediates) {
    const targetPaths = {};
    ...
    for (const product of products) {
        ...
        console.log(`[catalog-persistence] persisted targetPath target="${product.label}" intermediates=[${requiredIntermediates.join(", ")}]`);
    }
    return targetPaths;
}
/**
 ...
 */
function mergeTargetPaths(existing, discovered) {
    const merged = {};
 ...
 */
async function loadAppConfig(appSlug, automationRoot) {
    const appConfigPath = node_path_1.default.join(automationRoot, "automations", "apps", appSlug, "app.config.json");
    try {
        const content = await promises_1.default.readFile(appConfigPath, "utf-8");
 ...
 */
function getRouteProfile(appConfig) {
    if (!appConfig.routeProfile)
        return null;
    return appConfig.routeProfile;
}

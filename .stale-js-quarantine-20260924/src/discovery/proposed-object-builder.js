...
exports.buildProposedObject = buildProposedObject;
exports.buildProposedObjects = buildProposedObjects;
...
function inferObjectType(element) {
    switch (element.type) {
        case "button":
            return "button";
        case "link":
...
function buildObjectKey(element, objType) {
...
function buildObjectName(element, objType) {
    const text = element.text || element.label || element.placeholder || element.name || "";
    if (text) {
...
function pickBestLocator(element) {
...
function buildDescription(element, objType) {
    const parts = [];
    if (element.text) {
...
function buildAliases(element) {
    const aliases = [];
    const texts = [element.text, element.label, element.placeholder, element.name].filter(Boolean);
...
function buildProposedObject(element) {
...
function buildProposedObjects(elements) {

...
exports.isSubmitLikeTarget = isSubmitLikeTarget;
exports.promoteToClickableAncestor = promoteToClickableAncestor;
exports.detectSelectionSuccess = detectSelectionSuccess;
...
function isSelectionLikeTarget(target, context) {
...
function isSubmitLikeTarget(target, action) {
    const normalizedTarget = normalizeText(target);
...
async function promoteToClickableAncestor(locator) {
    try {
...
async function detectSelectionByLocator(page, target, locator) {
    const evidence = [];
    let confidence = 0;
    if (locator) {
        try {
        ...
        try {
            ...
            const targetInSummary = await page.evaluate((pattern) => {
                ...
                for (const input of inputs) {
                    const value = input.value || input.getAttribute("aria-valuenow") || "";
...
async function detectNextActionEnabled(page) {
    const evidence = [];
    try {
        const nextAction = await page.evaluate((patterns) => {
            ...
            for (const btn of buttons) {
                ...
                const matches = patterns.some((p) => text.includes(p) || ariaLabel.includes(p));
                if (matches) {
                    const disabled = btn.hasAttribute("disabled") || btn.getAttribute("aria-disabled") === "true";
                    const hidden = btn.getAttribute("aria-hidden") === "true" || btn.offsetParent === null;
...
async function detectSelectionSuccess(input) {

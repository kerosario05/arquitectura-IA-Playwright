...
exports.normalizeJiraIssue = normalizeJiraIssue;
exports.normalizeJiraIssues = normalizeJiraIssues;
const adf_extractor_1 = require("./adf-extractor");
...
function extractAcceptanceCriteriaSection(lines) {
    ...
    for (let i = startIdx + 1; i < lines.length; i++) {
        ...
        if (ACCEPTANCE_CRITERIA_HEADERS.some((re) => re.test(line)) ||
...
function normalizeJiraIssue(issue, options = {}) {
    ...
    const rawFieldValue = acceptanceCriteriaField === "description"
    ...
    if (typeof rawFieldValue === "string") {
        ...
        rawText = (0, adf_extractor_1.extractTextFromAdf)(rawFieldValue);
    }
    ...
    if (stepTexts.length === 0) {
        steps = [
            {
                ...
                action: title,
                expected: undefined,
                dataHints: (0, testrail_normalizer_1.extractDataHintsFromText)(title)
            }
        ];
    }
    else {
        steps = stepTexts.map((action, i) => ({
            ...
            action,
            expected: undefined,
            dataHints: (0, testrail_normalizer_1.extractDataHintsFromText)(action)
        }));
    }
    return {
        source: "jira",
...
function normalizeJiraIssues(issues, options) {

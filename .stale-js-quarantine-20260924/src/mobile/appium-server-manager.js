...
exports.__resetSpawnForTesting = __resetSpawnForTesting;
exports.__setHttpStatusCheckerForTesting = __setHttpStatusCheckerForTesting;
exports.__resetHttpStatusCheckerForTesting = __resetHttpStatusCheckerForTesting;
exports.resolveAppiumPortScanLimit = resolveAppiumPortScanLimit;
exports.resolveAvailableAppiumPort = resolveAvailableAppiumPort;
exports.getCurrentAppiumServer = getCurrentAppiumServer;
exports.getAppiumServerUrl = getAppiumServerUrl;
exports.resolveAppiumStartTimeoutMs = resolveAppiumStartTimeoutMs;
exports.resolveAppiumSpawnPlan = resolveAppiumSpawnPlan;
...
exports.__resetAppiumServerStateForTesting = __resetAppiumServerStateForTesting;
exports.__setPlatformForTesting = __setPlatformForTesting;
exports.__setReadinessPollIntervalForTesting = __setReadinessPollIntervalForTesting;
exports.__setPortProbeForTesting = __setPortProbeForTesting;
...
function __resetSpawnForTesting() {
    spawnFn = node_child_process_1.spawn;
}
async function defaultHttpStatusChecker(url) {
    try {
...
let httpStatusChecker = defaultHttpStatusChecker;
function __setHttpStatusCheckerForTesting(fn) {
    httpStatusChecker = fn;
}
function __resetHttpStatusCheckerForTesting() {
    httpStatusChecker = defaultHttpStatusChecker;
}
let currentServer = null;
let ready = false;
let lifecycleStatus = "stopped";
let lastError;
let startInFlight = null;
let platformOverride;
let readinessPollIntervalMs = 1500;
let portProbeForTesting;
...
function commandKindForExecutable(executable) {
...
function appendTail(current, nextChunk, maxChars) {
    const combined = current + nextChunk;
    if (combined.length <= maxChars)
        return combined;
    return combined.slice(combined.length - maxChars);
}
function previewText(value, maxChars = APPIUM_OUTPUT_PREVIEW_MAX_CHARS) {
...
function classifyPortError(error) {
    ...
    if (normalized.includes("econnrefused"))
        return "ECONNREFUSED";
    if (normalized.includes("etimedout") || normalized.includes("timed out"))
        return "ETIMEDOUT";
    if (normalized.includes("ehostunreach"))
        return "EHOSTUNREACH";
...
function isChildAlive(child) {
    ...
    if (typeof anyChild.exitCode === "number")
        return false;
    if (anyChild.killed)
        return false;
    return true;
}
async function defaultPortProbe(host, port, timeoutMs) {
...
async function killStartedProcessTree(proc, onLog) {
    const pid = proc.pid;
    if (!pid)
        return true;
...
async function waitForPortRelease(host, port, timeoutMs, onLog) {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
        if (!(await isPortListening(host, port, 800)))
            return true;
...
function resolveAppiumPortScanLimit(rawValue = process.env.APPIUM_PORT_SCAN_LIMIT) {
...
async function resolveAvailableAppiumPort(host, preferredPort, onLog) {
...
function getCurrentAppiumServer() {
    if (!currentServer)
        return null;
...
function getAppiumServerUrl() {
    return getCurrentAppiumServer()?.url;
}
function resolveAppiumStartTimeoutMs(rawValue = process.env.APPIUM_START_TIMEOUT_MS) {
    if (!rawValue || !rawValue.trim())
    ...
    if (parsed < MIN_APPIUM_START_TIMEOUT_MS)
        return MIN_APPIUM_START_TIMEOUT_MS;
    if (parsed > MAX_APPIUM_START_TIMEOUT_MS)
        return MAX_APPIUM_START_TIMEOUT_MS;
    return parsed;
}
function resolveAppiumSpawnPlan(executableRaw, args, options = {}) {
...
function classifySpawnError(err, context) {
    const e = err;
    const code = e?.code;
...
function attachProcessStreams(child, context, onLog, onTraceChunk) {
    onLog?.(`[appium] event=process_spawned pid=${child.pid ?? "unknown"} launcher=${context.launcher} executable=${context.executable}`);
    ...
    const finalize = (code, signal) => {
        ...
        onLog?.(`[appium] event=process_close exitCode=${exitCode ?? "null"} signal=${exitSignal ?? "null"}${outputDetail ? ` ${outputDetail}` : ""}`);
        currentServer = null;
        ready = false;
...
async function startAppiumServer(port, onLog, options) {
    ...
    const launch = (async () => {
        ...
        const bin = env_1.config.integrations.android?.appiumBin || "appium";
        const args = ["--port", String(selectedPort)];
        const plan = resolveAppiumSpawnPlan(bin, args, {
            platform: runtimePlatform(),
            comSpec: process.env.ComSpec,
        });
        onLog?.(`[appium] launch plan platform=${plan.platform} resolvedExecutable=${plan.resolvedExecutable} commandKind=${plan.commandKind} effectiveLauncher=${plan.effectiveLauncher} port=${selectedPort}`);
...
async function waitForReady(timeoutMs, onLog) {
    if (!currentServer) {
    ...
    while (Date.now() < deadline) {
        ...
        if (exitPromise) {
            const race = await Promise.race([
                ...
                new Promise((resolve) => setTimeout(() => resolve("tick"), waitSlice)),
            ]);
            if (race === "exit") {
                ...
                lifecycleStatus = "failed";
                lastError = message;
                onLog?.(`[appium] event=process_exit exitCode=${exitCode ?? "null"} signal=${signal ?? "null"} before_ready=true ` +
    ...
    let observedExitCode = typeof failedProcessRef?.exitCode === "number" ? failedProcessRef.exitCode : undefined;
...
async function refreshStatus(port) {
    const targetPort = port ?? currentServer?.port ?? env_1.config.integrations.android?.appiumPort ?? 4723;
...
async function stopAppiumServer(onLog) {
    if (!currentServer) {
...
function __resetAppiumServerStateForTesting() {
    currentServer = null;
    ready = false;
    lifecycleStatus = "stopped";
    lastError = undefined;
...
function __setPlatformForTesting(platform) {
    platformOverride = platform;
}
function __setReadinessPollIntervalForTesting(intervalMs) {
    readinessPollIntervalMs = intervalMs > 0 ? intervalMs : 1;
}
function __setPortProbeForTesting(probe) {

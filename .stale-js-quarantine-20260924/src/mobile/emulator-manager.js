...
exports.__setAdbRunnerForTesting = __setAdbRunnerForTesting;
exports.__resetAdbRunnerForTesting = __resetAdbRunnerForTesting;
exports.startEmulator = startEmulator;
exports.waitForBoot = waitForBoot;
exports.getStatus = getStatus;
exports.stopEmulator = stopEmulator;
exports.__resetEmulatorStateForTesting = __resetEmulatorStateForTesting;
exports.probeAdbHealth = probeAdbHealth;
...
function defaultAdbRunner(args) {
    const { adbPath } = (0, android_sdk_1.resolveAndroidSdk)();
    return new Promise((resolve) => {
        ...
        child.on("error", (err) => settle({ exitCode: 1, stdout, stderr: stderr + err.message }));
...
let adbRunner = defaultAdbRunner;
function __setAdbRunnerForTesting(fn) {
    adbRunner = fn;
}
function __resetAdbRunnerForTesting() {
...
let lastError;
function parseConnectedEmulatorDeviceId(adbDevicesOutput) {
...
async function detectConnectedEmulatorDeviceId() {
    const result = await adbRunner(["devices"]);
    if (result.exitCode !== 0)
        return undefined;
    return parseConnectedEmulatorDeviceId(result.stdout);
}
function clearTrackedState(nextStatus = "stopped", errorMessage) {
    currentEmulator = null;
    bootCompleted = false;
    lifecycleStatus = nextStatus;
    lastError = errorMessage;
}
async function startEmulator(avdName, opts = {}, onLog) {
    ...
    const { emulatorPath, sdkHome } = (0, android_sdk_1.resolveAndroidSdk)();
    ...
    child.stdout?.on("data", (chunk) => {
        ...
        if (text)
            onLog?.(`[emulator:stdout] ${text}`);
    });
    child.stderr?.on("data", (chunk) => {
        const text = chunk.toString("utf-8").trim();
        if (text)
            onLog?.(`[emulator:stderr] ${text}`);
    });
...
async function waitForBoot(timeoutMs, onLog) {
    if (!currentEmulator) {
    ...
    while (Date.now() < deadline) {
        ...
        const result = await adbRunner(deviceId
...
async function stopEmulator(onLog, options) {
    if (!currentEmulator) {
    ...
    if (currentEmulator.external) {
        onLog?.(`[mobile:infra] emulatorCleanup action=preserve reason=reused_external_emulator caller=${caller} runId=${runId} deviceId=${deviceId ?? "unknown"} emulatorStartedByRunner=false ownership=external_reused`);
        return;
    }
    onLog?.(`[mobile:infra] emulatorCleanup action=shutdown reason=started_and_owned_by_runner caller=${caller} runId=${runId} deviceId=${deviceId ?? "unknown"} emulatorStartedByRunner=true ownership=runner`);
...
function __resetEmulatorStateForTesting() {
    clearTrackedState("stopped");
}
/**
 ...
 */
async function probeAdbHealth(deviceId) {

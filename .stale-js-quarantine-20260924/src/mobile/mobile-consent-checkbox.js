...
exports.isSubmitLikeClick = isSubmitLikeClick;
exports.findUncheckedConsentCheckboxes = findUncheckedConsentCheckboxes;
exports.isConsentCheckboxAutoTickEnabled = isConsentCheckboxAutoTickEnabled;
exports.isSubmitGated = isSubmitGated;
exports.findConsentAcceptanceRow = findConsentAcceptanceRow;
exports.ensureConsentCheckboxChecked = ensureConsentCheckboxChecked;
...
function isSubmitLikeClick(step) {
...
function isCheckboxElement(tag, attrs) {
...
function buildSelector(attrs) {
    ...
    if (rid)
        return `android=new UiSelector().resourceId("${escapeForUiSelector(rid)}").checked(false)`;
    if (desc)
        return `android=new UiSelector().description("${escapeForUiSelector(desc)}").checked(false)`;
    if (text)
        return `android=new UiSelector().text("${escapeForUiSelector(text)}").checked(false)`;
 ...
 */
function findUncheckedConsentCheckboxes(pageSourceXml) {
    if (!pageSourceXml)
        return [];
    const found = [];
    const seen = new Set();
    ...
    while ((m = ELEMENT_RE.exec(pageSourceXml)) !== null) {
        ...
        if (!isCheckboxElement(tag, attrs))
            continue;
        if (attrs.checked !== "false")
            continue; // already ticked (or unknown) → leave it
...
function isConsentCheckboxAutoTickEnabled() {
 ...
 */
function isSubmitGated(pageSourceXml) {
 ...
 */
function observeGateState(pageSourceXml) {
 ...
 */
async function observePostTapGate(browser, tapTimestampMs, log) {
    const samples = [];
    ...
    for (const targetMs of checkpoints) {
        ...
        if (waitMs > 0) {
            ...
            catch { /* pause unavailable in tests */ }
        }
        let xml = "";
        try {
        ...
        log(`[mobile:consent-state] elapsedMs=${elapsedMs}` +
 ...
 */
function findConsentCheckboxBounds(pageSourceXml, parentBounds) {
    ...
    while ((m = ELEMENT_RE.exec(pageSourceXml)) !== null) {
        ...
        if (!b)
            continue;
        if (b.x1 < parentBounds.x1 || b.y1 < parentBounds.y1 || b.x2 > parentBounds.x2 || b.y2 > parentBounds.y2)
            continue;
        const w = b.x2 - b.x1;
        const h = b.y2 - b.y1;
        if (w <= 0 || h <= 0)
            continue;
        if (w > maxDimX || h > maxDimY)
            continue;
        const distFromLeft = b.x1 - parentBounds.x1;
        const distFromRight = parentBounds.x2 - b.x2;
        if (distFromLeft > edgeMargin && distFromRight > edgeMargin)
            continue;
        const area = w * h;
        if (area < bestArea) {
            bestArea = area;
 ...
 */
function findConsentAcceptanceRow(pageSourceXml) {
    let m;
    ELEMENT_RE.lastIndex = 0;
    while ((m = ELEMENT_RE.exec(pageSourceXml)) !== null) {
        const attrs = parseAttrs(m[2]);
 ...
 */
async function ensureConsentCheckboxChecked(browser, log = () => { }) {
    const ticked = [];
    if (!isConsentCheckboxAutoTickEnabled())
        return { ticked };
    let xml;
    try {
        xml = await browser.getPageSource();
    ...
    if ((0, mobile_modal_dismisser_1.detectBlockingModal)(xml).isTermsModal) {
        const closed = await (0, mobile_modal_dismisser_1.dismissBlockingModal)(browser, log);
        if (closed) {
            ...
            catch { /* pause may be unavailable in tests */ }
            try {
                xml = await browser.getPageSource();
            }
            catch { /* keep previous */ }

...
exports.detectBlockingModal = detectBlockingModal;
exports.isModalRecoveryEnabled = isModalRecoveryEnabled;
exports.dismissBlockingModal = dismissBlockingModal;
exports.dismissAndroidCompatibilityDialog = dismissAndroidCompatibilityDialog;
exports.detectAndroidCompatibilityDialog = detectAndroidCompatibilityDialog;
...
function closeSelectorFor(attrs) {
    ...
    const labelIsClose = label.length > 0 && label.length <= 14 && CLOSE_KEYWORDS.some((kw) => normalize(label) === normalize(kw) || normalize(label).includes(normalize(kw)));
    ...
    const ridIsClose = Boolean(rid &&
        !nrid.includes("background") &&
        ["close", "cerrar", "cancel", "dismiss", "descartar", "btnclose", "iconclose"].some((kw) => nrid.includes(kw)));
    if (!labelIsClose && !ridIsClose)
        return null;
    if (rid && ridIsClose)
        return `android=new UiSelector().resourceId("${escapeForUiSelector(rid)}")`;
    if (desc)
        return `android=new UiSelector().description("${escapeForUiSelector(desc)}")`;
    if (text)
        return `android=new UiSelector().text("${escapeForUiSelector(text)}")`;
    if (rid)
        return `android=new UiSelector().resourceId("${escapeForUiSelector(rid)}")`;
    return null;
}
 ...
 */
function findGeometricCloseTap(pageSourceXml) {
    ...
    while ((m = ELEMENT_RE.exec(pageSourceXml)) !== null) {
        ...
        const w = b.x2 - b.x1;
        const h = b.y2 - b.y1;
        if (w <= 0 || h <= 0 || w > 220 || h > 220)
            continue; // small control
        // Upper-right quadrant of the screen.
        if (b.x1 < screenW * 0.55 || b.y1 > screenH * 0.45)
            continue;
        const area = w * h;
        if (!best || area < best.area)
            best = { x: Math.round((b.x1 + b.x2) / 2), y: Math.round((b.y1 + b.y2) / 2), area };
    }
 ...
 */
function detectBlockingModal(pageSourceXml) {
    let hasModalContainer = false;
    let closeSelector = null;
    let closeLabel = null;
    let isTermsModal = false;
    if (!pageSourceXml)
        return { hasModalContainer, closeSelector, closeLabel, closeTap: null, isTermsModal };
...
function isModalRecoveryEnabled() {
 ...
 */
async function dismissBlockingModal(browser, log = () => { }) {
    if (!isModalRecoveryEnabled())
        return false;
    let xml;
    try {
 ...
 */
async function dismissAndroidCompatibilityDialog(browser, log = () => { }) {
    let xml;
    try {
        xml = await browser.getPageSource();
    }
 ...
 */
function detectAndroidCompatibilityDialog(pageSourceXml) {
    if (!pageSourceXml)
        return false;
    // The system dialog has package="android" at the hierarchy or window level.
    const hasSystemPackage = /package="android"/.test(pageSourceXml) ||
    ...
    const hasOkButton = /android:id\/button1/.test(pageSourceXml) ||

...
exports.MobileOtpResolverError = void 0;
exports.resolveMobileOtp = resolveMobileOtp;
 ...
 */
class MobileOtpResolverError extends Error {
    reasonCode;
    httpStatus;
    constructor(reasonCode, message, httpStatus) {
...
exports.MobileOtpResolverError = MobileOtpResolverError;
function otpBaseUrl(env = process.env) {
    const explicit = (env.OTP_INTERNAL_BASE_URL ?? "").trim();
    if (explicit)
        return explicit.replace(/\/+$/, "");
    const host = (env.HOST ?? "").trim() || "127.0.0.1";
    return `http://${host}:${(0, config_1.resolveServerPort)(env)}`;
}
function automationKey(env = process.env) {
...
async function loadOtpProfile(appSlug) {
    const cfg = await (0, app_auto_resolver_1.loadAppConfig)(appSlug);
    if (!cfg || typeof cfg !== "object")
        return null;
    const direct = cfg.otpProfile;
    const container = cfg.appProfile && typeof cfg.appProfile === "object" && !Array.isArray(cfg.appProfile)
        ? cfg.appProfile.otpProfile
        : undefined;
    const raw = (direct ?? container);
    ...
    return {
        ...
        allowedChannels: Array.isArray(raw.allowedChannels)
            ? raw.allowedChannels.filter((c) => typeof c === "string")
            : undefined,
    };
}
function resolveChannel(hint, profile) {
    const norm = (s) => s.trim().toUpperCase();
    const allowed = (profile.allowedChannels ?? []).map(norm).filter(Boolean);
    if (hint && allowed.includes(norm(hint)))
        return norm(hint);
    if (allowed.includes(norm(profile.defaultChannel ?? "")))
        return norm(profile.defaultChannel ?? "");
    if (allowed.length === 0) {
...
async function postOtp(path, body, env) {
 ...
 * the OTP.
 */
async function resolveMobileOtp(opts, env = process.env) {

...
exports.isContinueLabel = isContinueLabel;
exports.findPassthroughControl = findPassthroughControl;
exports.isPassthroughScreen = isPassthroughScreen;
exports.buildPassthroughStep = buildPassthroughStep;
exports.withPassthroughSteps = withPassthroughSteps;
/**
 ...
 */
function findPassthroughControl(screen) {
    const elements = screen.elements ?? [];
    if (elements.length === 0)
        return null;
    // Any real decision (a field to fill, a switch to flip) disqualifies the screen.
    if (elements.some((el) => DECISION_ROLES.has(el.role)))
        return null;
 ...
 */
function withPassthroughSteps(entrySteps, screensInOrder) {
    const covered = new Set(entrySteps.map((st) => st.target?.value).filter((v) => Boolean(v)));
    const result = [...entrySteps];
    for (const screen of screensInOrder) {
        const step = buildPassthroughStep(screen);
        if (!step?.target)
            continue;
        if (covered.has(step.target.value))
            continue;
        result.push(step);

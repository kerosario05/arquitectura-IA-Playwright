...
exports.detectSegmentedGroup = detectSegmentedGroup;
exports.analyzeSegmentedGroup = analyzeSegmentedGroup;
exports.joinSegmentValues = joinSegmentValues;
 ...
 */
function detectSegmentedGroup(candidates, expectedLength) {
 ...
 */
function analyzeSegmentedGroup(candidates, expectedLength) {
    if (expectedLength < 2)
        return { group: null, reason: "value_too_short" };
    const usable = candidates.filter((c) => c.displayed && c.bounds !== null);
    if (usable.length < 2) {
        return {
            group: null,
    ...
    if (viable.length === 0)
        return { group: null, reason: lastRejection || "no_viable_row" };
    if (viable.length === 1)
        return { group: viable[0].ordered.map((c) => c.index), reason: "ok" };
 ...
 */
function joinSegmentValues(values) {

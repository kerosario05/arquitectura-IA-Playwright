...
exports.resolveOtpIdentity = resolveOtpIdentity;
exports.planOtpAutoContinue = planOtpAutoContinue;
exports.planPostOtpSubmit = planPostOtpSubmit;
exports.executeMobileStep = executeMobileStep;
const mobile_consent_checkbox_1 = require("./mobile-consent-checkbox");
const mobile_modal_dismisser_1 = require("./mobile-modal-dismisser");
const mobile_text_normalization_1 = require("./mobile-text-normalization");
const mobile_otp_resolver_1 = require("./mobile-otp-resolver");
const mobile_segmented_input_1 = require("./mobile-segmented-input");
 ...
 */
function resolveOtpIdentity(step, ctx, stepIndex, env = process.env) {
    ...
    const valueFor = (f) => (ctx.dataOverrides?.[f.stepIndex] ?? f.exampleValue ?? "").trim();
    const nameOf = (f) => `${f.key ?? ""} ${f.label ?? ""}`;
    ...
    if (identityField) {
        const field = fields.find((f) => f.key === identityField || f.label === identityField || slugifyKey(f.label) === slugifyKey(identityField));
        const value = field ? valueFor(field) : "";
        if (value)
            return { identity: value, source: "field" };
    }
    // Tier 2 — exactly one field whose name reads as an identification document.
    const byName = fields.filter((f) => IDENTITY_LABEL_PATTERN.test(nameOf(f)) && !SECRET_LABEL_PATTERN.test(nameOf(f)) && isDigitsOnly(valueFor(f)));
    if (byName.length === 1)
        return { identity: valueFor(byName[0]), source: "inferred_label" };
    // Tier 3 — exactly one sensitive, all-digit field that is not a secret.
    const bySensitivity = fields.filter((f) => f.sensitive && !SECRET_LABEL_PATTERN.test(nameOf(f)) && isDigitsOnly(valueFor(f)));
    if (bySensitivity.length === 1)
        return { identity: valueFor(bySensitivity[0]), source: "inferred_sensitive" };
    ...
    if (isDigitsOnly(envIdentity))
        return { identity: envIdentity, source: "env_identity_provider" };
    return null;
}
async function resolveOtpForStep(step, ctx, stepIndex = -1, env = process.env) {
    if (!ctx?.appSlug) {
        throw new mobile_otp_resolver_1.MobileOtpResolverError("otp_identity_unresolved", "OTP step requires appSlug in runtime context.");
    }
    const resolvedIdentity = resolveOtpIdentity(step, ctx, stepIndex, env);
    if (!resolvedIdentity) {
        throw new mobile_otp_resolver_1.MobileOtpResolverError("otp_identity_unresolved", "OTP identity could not be resolved from runtime data.");
    }
    const { identity, source: identitySource } = resolvedIdentity;
    if (identitySource !== "field") {
        console.warn(`[mobile:otp] identity resolved without an explicit hint (source=${identitySource}) — declare otp.identityField on the step to pin it`);
    }
    ...
    const resolved = await (0, mobile_otp_resolver_1.resolveMobileOtp)({
        appSlug: ctx.appSlug,
        identity,
    ...
    if (!otp) {
        throw new mobile_otp_resolver_1.MobileOtpResolverError("otp_value_empty", "OTP resolver returned an empty or whitespace-only value.");
    }
    return otp;
}
...
const runtimeStateByBrowser = new WeakMap();
function getRuntimeState(browser) {
...
async function waitOnce(el, mode, timeoutMs) {
...
function extractUiAutomatorClassName(selectorValue) {
    const match = selectorValue.match(/className\("([^"]+)"\)/);
    return match?.[1]?.trim() || undefined;
}
function selectorHasOrdinalInstance(selectorValue) {
    return /\.instance\(\d+\)/.test(selectorValue);
}
function buildStableSelectorsForClassNodes(pageSource, className) {
    const selectors = [];
...
async function buildEvidenceFallbackSelectors(browser, target) {
    ...
    if (!pageSource)
        return [];
    const variants = (0, mobile_text_normalization_1.buildTextVariants)(value);
    const selectors = [];
...
function hasMaskSeparators(value) {
    return /[^a-zA-Z0-9]/.test(value);
}
/** Keeps letters/digits, drops only format/separator characters. */
function canonicalizeSignificant(value) {
    return value.replace(/[^a-zA-Z0-9]/g, "");
}
async function matchesForSelector(browser, selector) {
    try {
...
async function readElementText(el) {
    if (!el)
        return "";
    for (const read of [
 ...
 */
async function buildSegmentCandidates(elements) {
    const candidates = [];
    for (let i = 0; i < elements.length; i++) {
        ...
        if (displayed) {
            try {
                bounds = (0, mobile_segmented_input_1.parseBounds)(String((await elements[i].getAttribute("bounds")) ?? ""));
            }
            catch {
                bounds = null;
            }
 ...
 */
async function findEmptySegmentedGroup(browser, selector, length) {
    ...
    return (0, mobile_segmented_input_1.analyzeSegmentedGroup)(empty, length).group;
}
async function tryFillSegmentedInput(browser, selector, value) {
    ...
    const geometry = candidates
        ...
        .join(" ");
    const { group, reason } = (0, mobile_segmented_input_1.analyzeSegmentedGroup)(candidates, chars.length);
    if (!group) {
 ...
 */
function planOtpAutoContinue(steps, index) {
 ...
 */
function planPostOtpSubmit(steps, index, sendTarget) {
    if (steps.length === 0)
        return { proceed: false, reason: "no_steps_in_context" };
    if (index !== steps.length - 1)
        return { proceed: false, reason: "scenario_has_later_steps" };
    for (let i = index - 1; i >= 0; i--) {
        const candidate = steps[i];
        if (!candidate?.target || !(0, mobile_consent_checkbox_1.isSubmitLikeClick)(candidate))
            continue;
        // "Enviar código de validación" reads as submit-like too; it is the OTP trigger, not the
        // control that advances the screen.
        if (sameTarget(candidate.target, sendTarget))
            continue;
...
async function submitAfterOtpChallenges(browser, index, ctx, sendTarget) {
    ...
    if (!observation.enabledObserved) {
        ...
        throw new Error(`data_precondition_failure: submit gate not satisfied after OTP validation target=${JSON.stringify(submitTarget)} enabledSource=${observation.enabledSource}`);
    }
...
async function completeRemainingOtpChallenges(browser, step, index, ctx, fillSelector, otpLength) {
...
async function readSegmentedObservation(browser, selector, group, submittedValue) {
    const elements = await matchesForSelector(browser, selector);
    const observedChars = [];
    for (const index of group) {
        observedChars.push(await readElementText(elements[index]));
    }
    const observed = (0, mobile_segmented_input_1.joinSegmentValues)(observedChars);
    ...
    console.log(`[mobile:fill] segmented verify boxes=${group.length} filled=${observedChars.filter((c) => c.length > 0).length} masked=${masked} accepted=${accepted}`);
...
async function readFilledValueObservation(el, submittedValue, options) {
    ...
    const probes = [
        ...
        { source: "element:value", read: async () => String((await el.getValue?.()) ?? "") },
    ];
...
async function observeEnabledState(browser, selector, target) {
    try {
...
async function inferSubmitGateStatus(browser, selector, target) {
    ...
    const uncheckedConsent = (0, mobile_consent_checkbox_1.findUncheckedConsentCheckboxes)(xml);
    const termsAccepted = uncheckedConsent.length === 0;
    const hasFormGate = (0, mobile_consent_checkbox_1.isSubmitGated)(xml);
...
async function locateSelector(browser, selector, timeoutMs, mode) {
    const el = await browser.$(selector);
    if (await waitOnce(el, mode, timeoutMs))
        return el;
    for (let attempt = 0; attempt < 2; attempt++) {
        const dismissed = await (0, mobile_modal_dismisser_1.dismissBlockingModal)(browser, (line) => console.log(line));
        if (!dismissed)
            break;
        const retried = await browser.$(selector);
        if (await waitOnce(retried, mode, Math.min(timeoutMs, 5000)))
            return retried;
    }
    return null;
}
...
function extractTextSelectorValue(value) {
 ...
 */
async function findClickableAncestor(browser, el) {
    let current = el;
    for (let depth = 0; depth <= MAX_ANCESTOR_DEPTH && current; depth++) {
        ...
        if (clickable) {
            const combined = (0, mobile_text_normalization_1.normalizeComparableText)(`${label} ${text}`.trim());
            if (!combined)
                return null;
...
async function resolveClickableElement(browser, target, clickSelector) {
    // Rule applies ONLY to androidUiAutomator selectors built from a text(...) locator.
    if (target.strategy !== "androidUiAutomator" || !TEXT_SELECTOR_RE.test(target.value))
        return null;
    const searchedText = extractTextSelectorValue(target.value);
    if (searchedText == null)
        return null;
    const normalizedSearched = (0, mobile_text_normalization_1.normalizeComparableText)(searchedText);
    if (!normalizedSearched)
        return null;
    let matches;
    try {
    ...
    for (const m of matches) {
        ...
        if (!ancestor || ancestor.label !== normalizedSearched)
            continue;
        if (seen.has(ancestor.element))
            continue;
 ...
 */
async function locateTarget(browser, target, selector, timeoutMs, mode) {
    ...
    while (Date.now() < deadline) {
        await new Promise((resolve) => setTimeout(resolve, LATE_POLL_INTERVAL_MS));
        polls += 1;
...
function requireEnabledBeforeClick() {
 ...
 */
async function isEnabledNow(browser, selector) {
...
async function waitUntilEnabled(browser, selector, timeoutMs) {
    const deadline = Date.now() + timeoutMs;
    if (await isEnabledNow(browser, selector))
        return true;
    while (Date.now() < deadline) {
...
function classifyInfrastructureStepError(message) {
...
function isConsentAcceptanceClick(step) {
...
function assertNoConfirmedEncodingCorruption(step) {
    const fragments = [step.description, step.value, step.target?.value].filter((v) => typeof v === "string");
...
async function executeMobileStep(browser, step, index, screenshotPath, otpContext) {
        ...
        switch (effectiveStep.action) {
            ...
            case "click": {
                if (!effectiveStep.target)
                    throw new Error("click step requires a target");
                ...
                if (!el)
                    throw new Error(`click target not found (even after closing any blocking modal): ${JSON.stringify(effectiveStep.target)}`);
                ...
                if (isConsentAcceptanceClick(effectiveStep)) {
                    const consentResult = await (0, mobile_consent_checkbox_1.ensureConsentCheckboxChecked)(browser, (line) => console.log(line));
                ...
                if (!consentResolved && requireEnabledBeforeClick()) {
                    ...
                    if (!(await waitUntilEnabled(browser, clickSelector, Math.min(effectiveStep.timeoutMs ?? 10000, 4000)))) {
                        if ((0, mobile_consent_checkbox_1.isSubmitLikeClick)(effectiveStep)) {
                            await (0, mobile_consent_checkbox_1.ensureConsentCheckboxChecked)(browser, (line) => console.log(line));
                        }
                        try {
                        ...
                        if (!enabledAfterKeyboard) {
                            ...
                            throw new Error(`data_precondition_failure: submit gate not satisfied fieldAccepted=${gate.fieldAccepted} termsAccepted=${gate.termsAccepted} buttonEnabled=${gate.buttonEnabled} target=${JSON.stringify(effectiveStep.target)} buttonObservationSource=${gate.buttonObservationSource}`);
                        }
                    }
                }
                ...
                if (!consentResolved) {
                    ...
                    if (disambiguation) {
                        console.log(`[mobile:click] duplicateTextMatches=${disambiguation.duplicateTextMatches} originalClass=${disambiguation.originalClass} selectedClass=${disambiguation.selectedClass} disambiguationReason=exact_clickable_ancestor_label disambiguated=true`);
            ...
            case "fill": {
                if (!effectiveStep.target)
                    throw new Error("fill step requires a target");
                ...
                if (!el)
                    throw new Error(`fill target not found (even after closing any blocking modal): ${JSON.stringify(effectiveStep.target)}`);
                ...
                const maskAware = submittedValue.trim().length > 0 &&
                    hintAttr.length > 0 &&
                ...
                if (effectiveStep.otp?.required === true && segmentGroup && otpContext) {
                    const sendTarget = await completeRemainingOtpChallenges(browser, effectiveStep, index, otpContext, fillSelector, Array.from(inputPayload).length);
            ...
            case "assertVisible": {
                if (!effectiveStep.target)
                    throw new Error("assertVisible step requires a target");
                const el = await locateTarget(browser, effectiveStep.target, selectorFromTarget(effectiveStep.target), effectiveStep.timeoutMs ?? 10000, "displayed");
                if (!el) {
            ...
            case "assertEnabled": {
                ...
                if (!effectiveStep.target)
                    throw new Error("assertEnabled step requires a target");
                ...
                if (!el)
                    throw new Error(`Expected element to be present and enabled but not found: ${JSON.stringify(effectiveStep.target)}`);
            ...
            case "assertDisabled": {
                ...
                if (!effectiveStep.target)
                    throw new Error("assertDisabled step requires a target");
                ...
                if (!el)
                    throw new Error(`Expected element to be present but disabled; not found: ${JSON.stringify(effectiveStep.target)}`);
                const enabled = await el.isEnabled().catch(() => true);
                if (enabled)
                    throw new Error(`Expected element to be DISABLED but it is enabled: ${JSON.stringify(effectiveStep.target)}`);
                break;
            }
            case "waitFor": {
                if (!effectiveStep.target)
                    throw new Error("waitFor step requires a target");
                const el = await locateTarget(browser, effectiveStep.target, selectorFromTarget(effectiveStep.target), effectiveStep.timeoutMs ?? 10000, "exist");
                if (!el)
                    throw new Error(`waitFor target not found (even after closing any blocking modal): ${JSON.stringify(effectiveStep.target)}`);
                break;
            }
            case "screenshot": {
            ...
            default:
                throw new Error(`Unsupported mobile step action: ${step.action}`);
        }
        if (screenshotPath) {

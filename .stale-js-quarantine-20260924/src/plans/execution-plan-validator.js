...
exports.assertValidExecutionPlan = assertValidExecutionPlan;
...
function pushError(issues, code, message, stepIndex) {
    issues.push({ level: "error", code, message, stepIndex });
}
function pushWarning(issues, code, message, stepIndex) {
    issues.push({ level: "warning", code, message, stepIndex });
}
function validateStepRules(step, issues) {
    const stepIndex = step.index;
    const capability = (0, action_registry_1.getActionCapability)(step.action);
    if (!capability) {
    ...
    if (capability.requiresTarget) {
        if (step.action === "navigate") {
            if (!(step.target === "APP_BASE_URL" || isObject(step.target))) {
                pushError(issues, "STEP_NAVIGATE_TARGET_REQUIRED", "navigate requires target APP_BASE_URL or PlanTarget.", stepIndex);
            }
        }
...
function deriveStatus(current, issues) {
...
function validateExecutionPlan(plan) {
    ...
    if (Array.isArray(steps)) {
        ...
        for (const rawStep of steps) {
            ...
            if (!(0, action_registry_1.getActionCapability)(step.action)) {
            ...
            if (hasNonEmptyString(step.valueKey) && !requiredDataKeys.has(step.valueKey.trim())) {
                pushWarning(issues, "STEP_VALUEKEY_NOT_DECLARED", `valueKey '${step.valueKey}' is not declared in requiredData.`, step.index);
            }
        }
    }
    if (status === "validated" && unresolvedRequiredData) {
        pushError(issues, "PLAN_VALIDATED_WITH_UNRESOLVED_DATA", "Plan status 'validated' is not allowed when requiredData has unresolved required entries.");
    }
    if (unresolvedRequiredData && status === "draft") {
        pushWarning(issues, "PLAN_DRAFT_WITH_UNRESOLVED_DATA", "Draft plan has unresolved required data and may later require status 'needs_data'.");
    }
...
function assertValidExecutionPlan(plan) {

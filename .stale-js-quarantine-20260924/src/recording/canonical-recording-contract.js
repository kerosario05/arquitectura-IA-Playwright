...
exports.isQaOverridableRuntimeInput = isQaOverridableRuntimeInput;
...
function isQaOverridableRuntimeInput(requirement) {
...
function isQaEditSource(source) {
...
function transitionAfter(events, index, excludeSeqs) {
    for (let cursor = index + 1; cursor < events.length; cursor += 1) {
        const candidate = events[cursor];
        if (typeof candidate.seq === "number" && excludeSeqs?.has(candidate.seq))
            continue;
...
function actionablePointerAnchor(events, index) {
    ...
    for (let cursor = index - 1; cursor >= 0 && index - cursor <= 40; cursor -= 1) {
        ...
        // A navigation is part of the transition, not a user-action boundary.
        if (candidate.kind === "tap" || candidate.kind === "fill" || candidate.kind === "back")
            return undefined;
    }
    return undefined;
}
function nextPointerBoundaryT(events, fromT, anchor) {
    let boundary;
    for (const event of events) {
        if (event.kind !== "note" || event.observationType !== "pointer")
            continue;
        if (event.t <= fromT)
            continue;
        if (anchor && event.screenKey === anchor.screenKey && stableControlOf(event) === stableControlOf(anchor))
            continue;
        boundary = boundary === undefined ? event.t : Math.min(boundary, event.t);
    }
    return boundary;
}
function causalTransition(events, index, excludeSeqs) {
    ...
    for (const candidate of events) {
        if (candidate.kind !== "navigate" || !candidate.url)
            continue;
        if (typeof candidate.seq === "number" && excludeSeqs?.has(candidate.seq))
            continue;
        if (candidate.t < anchor.t || candidate.t > end)
            continue;
        if (candidate.url === anchor.url)
            continue;
        bound = candidate;
    }
    return bound ? { event: bound } : transitionAfter(events, index, excludeSeqs);
}
function ownershipForEvent(events, index, excludeSeqs) {
    const event = events[index];
    const target = event.target;
    const anchor = actionablePointerAnchor(events, index);
...
function isDerivedDisplayClick(events, index) {
    ...
    if (event.kind !== "tap" || event.target?.interactionType === "select" || event.target?.afterValue !== undefined)
        return false;
    // Capture V2 pointer identity proves a physical action; preserve it in the observed projection.
...
function buildCanonicalInteractions(events, editingSessions = []) {
    const result = [];
    const ownedTransitionSeqs = new Set();
    for (let candidateIndex = 0; candidateIndex < events.length; candidateIndex += 1) {
        const candidate = events[candidateIndex];
        if (candidate.kind !== "tap" && candidate.kind !== "fill")
            continue;
        const anchor = actionablePointerAnchor(events, candidateIndex);
        if (!anchor || !anchor.url) {
            const transition = causalTransition(events, candidateIndex).event;
            if (transition && typeof transition.seq === "number")
                ownedTransitionSeqs.add(transition.seq);
            continue;
        }
        const end = nextPointerBoundaryT(events, anchor.t, anchor) ?? Number.POSITIVE_INFINITY;
        for (const navigateEvent of events) {
            if (navigateEvent.kind !== "navigate" || typeof navigateEvent.seq !== "number")
                continue;
            if (navigateEvent.t < anchor.t || navigateEvent.t > end)
                continue;
    ...
    for (const [index, event] of events.entries()) {
        ...
        if (event.kind === "navigate" || event.kind === "back") {
            if (typeof event.seq === "number" && ownedTransitionSeqs.has(event.seq))
                continue;
...
function applyRuntimeDatasetValues(scenario, values) {
    ...
    const requiredData = scenario.requiredData.map((field) => {
        ...
        if (persistedQaValues[field.key] !== undefined || values[field.key] !== undefined)
            return field;
    ...
    const runtimeInputRequirements = materializedRequirements.map((requirement) => {
        ...
        return effectiveValues[requirement.valueKey] === undefined ? withLineage : {
            ...
            source: (explicitQaEdit ? "CURRENT_QA_EDIT" : requirement.source === "secure" ? "secure" : "RECORDED_CONFIRMED"),
...
function toSharedMcpScenario(scenario, appSlug, datasetValues = {}) {
    ...
    const executableInteractions = executableInteractionsForRoute
        ...
        return {
            ...
            // source of truth: its `routeAfter` is the surface the action must reach.
            ...
            ...(interaction.causedTransition
                ...
                : interaction.transitionObserved
                    ? { expectedOutcomeKind: "in_place_transition" }
                    : {}),
        };
    });

...
const android_session_recorder_1 = require("./android-session-recorder");
...
describe("parseBounds", () => {
    test("parses an Android bounds pair into x/y/width/height", () => {
        node_assert_1.default.deepStrictEqual((0, tap_hit_tester_1.parseBounds)("[60,400][1020,540]"), { x: 60, y: 400, width: 960, height: 140 });
    });
    test("rejects a degenerate rectangle", () => {
        node_assert_1.default.strictEqual((0, tap_hit_tester_1.parseBounds)("[10,10][10,10]"), null);
        node_assert_1.default.strictEqual((0, tap_hit_tester_1.parseBounds)("not-bounds"), null);
        node_assert_1.default.strictEqual((0, tap_hit_tester_1.parseBounds)(undefined), null);
    });
});
describe("extractBoundedNodes", () => {
    const nodes = (0, tap_hit_tester_1.extractBoundedNodes)(LOGIN_XML);
    test("captures every node that declares bounds", () => {
        node_assert_1.default.strictEqual(nodes.length, 7);
    });
    test("records the clickable and enabled attributes", () => {
        ...
        node_assert_1.default.strictEqual(next.clickable, true);
        node_assert_1.default.strictEqual(next.enabled, false);
    });
    test("tracks depth so a child outranks a same-sized wrapper", () => {
        ...
        node_assert_1.default.ok(wrapper && child);
        node_assert_1.default.ok(child.depth > wrapper.depth);
    });
});
describe("hitTest", () => {
    const nodes = (0, tap_hit_tester_1.extractBoundedNodes)(LOGIN_XML);
    test("a tap on the button's inner label resolves to the clickable container", () => {
        const hit = (0, tap_hit_tester_1.hitTest)(nodes, 540, 770, { appPackage: "com.bank.app" });
        node_assert_1.default.ok(hit);
        node_assert_1.default.strictEqual(hit.target.label, "Ingresar");
        node_assert_1.default.strictEqual(hit.fallback, false);
        node_assert_1.default.deepStrictEqual(hit.target.locators[0], {
    ...
    test("a tap on an input resolves to the input and marks its role", () => {
        const hit = (0, tap_hit_tester_1.hitTest)(nodes, 500, 470, { appPackage: "com.bank.app" });
        node_assert_1.default.ok(hit);
        node_assert_1.default.strictEqual(hit.target.role, "input");
        node_assert_1.default.strictEqual(hit.target.label, "Usuario");
    });
    test("carries the disabled state of a gated control", () => {
        const hit = (0, tap_hit_tester_1.hitTest)(nodes, 500, 970, { appPackage: "com.bank.app" });
        node_assert_1.default.ok(hit);
        node_assert_1.default.strictEqual(hit.target.enabled, false);
    });
    test("a tap on static text falls back and says so", () => {
        const hit = (0, tap_hit_tester_1.hitTest)(nodes, 500, 240, { appPackage: "com.bank.app" });
        node_assert_1.default.ok(hit);
        node_assert_1.default.strictEqual(hit.fallback, true);
        node_assert_1.default.strictEqual(hit.target.label, "Bienvenido");
    });
    test("ignores controls owned by another package", () => {
        const hit = (0, tap_hit_tester_1.hitTest)(nodes, 540, 40, { appPackage: "com.bank.app" });
        // Only the app's root frame contains that point once System UI is filtered out.
        node_assert_1.default.ok(hit === null || hit.target.label === "");
    });
    test("returns null when the coordinate falls outside every node", () => {
        node_assert_1.default.strictEqual((0, tap_hit_tester_1.hitTest)(nodes, 5000, 5000, { appPackage: "com.bank.app" }), null);
    });
});
describe("touch calibration", () => {
    test("parses the touchscreen ranges from a labelled probe (getevent -lp)", () => {
        ...
        node_assert_1.default.deepStrictEqual((0, android_touch_listener_1.parseTouchDevices)(probe)[0], {
    ...
    test("prefers a multitouch panel over a single-touch device listed first", () => {
        ...
        node_assert_1.default.strictEqual((0, android_touch_listener_1.parseTouchDevices)(probe)[0]?.devicePath, "/dev/input/event1");
    });
    test("does not mistake a key code for a position axis", () => {
        ...
        node_assert_1.default.deepStrictEqual((0, android_touch_listener_1.parseTouchDevices)(probe), []);
    });
    test("finds nothing when no device reports position axes", () => {
        node_assert_1.default.deepStrictEqual((0, android_touch_listener_1.parseTouchDevices)('add device 1: /dev/input/event0\n  name: "gpio-keys"'), []);
    });
    ...
    test("returns every candidate, not just the first, on a multi-node emulator", () => {
        ...
        node_assert_1.default.deepStrictEqual((0, android_touch_listener_1.parseTouchDevices)(probe).map((device) => device.devicePath), ["/dev/input/event12", "/dev/input/event11", "/dev/input/event10"]);
    });
    test("lists the probed devices so a failed calibration can say what it saw", () => {
        ...
        node_assert_1.default.deepStrictEqual((0, android_touch_listener_1.summarizeProbedDevices)(probe), [
    ...
    test("prefers an override screen size over the physical one", () => {
        ...
        node_assert_1.default.deepStrictEqual((0, android_touch_listener_1.parseScreenSize)(out), { width: 1080, height: 2340 });
    });
    test("scales a digitizer coordinate into display pixels", () => {
        ...
        node_assert_1.default.deepStrictEqual((0, android_touch_listener_1.scalePoint)(2048, 4096, cal), { x: 540, y: 1200 });
    });
});
describe("createTouchStreamParser", () => {
    ...
    test("reassembles a tap from a tracking-id framed contact", () => {
        const parser = (0, android_touch_listener_1.createTouchStreamParser)({ calibration });
        ...
        node_assert_1.default.strictEqual(gestures.length, 1);
        node_assert_1.default.deepStrictEqual(gestures[0], { kind: "tap", x: 540, y: 770, t: 0 });
    });
    test("reassembles a tap from a BTN_TOUCH framed contact", () => {
        ...
        node_assert_1.default.strictEqual(gestures.length, 1);
        node_assert_1.default.strictEqual(gestures[0].kind, "tap");
        node_assert_1.default.strictEqual(gestures[0].x, 100);
        node_assert_1.default.strictEqual(gestures[0].y, 200);
    });
    test("classifies a long travel as a swipe with its endpoints", () => {
        ...
        node_assert_1.default.strictEqual(swipe.kind, "swipe");
        if (swipe.kind !== "swipe")
            return;
        node_assert_1.default.strictEqual(swipe.y, 1600);
        node_assert_1.default.strictEqual(swipe.toY, 200);
        node_assert_1.default.strictEqual(swipe.durationMs, 300);
    });
    ...
    test("drops a contact that never reports a coordinate", () => {
        ...
        node_assert_1.default.strictEqual(gestures.length, 1);
        node_assert_1.default.deepStrictEqual(gestures[0], { kind: "tap", x: 100, y: 200, t: 0 });
    });
    test("reassembles a tap on a single-touch digitizer (ABS_X / ABS_Y)", () => {
        ...
        node_assert_1.default.deepStrictEqual(gestures, [{ kind: "tap", x: 500, y: 1000, t: 0 }]);
    });
    test("ignores lines from another input device", () => {
        const parser = (0, android_touch_listener_1.createTouchStreamParser)({ calibration, devicePath: "/dev/input/event1" });
        const gestures = feed(parser, [
            "[   4000.000000] /dev/input/event0: EV_KEY       KEY_POWER            00000001",
            "[   4000.000000] /dev/input/event0: EV_KEY       KEY_POWER            00000000",
        ]);
        node_assert_1.default.deepStrictEqual(gestures, []);
    });
    test("a jitter smaller than the tap threshold stays a tap", () => {
        const parser = (0, android_touch_listener_1.createTouchStreamParser)({ calibration, tapMaxDistancePx: 24 });
        const gestures = feed(parser, [
...
describe("readableTitle", () => {
    ...
    test("skips a title made of icon-font glyphs", () => {
        ...
        node_assert_1.default.strictEqual((0, android_session_recorder_1.readableTitle)(snapshot), "Hola, valida tus datos de contacto.");
    });
    test("keeps a real title", () => {
        node_assert_1.default.strictEqual((0, android_session_recorder_1.readableTitle)({ title: "Iniciar sesión", assertionTargets: ["Usuario"] }), "Iniciar sesión");
    });
    test("returns undefined when no text carries letters or digits", () => {
        node_assert_1.default.strictEqual((0, android_session_recorder_1.readableTitle)({ title: "", assertionTargets: ["···", "—"] }), undefined);
    });
});
...
describe("locator uniqueness", () => {
    const nodes = (0, tap_hit_tester_1.extractBoundedNodes)(DUPLICATE_LABEL_XML);
    test("a label that is unique on the screen is offered as-is", () => {
        const hit = (0, tap_hit_tester_1.hitTest)(nodes, 720, 2950, { appPackage: "com.bank.app" });
        node_assert_1.default.deepStrictEqual(hit?.target.locators[0], {
            strategy: "accessibilityId",
            value: "Salir",
            confidence: 0.95,
    ...
    test("two controls sharing a label resolve to different locators", () => {
        const email = (0, tap_hit_tester_1.hitTest)(nodes, 720, 1170, { appPackage: "com.bank.app" });
        const phone = (0, tap_hit_tester_1.hitTest)(nodes, 720, 1690, { appPackage: "com.bank.app" });
        const first = email?.target.locators[0];
        const second = phone?.target.locators[0];
        node_assert_1.default.ok(first && second);
        node_assert_1.default.notStrictEqual(first.value, second.value);
        node_assert_1.default.strictEqual(first.value, 'new UiSelector().description("Enviar código de validación").instance(0)');
        node_assert_1.default.strictEqual(second.value, 'new UiSelector().description("Enviar código de validación").instance(1)');
    });
    test("a positional locator is marked ambiguous and drops below the uncertainty threshold", () => {
        ...
        node_assert_1.default.strictEqual(locator?.ambiguous, true);
        node_assert_1.default.strictEqual(locator?.matchIndex, 1);
        node_assert_1.default.ok((locator?.confidence ?? 1) < 0.7);
    });
    test("a composite content-desc is anchored on its longest segment and stays unique", () => {
        const hit = (0, tap_hit_tester_1.hitTest)(nodes, 720, 980, { appPackage: "com.bank.app" });
        node_assert_1.default.strictEqual(hit?.target.locators[0].ambiguous, undefined);
        node_assert_1.default.match(hit?.target.locators[0].value ?? "", /descriptionContains\("kev\*\*\*\*\*05@gmail\.com"\)/);
    });
});

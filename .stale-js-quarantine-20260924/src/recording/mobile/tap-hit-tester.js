...
exports.extractBoundedNodes = extractBoundedNodes;
exports.buildLocators = buildLocators;
 ...
 */
function extractBoundedNodes(pageSourceXml) {
    const nodes = [];
    let depth = 0;
    let m;
 ...
 */
function buildLocators(node, tree = []) {
...
function nodeRole(node) {
    ...
    if (INPUT_CLASS_RE.test(cls))
        return "input";
    if (/Button/i.test(cls))
        return "button";
    if (/CheckBox|Switch|ToggleButton/i.test(cls))
        return "toggle";
    if (node.clickable)
        return "button";
    return "text";
 ...
 */
function hitTest(nodes, x, y, options = {}) {

...
(0, node_test_1.default)("structured extras are technical prerequisites, not unauthorized functional actions", () => {
    ...
    strict_1.default.equal(extraActions.filter((action) => action.actionType === "fill").length, 0);
    strict_1.default.equal(extraActions.filter((action) => action.actionType === "select").length, 1);
});

...
exports.renderWalkthrough = renderWalkthrough;
exports.buildCompactSemanticAiContext = buildCompactSemanticAiContext;
exports.parseNegatives = parseNegatives;
exports.enrichFromTrace = enrichFromTrace;
exports.applyStory = applyStory;
...
function renderWalkthrough(trace, segments) {
    ...
    for (const segment of segments) {
        ...
        for (const event of segment.events) {
            ...
            switch (event.kind) {
                case "tap":
                    lines.push(`  - El usuario presionó "${label ?? "un control"}"${event.target?.enabled === false ? " (el control estaba deshabilitado)" : ""}`);
                    break;
                case "fill":
                    lines.push(event.redactedKey
        ...
        if (texts.length > 0)
            lines.push(`  Textos visibles: ${texts.join(" | ")}`);
        lines.push("");
    }
    return lines.join("\n").trim();
}
 ...
 */
function isAnchored(basedOn, transcript) {
 ...
 */
function buildCompactSemanticAiContext(trace, segments, happyPath) {
    ...
    const compact = {
        ...
        primaryFunctionalActions: actions.slice(0, 100).map((event) => ({
            ...
            value: event.kind === "fill" && !(0, semantic_recording_1.isSensitiveRecordedEvent)(event) ? event.value : undefined,
...
function buildUserPrompt(context, happyPath) {
    return [
        ...
        (0, ai_scenario_contract_1.recordingAiSchemaInstruction)(),
 ...
 */
function parseNegatives(raw, transcript, onLog) {
...
function isTechnicalStoryDescription(value) {
 ...
 */
async function enrichFromTrace(trace, segments, happyPath, ai, onLog) {
    ...
    try {
        ...
        const response = await ai.completeJson({
            ...
            jsonSchema: {
                name: ai_scenario_contract_1.RECORDING_AI_SCENARIO_SCHEMA_NAME,
                schema: ai_scenario_contract_1.RECORDING_AI_SCENARIO_SCHEMA,
            },
            purpose: "scenario_generation",
        });
        const validation = (0, ai_scenario_contract_1.validateRecordingAiScenarioResponse)(response.parsedJson);
        ...
        onLog?.(`[recording:ai] contrato rechazado o IA no disponible (${err instanceof Error ? err.message : String(err)}); se conserva el escenario determinista fallbackUsed=true`);
        return { narrative, story: null, extraNegatives: [], aiProposals: [], aiRejected: [], schemaValid: false, fallbackUsed: true };
    }
}
/** Applies the model's story to the deterministic scenario without touching its steps. */
function applyStory(scenario, result) {
    if (!result.story)
        return scenario;
    return {
        ...scenario,
        ...
        title: (0, trace_to_scenario_1.capTitle)(result.story.title || scenario.title),

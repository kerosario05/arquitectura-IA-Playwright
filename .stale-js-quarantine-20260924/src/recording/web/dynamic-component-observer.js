...
exports.captureDynamicComponentSnapshot = captureDynamicComponentSnapshot;
exports.technicalTargetsForSnapshot = technicalTargetsForSnapshot;
exports.observeDynamicComponentActivation = observeDynamicComponentActivation;
const ROOT_MARKER = "data-recording-dynamic-root";
const TRIGGER_MARKER = "data-recording-dynamic-trigger";
async function markRoot(page, trigger, marker) {
...
async function captureDynamicComponentSnapshot(root, trigger) {
    return root.evaluate((element, triggerMarker) => {
        const clean = (value) => value?.replace(/\s+/g, " ").trim() || undefined;
        const visible = (node) => {
            const html = node;
            if (html.hidden || node.getAttribute("aria-hidden") === "true")
                return false;
        ...
        const stableAttributes = (node) => Object.fromEntries(["data-testid", "data-test-id", "name", "type", "aria-label", "aria-labelledby", "role", "data-field", "data-column"]
        ...
        const toControl = (node) => {
            ...
            const value = "value" in node && typeof node.value === "string" ? node.value : clean(node.textContent);
            return {
                tag,
                role,
                ...
                enabled: !("disabled" in node) || !node.disabled,
        ...
        const surface = surfaceElement ? {
            ...
            options: Array.from(surfaceElement.querySelectorAll("option, [role='option'], [data-option], [data-value], li"))
                .filter(visible)
                .map((option) => ({ text: clean(option.textContent || option.getAttribute("aria-label") || option.getAttribute("data-value")) || "", identity: identity(option), selected: option.getAttribute("aria-selected") === "true" || option.selected === true, disabled: option.disabled === true })),
        } : undefined;
...
function technicalTargetsForSnapshot(snapshot, validatedByInteraction = false) {
    const targets = [];
    for (const control of [snapshot.editableControl, snapshot.selectionControl].filter((value) => Boolean(value))) {
        const locators = [];
        const attrs = control.stableAttributes;
        if (attrs["data-testid"] || attrs["data-test-id"])
            locators.push({ strategy: "data-testid", value: attrs["data-testid"] || attrs["data-test-id"], confidence: 0.98 });
        if (attrs["aria-label"])
            locators.push({ strategy: "aria-label", value: attrs["aria-label"], confidence: 0.9 });
        if (attrs.name)
            locators.push({ strategy: "css", value: `[name="${attrs.name.replace(/"/g, "\\\"")}"]`, confidence: 0.65 });
...
async function observeDynamicComponentActivation(page, trigger, options = {}) {
    ...
    await page.evaluate(({ marker, rootMarker }) => {
        ...
        const state = { mutations: [], escalated: false, portal: false, ariaControls: false, focus: [], localObserver: undefined, documentObserver: undefined, focusHandler: undefined };
        const inspect = () => {
            ...
            const ids = controls.flatMap((node) => [node.getAttribute("aria-controls"), node.getAttribute("aria-owns")].filter(Boolean));
            if (ids.length > 0) {
        ...
        const escalate = () => {
            if (state.escalated || !document.body)
                return;
            state.escalated = true;
        ...
        state.localObserver = new MutationObserver((records) => {
            ...
            inspect();
            if (state.ariaControls || state.portal)
                escalate();
        });
    ...
    if (after.editableControl && before.editableControl?.disabled && !after.editableControl.disabled)
        states.push("DEPENDENT_CONTROL_ENABLED");

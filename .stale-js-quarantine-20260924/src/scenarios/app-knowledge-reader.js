...
exports.readAppKnowledge = readAppKnowledge;
exports.selectLearnedRoute = selectLearnedRoute;
...
function classifyHuScope(issue) {
    ...
    const authenticatedPrecondition = /\b(ses[ií]on\s+(v[aá]lida|iniciada|activa|existente)|ya\s+(est[aá]|se\s+encuentra)\s+(autenticado|logueado)|autenticaci[oó]n\s+(completada|exitosa|ya\s+realizada)|precondici[oó]n\s+autenticada)\b/i.test(corpus);
...
function isItemRelevant(item, huScope, authenticatedPrecondition) {
    if (!item.steps || item.steps.length === 0)
        return { relevant: false, reason: "no_steps" };
    if (!item.coverageRefs || item.coverageRefs.length === 0)
        return { relevant: false, reason: "no_coverage_refs" };
    if (item.source !== "scenario_derived")
        return { relevant: false, reason: "invalid_source" };
    const itemText = [
        item.scenarioTitle,
        ...item.steps,
        ...item.clickTargets,
    ...
    if (huScope === "public_info") {
        ...
        if (hasAuth)
            return { relevant: false, reason: "public_hu_skips_auth_route" };
    ...
    if (huScope === "private_transactional") {
        ...
        const hasEntryClick = item.clickTargets.some(ct => ct.toLowerCase().includes("iniciar") || ct.toLowerCase().includes("ingresar") || ct.toLowerCase().includes("acceder"));
...
function readAppKnowledge(appSlug, primaryIssue) {
    ...
    // Classify HU scope
    const classification = primaryIssue ? classifyHuScope(primaryIssue) : { scope: "unknown", authenticatedPrecondition: false };
    ...
    for (const candidate of candidates) {
        if (selected.length >= MAX_ITEMS)
            break;
        if (totalSteps + candidate.item.steps.length > MAX_STEPS)
            continue;
        const hintChars = candidate.item.steps.join(" ").length + candidate.reason.length;
        if (totalChars + hintChars > MAX_CHARS)
            continue;
        selected.push({
 ...
 */
function selectLearnedRoute(appSlug, huTerms, huScope, contractTerms, contractAuthPrecondition) {
    const knowledgePath = path.join(process.cwd(), "automations", "apps", appSlug, "app.knowledge.json");
    if (!fs.existsSync(knowledgePath))
        return null;
    let knowledge;
    ...
    const MIN_SCORE = 1;
    let bestFunctional = null;
    let bestAuthEntry = null;
    for (const item of knowledge.items) {
        if (item.source !== "scenario_derived")
            continue;
        if (!item.coverageRefs?.includes("learned_navigation_path"))
            continue;
        if (!item.steps || item.steps.length === 0)
            continue;
        ...
        if (item.validationStatus === "rejected")
            continue;
        if (item.knowledgeKind === "rejected_route" || item.knowledgeKind === "rejected_scenario")
            continue;
        ...
        for (const ht of huNorm) {
            if (itemCorpusJoined.includes(ht))
                termMatches++;
        }
        if (termMatches > 0) {
        ...
        for (const ct of contractNorm) {
            if (itemCorpusJoined.includes(ct))
                contractMatches++;
        }

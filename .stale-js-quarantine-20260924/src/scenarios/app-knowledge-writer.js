...
exports.writeAppKnowledge = writeAppKnowledge;
exports.appendRouteSuggestionToKnowledge = appendRouteSuggestionToKnowledge;
...
function computeStableId(appSlug, issueKey, normalizedSteps) {
...
function semanticStepsKey(issueKey, steps) {
...
function finalDedupKey(appSlug, issueKey, coverageRefs, steps) {
...
function extractClickTargets(steps) {
    const targets = [];
    const seen = new Set();
...
function extractAssertionTargets(steps) {
    const targets = [];
    const seen = new Set();
    for (const step of steps) {
...
function extractNegativeAssertions(steps) {
    const assertions = [];
    const seen = new Set();
    for (const step of steps) {
...
function extractAuthTerms(scenarioText, flow) {
    if (!flow || !flow.requiresAuth)
        return [];
    const terms = [];
    const seen = new Set();
...
function findMatchingOptionFlow(scenarioSteps, scenarioTitle, optionFlows) {
...
function findCoverageRefs(scenarioTitle, steps, optionLabels, contract, routeProfile) {
    if (!contract)
        return [];
    const refs = [];
    const titleLower = scenarioTitle.toLowerCase().normalize("NFC");
    ...
    const allItems = [
        ...
        ...contract.listScreens || [],
        ...contract.selectionFlows || [],
        ...contract.singleItemShortcuts || [],
        ...contract.detailFields || [],
        ...contract.formatRules || [],
        ...contract.actionOptions || [],
        ...contract.conditionalAlerts || [],
        ...contract.failureCases || [],
        ...contract.navigationItems || [],
        ...contract.selectableOptions || [],
        ...contract.visibleMessages || [],
        ...contract.confirmationFlows || [],
        ...contract.deliveryOptions || [],
        ...contract.referenceNumbers || [],
    ];
    for (const item of allItems) {
        if (refs.includes(item.id))
            continue;
        const searchTerms = [
            item.label,
            item.id,
...
function normalizeTargetForDedupe(target) {
    return target
...
function buildAllowedKnowledgeTerms(contract, optionFlows, routeProfile) {
    const assertions = new Set();
    const negatives = new Set();
    const clicks = new Set();
    const addTerm = (s) => { if (s) {
        const t = normalizeCompare(s);
        if (t.length > 1)
            assertions.add(t);
    } };
    const addClick = (s) => { if (s) {
        const t = normalizeCompare(s);
        if (t.length > 1)
            clicks.add(t);
    } };
    const addNegative = (s) => { if (s) {
        const t = normalizeCompare(s);
        if (t.length > 1)
            negatives.add(t);
    } };
    if (contract) {
        ...
        for (const item of allItems) {
            ...
            for (const el of item.initialScreenElements || [])
                addTerm(el);
            for (const el of item.postActionElements || [])
                addTerm(el);
            ...
            if (item.requiresAuth && item.expectedResult) {
                ...
                if (lower.includes("identificaci"))
                    addTerm("identificación");
                if (lower.includes("autenticaci"))
                    addTerm("autenticación");
    ...
    if (routeProfile) {
        ...
        const entryTargets = [];
        for (const step of rp._entryPath || []) {
            if (step?.target) {
                addClick(step.target);
                entryTargets.push(step.target);
            }
        }
        for (const step of rp._authenticatedPathArr || []) {
            if (step?.target) {
                addClick(step.target);
                entryTargets.push(step.target);
            }
        ...
        for (const sig of rp._terminalSignals || []) {
            addTerm(sig);
            entryTargets.push(`signal:${sig}`);
        }
        for (const da of rp._detailAssertions || []) {
            addTerm(da);
            entryTargets.push(`assertion:${da}`);
        }
...
function writeAppKnowledge(appSlug, validatedScenarios, contract, optionFlows, branchConflictTitles, outOfScopeRejectedTitles, currentIssueKey, routeProfile, routeResolutionBlocked, rejectedRouteProfileKeys) {
    ...
    const finalScenarios = validatedScenarios.filter(sc => {
        ...
        if (branchConflictTitles.has(title)) {
            skipped.branchConflict++;
            return false;
        }
        if (outOfScopeRejectedTitles.has(title)) {
            skipped.outOfScope++;
            return false;
        }
        return true;
    ...
    if (skipped.branchConflict > 0)
        console.log(`[app-knowledge] skippedBranchConflict count=${skipped.branchConflict}`);
    if (skipped.outOfScope > 0)
        console.log(`[app-knowledge] skippedOutOfScope count=${skipped.outOfScope}`);
    const hasScenarioData = finalScenarios.length > 0;
    const hasRouteProfileData = routeProfile &&
        routeProfile?._selectionTarget &&
        routeProfile?._entryPath?.length > 0 &&
        routeProfile?._authenticatedPathArr?.length > 0 &&
        routeProfile?._accessMode === "private";
    ...
    const normalizeTerm = (s) => normalizeCompare(s);
    const now = nowIso();
    // --- Route classification helpers (generic signal-based, no hardcoded names) ---
    const _classifyAccessLevel = (item, matchingFlows, contract) => {
        if (item.authTerms.length > 0)
            return "private";
        if (contract?.authenticatedPrecondition)
            return "authenticated";
        if (matchingFlows.some(f => f.requiresAuth))
            return "private";
        if (matchingFlows.length > 0 && matchingFlows.every(f => !f.requiresAuth))
            return "public";
        if (contract?.functionalScope === "public_info" || contract?.functionalScope === "entry_menu_or_landing")
            return "public";
        if (contract?.functionalScope === "private_transactional")
            return "private";
        return "unknown";
    };
    const _classifyIntent = (item, matchingFlows, contract) => {
        if (matchingFlows.some(f => f.requiresAuth))
            return "auth_entry";
        if (item.authTerms.length > 0)
            return "auth_entry";
        if (contract?.functionalScope === "entry_menu_or_landing")
            return "menu_entry";
        if (contract?.functionalScope === "public_info")
            return "informational";
        if (contract?.functionalScope === "private_transactional")
            return "transactional";
        if (matchingFlows.length > 0 && !matchingFlows[0].requiresAuth) {
            const result = (matchingFlows[0].expectedResult || "").toLowerCase();
            if (/informaci.n|detalle|resumen|consulta/i.test(result))
                return "informational";
            return "transactional";
        }
        const allText = [item.scenarioTitle, ...item.steps].join(" ").toLowerCase();
        if (/informaci.n|detalle|resumen|saldo/i.test(allText) && !/transferir|pagar|enviar/i.test(allText))
            return "informational";
        if (/transferir|pagar|enviar|solicitar|contratar/i.test(allText))
            return "transactional";
        if (/men.|opciones|categorias|inicio/i.test(allText))
            return "menu_entry";
        return "functional";
    };
    const _classifyStartsFrom = (_item, contract) => {
        if (contract?.authenticatedPrecondition)
            return "authenticated_state";
        return "public_initial";
    };
    const _classifyEndsAt = (_item, matchingFlows, contract) => {
        if (matchingFlows.some(f => f.requiresAuth))
            return "auth_gate";
        if (_item.authTerms.length > 0)
            return "auth_gate";
        if (contract?.functionalScope === "private_transactional")
    ...
    const _isLearnedRoute = (coverageRefs) => {
        return coverageRefs.includes("learned_navigation_path") || coverageRefs.includes("private_entry_path");
    };
    const _isSharedOrAuthRoute = (item) => {
        const refs = item.coverageRefs || [];
        if (_isLearnedRoute(refs))
            return true;
        if (refs.includes("shared_entry_path"))
            return true;
        if (refs.some(r => r === "intent:auth_entry"))
            return true;
        if (refs.some(r => r === "endsAt:auth_gate"))
            return true;
        if (refs.some(r => r === "startsFrom:public_initial") && refs.some(r => r === "endsAt:auth_gate"))
            return true;
        return false;
    };
    const existingMap = new Map();
    // ── Knowledge scoring and classification helpers ──
    const _computeRouteSignature = (item) => {
        const refs = item.coverageRefs || [];
    ...
    const _computeCoverageSignature = (item) => {
    ...
    const _scoreAndClassify = (item, isRoute) => {
        const refs = item.coverageRefs || [];
        ...
        let score = 0;
        if (item.knowledgeKind === "route_prefix")
            score += 40;
        else if (item.knowledgeKind === "route_functional")
            score += 30;
        else if (item.knowledgeKind === "scenario_validated")
            score += 20;
        if (refs.length > 0)
            score += 20;
        ...
        if (item.runCount >= 2)
            score += 10;
        if (item.trustedForReuse === false)
            score -= 50;
        if (item.rejectedReason)
            score -= 30;
        if (item.sourceQuality === "fallback")
            score -= 40;
        ...
        if (isLearnedRoute || isRoute) {
            if (item.validationStatus !== "validated") {
                gatePassed = false;
                gateReason = "validation_status_not_validated";
            }
            else if (item.knowledgeKind !== "route_prefix" && item.knowledgeKind !== "route_functional") {
                gatePassed = false;
                gateReason = "wrong_knowledge_kind_for_route";
            }
            else if ((item.confidenceScore ?? 0) < 50) {
                gatePassed = false;
                gateReason = "score_below_50";
            }
            else if (!item.routeSignature) {
                gatePassed = false;
                gateReason = "missing_route_signature";
            }
            else if (item.containsRejectedTargets) {
                gatePassed = false;
                gateReason = "contains_rejected_targets";
            }
            else if (item.sourceQuality === "fallback") {
                gatePassed = false;
                gateReason = "fallback_source";
            }
            else {
                gateReason = "passed";
            }
            ...
            if (item.validationStatus !== "validated") {
                ...
                gatePassed = false;
                gateReason = "wrong_knowledge_kind_for_scenario";
            }
            else if ((item.confidenceScore ?? 0) < 40) {
                gatePassed = false;
                gateReason = "score_below_40";
            }
            else if ((item.coverageRefs?.length ?? 0) === 0) {
                gatePassed = false;
                gateReason = "no_coverage_refs";
            }
            else if (!item.coverageSignature) {
                gatePassed = false;
                gateReason = "missing_coverage_signature";
            }
            else if (item.sourceQuality === "fallback") {
                gatePassed = false;
                gateReason = "fallback_source";
    ...
    if (existing && existing.items.length > 0) {
        ...
        const removedDuplicateRouteIds = new Set();
        {
            ...
            if (!canonicalId && routeProfile) {
                ...
                if (rpAccessMode === "private") {
                    ...
                    for (const step of rp._entryPath || []) {
                        if (step?.target)
                            rpTargets.push(normalizeTargetForDedupe(step.target));
                    }
                    for (const step of rp._authenticatedPathArr || []) {
                        if (step?.target)
                            rpTargets.push(normalizeTargetForDedupe(step.target));
                    ...
                    if (rp._selectionTarget)
                        rpTargets.push(normalizeTargetForDedupe(rp._selectionTarget));
            ...
            if (canonicalId && canonicalTargets.length > 0) {
                ...
                for (const item of baseItems) {
                    ...
                    if (!_isLearnedRoute(item.coverageRefs))
                        continue;
                    if (!item.id.startsWith("learned_path_"))
                        continue;
                    ...
                    const isValidLearnedIntent = dRefIntent && (dRefIntent.endsWith("informational") || dRefIntent.endsWith("transactional") || dRefIntent.endsWith("menu_entry"));
        ...
        const filteredItems = baseItems.filter(item => {
            ...
            if (removedDuplicateRouteIds.has(item.id))
                return false;
            ...
            if (shouldCleanupCurrentIssue) {
                ...
                if (outContractTitle)
                    allBad.push("__title_out_of_contract__");
                if (allBad.length > 0) {
        ...
        const blockedFilteredItems = routeCompleteItems.filter(item => {
            if (!routeResolutionBlocked || !currentIssueKey || item.issueKey !== currentIssueKey) {
                ...
                if (currentIssueKey && item.issueKey !== currentIssueKey)
                    preservedOtherHistoricalBlocked++;
                return true;
            }
        ...
        for (const item of blockedFilteredItems) {
            ...
            if (existing) {
                ...
                if (item.createdAt < keeper.createdAt)
                    keeper.createdAt = item.createdAt;
                if (item.lastSeenAt > keeper.lastSeenAt)
                    keeper.lastSeenAt = item.lastSeenAt;
                removedDuplicates++;
            }
            else {
                semanticMap.set(sk, item);
        ...
        for (const [key, items] of refGroups) {
            ...
            for (let i = 1; i < items.length; i++) {
                ...
                const isSubset = subset.steps.every(ss => best.steps.some(bs => normalizeCompare(bs).includes(normalizeCompare(ss))));
                if (isSubset) {
                    best.runCount = Math.max(best.runCount || 1, subset.runCount || 1);
                    if (subset.createdAt < best.createdAt)
                        best.createdAt = subset.createdAt;
        ...
        for (const item of existingMap.values()) {
            if (!item.knowledgeKind)
                _scoreAndClassify(item, _isLearnedRoute(item.coverageRefs));
        }
    }
    ...
    for (const sc of finalScenarios) {
        ...
        const coverageRefs = sc.coverageRefs?.length > 0
            ? sc.coverageRefs
    ...
    if (skippedIncompleteRoute > 0)
        console.log(`[app-knowledge] skippedIncompleteRoute count=${skippedIncompleteRoute}`);
    ...
    const learnedRouteItems = [];
    let routeSkippedNoClick = 0;
    let routePersisted = 0;
    const routeDedupeSeen = new Set();
    for (const item of newItems) {
        ...
        if (routeProfile) {
            ...
            if (rpAccessMode === "private") {
                ...
                for (const step of rp._entryPath || []) {
                    if (step?.target)
                        knownTargets.push(normalizeTargetForDedupe(step.target));
                }
                for (const step of rp._authenticatedPathArr || []) {
                    if (step?.target)
                        knownTargets.push(normalizeTargetForDedupe(step.target));
                ...
                if (rp._selectionTarget)
                    knownTargets.push(normalizeTargetForDedupe(rp._selectionTarget));
        ...
        let routeConfidence = "scenario_derived";
        ...
        if (routeDedupeSeen.has(dedupeKey))
            continue;
        ...
        const learnedRouteItem = {
            ...
            lastSeenAt: now,
            runCount: 1,
            destinationSignals: Array.isArray(routeProfile?._terminalSignals)
                ? routeProfile._terminalSignals
                : undefined,
        };
    ...
    if (routeProfile) {
        const rp = routeProfile;
        const accessMode = rp._accessMode;
        const intent = rp._intent;
        const selectionTarget = rp._selectionTarget;
        const entryPath = rp._entryPath;
        const authPath = rp._authenticatedPathArr;
        const rpSource = rp._rpSource;
        const rpConfidence = rp._rpConfidence;
        const terminalSignals = rp._terminalSignals;
        const detailAssertions = rp._detailAssertions;
        ...
        if (hasValidAuthPath) {
            ...
            const steps = clickTargets.map(t => `Clic en "${t}"`);
            if (selectionTarget) {
            ...
            if (existingMatch) {
                ...
                if (rpSource === "user_confirmed")
                    existingMatch.source = "scenario_derived"; // keep type
                if (rpConfidence === "high")
                    existingMatch.confidence = "scenario_derived";
    ...
    const removedDuplicateRouteIds = new Set();
    {
        ...
        if (canonicalId && canonicalTargets.length > 0) {
            ...
            for (const [id, item] of existingMap.entries()) {
                ...
                if (!_isLearnedRoute(item.coverageRefs))
                    continue;
                if (!id.startsWith("learned_path_"))
                    continue;
                const dRefAccess = item.coverageRefs.find(r => r.startsWith("accessLevel:"));
                const dRefIntent = item.coverageRefs.find(r => r.startsWith("intent:"));
                const isAuthNonPrivate = dRefAccess && dRefAccess !== "accessLevel:private";
    ...
    // If an equivalent list_screen_0 item already exists (same clickTargets), remove the misclassified one.
    {
        ...
        for (const [id, item] of existingMap.entries()) {
            ...
            if (!/visualiz(?:aci[oó]n|ar)|validar\s+.*listad|\blistad/i.test(title))
                continue;
            if (/seleccionar|escoger|elegir|detalle|formato|opciones|alerta|falla|error|restringido|no\s+se\s+muestre|regresar|volver|bloqueo|inconsistencia|incompleto|indisponibilidad|ocultamiento|n[uú]mero\s+completo/i.test(title))
                continue;
            if (!item.coverageRefs.some(r => /^selection_flow_\d+$/.test(r)))
                continue;
            // Check if an equivalent list_screen_0 item already exists with compatible clickTargets
            const itemClicks = (item.clickTargets || []).map(t => normalizeTargetForDedupe(t));
            let existingListId = null;
            for (const [oid, oitem] of existingMap.entries()) {
                if (oid === id)
                    continue;
                if (_isLearnedRoute(oitem.coverageRefs))
                    continue;
                if (!oitem.coverageRefs.some(r => /^list_screen_\d+$/.test(r)))
                    continue;
    ...
    for (const [id, existingItem] of existingMap.entries()) {
        if (!_isLearnedRoute(existingItem.coverageRefs))
            continue;
        ...
        const isGeneric = (t) => genericTerms.some(g => t.toLowerCase().includes(g));
        let bestAssertion = null;
        if (endsAt === "auth_gate") {
            ...
            if (authRelated.length > 0)
                bestAssertion = authRelated[authRelated.length - 1];
        }
        if (!bestAssertion) {
            const nonGeneric = assertionTargets.filter(t => !isGeneric(t));
            if (nonGeneric.length > 0)
                bestAssertion = nonGeneric[nonGeneric.length - 1];
        }
    ...
    for (const item of preDedupItems) {
        ...
        if (existing) {
            ...
            if (existing.createdAt < keeper.createdAt)
                keeper.createdAt = existing.createdAt;
            if (item.createdAt < keeper.createdAt)
                keeper.createdAt = item.createdAt;
            if (existing.lastSeenAt > keeper.lastSeenAt)
                keeper.lastSeenAt = existing.lastSeenAt;
            if (item.lastSeenAt > keeper.lastSeenAt)
                keeper.lastSeenAt = item.lastSeenAt;
            // Merge arrays without duplicates
            for (const t of existing.clickTargets) {
                if (!keeper.clickTargets.includes(t))
                    keeper.clickTargets.push(t);
            }
            for (const t of item.clickTargets) {
                if (!keeper.clickTargets.includes(t))
                    keeper.clickTargets.push(t);
            }
            for (const t of existing.assertionTargets) {
                if (!keeper.assertionTargets.includes(t))
                    keeper.assertionTargets.push(t);
            }
            for (const t of item.assertionTargets) {
                if (!keeper.assertionTargets.includes(t))
                    keeper.assertionTargets.push(t);
            }
            for (const a of existing.negativeAssertions) {
                if (!keeper.negativeAssertions.includes(a))
                    keeper.negativeAssertions.push(a);
            }
            for (const a of item.negativeAssertions) {
                if (!keeper.negativeAssertions.includes(a))
                    keeper.negativeAssertions.push(a);
            }
            for (const ol of existing.optionLabels) {
                if (!keeper.optionLabels.includes(ol))
                    keeper.optionLabels.push(ol);
            }
            for (const ol of item.optionLabels) {
                if (!keeper.optionLabels.includes(ol))
                    keeper.optionLabels.push(ol);
            }
            for (const at of existing.authTerms) {
                if (!keeper.authTerms.includes(at))
                    keeper.authTerms.push(at);
            }
            for (const at of item.authTerms) {
                if (!keeper.authTerms.includes(at))
                    keeper.authTerms.push(at);
            ...
            for (const step of existing.steps) {
                if (!keeper.steps.some(s => normalizeCompare(s) === normalizeCompare(step)))
                    keeper.steps.push(step);
            }
            for (const step of item.steps) {
                if (!keeper.steps.some(s => normalizeCompare(s) === normalizeCompare(step)))
                    keeper.steps.push(step);
    ...
    for (const item of finalDedupMap.values()) {
        ...
        if (isOptionFlow) {
            ...
            if (!functionalGroups.has(groupKey))
                functionalGroups.set(groupKey, []);
    ...
    const functionalDedupMap = new Map();
    for (const [groupKey, items] of functionalGroups) {
        ...
        for (let i = 1; i < items.length; i++) {
            ...
            if (variant.createdAt < best.createdAt)
                best.createdAt = variant.createdAt;
            if (variant.lastSeenAt > best.lastSeenAt)
                best.lastSeenAt = variant.lastSeenAt;
    ...
    if (mergedRouteDups > 0) {
        const dedupedRouteItems = new Map();
        for (const item of routeSigMap.values())
            dedupedRouteItems.set(item.id, item);
        ...
        for (const [id, item] of dedupedRouteItems)
            functionalDedupMap.set(id, item);
    ...
    if (mergedCovDups > 0) {
        const dedupedCovItems = new Map();
        for (const item of covSigMap.values())
            dedupedCovItems.set(item.id, item);
        for (const [id, item] of functionalDedupMap) {
        ...
        functionalDedupMap.clear();
        for (const [id, item] of dedupedCovItems)
            functionalDedupMap.set(id, item);
    ...
    const _extractRef = (coverageRefs, prefix) => {
        const ref = coverageRefs.find(r => r.startsWith(prefix));
        return ref ? ref.slice(prefix.length) : null;
    };
    const _isCompletePrivateRoute = (rp) => {
        if (!rp || rp.accessMode !== "private")
            return false;
        const intent = rp.intent;
        if (!intent || intent === "none" || intent === "auth_entry")
            return false;
        if (!rp.entryPath || !Array.isArray(rp.entryPath) || rp.entryPath.length < 1)
            return false;
    ...
    const stablePrivateRoutes = result.items.filter(item => {
        if (!_isLearnedRoute(item.coverageRefs))
            return false;
        if (item.authTerms.length === 0)
            return false;
        if ((item.runCount || 1) < 2)
            return false;
        ...
        if (intent === "auth_entry" || intent === "none")
            return false;
        if (endsAt === "auth_gate")
            return false;
        // Must have enough steps for a functional route
        if (item.clickTargets.length < 2)
            return false;
        return true;
    });
    if (stablePrivateRoutes.length > 0) {
        ...
        try {
            ...
            if (fs.existsSync(appConfigPath)) {
                ...
                const appConfigContent = JSON.parse(raw);
                if (!appConfigContent.routeProfiles)
                    appConfigContent.routeProfiles = {};
                const routeProfiles = appConfigContent.routeProfiles;
                const existingPrivate = Object.values(routeProfiles).find((p) => p?.accessMode === "private");
                if (existingPrivate && _isCompletePrivateRoute(existingPrivate)) {
                    ...
                    const recoveryCandidates = result.items.filter(item => _isLearnedRoute(item.coverageRefs) &&
                    ...
                    if (recoveryCandidates.length > 0) {
                        ...
                        const clickTargets = recoveryRoute.clickTargets.map(t => ({ action: "click", target: t }));
                        // Partition: use existing incomplete route's entryPath as auth prefix, no overlap
                        const existingEntryCount = existingPrivate?.entryPath?.length || 1;
                        ...
                        if (!appConfigContent.routeProfiles[recKey]) {
                            appConfigContent.routeProfiles[recKey] = {
                                ...
                                terminalSignals: recoveryRoute.assertionTargets?.filter((a) => a.length < 40) || [],
                            ...
                            for (const [k, v] of Object.entries(routeProfiles)) {
                                if (v?.accessMode === "private" && k !== recKey) {
                    ...
                    const clickTargets = bestFunctional.clickTargets.map(t => ({ action: "click", target: t }));
                    ...
                    appConfigContent.routeProfiles[profileKey] = {
                        ...
                        terminalSignals: bestFunctional.assertionTargets?.filter((a) => a.length < 40) || [],
...
async function appendRouteSuggestionToKnowledge(appSlug, suggestion, automationsRoot) {
    ...
    if (!existing) {
        existing = knowledge.items.find(i => i.routeFrom != null &&
            i.actionTarget != null &&

...
exports.validateCanonicalSemanticNormalizationProposal = validateCanonicalSemanticNormalizationProposal;
exports.summarizeCanonicalSemanticProposalShape = summarizeCanonicalSemanticProposalShape;
...
function validateOrigin(value, path, errors) {
...
function validateCanonicalSemanticNormalizationProposal(value, context) {
    const errors = [];
    ...
    for (const [index, requirement] of value.requirements.entries()) {
        ...
        if (requirementIds.has(requirement.proposalId))
            addError(errors, "duplicate_proposal_id", "proposalId must be unique", `${path}.proposalId`);
        requirementIds.add(requirement.proposalId);
        if (requirement.branchProposalId !== undefined && !validString(requirement.branchProposalId))
            addError(errors, "invalid_branch_proposal_ref", "branchProposalId must be a non-empty string", `${path}.branchProposalId`, { expected: "non-empty string", receivedType: valueType(requirement.branchProposalId), receivedKeys: keysOf(requirement.branchProposalId) });
    ...
    for (const [index, branch] of value.branches.entries()) {
        ...
        if (branchIds.has(branch.branchProposalId))
            addError(errors, "duplicate_branch_proposal_id", "branchProposalId must be unique", `${path}.branchProposalId`);
        branchIds.add(branch.branchProposalId);
        if (branch.conditions !== undefined && !validStringArray(branch.conditions))
            addError(errors, "invalid_branch_conditions", "conditions must contain non-empty strings", `${path}.conditions`, { expected: "array of non-empty strings", receivedType: valueType(branch.conditions), receivedKeys: keysOf(branch.conditions) });
        if (branch.destinationIntent !== undefined && !validString(branch.destinationIntent))
            addError(errors, "invalid_destination_intent", "destinationIntent must be a non-empty string", `${path}.destinationIntent`, { expected: "non-empty string", receivedType: valueType(branch.destinationIntent), receivedKeys: keysOf(branch.destinationIntent) });
        for (const ref of branch.requirementProposalRefs)
            if (!requirementIds.has(ref))
                addError(errors, "dangling_requirement_proposal_ref", "branch references an unknown requirement proposal", path, { expected: "existing proposalId", receivedType: "string" });
    }
    for (const [index, requirement] of value.requirements.entries()) {
    ...
    for (const [index, link] of value.stepRequirementLinks.entries()) {
        ...
        if (!linkRecord || !hasOnlyKeys(linkRecord, linkKeys) || !Number.isInteger(linkRecord.stepOrder) || linkRecord.stepOrder <= 0 || !validStringArray(linkRecord.requirementProposalRefs)) {
        ...
        const stepOrder = linkRecord.stepOrder;
        if (!validOrders.has(stepOrder))
            addError(errors, "unknown_step_order", "stepOrder does not exist in the canonical scenario", `${path}.stepOrder`, { expected: "existing canonical step order", receivedType: "number" });
        for (const ref of linkRecord.requirementProposalRefs)
            if (typeof ref === "string" && !requirementIds.has(ref))
                addError(errors, "dangling_requirement_proposal_ref", "step link references an unknown requirement proposal", path, { expected: "existing proposalId", receivedType: "string" });
    }
...
function summarizeCanonicalSemanticProposalShape(value) {

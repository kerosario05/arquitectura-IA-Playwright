...
exports.evaluateCatalogQuality = evaluateCatalogQuality;
/**
 ...
 */
function evaluateCatalogQuality(routeProfile, options) {
    const reasons = [];
    ...
    for (const [target, tp] of Object.entries(targetPaths)) {
        ...
        if (!tp.productMetadata) {
            ...
            if (tp.productMetadata.presentationType === "product_card" &&
            ...
            const isDetailType = tp.productMetadata.presentationType === "detail_page" ||
    ...
    if (options.issueContext) {
        const huText = (options.issueContext.summary +
            " " +
            options.issueContext.description +
            " " +
            (options.issueContext.acceptanceCriteria || "")).toLowerCase();
        const mentionsProducts = huText.includes("producto") ||
    ...
    if (options.coverageMode === "exhaustive") {
        ...
        const hasRepresentativeFlag = Object.values(targetPaths).some((tp) => tp.productMetadata?.isRepresentative === true);

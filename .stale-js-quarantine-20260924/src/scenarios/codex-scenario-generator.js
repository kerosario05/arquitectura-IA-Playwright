...
exports.deriveProviderStepRequirementRefs = deriveProviderStepRequirementRefs;
exports.evaluateProviderRequirementCompliance = evaluateProviderRequirementCompliance;
exports.runCoverageCompletionPass = runCoverageCompletionPass;
exports.evaluateProviderClaimCompliance = evaluateProviderClaimCompliance;
exports.applyScenarioQualityGate = applyScenarioQualityGate;
exports.generateScenariosWithAi = generateScenariosWithAi;
const mcp_scenario_prompt_builder_1 = require("./mcp-scenario-prompt-builder");
...
const scenario_output_parser_1 = require("./scenario-output-parser");
...
const scenario_route_resolver_1 = require("./scenario-route-resolver");
const route_profile_derived_context_1 = require("./route-profile-derived-context");
const route_profile_quality_validator_1 = require("./route-profile-quality-validator");
const scenario_route_compliance_validator_1 = require("./scenario-route-compliance-validator");
 ...
 */
function convertSeedsToValidatedScenarios(seeds, appSlug, routeProfile, derivedContext, routeResolutions) {
    ...
    for (const scenario of seedScenarios) {
        const result = (0, scenario_route_compliance_validator_1.validateScenarioCompliance)(scenario, derivedContext, routeResolutions.get(scenario.sourceIssueKey));
        if (result.valid) {
 ...
 */
function normalizeAiScenario(scenario, appSlug) {
...
function extractHuCoverage(issue, pathSelection) {
...
function buildHuFallbackScenarios(issues, appSlug, routeProfile, routeResolutions, entrySteps, pathSelectionMap, functionalBranches = [], requirementManifest) {
    ...
    const entryPrefix = isPrivateSynthetic
        ...
        : (entrySteps || []).slice(0, 2).map(step => step.action === "click"
            ? `Clic en "${step.target}".`
            : `Validar que se muestre "${step.target}".`);
    const routePrefix = (resolution?.executableRouteSteps || []).slice(0, 3);
    const syntheticPrefix = (pathSelection?.selectedPath?.requiredIntermediates || [])
        .slice(0, 3)
    ...
    const navigationPrefix = [...entryPrefix, ...(routePrefix.length > 0 && !isPrivateSynthetic ? routePrefix : syntheticPrefix)]
        .filter((step, index, allSteps) => allSteps.findIndex(candidate => candidate.toLowerCase() === step.toLowerCase()) === index);
    ...
    const removedPublicSteps = scenarios.reduce((total, scenario, index) => total + (scenario.steps.length - sanitizedScenarios[index].steps.length), 0);
...
function writeScenarioGenerationTrace(issues, requirementManifest, scenarios, stages = {}, canonicalClaims = []) {
    const issueKey = issues[0]?.key ?? "unknown";
    const directory = (0, node_path_1.join)(process.cwd(), ".artifacts", "scenario-generation-traces");
    ...
    const claimTrace = canonicalClaims.map((claim) => {
        ...
        const referenced = (stage) => (stagesByName(stageNames, stage, stages) ?? []).some((scenario) => (scenario.stepClaims ?? []).some((ref) => ref.claimId === claim.claimId));
    ...
    (0, node_fs_1.writeFileSync)((0, node_path_1.join)(directory, `${issueKey}.json`), JSON.stringify({
        issueKey,
    ...
    console.log(`[scenarios:trace] captured issue=${issueKey} path=${(0, node_path_1.join)(directory, `${issueKey}.json`)}`);
}
function stagesByName(names, name, stages) {
    return names.includes(name) ? stages[name] : undefined;
}
function deriveProviderStepRequirementRefs(scenarios, canonicalClaims) {
    ...
    return scenarios.map((scenario) => {
        ...
        for (const ref of Array.isArray(scenario.stepRequirementRefs) ? scenario.stepRequirementRefs : []) {
            if (!canonicalClaims.some((claim) => claim.requirementId === ref.requirementId))
                continue;
            if (!Number.isInteger(ref.stepIndex) || ref.stepIndex < 0 || ref.stepIndex >= (scenario.steps?.length ?? 0))
                continue;
        ...
        refs.push(...(Array.isArray(scenario.stepClaims) ? scenario.stepClaims : []).flatMap((claim) => {
            const descriptor = canonicalById.get(claim.claimId);
            const stepIndex = claim.stepIndex;
            if (!descriptor || !Number.isInteger(stepIndex) || stepIndex < 0 || stepIndex >= (scenario.steps?.length ?? 0))
                return [];
            if (descriptor.scope === "branch" && descriptor.scopeId !== scenario.functionalBranch?.branchId)
                return [];
...
function evaluateProviderRequirementCompliance(scenarios, requirementManifest, canonicalClaims = []) {
    const canonical = new Set(requirementManifest.map((requirement) => requirement.requirementId ?? requirement.id));
    const claimRequirementIds = new Map(canonicalClaims.map((claim) => [claim.claimId, claim.requirementId]));
    const expected = requirementManifest
        .filter((requirement) => (0, scenario_functional_quality_1.isRequirementFunctionallyCoverable)(requirement))
    ...
    for (const scenario of scenarios) {
        for (const ref of scenario.stepRequirementRefs ?? []) {
            ...
            if (!requirement || !canonical.has(ref.requirementId))
                continue;
            if (!Number.isInteger(ref.stepIndex) || ref.stepIndex < 0 || ref.stepIndex >= (scenario.steps?.length ?? 0))
                continue;
            if (requirement.associatedBranchId && scenario.functionalBranch?.branchId !== requirement.associatedBranchId)
                continue;
        ...
        for (const claim of scenario.stepClaims ?? []) {
            ...
            if (!requirementId || !canonical.has(requirementId))
                continue;
...
function buildCoverageCompletionMessages(issue, missingRequirements, canonicalClaims) {
...
function mergeScenariosByLineage(firstPass, completionPass) {
    ...
    for (const scenario of [...firstPass, ...completionPass]) {
        ...
        const refs = (scenario.stepRequirementRefs ?? []).map((ref) => `${ref.requirementId}:${ref.stepIndex}`).sort().join("|");
        const claims = (scenario.stepClaims ?? []).map((claim) => `${claim.claimId}:${claim.stepIndex}`).sort().join("|");
        const key = `${scenario.sourceIssueKey ?? ""}::${branchId}::${refs}::${claims}`;
        if (!byLineage.has(key))
            byLineage.set(key, scenario);
    }
    return [...byLineage.values()];
}
async function runCoverageCompletionPass(firstPassScenarios, requirementManifest, canonicalClaims, provider, issue, prepareCompletionScenarios) {
    const coverableRequirements = requirementManifest.filter(scenario_functional_quality_1.isRequirementFunctionallyCoverable);
    ...
    try {
        ...
        const parsed = response.parsedJson ? (0, scenario_output_parser_1.parseAiResponse)(response.rawText) : null;
        const completionScenarios = await prepareCompletionScenarios(parsed?.scenarios ?? []);
        const missingIds = new Set(missingRequirements.map((requirement) => requirement.requirementId ?? requirement.id));
        scenarios = mergeScenariosByLineage(scenarios, completionScenarios.filter((scenario) => (scenario.stepRequirementRefs ?? []).some((ref) => missingIds.has(ref.requirementId))));
...
function evaluateProviderClaimCompliance(scenarios, canonicalClaims) {
    ...
    for (const scenario of scenarios) {
        ...
        for (const claim of Array.isArray(scenario.stepClaims) ? scenario.stepClaims : []) {
            ...
            const invalid = (reason, descriptor) => {
                invalidClaims.push({
                    scenarioId,
...
function repairUnquotedValidationStep(normalized) {
...
function applyScenarioQualityGate(scenarios, issues, routeProfile, pathSelectionMap, huEvidenceMap) {
    const normalizedScenarios = [];
    const rejected = [];
    const rejectedReasons = new Set();
    let normalizedCount = 0;
    const optionFlowLabelsByIssue = new Map();
    for (const issue of issues) {
        const flowLabels = new Set();
        const detected = (0, hu_scope_guard_1.detectOptionFlows)(issue);
        for (const flow of detected.flows) {
            if (!flow.optionLabel?.trim())
                continue;
    ...
    for (const scenario of scenarios) {
        ...
        let rejectedReason;
        const visibleControls = new Set((routeProfile?.visibleControls || []).map((value) => value.toLowerCase()));
        ...
        for (const [originalIndex, rawStep] of (scenario.steps || []).entries()) {
            ...
            if (ordinalMatch) {
                ...
                const domainTerm = pathSelection?.selectedPath?.target ||
                    routeProfile?._selectionTarget ||
                    routeProfile?.domainTerms?.ORDINAL_SELECTION;
                if (domainTerm && typeof domainTerm === "string") {
                    ...
                    const termSource = pathSelection?.selectedPath?.target ? "pathSelection.target" :
                        routeProfile?._selectionTarget ? "routeProfile.selectionTarget" : "routeProfile.domainTerms";
        ...
        const remappedClaims = (0, step_authority_1.remapStepClaimsByOrigins)(scenario.stepClaims, normalizedOrigins);
 ...
 */
function dedupeConsecutiveStepsWithOrigins(steps) {
    if (steps.length === 0)
 ...
 */
function repairUnbackedClicks(steps, allowedTargets, requiredIntermediates, preservedPrivateTargets) {
    return repairUnbackedClicksWithOrigins(steps, allowedTargets, requiredIntermediates, preservedPrivateTargets).steps;
}
function repairUnbackedClicksWithOrigins(steps, allowedTargets, requiredIntermediates, preservedPrivateTargets, canonicalClaims = [], stepClaims = []) {
    if (steps.length === 0)
        return { steps, origins: [], canonicalActionsSeen: 0, unsupportedProviderActionsSeen: 0 };
    const _norm = (s) => s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
    const requiredNavigation = new Set((requiredIntermediates || []).map(intermediate => _norm(intermediate)));
    const privateNavNormalized = new Set((preservedPrivateTargets || []).map(t => _norm(t)));
    ...
    for (const [origin, step] of steps.entries()) {
        ...
        if (clickTarget) {
            ...
            if (!isAllowed) {
                ...
                if (claimsForStep.some((claim) => canonicalActionClaims.has(claim.claimId)))
                    canonicalActionsSeen++;
                else
                    unsupportedProviderActionsSeen++;
 ...
 */
function getScenarioGenerationMode() {
 ...
 */
function tryDeterministicGeneration(issues, appSlug, routeResolutions, entrySteps) {
    const seeds = [];
    for (const issue of issues) {
        ...
        const stringSteps = (0, step_formatter_1.ensureStepStrings)(dedupedSteps);
        const seed = {
...
async function generateScenariosWithAi(issues, appSlug, testrailMeta, targetAppSlug, targetAppName, routeProfile, entrySteps, loginMode, _testProvider, huEvidenceMap, // HU-driven evidence per issue
pathSelectionMap, // HU-driven path selections per issue
huScopeGuard, // HU scope guard for prompt guidance (NEW)
preservedPrivateTargets, // Private navigation targets to preserve as clicks
huScenarioModel, routePendingScenarioPlan, functionalBranches, telemetryContext) {
    const primaryIssue = issues[0];
    const requirementManifest = (0, scenario_functional_quality_1.buildRequirementManifest)(primaryIssue
        ? [primaryIssue.summary, primaryIssue.description, primaryIssue.acceptanceCriteria ?? ""].filter(Boolean).join("\n")
        : "", functionalBranches ?? []);
    const canonicalClaims = (0, scenario_functional_quality_1.buildCanonicalClaims)(requirementManifest);
    ...
    for (const issue of issues) {
        ...
        const issueIntent = (0, hu_intent_classifier_1.detectHuIntent)({
        ...
        issue._huIntent = issueIntent.intent;
        issue._huIntentConfidence = issueIntent.confidence;
        ...
        if (isNonCatalogIntent && routeIsCatalogListing) {
            console.log(`[route-profile-compatibility] compatible=false issue=${issue.key} huIntent=${issueIntent.intent} routeMode=listing_validation routeProfile=${routeProfile?.name ?? "unknown"} reason=intent_mismatch`);
            blockedIssues.push({
                key: issue.key,
                title: issue.summary,
            ...
            routeResolutions.set(issue.key, {
                ...
                diagnostics: [{
                        ...
                        context: { huIntent: issueIntent.intent, routeProfileName: routeProfile?.name }
                    }],
                canGenerate: false,
        ...
        const resolution = (0, scenario_route_resolver_1.resolveScenarioRoute)(issue, routeProfile ?? null, appSlug);
        ...
        if (resolution.canGenerate) {
            ...
            if (isHuComposed) {
                const steps = (routeProfile?.entrySteps?.length ?? 0);
    ...
    const derivedContext = (0, route_profile_derived_context_1.buildDerivedExecutionContext)(appSlug, routeProfile ?? null, routeResolutions, additionalEntryTargets);
    (0, route_profile_derived_context_1.logDerivedContext)(derivedContext);
    const branchRequiredClicks = (0, effective_click_authority_1.collectBranchRequiredClicks)(functionalBranches);
    console.log(`[coverage-contract] branchRequiredClicks=${branchRequiredClicks.length} added=0 effectiveAllowedClicksBeforeRepair=${derivedContext.allowedExecutableClicks.length}`);
    // Supplement allowedExecutableClicks with preserved private targets (selected learned navigation path)
    const learnedPathNormalizedTargets = new Set();
    ...
    console.log(`[coverage-contract] effectiveAllowedClicks source=trusted_route+protected_private total=${derivedContext.allowedExecutableClicks.length} branchRequiredClicks=${branchRequiredClicks.length}`);
    const profileQuality = (0, route_profile_quality_validator_1.validateRouteProfileQuality)(appSlug, routeProfile ?? null, routeResolutions, derivedContext);
    (0, route_profile_quality_validator_1.logRouteProfileQuality)(profileQuality);
    ...
    if (routeBackedIssues.length === 0) {
        ...
        if (hasIntentMismatch && issues.length > 0) {
            ...
            try {
                const messages = await (0, mcp_scenario_prompt_builder_1.buildMcpScenarioMessages)(routePendingIssues, appSlug, testrailMeta, targetAppSlug, targetAppName, null, entrySteps, loginMode, routePendingResolutions, pendingDeterministicSeeds || undefined, huScenarioModel?.mainIntent, huEvidenceMap, pathSelectionMap, huScopeGuard, huScenarioModel, routePendingScenarioPlan, functionalBranches, requirementManifest, canonicalClaims);
                ...
                const provider = _testProvider ?? await (0, ai_provider_factory_1.createScenarioAiProvider)();
                ...
                if (!response.parsedJson) {
                    const completion = await runCoverageCompletionPass([], requirementManifest, canonicalClaims, provider, routePendingIssues[0], (scenarios) => deriveProviderStepRequirementRefs(scenarios.map((scenario) => normalizeAiScenario((0, step_formatter_1.normalizeScenarioSteps)({
                        ...scenario,
                        nonExecutableCriteria: "requires_route_discovery",
                    }), appSlug)), canonicalClaims));
                ...
                const routePendingScenarios = parsed.scenarios.map((sc) => ({
                    ...sc,
                    nonExecutableCriteria: "requires_route_discovery",
                }));
                const completion = await runCoverageCompletionPass(deriveProviderStepRequirementRefs(routePendingScenarios, canonicalClaims), requirementManifest, canonicalClaims, provider, routePendingIssues[0], (scenarios) => deriveProviderStepRequirementRefs(scenarios.map((scenario) => normalizeAiScenario((0, step_formatter_1.normalizeScenarioSteps)({
                    ...scenario,
                    nonExecutableCriteria: "requires_route_discovery",
                }), appSlug)), canonicalClaims));
                ...
                console.log(`[scenario-preview] aiGeneration routePending empty fallback=plan_based error=${err?.message ?? "unknown"}`);
                return {
                    appSlug, targetAppSlug, targetAppName, confidence: "low",
    ...
    if (shouldSkipAI) {
        ...
        const { validScenarios, invalidScenarios } = convertSeedsToValidatedScenarios(deterministicSeeds, appSlug, routeProfile || null, derivedContext, routeResolutions);
        ...
        return {
            ...
            warnings: invalidScenarios.map((inv) => `Scenario ${inv.scenario.sourceIssueKey} failed compliance: ${inv.result.reasonCode}`),
            rejected: invalidScenarios.map((inv) => ({
    ...
    const canonicalContext = primaryIssue ? (0, canonical_hu_context_1.buildCanonicalHuContext)(primaryIssue) : null;
    ...
    const messages = await (0, mcp_scenario_prompt_builder_1.buildMcpScenarioMessages)(routeBackedIssues, appSlug, testrailMeta, targetAppSlug, targetAppName, routeProfile, entrySteps, loginMode, routeResolutions, // Pass route resolutions to prompt builder
    ...
    try {
        ...
        const parsed = (0, scenario_output_parser_1.parseAiResponse)(response.rawText);
        if (!parsed) {
        ...
        const traceStages = {};
        const traceScenarios = (scenarios) => scenarios.map((scenario) => ({
            scenarioId: scenario.scenarioId,
            steps: scenario.steps,
            stepRequirementRefs: scenario.stepRequirementRefs,
            stepClaims: scenario.stepClaims,
        }));
        traceStages.rawProvider = traceScenarios(response.parsedJson?.scenarios ?? []);
        ...
        console.log(`[provider-claim-compliance] expected=${providerClaimCompliance.expectedClaims.length} ` +
        ...
        console.log(`[provider-requirement-compliance] expected=${providerCompliance.expectedCoverableRequirementIds.join(",")} ` +
        ...
        const normalizedScenarios = parsed.scenarios.map((scenario) => (0, step_formatter_1.normalizeScenarioSteps)(scenario));
        ...
        const fullyNormalizedScenarios = normalizedScenarios.map((scenario) => normalizeAiScenario(scenario, appSlug));
        ...
        const dedupedScenarios = fullyNormalizedScenarios.map((scenario) => {
            ...
            return {
                ...scenario,
                steps: dedupedSteps,
                ...(scenario.stepClaims ? { stepClaims: (0, step_authority_1.remapStepClaimsByOrigins)(scenario.stepClaims, deduped.origins) } : {}),
            };
        });
        console.log(`[scenarios:dedupe] afterDedupe=${dedupedScenarios.length}`);
        const qualityGate = applyScenarioQualityGate(dedupedScenarios, routeBackedIssues, routeProfile || null, pathSelectionMap, huEvidenceMap);
        traceStages.quality = traceScenarios(qualityGate.scenarios);
        console.log(`[scenarios:quality] normalized=${qualityGate.normalizedCount} rejected=${qualityGate.rejected.length} reasons=${qualityGate.rejectedReasons.join("|") || "none"}`);
        ...
        const repairedScenarios = enrichedScenarios.map(scenario => {
            ...
            const repaired = repairUnbackedClicksWithOrigins(scenario.steps ?? [], derivedContext.allowedExecutableClicks, requiredIntermediates, preservedPrivateTargets, canonicalClaims, scenario.stepClaims);
            ...
            console.log(`[scenarios:repair] canonicalActionsSeenByUnbackedRepair=${repaired.canonicalActionsSeen} ` +
            ...
            return {
                ...scenario,
                steps: repairedSteps,
                ...(scenario.stepClaims ? { stepClaims: (0, step_authority_1.remapStepClaimsByOrigins)(scenario.stepClaims, repaired.origins) } : {}),
            };
        });
        ...
        const complianceValidation = (0, scenario_route_compliance_validator_1.validateScenariosCompliance)(repairedScenarios, derivedContext, routeResolutions, canonicalClaims);
        // Log compliance results
        for (const invalidEntry of complianceValidation.invalidScenarios) {
            (0, scenario_route_compliance_validator_1.logComplianceResult)(appSlug, invalidEntry.scenario.sourceIssueKey, invalidEntry.scenario.title, invalidEntry.result);
        }
        (0, scenario_route_compliance_validator_1.logComplianceSummary)(appSlug, complianceValidation.validScenarios.length, complianceValidation.invalidScenarios.length);
        const completion = await runCoverageCompletionPass(complianceValidation.validScenarios, requirementManifest, canonicalClaims, provider, routeBackedIssues[0], (scenarios) => {
            const normalized = deriveProviderStepRequirementRefs(scenarios.map((scenario) => normalizeAiScenario((0, step_formatter_1.normalizeScenarioSteps)(scenario), appSlug)), canonicalClaims);
            const quality = applyScenarioQualityGate(normalized, routeBackedIssues, routeProfile || null, pathSelectionMap, huEvidenceMap);
            return (0, scenario_route_compliance_validator_1.validateScenariosCompliance)(quality.scenarios, derivedContext, routeResolutions, canonicalClaims).validScenarios;
        });
        const coverableRequirements = requirementManifest.filter(scenario_functional_quality_1.isRequirementFunctionallyCoverable);
        ...
        if (complianceValidation.validScenarios.length === 0 && hasHuEvidence && hasSelectedPath) {
            const fallbackScenarios = buildHuFallbackScenarios(routeBackedIssues, appSlug, routeProfile || null, routeResolutions, entrySteps, pathSelectionMap, functionalBranches, requirementManifest);
        ...
        if (coverableRequirements.length === 0 && complianceValidation.validScenarios.length > 0 && complianceValidation.validScenarios.length < 3 && hasHuEvidence && hasSelectedPath) {
            const fallbackScenarios = buildHuFallbackScenarios(routeBackedIssues, appSlug, routeProfile || null, routeResolutions, entrySteps, pathSelectionMap, functionalBranches, requirementManifest);
            const existingTitles = new Set(complianceValidation.validScenarios.map((scenario) => String(scenario.title).toLowerCase()));
            const fallbackToAdd = fallbackScenarios.filter((scenario) => !existingTitles.has(String(scenario.title).toLowerCase()));
        ...
        console.log(`[scenarios:ai] success ` +
        ...
        if (error instanceof ai_provider_types_1.AiProviderError) {
            if (error.code === "ai_provider_timeout") {
                ...
                const deterministicPossible = routeBackedIssues.some(issue => routeResolutions.get(issue.key)?.scenarioMode === "action_button_validation");
                if (deterministicPossible) {
                    suggestions.push("some issues support deterministic generation (action_button_validation mode)");
                }
                console.log(`[scenarios:timeout] provider=${provider.providerType} ` +
                ...
                if ((generationMode === "fallback_deterministic_on_ai_failure" || generationMode === "ai_supported_by_deterministic") &&
                    deterministicSeeds &&
                    ...
                    return {
                        ...
                        warnings: [
                            ...
                            ...blockedIssues.map(b => `Blocked ${b.key}: ${b.reason}`),
                            ...invalidScenarios.map((inv) => `Scenario ${inv.scenario.sourceIssueKey} failed compliance: ${inv.result.reasonCode}`)
                        ],
                        rejected: [
                            ...blockedIssues.map(b => ({ sourceIssueKey: b.key, reason: b.reason })),
                            ...invalidScenarios.map((inv) => ({
                                sourceIssueKey: inv.scenario.sourceIssueKey,
                                reason: `compliance_failed: ${inv.result.reasonCode}`
                            }))
                        ],
                ...
                throw new Error(`AI_GENERATION_TIMEOUT|AI provider (${provider.providerType}) timed out after ${error.diagnostics?.timeoutMs || "unknown"}ms. ` +
            ...
            if ((generationMode === "fallback_deterministic_on_ai_failure" || generationMode === "ai_supported_by_deterministic") &&
                ...
                let fallbackReason = "ai_generation_error";
 ...
 */
function resolutionIsCatalogListing(routeProfile) {
    if (!routeProfile)
        return false;
    const rp = routeProfile;
    ...
    if (entryLabels.some(l => catalogEntryPatterns.some(p => p.test(l))))
        score += 1;
    ...
    if (controls.length >= 5) {
        ...
        if (catalogControls.length / Math.max(controls.length, 1) >= 0.3)
            score += 1;
    }
    return score >= 2;
}
 ...
 */
function parseAiResponseWithMode(parsedJson, mode) {
    const raw = Array.isArray(parsedJson["scenarios"]) ? parsedJson["scenarios"] : [];
    const scenarios = raw.filter((s) => s && typeof s === "object" && s.title && Array.isArray(s.steps) && s.steps.length > 0);
    return { scenarios };
}

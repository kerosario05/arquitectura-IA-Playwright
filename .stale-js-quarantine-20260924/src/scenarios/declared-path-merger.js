...
exports.extractClickTarget = extractClickTarget;
exports.mergeDeclaredPathsIntoScenario = mergeDeclaredPathsIntoScenario;
function _norm(s) {
    return s
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
...
function mergeDeclaredPathsIntoScenario(steps, paths) {
    const result = {
        steps: [...steps],
        insertedCount: 0,
        existingCount: 0,
    ...
    for (const path of paths) {
        ...
        const terminalIndex = result.steps.findIndex((step) => clickTargetOf(step) !== null && _norm(clickTargetOf(step)) === terminalNorm);
        if (terminalIndex < 0)
            continue;

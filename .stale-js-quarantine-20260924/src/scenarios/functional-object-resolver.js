...
exports.TITLE_ACTION_VERBS = exports.TRAILING_FILLER_TERMS = exports.OPERATIONAL_OBJECT_TERMS = exports.INTERNAL_OBJECT_TERMS = exports.BDD_FRAMING_TERMS = void 0;
exports.normalizeObjText = normalizeObjText;
exports.classifySemanticObject = classifySemanticObject;
exports.stripBddFraming = stripBddFraming;
exports.extractRequirementNounPhrase = extractRequirementNounPhrase;
exports.mapBusinessEntityLabel = mapBusinessEntityLabel;
exports.inferFunctionalAction = inferFunctionalAction;
exports.resolveFunctionalObject = resolveFunctionalObject;
exports.resolveBackedObjectForTitle = resolveBackedObjectForTitle;
exports.extractTitleObjectSegment = extractTitleObjectSegment;
exports.validateSemanticScenarioTitle = validateSemanticScenarioTitle;
exports.repairSemanticScenarioTitle = repairSemanticScenarioTitle;
/** BDD framing + desire/role clause words. Never business objects. */
exports.BDD_FRAMING_TERMS = new Set([
 ...
 */
exports.INTERNAL_OBJECT_TERMS = new Set([
...
exports.OPERATIONAL_OBJECT_TERMS = new Set([
...
exports.TRAILING_FILLER_TERMS = new Set([
...
exports.TITLE_ACTION_VERBS = new Set([
...
function isLikelySpanishVerb(token) {
    const t = token.toLowerCase();
    if (/[aeiouáéíóú](?:ar|er|ir|arse|erse|irse)$/i.test(t))
        return true;
 ...
 */
function classifySemanticObject(term) {
    const raw = (term ?? "").trim();
    if (!raw)
        return { valid: false, kind: "empty", reason: "empty_object" };
    const normalized = normalizeObjText(raw);
    const tokens = normalized.split(/\s+/).filter(Boolean);
    if (tokens.length === 0)
        return { valid: false, kind: "empty", reason: "empty_object" };
    const meaningful = tokens.filter((t) => t.length >= 3
        && !exports.INTERNAL_OBJECT_TERMS.has(t)
        && !exports.OPERATIONAL_OBJECT_TERMS.has(t)
        && !exports.BDD_FRAMING_TERMS.has(t)
        && !exports.TRAILING_FILLER_TERMS.has(t)
        && !isLikelySpanishVerb(t));
    if (meaningful.length === 0) {
        const hasInternal = tokens.some((t) => exports.INTERNAL_OBJECT_TERMS.has(t));
        return {
            valid: false,
    ...
    if (exports.TRAILING_FILLER_TERMS.has(last) || exports.BDD_FRAMING_TERMS.has(last)) {
 ...
 */
function extractRequirementNounPhrase(text) {
    ...
    while (end > 0) {
        ...
        if (exports.INTERNAL_OBJECT_TERMS.has(l) || exports.OPERATIONAL_OBJECT_TERMS.has(l)
            || exports.BDD_FRAMING_TERMS.has(l) || exports.TRAILING_FILLER_TERMS.has(l)
    ...
    while (start > 0 && count < 3) {
        ...
        if (exports.INTERNAL_OBJECT_TERMS.has(l) || exports.OPERATIONAL_OBJECT_TERMS.has(l)
            || exports.BDD_FRAMING_TERMS.has(l) || isLikelySpanishVerb(l) || l.length < 3) {
            break;
        }
        start--;
        if (!exports.TRAILING_FILLER_TERMS.has(l))
            count++;
    }
    ...
    while (phraseStart < end) {
        ...
        if (exports.TRAILING_FILLER_TERMS.has(l) || exports.BDD_FRAMING_TERMS.has(l) || l.length < 3)
            phraseStart++;
        else
            break;
    }
...
function mapBusinessEntityLabel(entity) {
    const map = {
...
function inferFunctionalAction(text) {
    const t = text.toLowerCase();
    if (/navegar|ir\s+a|acceder\s+a/.test(t))
        return "navigate";
    if (/seleccionar|elegir|escoger/.test(t))
        return "select";
    if (/ingresar|completar|llenar|digitar/.test(t))
        return "fill";
    if (/buscar|consultar|filtrar/.test(t))
        return "search";
    if (/enviar|procesar|ejecutar/.test(t))
        return "submit";
    if (/confirmar|aceptar/.test(t))
        return "confirm";
    if (/cancelar|rechazar/.test(t))
        return "cancel";
    if (/volver|regresar|retornar/.test(t))
        return "return";
    if (/descargar/.test(t))
        return "download";
    if (/exportar/.test(t))
    ...
    if (/generar/.test(t))
        return "generate";
    if (/validar|verificar|mostrar|visualizar/.test(t))
        return "validate";
    return "unknown";
}
/**
 ...
 */
function resolveFunctionalObject(requirement, context = {}) {
    ...
    for (const f of (hu.requiredFields ?? [])) {
        if (typeof f === "string" && f.trim() && classifySemanticObject(f).valid)
            candidates.push({ value: f.trim(), source: "field" });
    }
    for (const b of (hu.visibleButtons ?? [])) {
        if (typeof b === "string" && b.trim() && classifySemanticObject(b).valid)
            candidates.push({ value: b.trim(), source: "button" });
    }
    for (const e of (hu.selectableEntities ?? [])) {
        const label = mapBusinessEntityLabel(String(e));
        if (label && classifySemanticObject(label).valid)
            candidates.push({ value: label, source: "selectable" });
    }
    for (const w of (hu.visibleWarnings ?? [])) {
        if (typeof w === "string" && w.trim() && classifySemanticObject(w).valid)
            candidates.push({ value: w.trim(), source: "warning" });
    }
    for (const s of (hu.requiredScreens ?? [])) {
        if (typeof s === "string" && s.trim() && classifySemanticObject(s).valid)
            candidates.push({ value: s.trim(), source: "screen" });
    }
...
function resolveBackedObjectForTitle(context) {
    if (!context)
        return null;
    const hu = context.huModel ?? {};
    const candidates = [];
    if (context.featureName && classifySemanticObject(context.featureName).valid)
        candidates.push(context.featureName);
    for (const f of (hu.requiredFields ?? [])) {
        if (typeof f === "string" && classifySemanticObject(f).valid)
            candidates.push(f);
    }
    for (const e of (hu.selectableEntities ?? [])) {
        const label = mapBusinessEntityLabel(String(e));
        if (classifySemanticObject(label).valid)
            candidates.push(label);
    }
    for (const b of (hu.visibleButtons ?? [])) {
        if (typeof b === "string" && classifySemanticObject(b).valid)
            candidates.push(b);
    }
    for (const w of (hu.visibleWarnings ?? [])) {
        if (typeof w === "string" && classifySemanticObject(w).valid)
            candidates.push(w);
    }
    if (context.entityTerm && classifySemanticObject(context.entityTerm).valid)
        candidates.push(context.entityTerm);
...
function sourceOfTitleObject(object, context) {
    if (!context)
        return "unknown";
    const oNorm = normalizeObjText(object);
    const hu = context.huModel ?? {};
    if (context.featureName && normalizeObjText(context.featureName) === oNorm)
        return "featureName";
    if (context.entityTerm && normalizeObjText(context.entityTerm) === oNorm)
        return "entityTerm";
    if ((hu.requiredFields ?? []).some((f) => normalizeObjText(f) === oNorm))
        return "huModel.field";
    if ((hu.visibleButtons ?? []).some((b) => normalizeObjText(b) === oNorm))
        return "huModel.button";
    if ((hu.visibleWarnings ?? []).some((w) => normalizeObjText(w) === oNorm))
        return "huModel.warning";
    if ((hu.selectableEntities ?? []).some((e) => normalizeObjText(mapBusinessEntityLabel(String(e))) === oNorm))
        return "huModel.selectable";
...
function extractTitleObjectSegment(title) {
    const t = title.trim();
    const specific = [
    ...
    const actionMatch = t.match(/^(ejecutar|completar|cancelar|validar|seleccionar|confirmar|buscar|generar|revisar|volver|descargar|enviar|ingresar|registrar|crear|consultar|visualizar|navegar|verificar|esperar|solicitar)\s+(.+)$/i);
    if (actionMatch) {
...
function validateSemanticScenarioTitle(title, context) {
    const t = title.trim();
    if (!t)
        return { valid: false, reason: "empty_title", object: null, objectSource: null, template: null };
    const firstWord = normalizeObjText(t.split(/\s+/)[0] ?? "");
    if (!exports.TITLE_ACTION_VERBS.has(firstWord)) {
...
function repairSemanticScenarioTitle(title, context) {
    const initial = validateSemanticScenarioTitle(title, context);
    if (initial.valid)
        return { title, repaired: false, valid: true, object: initial.object, objectSource: initial.objectSource };

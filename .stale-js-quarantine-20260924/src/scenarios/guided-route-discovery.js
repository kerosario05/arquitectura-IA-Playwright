...
exports.runGuidedRouteDiscovery = runGuidedRouteDiscovery;
...
function getBaseUrlFromConfig(appConfig) {
    return appConfig?.baseUrl || appConfig?.appProfile?.baseUrl || null;
}
function getRouteProfileFromConfig(appConfig) {
    if (!appConfig)
        return null;
    return appConfig?.routeProfile ?? null;
}
function extractEntrySteps(configRp) {
    if (!configRp)
        return [];
    const steps = [];
    ...
    if (entry) {
        for (const e of entry) {
            if (e.visibleLabel)
                steps.push(e.visibleLabel);
        }
    }
    return steps;
}
function extractVisibleControls(appConfig) {
    if (!appConfig)
        return [];
    const controls = appConfig.visibleControls;
    if (controls && controls.length > 0)
        return controls;
    const profileControls = appConfig.appProfile?.visibleControls;
    return profileControls ?? [];
}
function buildCandidateFromConfig(appSlug, entrySteps, visibleControls, huIntent) {
    return {
...
function loadKnowledgeItems(appSlug) {
    try {
...
function filterEligibleKnowledge(items) {
    return items.filter((item) => item.trustedForReuse === true &&
...
function scoreKnowledgeItems(items) {
    return items.map((item) => {
        let score = 0;
        const refs = (item.coverageRefs ?? []).join(" ");
        if (item.trustedForReuse)
            score += 50;
        ...
        if (Array.isArray(item.authTerms) && item.authTerms.length > 0)
            score += 15;
        if ((item.successCount ?? 0) > 0)
            score += 10;
        if (item.confidenceScore && item.confidenceScore >= 60)
            score += 10;
        const allClicks = (item.clickTargets ?? []).join(" ").toLowerCase();
        if (/informaci\u00f3n de productos|catalogo de productos|listado de productos/.test(allClicks))
            score -= 25;
        return { item, score };
    }).sort((a, b) => b.score - a.score);
}
function buildKnowledgeCandidate(best, huIntent) {
    return {
...
async function runGuidedRouteDiscoveryInternal(request) {
    const { appSlug, issueKey, huIntent, reasonCode } = request;
    const dryRun = request.dryRun !== false;
    const mode = request.mode;
    console.log(`[guided-route-discovery] requested appSlug=${appSlug} issue=${issueKey} huIntent=${huIntent} reason=${reasonCode} dryRun=${dryRun} mode=${mode ?? "standard"}`);
    // ── Validate payload ──
    const missing = [];
    if (!appSlug?.trim())
        missing.push("appSlug");
    if (!issueKey?.trim())
        missing.push("issueKey");
    if (!huIntent?.trim())
        missing.push("huIntent");
    if (!reasonCode?.trim())
    ...
    if (!dryRun && mode === "candidate_only") {
        ...
        try {
            ...
            try {
                ...
                const { scanCurrentPage } = await Promise.resolve().then(() => __importStar(require("../explorer/page-scanner")));
                ...
                const buttons = snapshotElements
                    .filter((e) => (e.tagName === "button" || e.role === "button") && e.text && e.visible !== false)
                    .map((e) => e.text)
                    .filter(Boolean);
                const links = snapshotElements
                    .filter((e) => (e.tagName === "a" || e.role === "link") && e.text && e.visible !== false)
                    .map((e) => e.text)
                    .filter(Boolean);
                ...
                const candidateClicks = [...buttons, ...links].filter((t) => !sensitiveActions.some((s) => t.toLowerCase().includes(s)));
                const scoredCandidates = candidateClicks.map((t) => {
...
async function runGuidedRouteDiscovery(request) {

...
exports.enrichWithPassiveHUEvidence = enrichWithPassiveHUEvidence;
exports.formatHUEvidenceForPrompt = formatHUEvidenceForPrompt;
function normalizeText(text) {
    const accents = { á: "a", é: "e", í: "i", ó: "o", ú: "u" };
    return (text || "").toLowerCase().replace(/[áéíóú]/g, (m) => accents[m] || m);
}
function detectBusinessIntent(summary, description) {
    const combined = normalizeText(`${summary} ${description}`);
    if (/consulta.*balance|balance|saldo/.test(combined))
        return "balance_inquiry";
    if (/estado.*cuenta|extracto/.test(combined))
        return "account_statement";
    if (/pago|transferencia/.test(combined))
        return "product_payment";
    if (/deposito.*plazo|certificado/.test(combined))
        return "term_deposit";
    if (/tarjeta.*credito|credito/.test(combined))
        return "credit_card";
    if (/cuenta.*efectivo|cuenta.*corriente|ahorro/.test(combined))
        return "cash_account";
    if (/prestamo|credito/.test(combined))
        return "loan";
    return "generic";
}
function detectProductCategory(summary, description) {
    const combined = normalizeText(`${summary} ${description}`);
    if (/deposito.*plazo|certificado|plazo/.test(combined))
        return "term_deposit";
    if (/tarjeta.*credito/.test(combined))
        return "credit_card";
    if (/cuenta.*efectivo|cuenta.*corriente|ahorro/.test(combined))
        return "cash_account";
    if (/prestamo/.test(combined))
        return "loan";
    return "generic";
}
function detectAccessMode(summary, description) {
    const combined = normalizeText(`${summary} ${description}`);
    ...
    if (publicIndicators.test(combined))
        return "public";
    if (privateIndicators.test(combined))
        return "private";
    return "private"; // default
}
function buildAllowedTerms(productCategory) {
    const terms = {
        term_deposit: [
            "Consulta de balance",
            "Depósitos a Plazo",
...
function buildBlockedTerms(productCategory) {
    const blocked = {
        term_deposit: [
            "Tarjeta de Crédito",
            "Tarjetas",
...
function enrichWithPassiveHUEvidence(input) {
    ...
    const labelHints = productCategory === "term_deposit"
    ...
    console.log(`[hu-evidence] issue=${input.jiraKey} intent=${businessIntent} product=${productCategory} ` +
...
function formatHUEvidenceForPrompt(evidence, issueKey) {

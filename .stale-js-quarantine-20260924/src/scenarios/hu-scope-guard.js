...
exports.evaluateHuScopeGuard = evaluateHuScopeGuard;
exports.detectOptionFlows = detectOptionFlows;
exports.buildCoverageContract = buildCoverageContract;
/**
 ...
 */
function evaluateHuScopeGuard(issue, pathSelection, appSlug) {
    if (!issue) {
    ...
    console.log(`[hu-scope-boundary] issue=${issue.key} scope=${guard.functionalScope} ` +
    ...
    console.log(`[hu-scope-boundary] allowedTerms=${guard.allowedTermsFromHu.length} ` +
 ...
 */
function buildSearchCorpus(issue) {
    const parts = [];
    if (issue.summary)
    ...
    if (issue.acceptanceCriteria)
        parts.push(issue.acceptanceCriteria);
    if (issue.labels)
        parts.push(issue.labels.join(" "));
    if (issue.components)
        parts.push(issue.components.join(" "));
    return parts
        .join(" ")
        .toLowerCase()
        .normalize("NFD")
 ...
 */
function extractAllowedTermsFromHu(issue) {
    const terms = new Set();
    const text = [
        issue.summary,
        issue.description,
    ...
    const filtered = Array.from(terms).filter(t => t.length > 1 && !/^\d+$/.test(t) && t !== " ");
    return filtered;
}
/**
 * Detect functional scope from corpus
 */
function detectFunctionalScope(corpus, issue) {
    const matched = [];
    const keywords = [];
 ...
 */
function loadAppExpansionCandidatesSync(appSlug) {
    if (!appSlug) {
    ...
    try {
        ...
        if (catalog?.products && catalog.products.length > 0) {
            const productNames = catalog.products
                .flatMap((p) => [p.name, p.category, ...(p.fields || [])])
                .filter(Boolean);
            return { terms: Array.from(new Set(productNames.map((t) => String(t)))), source: "catalog" };
        }
        ...
        if (routeProfile?.targetPaths && routeProfile.targetPaths.length > 0) {
            const pathNames = routeProfile.targetPaths.flatMap((p) => [p.label, p.category]);
            return { terms: Array.from(new Set(pathNames.filter(Boolean).map((t) => String(t)))), source: "routeProfile" };
        }
    }
    catch {
 ...
 */
function applyGuardRules(detection, allowedTerms, appSlug) {
    ...
    if (appContext.source !== "none") {
        console.log(`[hu-scope-boundary] blockedExpansionSource=${appContext.source} blockedExpansion=${blockedExpansion.length}`);
    }
    // Gate catalog expansion: only if HU explicitly mentions terms from app context
    const mentionedAppTerms = appContext.terms.filter(term => allowedTermsLower.some(aTerm => aTerm.includes(term.toLowerCase())));
 ...
 */
function detectOptionFlows(issue) {
    if (!issue) {
    ...
    while ((match = optionColonPattern.exec(corpus)) !== null) {
        let optionLabel = match[1]?.trim().replace(/[,.:;]/g, "").replace(/^\s+|\s+$/g, "");
        const resultRaw = match[2]?.trim().replace(/[,.:;]/g, "").replace(/^\s+|\s+$/g, "");
        if (!optionLabel || optionLabel.length < 2)
            continue;
    ...
    if (flows.length > 0) {
        ...
        for (const flow of flows) {
            console.log(`[hu-option-flows] optionLabel="${flow.optionLabel}" expectedResult="${flow.expectedResult.substring(0, 60)}" requiresAuth=${flow.requiresAuth}`);
        }
    }
    return {
        flows,
 ...
 */
function buildCoverageContract(issue, optionFlows) {
    ...
    const funcNavigationItems = [];
    const funcListScreens = [];
    const funcSelectionFlows = [];
    const funcSingleItemShortcuts = [];
    const funcDetailFields = [];
    const funcFormatRules = [];
    const funcActionOptions = [];
    const funcConditionalAlerts = [];
    const funcFailureCases = [];
    ...
    if (funcListScreens.length > 0)
        console.log(`[coverage-contract] extractedListScreens count=${funcListScreens.length}`);
    ...
    if (funcSelectionFlows.length > 0)
        console.log(`[coverage-contract] extractedSelectionFlows count=${funcSelectionFlows.length}`);
    ...
    if (funcSingleItemShortcuts.length > 0)
        console.log(`[coverage-contract] extractedSingleItemShortcuts count=${funcSingleItemShortcuts.length}`);
    ...
    if (funcDetailFields.length > 0)
        console.log(`[coverage-contract] extractedDetailFields count=${extractedFieldNames.length}`);
    ...
    const formatTypes = [];
    if (/moneda|signo\s+monetario|s[íi]mbolo\s+(de\s+)?moneda/i.test(lower))
        formatTypes.push("Moneda");
    if (/porcentaje/i.test(lower))
        formatTypes.push("Porcentaje");
    if (/fecha/i.test(lower))
        formatTypes.push("Fecha");
    if (/hora/i.test(lower))
        formatTypes.push("Hora");
    if (/enmascaramiento|m[áa]scara|caracteres\s+enmascarados/i.test(lower))
        formatTypes.push("Enmascaramiento");
    if (/decimales|miles/i.test(lower))
        formatTypes.push("Decimales");
    let formatLabel = "Validate format rules";
    if (formatTypes.length > 0)
        formatLabel += ` (${formatTypes.join(", ")})`;
    ...
    if (funcFormatRules.length > 0)
        console.log(`[coverage-contract] extractedFormatRules count=${formatTypes.length}`);
    ...
    if (actionOpts.length > 0)
        actionOptionsLabel += ` (${actionOpts.join(", ")})`;
    ...
    if (funcActionOptions.length > 0)
        console.log(`[coverage-contract] extractedActionOptions count=${actionOpts.length}`);
    ...
    if (funcFailureCases.length > 0)
        console.log(`[coverage-contract] extractedFailureCases count=${funcFailureCases.length}`);
    // --- T3: selectable_options (periods, ranges, numeric options) ---
    const funcSelectableOptions = [];
    ...
    if (funcSelectableOptions.length > 0)
        console.log(`[coverage-contract] extractedSelectableOptions count=${funcSelectableOptions.length}`);
    // --- T4: visible_message (quoted or described in HU) ---
    const funcVisibleMessages = [];
    ...
    if (funcVisibleMessages.length > 0)
        console.log(`[coverage-contract] extractedVisibleMessages count=${funcVisibleMessages.length}`);
    // --- T5: confirmation_flow (confirm, send, finalize) ---
    const funcConfirmationFlows = [];
    ...
    if (funcConfirmationFlows.length > 0)
        console.log(`[coverage-contract] extractedConfirmationFlows count=${funcConfirmationFlows.length}`);
    // --- T7: delivery_option (email, delivery method) ---
    const funcDeliveryOptions = [];
    ...
    if (funcDeliveryOptions.length > 0)
        console.log(`[coverage-contract] extractedDeliveryOptions count=${funcDeliveryOptions.length}`);
    // --- T8: reference_number (unique visible identifier) ---
    const funcReferenceNumbers = [];
    const refPatterns = [
    ...
    if (funcReferenceNumbers.length > 0)
        console.log(`[coverage-contract] extractedReferenceNumbers count=${funcReferenceNumbers.length}`);
    // --- Detect authenticatedPrecondition ---
    const authenticatedPrecondition = /\b(sesi[oó]n\s+(v[aá]lida|iniciada|activa|existente)|ya\s+(est[aá]|se\s+encuentra)\s+(autenticado|logueado)|autenticaci[oó]n\s+(completada|exitosa|ya\s+realizada)|precondici[oó]n\s+autenticada)\b/i.test(lower);
    ...
    if (funcListScreens.length > 0 && funcSelectionFlows.length > 0) {
        ...
        for (const { arr, reason } of postSelectionTypes) {
            for (const item of arr) {
                item.requiresSelection = true;
                item.requiresSelectionRef = selectionRefId;
                item.selectionTarget = inferredTarget;

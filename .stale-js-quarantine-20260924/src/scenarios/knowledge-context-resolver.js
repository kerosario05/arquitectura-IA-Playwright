...
exports.loadOrCreateKnowledgeContext = loadOrCreateKnowledgeContext;
exports.buildKnowledgeContextForScenarioGeneration = buildKnowledgeContextForScenarioGeneration;
const fs = __importStar(require("node:fs"));
const path = __importStar(require("node:path"));
function loadKnowledgeRaw(appSlug) {
    try {
...
function loadOrCreateKnowledgeContext(appSlug) {
 ...
 */
function scoreKnowledgeItem(item, huNorm, huIntent, huSubIntent, huConcepts, huRouteTokens) {
    let score = 0;
    const reasons = [];
    // ── Base trust signals ──
    if (item.trustedForReuse === true) {
        score += 30;
    ...
    if (successes > 0)
        score += Math.min(successes * 2, 10);
    // ── Knowledge kind priority ──
    const kind = item.knowledgeKind ?? "";
    if (kind === "route_menu_snapshot" && item.validationStatus === "validated") {
        score += 40;
        reasons.push("kind=route_menu_snapshot_validated");
    }
    else if (kind === "route_functional_observed" && item.validationStatus === "validated") {
        score += 35;
        reasons.push("kind=route_functional_observed_validated");
    }
    else if (kind === "route_functional") {
        score += 25;
        reasons.push("kind=route_functional");
    }
    else if (kind === "scenario_validated") {
        score += 15;
        reasons.push("kind=scenario_validated");
    }
    else if (kind === "route_prefix") {
        score += 10;
        reasons.push("kind=route_prefix");
    }
    else if (kind === "scenario_candidate") {
        score += 5;
        reasons.push("kind=scenario_candidate");
    }
    else if (kind === "route_menu_snapshot") {
        score += 8;
        reasons.push("kind=route_menu_snapshot");
    }
    else if (kind === "route_functional_observed") {
        score += 8;
        reasons.push("kind=route_functional_observed");
    }
    else if (kind === "runtime_click_target") {
        score += 6;
        reasons.push("kind=runtime_click_target");
    }
    else if (kind === "runtime_screen_snapshot") {
        score += 6;
        reasons.push("kind=runtime_screen_snapshot");
    }
    // ── Extract item concepts and combined text ──
    const itemConcepts = huConcepts ? extractItemConcepts(item) : new Set();
    ...
    if (huRouteTokens && huRouteTokens.length > 0) {
        for (const rt of huRouteTokens) {
            if (combinedText.includes(rt))
                routeOverlapCount++;
        }
    }
    if (routeOverlapCount >= 2) {
        score += 50;
        reasons.push(`route_overlap=${routeOverlapCount}`);
    }
    else if (routeOverlapCount >= 1) {
        score += 35;
        reasons.push(`route_segment_match=${routeOverlapCount}`);
    }
    ...
    if (huConcepts && huConcepts.size > 0 && itemConcepts.size > 0) {
        ...
        if (overlapCount >= 8) {
            score += 35;
            reasons.push(`overlap_strong=${overlapCount}`);
        }
        else if (overlapCount >= 4) {
            score += 20;
            reasons.push(`overlap_good=${overlapCount}`);
        }
        else if (overlapCount >= 2) {
            score += 10;
            reasons.push(`overlap_ok=${overlapCount}`);
        }
        ...
        if (routeOverlapCount === 0) {
            ...
            if (foreignRatio > 0.7) {
                score -= 50;
                reasons.push(`foreign_dominant=${foreignRatio.toFixed(2)}`);
                ...
                score -= 20;
                reasons.push(`foreign_significant=${foreignRatio.toFixed(2)}`);
            }
        }
        else {
            ...
            if (foreignRatio > 0.85 && overlapCount < 3) {
                score -= 30;
                reasons.push(`foreign_despite_route=${foreignRatio.toFixed(2)}`);
    ...
    if (combinedText) {
        const huTokens = huNorm.split(/\s+/).filter((t) => t.length > 3);
        const matchCount = huTokens.filter((t) => combinedText.includes(t)).length;
        if (matchCount > 0) {
            const sim = matchCount / Math.max(huTokens.length, 1);
            score += Math.round(sim * 25);
            if (sim > 0.3)
                reasons.push(`text_sim=${sim.toFixed(2)}`);
        }
    }
    // ── Coverage refs / routeSignature alignment ──
    const refs = (item.coverageRefs ?? []).join(" ").toLowerCase();
    if (/accessLevel:private|accessLevel:authenticated/.test(refs)) {
        score += 15;
        reasons.push("access_private");
    }
    if (/startsFrom:public_initial|startsFrom:authenticated_state/.test(refs)) {
        score += 5;
        reasons.push("starts_from_known");
    }
    if (/endsAt:auth_gate/.test(refs)) {
        score += 5;
        reasons.push("ends_at_auth");
    }
    if (Array.isArray(item.authTerms) && item.authTerms.length > 0) {
        score += 10;
        reasons.push("has_auth_terms");
    }
    // ── Intent alignment (generic) ──
    const intentTermCount = huIntent.split(/[_\-\s]+/).filter((t) => t.length > 2 && combinedText.includes(normalize(t))).length;
    ...
    if (huSubIntent && huSubIntent !== "standard" && routeOverlapCount === 0) {
        if (itemConcepts.size > 0 && overlapCount < 3) {
            score -= 60;
            reasons.push("subintent_low_overlap");
...
function buildKnowledgeContextForScenarioGeneration(appSlug, huText, huIntent, huSubIntent, huExplicitRoutePath) {
    const raw = loadKnowledgeRaw(appSlug);
    const allItems = raw && Array.isArray(raw.items) ? raw.items : [];
    ...
    const selectedKinds = selected.map((s) => s.item.knowledgeKind).join(",");
    ...
    const navigationHints = selected
        .filter((s) => s.item.knowledgeKind === "route_prefix" ||
        ...
        const steps = Array.isArray(item.steps) && item.steps.length > 0
            ? item.steps
            : item.clickTargets.map((t) => `Clic en "${t}".`);
    ...
    const functionalHints = selected
        .filter((s) => s.item.knowledgeKind === "scenario_validated" ||
    ...
    const termsSeenBefore = Array.from(new Set(selected.flatMap((s) => s.item.clickTargets ?? []))).slice(0, 20);

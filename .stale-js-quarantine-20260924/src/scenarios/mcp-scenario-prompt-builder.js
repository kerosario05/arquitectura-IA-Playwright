...
exports.calculateDynamicScenarioLimit = calculateDynamicScenarioLimit;
exports.buildMcpScenarioMessages = buildMcpScenarioMessages;
 ...
 */
function getMaxScenariosPerIssue() {
 ...
 */
function calculateDynamicScenarioLimit(issues, routeProfile, alignedCategories) {
    ...
    const mentionsProducts = huCorpus.includes("producto") ||
    ...
    const isBroadScope = huCorpus.includes("todos") ||
    ...
    if (mentionsProducts && hasBroadSignal) {
        ...
        console.log(`[scenarios:limit] broad product HU detected: ` +
    ...
    if (mentionsFailures && failureScenarioCount > 0) {
        ...
        limit += failureQuota;
        console.log(`[scenarios:limit] failure scenarios detected: count=${failureScenarioCount} addedQuota=${failureQuota} newLimit=${limit}`);
    }
 ...
 */
function buildCompactMcpRules(scenarioLimit) {
 ...
 */
function buildRouteResolutionContext(issues, routeResolutions) {
 ...
 */
function buildDiscoveredProductsBlock(profileCtx) {
    if (!profileCtx || !profileCtx.present) {
        return "";
    }
    // Extract discovered products from targetPaths
    const targetPaths = profileCtx.targetPaths;
...
function buildFunctionalBranchBlock(functionalBranches) {
...
function buildBranchRequiredClicksBlock(functionalBranches, suppressRouteProfile) {
    if (!functionalBranches || functionalBranches.length === 0)
        return "";
    const requiredClicks = Array.from(new Set(functionalBranches
        .map((branch) => branch.sourceLabel?.trim())
        .filter((label) => Boolean(label))));
    if (requiredClicks.length === 0)
        return "";
    const lines = [];
...
function buildSystemPrompt(skillMd, appSlug, testrailMeta, targetAppSlug, targetAppName, profileCtx, derivedCtx, useCompactMode, scenarioLimit, suppressRouteProfile, functionalBranches) {
    ...
    const routeProfileBlock = profileCtx?.present && !suppressRouteProfile
        ? `\n\n## App Configuration / RouteProfile\n${(0, scenario_prompt_context_1.formatAppProfileContext)(profileCtx)}`
        : "";
    const derivedContextBlock = derivedCtx && !suppressRouteProfile
        ? `\n\n${(0, route_profile_derived_context_1.formatDerivedContextForPrompt)(derivedCtx)}`
        : "";
    ...
    const entryPathBlock = (0, scenario_prompt_context_1.buildEntryPathBlockFromContext)(profileCtx ?? { appSlug, entrySteps: [], navigationHints: {}, aliases: {}, domainTerms: {}, visibleControls: [], present: false });
...
async function buildMcpScenarioMessages(issues, appSlug, testrailMeta, targetAppSlug, targetAppName, routeProfile, entrySteps, loginMode, routeResolutions, deterministicSeeds, effectiveIntent, _huEvidenceMap, _pathSelectionMap, _huScopeGuard, _huScenarioModel, _routePendingScenarioPlan, functionalBranches, requirementAccounting, canonicalClaims) {
    const skillMd = await loadSkillMarkdown();
    // Detect intent from explicit effectiveIntent first, fallback to annotated issues
    const classifierIntent = issues[0]?._huIntent ?? "unknown_flow";
    ...
    if (routeProfile) {
        ...
        const hasProductMetadata = targetPathKeys.some((k) => !!rp.targetPaths?.[k]?.productMetadata?.subcategory);
        const entryLabels = (rp.entry ?? []).map((e) => (e.businessLabel ?? e.visibleLabel ?? "").toLowerCase()).join(" ");
        const hasCatalogEntry = /informacion_de_productos|productos|catalogo/i.test(entryLabels);
        const controls = (rp.visibleControls ?? []);
        const catalogControls = controls.filter((c) => /beneficios|requisitos|condiciones relevantes|descripci[oó]n general|informaci[oó]n legal|nombre del producto|solicitar/i.test(c));
    ...
    const profileCtx = (0, scenario_prompt_context_1.buildAppProfilePromptContext)(appSlug, {
        targetAppSlug,
        targetAppName,
        routeProfile,
        entrySteps,
    ...
    (0, scenario_prompt_context_1.logAppProfileContext)(profileCtx);
    ...
    const derivedCtx = routeProfile
        ? (0, route_profile_derived_context_1.buildDerivedExecutionContext)(appSlug, routeProfile, routeResolutions ?? new Map(), additionalEntryTargets)
        : undefined;
    if (derivedCtx) {
        (0, route_profile_derived_context_1.logDerivedContext)(derivedCtx);
    }
    ...
    if (issues.length > 0 && routeProfile?.targetPaths) {
        ...
        const { alignedTargetPaths, diagnostics } = filterTargetPathsByIssueScope(issueContext, routeProfile.targetPaths, routeProfile ?? null);
        ...
        console.log(`[scenarios:limit] HU scope analysis: totalProducts=${diagnostics.totalProducts} ` +
    ...
    let systemContent = buildSystemPrompt(skillMd, appSlug, testrailMeta, targetAppSlug, targetAppName, profileCtx, derivedCtx, useCompactMode, dynamicScenarioLimit, suppressRouteProfile, functionalBranches);
    ...
    const safeUserContent = (0, scenario_prompt_context_1.sanitizeForPrompt)(formatIssues(issues));
    ...
    const rawBranchRequiredClicksCount = new Set((functionalBranches ?? [])
        .map((branch) => branch.sourceLabel?.trim())
        .filter((label) => Boolean(label))).size;

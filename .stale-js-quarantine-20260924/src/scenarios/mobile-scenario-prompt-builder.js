...
exports.buildMobileScenarioMessages = buildMobileScenarioMessages;
...
function formatFlows(routeProfile) {
    if (!routeProfile.flows || Object.keys(routeProfile.flows).length === 0)
        return "";
    const lines = [
    ...
    for (const [flowId, flow] of Object.entries(routeProfile.flows)) {
        ...
        if (flow.entryFromScreen)
            lines.push(`- Pantalla destino: ${flow.entryFromScreen}`);
...
function formatRouteProfile(routeProfile) {
    const lines = [
    ...
    for (const screen of Object.values(routeProfile.screens)) {
        ...
        if (screen.flowNotes && screen.flowNotes.length > 0) {
            ...
            for (const note of screen.flowNotes)
                lines.push(`  - ${note}`);
        }
        lines.push("");
    }
 ...
 */
function formatFunctionalDataProfiles(routeProfile) {
 ...
 */
function formatLearnedKnowledge(learnedScreens) {
    ...
    for (const screen of learnedScreens) {
        ...
        if (screen.disabledTargets.length > 0) {
            ...
            lines.push(`- Controles DESHABILITADOS en este estado: ${screen.disabledTargets.map((t) => `"${t}"`).join(", ")}. ` +
...
function formatDestinationClaimManifest(manifest) {
...
function buildMobileScenarioMessages(issue, routeProfile, learnedScreens, destinationClaimManifest) {
    const userParts = [
    ...
    console.log(`[mobile:prompt-size] issue=${issue.key} ` +

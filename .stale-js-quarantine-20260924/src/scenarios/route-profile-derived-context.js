...
exports.buildDerivedExecutionContext = buildDerivedExecutionContext;
exports.formatDerivedContextForPrompt = formatDerivedContextForPrompt;
exports.logDerivedContext = logDerivedContext;
/**
 ...
 */
function isTargetBacked(target, routeProfile, allowedSoFar) {
    if (!routeProfile)
        return false;
    // Already in allowed set from earlier resolution stages
    if (allowedSoFar.has(target))
        return true;
    // Entry targets
 ...
 */
function deriveAllowedExecutableClicks(routeProfile, routeResolutions, additionalEntryTargets) {
            ...
            for (const step of resolution.executableRouteSteps) {
                ...
                if (clickMatch) {
                    ...
                    if (isBacked) {
                        ...
                        console.warn(`[route-profile-derived] executableRouteStep contains unbacked target: "${target}". ` +
 ...
 */
function deriveAssertionOnlyTerms(routeProfile) {
    const assertionOnly = new Set();
    if (!routeProfile)
        return [];
 ...
 */
function deriveVisibleButNotExecutableTerms(routeProfile, allowedExecutableClicks) {
    const visibleButNotExecutable = new Set();
    if (!routeProfile)
        return [];
    const visibleControls = Array.isArray(routeProfile.visibleControls)
        ? routeProfile.visibleControls
 ...
 */
function deriveSensitiveActions(routeProfile) {
 ...
 */
function buildAliasesByTarget(routeProfile) {
 ...
 */
function determineProfileConfidence(routeProfile, routeResolutions) {
    if (!routeProfile)
        return "none";
    let score = 0;
    const diagnostics = [];
    ...
    const hasExecutableSteps = Array.from(routeResolutions.values()).some(r => r.executableRouteSteps && r.executableRouteSteps.length > 0);
 ...
 */
function buildDerivedExecutionContext(appSlug, routeProfile, routeResolutions, additionalEntryTargets) {
    const diagnostics = [];
    const { allowed: allowedExecutableClicks, sources: clickSources } = deriveAllowedExecutableClicks(routeProfile, routeResolutions, additionalEntryTargets);
 ...
 */
function formatDerivedContextForPrompt(context) {
    const sections = [];
...
function logDerivedContext(context) {
    console.log(`[route-profile-derived] appSlug=${context.appSlug} ` +
    ...
    if (context.allowedExecutableClicks.length > 0) {
        ...
        if (context.clickSources) {
            ...
            const examples = context.allowedExecutableClicks.slice(0, 3).map((target) => `"${target}"(${context.clickSources?.get(target) || "unknown"})`);

...
exports.validateRouteProfileQuality = validateRouteProfileQuality;
exports.logRouteProfileQuality = logRouteProfileQuality;
/**
 * Validate route profile quality
 */
function validateRouteProfileQuality(appSlug, routeProfile, routeResolutions, derivedContext) {
    const diagnostics = [];
    // Missing profile
    if (!routeProfile) {
        diagnostics.push({
    ...
    if (derivedContext.allowedExecutableClicks.length > 0 && derivedContext.assertionOnlyTerms.length > 0) {
        const overlap = derivedContext.allowedExecutableClicks.filter(c => derivedContext.assertionOnlyTerms.includes(c));
        if (overlap.length > 0) {
            diagnostics.push({
                level: "warning",
 ...
 */
function logRouteProfileQuality(result) {
    console.log(`[route-profile-quality] appSlug=${result.appSlug} ` +
    ...
    for (const diag of result.diagnostics) {
        console.log(`[route-profile-quality] ${diag.level.toUpperCase()} ${diag.code}: ${diag.message}`);
    }
}

...
exports.classifyScenarioAutomatability = classifyScenarioAutomatability;
exports.filterScenariosByAutomatability = filterScenariosByAutomatability;
/**
...
function hasSupportedUiWaitStep(steps) {
    return steps.some((step) => /^(?:\d+[\.)]\s*)?(esperar|wait)/i.test(step)
        && /\b(\d+)\s*(segundos?|minutos?|ms|s|m)\b/i.test(step));
}
function normalizeWhitespace(value) {
    return value.replace(/\s+/g, " ").trim();
}
function findPatternMatch(patterns, sources) {
 ...
 */
function classifyScenarioAutomatability(scenario, huContext) {
    const detectedPatterns = [];
    const scenarioWithAuthority = scenario;
    ...
    if (contextOnlyMatches.length > 0) {
        console.log(`[automatability-filter] contextOnlySignals ignored rules=${contextOnlyMatches.slice(0, 3).join("|")} issue=${huContext?.key ?? "none"}`);
    }
 ...
 */
function filterScenariosByAutomatability(scenarios, huContext) {
    const automatable = [];
    const excluded = [];
    for (const scenario of scenarios) {
        ...
        console.log(`[automatability-filter] scenarioId="${scenarioId}" title="${safeTitle}" automatable=${classification.isAutomatable} ` +
    ...
    console.log(`[automatability-filter] total=${scenarios.length} automatable=${automatable.length} excluded=${excluded.length}`);

...
exports.generateDeterministicSeeds = generateDeterministicSeeds;
const scenario_hu_scope_filter_1 = require("./scenario-hu-scope-filter");
...
function normalizeLabel(label) {
    return (0, target_normalization_1.normalizeTarget)(label);
}
/**
 * Deduplicate navigation steps by normalized label
 * Preserves order and keeps first occurrence
 */
function deduplicateNavigationSteps(steps) {
    const seen = new Set();
    const deduplicated = [];
 ...
 */
function resolveFunctionalGroup(tp) {
    ...
    if (catalogRootIndex !== -1 && catalogRootIndex < intermediates.length - 1) {
        ...
        console.log(`[catalog-seeds:group] resolved functional group for "${tp.productMetadata?.productLabel}" from intermediates: "${functionalGroup}"`);
        return functionalGroup;
    }
    // Fallback: use first intermediate as category
    if (intermediates.length > 0) {
        console.log(`[catalog-seeds:group] using first intermediate as functional group for "${tp.productMetadata?.productLabel}": "${intermediates[0]}"`);
        return intermediates[0];
    }
    // Final fallback: use "General"
    console.log(`[catalog-seeds:group] no functional group found for "${tp.productMetadata?.productLabel}", using "General"`);
    return "General";
}
/**
 ...
 */
function generateDeterministicSeeds(routeProfile, existingScenarios, issueContext, appSlug, coverageMode, appConfig, huEvidence, huIntent) {
    ...
    if (huIntent && !(0, hu_intent_classifier_1.isCatalogListingIntent)(huIntent)) {
        const issueKey = issueContext?.key ?? "unknown";
    ...
    if (huEvidence) {
        ...
        targetPathsToFilter = Object.fromEntries(Object.entries(targetPathsToFilter).filter(([_, tp]) => {
        ...
        console.log(`[catalog-seeds] huEvidenceScope=true product=${huEvidence.productCategory} ` +
    ...
    const { alignedTargetPaths, diagnostics: scopeDiagnostics } = (0, scenario_hu_scope_filter_1.filterTargetPathsByIssueScope)(issueContext, targetPathsToFilter, routeProfile);
    console.log(`[catalog-seeds] HU scope filter: ${scopeDiagnostics.totalProducts} total → ` +
    ...
    console.log(`[catalog-seeds] products grouped by functional category: ${Array.from(productsByCategory.entries())
    ...
    for (const [category, products] of productsByCategory.entries()) {
        if (coverageMode === "representative") {
            ...
            if (isMajorCategory && productsByVariant.size < 2 && products.length >= 2) {
                ...
                for (const product of products) {
                    const alreadyIncluded = representativeProducts.some((rp) => rp.target.toLowerCase() === product.target.toLowerCase());
                    if (!alreadyIncluded) {
                        representativeProducts.push({
                            ...product,
                            category,
                        });
                        console.log(`[catalog-seeds:coverage] added second product for major category "${category}": "${product.target}"`);
                        break;
                    }
                }
            }
    ...
    console.log(`[catalog-seeds] selected ${representativeProducts.length} representative products ` +
    ...
    for (const scenario of existingScenarios) {
        for (const step of scenario.steps || []) {
            ...
            const validateMatch = step.match(/Validar que (?:se muestre|esté visible|el botón) [""]([^""]+)[""]|Validar que (?:se muestre|esté visible|el botón) ['']([^'']+)['']/i);
    ...
    console.log(`[catalog-seeds] AI coverage: products=${coveredProducts.size}/${discoveredProducts.length} ` +
        `categories=${coveredCategories.size}/${productsByCategory.size}`);
    // Generate seeds based on coverage mode
    const seeds = [];
    const seedsByCategoryCount = {};
    ...
    if (coverageMode === "representative") {
        ...
        for (const { target, tp, category, variant } of representativeProducts) {
            ...
            if (coveredProducts.has(normalized)) {
                console.log(`[catalog-seeds] skipping ${target} (${category}${variant ? ` ${variant}` : ""}) - already covered by AI`);
                continue;
            }
            ...
            if (seed) {
                ...
                console.log(`[catalog-seeds] generated seed for ${target} (${category}${variant ? ` ${variant}` : ""}) [representative mode]`);
            }
        }
    }
    else {
    ...
    console.log(`[catalog-seeds] generated ${seeds.length} seeds (mode=${coverageMode}) ` +
 ...
 */
function generateSeedForProduct(target, tp, routeProfile, appSlug, sourceIssueKey, appConfig) {
    ...
    console.log(`[catalog-seeds:steps] "${target}" rawSteps=${rawEntrySteps.length} dedupSteps=${entrySteps.length} ` +
        ...
        if (clickableToDetail) {
            ...
            if (metadata.expectedCardSignals) {
                if (metadata.expectedCardSignals.cardTextPreview) {
                    steps.push(`${stepNumber}. Validar que se muestre el texto "${metadata.expectedCardSignals.cardTextPreview}".`);
                    stepNumber++;
                }

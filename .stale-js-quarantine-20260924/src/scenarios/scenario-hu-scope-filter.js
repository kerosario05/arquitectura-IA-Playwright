...
exports.filterTargetPathsByIssueScope = filterTargetPathsByIssueScope;
 ...
 */
function filterTargetPathsByIssueScope(issueContext, targetPaths, routeProfile, huIntent) {
    const diagnostics = {
    ...
    const explicitlyMentionedCategories = new Set();
    ...
    if (explicitlyMentionedCategories.size > 0) {
        console.log(`[hu-scope-filter] HU mentions ${explicitlyMentionedCategories.size} categories explicitly: ` +
            Array.from(explicitlyMentionedCategories).join(", "));
    }
    // Filter targetPaths by HU alignment
    const alignedTargetPaths = {};
    for (const [target, tp] of Object.entries(targetPaths)) {
        ...
        if (alignmentScore.isAligned) {
            ...
            console.log(`[hu-scope-filter] filtered out "${target}" - score=${alignmentScore.score} threshold=0.3`);
        }
    }
    // Deduplicate matched keywords
    diagnostics.matchedKeywords = Array.from(new Set(diagnostics.matchedKeywords));
    console.log(`[hu-scope-filter] total=${diagnostics.totalProducts} aligned=${diagnostics.alignedProducts} ` +
    ...
    if (diagnostics.alignedProducts === 0) {
        diagnostics.warnings.push(`No products aligned with HU scope - HU may be too generic or products not related. Falling back to all products.`);
        diagnostics.alignedProducts = diagnostics.totalProducts;
 ...
 */
function buildHuSearchCorpus(issue) {
    const parts = [];
    if (issue.summary)
 ...
 */
function buildDomainTermsMap(routeProfile) {
    const map = new Map();
 ...
 */
function buildAliasesMap(routeProfile) {
    const map = new Map();
 ...
 */
function calculateAlignmentScore(target, tp, huKeywords, domainTerms, aliases) {

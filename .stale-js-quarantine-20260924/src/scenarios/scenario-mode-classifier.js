...
exports.classifyScenarioMode = classifyScenarioMode;
/**
 ...
 */
function detectModeFromIntent(huText) {
 ...
 */
function getRequiredRouteDepth(mode) {
    switch (mode) {
 ...
 */
function validateRouteBacking(mode, routeProfile) {
    const missingSteps = [];
    if (!routeProfile) {
    ...
    switch (mode) {
        ...
        case "return_navigation":
            ...
            const hasReturnControl = visibleControls.some(control => normalizeText(control).includes("volver"));
            if (!hasReturnControl) {
 ...
 */
function assignConfidence(mode, hasBacking, missingSteps) {
    if (mode === "unknown") {
        return "low";
    }
    if (hasBacking) {
    ...
    if (missingSteps.includes("intermediates") ||
 ...
 */
function classifyScenarioMode(huText, routeProfile) {

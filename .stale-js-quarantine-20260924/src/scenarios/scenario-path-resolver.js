...
exports.buildCompleteExecutablePath = buildCompleteExecutablePath;
exports.logPathResolution = logPathResolution;
 ...
 */
function resolveExecutablePath(target, currentContext, routeProfile, derivedContext, routeResolution) {
    const diagnostics = [];
    const pathSteps = [];
    const insertedSteps = [];
    ...
    const isAllowedClick = derivedContext.allowedExecutableClicks.some((click) => (0, target_normalization_1.targetsMatch)(target, click));
    if (!isAllowedClick) {
        return {
            canResolve: false,
            pathSteps: [],
    ...
    if (routeProfile.targetPaths && routeProfile.targetPaths[target]) {
        ...
        for (const intermediate of pathDef.requiredIntermediates) {
            if (!currentContext.some((ctx) => (0, target_normalization_1.targetsMatch)(ctx, intermediate))) {
                pathSteps.push({
                    action: "click",
                    target: intermediate,
    ...
    if (routeResolution?.executableRouteSteps) {
        const stepsWithTarget = routeResolution.executableRouteSteps.filter((step) => step.includes(target));
        if (stepsWithTarget.length > 0) {
            ...
            for (const step of routeResolution.executableRouteSteps) {
                ...
                if (clickMatch) {
                    ...
                    if (!currentContext.some((ctx) => (0, target_normalization_1.targetsMatch)(ctx, clickTarget))) {
                        pathSteps.push({
                            action: "click",
                            target: clickTarget,
                            description: `Navigate to ${clickTarget}`,
                        });
                        if (!(0, target_normalization_1.targetsMatch)(clickTarget, target)) {
                            insertedSteps.push({
                                action: "click",
                                target: clickTarget,
    ...
    if (routeProfile.intermediates) {
        for (const [groupKey, intermediateList] of Object.entries(routeProfile.intermediates)) {
            const targetIndex = intermediateList.findIndex((item) => (0, target_normalization_1.targetsMatch)(item, target));
            if (targetIndex !== -1) {
    ...
    if (routeProfile.entry) {
        const isEntry = routeProfile.entry.some((entry) => (0, target_normalization_1.targetsMatch)(entry.visibleLabel, target) || (0, target_normalization_1.targetsMatch)(entry.businessLabel, target));
        if (isEntry) {
            pathSteps.push({
                action: "click",
                target: target,
 ...
 */
function buildCompleteExecutablePath(target, routeProfile, derivedContext, routeResolution, entrySteps) {
    ...
    const resolution = resolveExecutablePath(target, currentContext, routeProfile, derivedContext, routeResolution);
 ...
 */
function logPathResolution(target, resolution, appSlug) {
    if (resolution.canResolve) {
        console.log(`[path-resolver] appSlug=${appSlug} target="${target}" ` +

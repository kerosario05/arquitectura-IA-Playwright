...
exports.selectScenarioPathFromScannedCatalog = selectScenarioPathFromScannedCatalog;
/**
 * Detect action keywords from HU description.
 * Matches HU-driven actions without modifying scenario generation.
 */
function detectActionsFromHU(issueDescription) {
    const actions = [];
 ...
 */
function extractKeywordsFromHU(issue) {
 ...
 */
function calculatePathMatchScore(targetPath, huEvidence, huKeywords) {
    let score = 0;
    ...
    if (huEvidence.accessMode === "private") {
        ...
        if (!firstIntermediate.includes("información de productos") &&
 ...
 */
function getRejectionReason(targetPath, huEvidence) {
    const firstIntermediate = (targetPath.requiredIntermediates?.[0] || "").toLowerCase();
 ...
 */
function selectScenarioPathFromScannedCatalog(input) {
    ...
    let selectedPath = selectedCandidates.length > 0
        ? {
    ...
    if (selectedPath === null && huEvidence.accessMode === "private") {
        ...
        if (syntheticPath) {
            ...
            console.log(`[path-selector] syntheticPathBuilt=true issue=${issue.key} ` +
    ...
    if (rejectedPaths.length > 0 && selectedPath === null) {
        gaps.push(`All ${rejectedPaths.length} evaluated paths were rejected: ${rejectedPaths
            .map((p) => p.reason)
            .join(", ")}`);
    }
    // Extract matched keywords (those that contributed to score)
    diagnostics.matchedKeywords = huKeywords.filter((kw) => selectedCandidates.some((c) => c.target.toLowerCase().includes(kw) || c.requiredIntermediates.some((i) => i.toLowerCase().includes(kw))));
    const selection = {
    ...
    console.log(`[path-selector] issue=${issue.key} intent=${huEvidence.businessIntent} ` +
    ...
    if (selectedPath) {
        ...
        console.log(`[path-selector] selectedPath=${selectedPath.targetPathKey} target="${selectedPath.target}" ` +
            `source=${pathSource}`);
        console.log(`[path-selector] selectedLabels=${selectedPath.requiredIntermediates.length} ` +
            `labels="${selectedPath.requiredIntermediates.join(" → ")}"`);
    }
    else {
        console.log(`[path-selector] selectedPath=none`);
    }
    console.log(`[path-selector] selectedActions=${actions.length} rejected=${rejectedPaths.length} ` +

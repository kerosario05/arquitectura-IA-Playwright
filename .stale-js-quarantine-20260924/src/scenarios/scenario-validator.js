...
exports.findMissingOptionFlowCoverage = findMissingOptionFlowCoverage;
...
function containsSensitiveAction(step) {
...
function matchesMcpPattern(step) {
    const trimmed = step.trim();
 ...
 */
function validatePrivateRouteJumps(scenario, huDrivenNavigationPath) {
...
function validateEntrySteps(scenario, routeProfile, huDrivenNavigationPath, functionalScope, optionFlows, authenticatedPrecondition, effectiveIntent) {
    ...
    if (huDrivenNavigationPath) {
        ...
        if (requiredEntryLabels.length === 0)
            return null;
    ...
    if (!scenario.steps || scenario.steps.length === 0)
        return null;
    const requiredEntryLabelsSet = new Set();
...
function validateProductRules(scenario, routeProfile) {
    ...
    for (const step of scenario.steps || []) {
        ...
        if (!matchedProduct)
            continue; // Not a product click
        // Check if it's a product_card that's not clickable
        if (matchedProduct.presentationType === "product_card" &&
 ...
 */
function validateAgainstHuScope(scenario, blockedTerms) {
 ...
 */
function validateOptionCoverage(scenario, optionFlows) {
    if (!optionFlows || optionFlows.length <= 1) {
    ...
    const hasOrdinalSelection = (scenario.steps || []).some(step => /^(\d+[\.)]\s*)?Seleccionar\s+(el\s+)?(primer|primera|[uú]ltimo|[uú]ltima)\s+/i.test(step));
    ...
    const mentionedOptions = optionFlows.filter(flow => scenarioText.includes(flow.optionLabel.toLowerCase()));
    ...
    if (targetOption.requiresAuth) {
        const hasAuthValidation = (scenario.steps || []).some(step => /validar\s+que\s+se\s+muestre/i.test(step) &&
 ...
 */
function findMissingOptionFlowCoverage(scenarios, optionFlows) {
    if (!optionFlows || optionFlows.length <= 1) {
        return [];
    }
...
function validateScenario(scenario, routeProfile, huDrivenNavigationPath, functionalScope, optionFlows, authenticatedPrecondition, effectiveIntent) {
    const errors = [];
    const warnings = [];
    ...
    if (!scenario.steps || scenario.steps.length === 0) {
        ...
        for (let i = 0; i < scenario.steps.length; i++) {
            ...
            const mojibakeResult = (0, target_normalization_1.detectMojibake)(rawStep);
            const step = mojibakeResult.corrected;
            if (mojibakeResult.hasMojibake) {
                console.log(`[validator:mojibake] corrected step ${i + 1}: "${rawStep.substring(0, 50)}" → "${step.substring(0, 50)}"`);
            }
            if (!isNumbered(step)) {
...
function validateScenarios(scenarios, routeProfile) {

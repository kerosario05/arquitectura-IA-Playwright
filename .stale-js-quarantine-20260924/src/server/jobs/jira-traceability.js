...
exports.linkTestRunToJiraIssue = linkTestRunToJiraIssue;
...
function buildJiraComment(input) {
    ...
    if (input.summary) {
        lines.push("", `Summary: total=${input.summary.total}, passed=${input.summary.passed}, failed=${input.summary.failed}, synced=${input.summary.synced}, syncFailed=${input.summary.syncFailed}`);
    }
    if (input.appSlug)
        lines.push(`App: ${input.appSlug}`);
    if (input.sectionSlug)
        lines.push(`Section: ${input.sectionSlug}`);
    return lines.join("\n");
}
async function linkTestRunToJiraIssue(input) {

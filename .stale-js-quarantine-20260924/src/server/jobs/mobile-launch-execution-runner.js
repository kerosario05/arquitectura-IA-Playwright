...
exports.deriveSourceIssueKey = deriveSourceIssueKey;
exports.classifyScenarioFailure = classifyScenarioFailure;
exports.startMobileLaunchExecutionJob = startMobileLaunchExecutionJob;
exports.startMobileLaunchExecutionJobWithDeps = startMobileLaunchExecutionJobWithDeps;
...
function deriveSourceIssueKey(scenarioId) {
    if (!scenarioId.startsWith("MOBILE-"))
        return undefined;
...
function mapMobileErrorToReasonCode(errorMessage) {
    const e = (errorMessage || "").toLowerCase();
    if (e.includes("expected element to be enabled"))
        return "assert_enabled_failed";
    if (e.includes("expected element to be disabled"))
        return "assert_disabled_failed";
    if (e.includes("target_disabled") || e.includes("deshabilitado"))
        return "target_disabled";
    if (e.includes("not found") || e.includes("no such element") || e.includes("could not be located"))
        return "target_not_found";
    if (e.includes("timeout") || e.includes("timed out"))
        return "timeout";
    if (e.includes("expected element to be visible"))
        return "assertion_not_found";
    return "execution_exception";
}
function resolveCaseId(publishedCases, scenarioId) {
    const match = publishedCases.find((pc) => pc.scenarioId === scenarioId ||
...
function classifyScenarioFailure(failedSteps) {
...
function classifyScenarioFailureSafe(failedSteps, onLog) {
    try {
...
function markRemainingScenariosSkippedInfrastructure(launchId, scenarios, publishedCases, totalScenarioCount, reasonCode, onLog) {
    let skipped = 0;
    for (const pending of scenarios) {
        const caseId = resolveCaseId(publishedCases, pending.scenarioId);
        if (!caseId)
            continue;
        (0, testrail_result_sync_1.updateLaunchManifestWithResult)(launchId, {
...
function classifyInfrastructureExecutionFailure(message) {
...
async function startMobileLaunchExecutionJob(jobId) {
    const deps = {
        ensureMobileInfraFn: mobile_test_runner_1.ensureMobileInfra,
        runOneScenarioFn: mobile_test_runner_1.runOneScenario,
        syncResultFn: testrail_result_sync_1.syncDiscoveryResultToTestRail,
        consolidateEvidenceFn: mobile_test_runner_1.consolidateMobileRunEvidence,
    };
    return startMobileLaunchExecutionJobWithDeps(jobId, deps);
}
async function startMobileLaunchExecutionJobWithDeps(jobId, deps) {
    ...
    try {
        (0, mobile_rerun_artifacts_1.persistMobileExecutionManifest)(jobId, params, {
            sourceJobId: typeof params.sourceJobId === "string"
                ? params.sourceJobId
                : undefined,
        });
    }
    ...
    let target;
    try {
        target = (0, mobile_test_runner_1.validateResolvedMobileTarget)((0, mobile_test_runner_1.resolveMobileTarget)(params));
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        const reasonCode = err instanceof mobile_test_runner_1.MobileTargetValidationError ? err.reasonCode : "mobile_apk_not_accessible";
        const apkPath = err instanceof mobile_test_runner_1.MobileTargetValidationError ? err.apkPath : undefined;
    ...
    onLog(`[mobile:launch] target resolved apkPathSource=${target.apkPathSource ?? "none"} apkPath=${target.apkPath ?? "none"} appPackage=${target.appPackage ?? "none"}`);
    ...
    try {
        ...
        for (const scenario of params.scenarios) {
            ...
            if (scenario.requiresRouteLearning === true) {
                ...
                const syncResult = {
                    statusId: 0,
                    syncStatus: "skipped_non_functional_failure",
                ...
                onLog(`[mobile:summary] scenarioId=${scenario.scenarioId} status=blocked passed=0 failed=0 skipped=0 blocked=1 firstFailureStep=-1 causalStep=-1 causalStepIndex=-1 reasonCode=requires_route_learning failureCategory=non_executable_precondition jiraAction=skipped testRailAction=${syncResult.syncStatus}`);
            ...
            const stepsToRun = (0, mobile_step_types_1.applyDataOverrides)(scenario.steps, params.dataOverrides?.[scenario.scenarioId], scenario.requiredData);
            ...
            const precheck = (0, mobile_test_runner_1.evaluateScenarioPrecheck)(stepsToRun, params.appSlug, undefined, scenario.requiredDataProfile);
            if (precheck.blocked) {
                totalBlocked++;
            ...
            try {
                ...
                if (block.blocked) {
                    ...
                    const scenariosSkipped = markRemainingScenariosSkippedInfrastructure(params.launchId, remaining, params.publishedCases, params.scenarios.length, reasonCode, onLog);
                    ...
                    if ((0, testrail_result_sync_1.hasInfrastructureBlockedStatusMapping)()) {
                        ...
                        onLog(`[mobile:launch] scenario ${scenario.scenarioId} infra-sync=${infraSync.syncStatus} statusId=${infraSync.statusId}${infraSync.error ? ` error=${infraSync.error}` : ""}`);
                        if (infraSync.syncStatus === "synced")
                            syncedCount++;
                        (0, testrail_result_sync_1.updateLaunchManifestWithResult)(params.launchId, {
                            scenarioId: scenario.scenarioId,
                            caseId,
                    ...
                    (0, mobile_rerun_artifacts_1.persistMobileExecutionResults)(jobId, scenarioOutcomes);
            ...
            const stepResults = (execResult.results ?? []);
            ...
            let discoveryStatus = scenarioBlocked
                ? "blocked"
                : (execResult.failed > 0 ? "failed" : "passed");
            let jiraAction = "skipped";
            totalPassed += execResult.passed;
            if (discoveryStatus === "failed")
                totalFailed += execResult.failed;
            if (discoveryStatus === "blocked")
                totalBlocked += execResult.blocked ?? 0;
            ...
            if (shouldSyncAsFunctionalFailure && sourceIssueKey) {
                ...
                if (primaryNavigationFailure && !defectEligible) {
                    jiraAction = "skipped";
                    onLog(`[mobile:launch] defect skipped scenario=${scenario.scenarioId} reason=navigation_primary_without_screen_evidence diagnostics=${failedStep?.diagnosticsPath ?? "n/a"}`);
                }
                if (defectEligible) {
                    try {
                        ...
                        Object.keys(technicalContext).forEach((k) => { if (technicalContext[k] === undefined)
                            delete technicalContext[k]; });
                        const failureReason = errorMessage || failedStep?.description || "El escenario mobile falló durante la ejecución.";
                        const sv = (0, defect_content_builder_1.inferDefectSeverity)(`${reasonCode} ${failureReason}`, scenario.scenarioId, scenario.title);
                        const title = (0, defect_content_builder_1.buildDefectTitle)({ scenarioId: scenario.scenarioId, scenarioTitle: scenario.title, severity: sv.severity, technicalContext, failureReason });
                        const description = (0, defect_content_builder_1.buildStructuredDefectDescription)({ scenarioId: scenario.scenarioId, scenarioTitle: scenario.title, failureReason, technicalContext, jobId });
                        const list = defect_checklist_store_1.defectChecklistStore.getOrCreate(sourceIssueKey);
                        ...
                        if (existing) {
                            ...
                            existing.technicalContext = technicalContext;
                            ...
                            defect_checklist_store_1.defectChecklistStore.addDefect(sourceIssueKey, {
                                description,
        ...
        (0, testrail_result_sync_1.finalizeLaunchManifest)(params.launchId, finalStatus, params.scenarios.length - syncedCount, params.scenarios.length);
        job_store_1.jobStore.update(jobId, {
        ...
        onLog(`[mobile:run] finished status=${totalFailed === 0 && totalBlocked === 0 ? "done" : "completed_with_failures"} passed=${totalPassed} failed=${totalFailed} blocked=${totalBlocked} durationMs=${job_store_1.jobStore.get(jobId)?.durationMs ?? "n/a"}`);
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        const block = classifyInfrastructureExecutionFailure(message);
        if (block.blocked) {
            ...
            const scenariosSkipped = markRemainingScenariosSkippedInfrastructure(params.launchId, params.scenarios, params.publishedCases, params.scenarios.length, reasonCode, onLog);
            onLog(`[mobile:launch] blocked reasonCode=${reasonCode} message=${message}`);

...
exports.ROUTE_LEARNING_COOLDOWN_MS = void 0;
exports.findDeviceBusyJob = findDeviceBusyJob;
exports.findActiveRouteLearningJob = findActiveRouteLearningJob;
exports.findRecentRouteLearningJob = findRecentRouteLearningJob;
exports.extractSteeringLabel = extractSteeringLabel;
exports.derivePreferLabels = derivePreferLabels;
exports.planRouteLearningAutostart = planRouteLearningAutostart;
exports.isFlowAlreadyLearned = isFlowAlreadyLearned;
/**
...
const DEVICE_JOB_TYPES = new Set([
    "mobile-test-run",
...
function findDeviceBusyJob(jobs) {
 ...
 */
function findActiveRouteLearningJob(jobs) {
...
exports.ROUTE_LEARNING_COOLDOWN_MS = 30 * 60 * 1000;
/**
 ...
 */
function findRecentRouteLearningJob(jobs, appSlug, flowId, now, withinMs = exports.ROUTE_LEARNING_COOLDOWN_MS) {
    return (jobs.find((j) => {
        if (j.type !== "mobile-route-learning")
            return false;
        if (ACTIVE_JOB_STATUSES.has(j.status))
            return false;
        if (j.params?.appSlug !== appSlug)
            return false;
        // A walk for a different story explored a different branch; it does not cover this one.
        if (flowId && j.params?.flowId !== flowId)
            return false;
 ...
 */
function extractSteeringLabel(target) {
...
function derivePreferLabels(scenarios) {
    const labels = [];
    for (const scenario of scenarios) {
        for (const step of (scenario.steps ?? []).slice(0, 4)) {
            ...
            if (labels.length >= MAX_PREFER_LABELS)
                return labels;
        }
    }
    return labels;
}
function planRouteLearningAutostart(input) {
 ...
 * history could not.
 */
function isFlowAlreadyLearned(profile, flowId) {
    if (!flowId || !profile?.flows)
        return false;

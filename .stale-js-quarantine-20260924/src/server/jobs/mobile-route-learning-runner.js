...
exports.fieldAlreadyHolds = fieldAlreadyHolds;
exports.buildLearnedScreens = buildLearnedScreens;
exports.upsertFlowFirst = upsertFlowFirst;
exports.deriveTriggerKeywords = deriveTriggerKeywords;
exports.startMobileRouteLearningJob = startMobileRouteLearningJob;
...
const mobile_route_learner_1 = require("../../mobile/mobile-route-learner");
const mobile_passthrough_screen_1 = require("../../mobile/mobile-passthrough-screen");
const ai_provider_factory_1 = require("../../ai/ai-provider-factory");
const mobile_route_learner_ai_1 = require("../../mobile/mobile-route-learner-ai");
const mobile_consent_checkbox_1 = require("../../mobile/mobile-consent-checkbox");
 ...
 */
function fieldAlreadyHolds(fill) {
    if (fill.otp)
        return false;
 ...
 */
async function snapshotScreen(browser, opts = {}) {
    const retries = opts.retries ?? 4;
    const delayMs = opts.delayMs ?? 2500;
    let snapshot = (0, mobile_knowledge_extractor_1.extractMobileScreenSnapshot)(await browser.getPageSource());
    for (let attempt = 1; attempt <= retries && snapshot.observedControls.length === 0; attempt++) {
        ...
        snapshot = (0, mobile_knowledge_extractor_1.extractMobileScreenSnapshot)(await browser.getPageSource());
    }
    return snapshot;
}
/**
 ...
 */
async function ensureAppInForeground(browser, appPackage, onLog, opts = {}) {
    if (!appPackage)
        return true;
 ...
 */
function buildLearnedScreens(snapshots, appPackage, now = new Date().toISOString()) {
    const screens = {};
    ...
    for (const [index, snapshot] of snapshots.entries()) {
        ...
        if (fingerprintById.has(screenId) && fingerprintById.get(screenId) !== snapshot.fingerprint) {
            let suffix = 2;
            while (fingerprintById.has(`${screenId}_${suffix}`) && fingerprintById.get(`${screenId}_${suffix}`) !== snapshot.fingerprint)
                suffix++;
            screenId = `${screenId}_${suffix}`;
        }
        if (fingerprintById.get(screenId) === snapshot.fingerprint)
            continue; // already captured
        fingerprintById.set(screenId, snapshot.fingerprint);
        const inputs = (0, mobile_route_learner_1.findInputControls)(snapshot, appPackage);
        const elements = [];
        const seen = new Set();
        for (const input of inputs) {
            const locator = (0, mobile_route_learner_1.inputControlToTarget)(input, inputs);
            if (!locator)
                continue;
            ...
            seen.add(key);
            elements.push({
                label: (0, mobile_route_learner_1.controlLabel)(input) || input.label || "(campo sin etiqueta)",
                role: "input",
                locator,
                hasAccessibilityLabel: Boolean((input.contentDesc ?? "").trim()),
            });
        }
        for (const actionable of (0, mobile_route_learner_1.findActionableControls)(snapshot, appPackage)) {
 ...
 */
function upsertFlowFirst(profile, flowId, flow) {
 ...
 */
function deriveTriggerKeywords(callerKeywords, hu, flowId) {
    ...
    const corpus = `${hu?.summary ?? ""} ${hu?.intent ?? ""}`
        ...
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, " ");
    const words = Array.from(new Set(corpus.split(/\s+/).filter((w) => w.length >= 5 && !GENERIC_KEYWORDS.has(w))));
    if (words.length > 0)
...
async function startMobileRouteLearningJob(jobId) {
    ...
    let aiProvider;
    try {
        aiProvider = await (0, ai_provider_factory_1.createGeneralAiProvider)();
    ...
    const aiCtx = { ai: aiProvider, huContext: params.huContext };
    const screens = [];
    ...
    try {
        const target = (0, mobile_test_runner_1.resolveMobileTarget)({
        ...
        await (0, mobile_test_runner_1.ensureMobileInfra)(target, onLog, { runId });
        const appiumHost = process.env.APPIUM_SERVER_HOST?.trim() || "127.0.0.1";
        const deviceId = (0, emulator_manager_1.getStatus)().deviceId || `${appiumHost}:${target.appiumPort}`;
        const lockOwner = `${runId}:route-learning`;
        pendingLock = { lockKey: (0, appium_session_coordinator_1.buildLockKey)(appiumHost, target.appiumPort, deviceId, target.systemPort), lockOwner };
        const handle = await (0, appium_session_coordinator_1.acquireSession)({
            ...
            factory: () => (0, appium_session_1.createSession)({
                appiumPort: target.appiumPort,
            ...
            canRetry: async () => {
                const appiumReady = (0, appium_server_manager_1.getStatus)().ready === true;
                const emu = (0, emulator_manager_1.getStatus)();
        ...
        for (let i = 0; i < seed.length; i++) {
            ...
            const result = await (0, mobile_step_executor_1.executeMobileStep)(browser, step, i);
            ...
            if (step.action !== "launchApp" && step.action !== "screenshot" && step.target) {
                traversed.push({ action: step.action, description: step.description, target: step.target, value: step.value });
                if (step.action === "click")
                    clickedLabels.push(step.target.value);
            }
        ...
        if (stopReason === "completed") {
            for (let i = 0; i < maxScreens; i++) {
                ...
                if (persisted) {
                    await (0, mobile_knowledge_persister_1.persistMobileScreen)(params.appSlug, snapshot, {
                ...
                const decision = await (0, mobile_route_learner_ai_1.decideNextActionInterpreted)(snapshot, decideOpts, aiCtx, onLog);
                ...
                if (decision.kind === "stop" && decision.reason === "needs_input") {
                    let resolved = (0, mobile_route_learner_1.resolveScreenData)(snapshot, params.screenData, target.appPackage);
                    if (!resolved.ok) {
                        ...
                        const aiFills = await (0, mobile_route_learner_ai_1.aiSynthesizeScreenData)(snapshot, decideOpts, aiCtx, onLog, params.screenData);
                        if (aiFills) {
                            const retry = (0, mobile_route_learner_1.resolveScreenData)(snapshot, [...(params.screenData ?? []), ...aiFills], target.appPackage);
                            if (retry.ok)
                                resolved = retry;
                        }
                    }
                    ...
                    if (resolved.ok && (resolved.fills.length === 0 || fillSignature === lastFillSignature)) {
                        const advance = (0, mobile_route_learner_1.findActionableControls)(snapshot, target.appPackage).find((a) => (0, mobile_passthrough_screen_1.isContinueLabel)(a.label) && !(0, mobile_route_learner_1.isBlockedSubmitLabel)(a.label, params.allowSubmitLabels));
                        if (!advance) {
                        ...
                        const advRes = await (0, mobile_step_executor_1.executeMobileStep)(browser, advStep, screens.length);
                    ...
                    for (const fill of resolved.fills) {
                        if (fill.selectFirst) {
                            ...
                            const selRes = await (0, mobile_step_executor_1.executeMobileStep)(browser, selectStep, screens.length);
                        ...
                        const fillStep = {
                            ...
                            ...(fill.otp ? { otp: { required: true } } : {}),
                        };
                        onLog(`[mobile:learn] fill -> "${fill.label}"${fill.otp ? " (OTP resuelto en runtime)" : ""}`);
                        const fillRes = await (0, mobile_step_executor_1.executeMobileStep)(browser, fillStep, screens.length, undefined, {
                            appSlug: params.appSlug,
                        });
                    ...
                    if (params.autoAcceptConsent !== false) {
                        try {
                            const { ticked } = await (0, mobile_consent_checkbox_1.ensureConsentCheckboxChecked)(browser, onLog);
                            if (ticked.length > 0)
                                onLog(`[mobile:learn] consentimiento marcado: ${ticked.join(" | ")}`);
                        }
                        catch (e) {
                ...
                const result = await (0, mobile_step_executor_1.executeMobileStep)(browser, step, screens.length);
                if (result.status !== "passed") {
                    stopReason = "no_actionable";
        ...
        if (clickedLabels.length > 0) {
            await (0, mobile_knowledge_persister_1.persistMobileRoute)(params.appSlug, clickedLabels, {
                scenarioTitle: `route-learning:${params.flowId ?? "walk"}`,
                status: "partial",
        ...
        if (observedSnapshots.length > 0) {
            const profile = (0, mobile_route_profile_1.loadMobileRouteProfile)(params.appSlug);
            if (!profile) {
                ...
                (0, mobile_route_profile_1.saveMobileRouteProfile)(params.appSlug, next);
                profileWritten = true;
            }
        }
        const result = {
        ...
        job_store_1.jobStore.update(jobId, { status: "failed", completedAt: new Date().toISOString(), errorMessage: message });
    }
    finally {
        if (sessionHandle) {
            try {
                await (0, appium_session_1.closeSession)(sessionHandle.browser);
            }
            catch {
        ...
        if (lock) {
            (0, appium_session_coordinator_1.releaseSessionLock)({ lockKey: lock.lockKey, lockOwner: lock.lockOwner, runId, onLog });
        }
    }
}

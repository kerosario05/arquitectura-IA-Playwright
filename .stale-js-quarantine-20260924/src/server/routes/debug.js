...
// ── Helpers ───────────────────────────────────────────────────────────────────
function buildCacheKey(storyKey, scenarioId) {
...
// ── Route ──────────────────────────────────────────────────────────────────────
exports.debugRouter.post("/testrail/publish-scenario", async (req, res) => {
    // ── Gate ────────────────────────────────────────────────────────────────────
    if (!isDebugEnabled) {
        res.status(403).json({
            ok: false,
            error: "forbidden",
            message: "This endpoint is disabled in production. Set DEBUG_TESTRAIL_ENDPOINTS=true to enable it.",
        });
        return;
    }
    ...
    const { projectId, suiteId, sectionId, appSlug, storyKey, scenarioId, title, steps, expectedResult, caseOracle, createTestRun, } = validation;
    ...
    const previewRefs = (0, testrail_case_publisher_1.buildSafeTestRailRefs)(scenario, scenarioId, storyKey);
    const previewExpected = (0, testrail_case_publisher_1.buildCustomExpected)(scenario);
    ...
    if (isPayloadDebugEnabled) {
        console.log(`[testrail-debug] final add_case body=${JSON.stringify(previewPayload, null, 2)}`);
    }
    else {
        console.log(`[testrail-debug] final add_case body (keys)=${Object.keys(previewPayload).join(",")}`);
    }
    ...
    try {
        const publishResult = await (0, testrail_case_publisher_1.publishScenariosToTestRail)(testRailClient, ctx);
        ...
        if (createTestRun) {
            if (publishResult.caseIds.length === 0) {
                ...
                try {
                    ...
                    console.log(`[testrail-debug] createdTestRun id=${run.id} url=${run.url ?? "n/a"}`);
                }
                catch (runErr) {
        ...
        console.log(`[testrail-debug] publish complete created=${publishResult.created} updated=${publishResult.updated} reused=${publishResult.reused} caseIds=${publishResult.caseIds.join(",")}`);
...
exports.debugRouter.post("/testrail/add-case-smoke", async (req, res) => {
    if (!isDebugEnabled) {
    ...
    let trClient;
    try {
        trClient = new testrail_client_1.TestRailClient((0, env_1.requireTestRailConfig)(env_1.config));
    }
    catch (err) {
    ...
    if (mode === "steps_preconds_expected") {
        const s = payload.custom_steps;
        const p = payload.custom_preconds;
        const e = payload.custom_expected;
    ...
    if (mode === "steps_preconds_expected_oracle") {
        ...
        const p = payload.custom_preconds;
        const e = payload.custom_expected;
        const o = payload.custom_case_oracle;
    ...
    if (mode === "steps_separated") {
        const arr = payload.custom_steps_separated;
        console.log(`[testrail-smoke] mode=steps_separated keys=${keys} separatedCount=${arr?.length ?? 0}`);
    }
    try {
        const result = await trClient.addCase(String(sectionId), payload, { preservePayload: true });
...
exports.debugRouter.post("/testrail/diagnose-refs", async (req, res) => {
    if (!isDebugEnabled) {
        res.status(403).json({ ok: false, error: "forbidden", message: "Debug endpoints are disabled in production." });
        return;
    }
...
exports.debugRouter.get("/testrail/status", (_req, res) => {
    if (!isDebugEnabled) {
        res.status(403).json({
            ok: false,
            error: "forbidden",

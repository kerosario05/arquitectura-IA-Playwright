...
const evidence_docx_generator_1 = require("../../evidence/evidence-docx-generator");
exports.jiraRouter = (0, express_1.Router)();
function client() {
    return new jira_client_1.JiraClient((0, env_1.requireJiraConfig)(env_1.config));
}
exports.jiraRouter.get("/projects", async (_req, res, next) => {
    try {
...
exports.jiraRouter.get("/projects/:key/sprints", async (req, res, next) => {
    try {
        const jira = client();
...
exports.jiraRouter.get("/projects/:key/sprint/active", async (req, res, next) => {
    try {
...
exports.jiraRouter.get("/issues", async (req, res, next) => {
    try {
        ...
        const jqlParts = [`project = "${projectId.replace(/"/g, '\\"')}"`];
        if (sprintId)
            jqlParts.push(`sprint = ${parseInt(sprintId, 10)}`);
        if (status)
...
exports.jiraRouter.post("/issues/upload-defects", async (req, res, next) => {
    try {
        const { sourceIssueKey, jiraProjectKey, defects = [], assigneeAccountId, } = req.body || {};
        ...
        const failed = [];
        const skipped = [];
        const evidenceRoot = path.resolve(process.env.MCP_EVIDENCE_ROOT ||
        ...
        for (const d of defects) {
            ...
            const storedDefect = defectId ? defect_checklist_store_1.defectChecklistStore.findDefect(sourceIssueKey, defectId) : null;
            ...
            if (defectId) {
                try {
                    const existing = defect_checklist_store_1.defectChecklistStore.findDefect(sourceIssueKey, defectId);
                    if (existing) {
...
exports.jiraRouter.post("/issues/:issueKey/retry-evidence", async (req, res, next) => {
    try {
        ...
        const stored = defect_checklist_store_1.defectChecklistStore.findDefect(sourceIssueKey, defectId);
        if (!stored) {
...
function resolveScreenshotPath(screenshotPath, scenarioDir, evidenceRoot, cwd, screenshotsDir, _stepIndex) {
    ...
    if (basename && basename !== sp) {
        ...
        if (fs.existsSync(basenameCandidate) && isImageFile(basenameCandidate))
            return basenameCandidate;
    }
    // 6. Raw file is directly in screenshotsDir
    const directCandidate = path.join(screenshotsDir, sp);
    if (fs.existsSync(directCandidate) && isImageFile(directCandidate))
        return directCandidate;
    return undefined;
}
function resolveFinalEvidenceStatus(defectStatus, evidenceJsonStatus) {
    const s = defectStatus.toLowerCase();
    if (s === "failed" || s === "fallido" || s === "exploration_failed" || s === "error" || s === "failure")
        return "Fallido";
    if (s === "blocked" || s === "bloqueado")
        return "Bloqueado";
    if (s === "passed" || s === "success" || s === "successful" || s === "exitoso" || s.includes("passed"))
        return "Exitoso";
    if (s === "pending_review")
        return evidenceJsonStatus || "Pendiente";
...
async function tryGenerateScenarioDocx(jobId, scenarioId, evidenceRoot, templatePath, defectId, defectFinalStatus) {
    try {
        ...
        for (const app of apps) {
            ...
            for (const section of sections) {
                ...
                try {
                    ...
                    const imagesDeclared = normalizedSteps.filter((s) => s.screenshotPath).length;
                    const imagesExisting = normalizedSteps.filter((s) => s.screenshotPath && isImageFile(s.screenshotPath) && fs.existsSync(s.screenshotPath)).length;
                    ...
                    if (templatePath) {
                        ...
                        try {
                            const { loadEvidenceConfig } = await Promise.resolve().then(() => __importStar(require("../../evidence/evidence-types")));
                    ...
                    try {
                        const result = await (0, evidence_docx_generator_1.generateEvidenceDocx)(normalizedRecord, resolvedTemplate, docxOut);

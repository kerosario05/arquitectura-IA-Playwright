...
exports.runtimeInputsRouter = void 0;
exports.resolveScenarioSyntheticBatch = resolveScenarioSyntheticBatch;
const express_1 = require("express");
const scenario_synthetic_value_resolver_1 = require("../../data/scenario-synthetic-value-resolver");
const scenario_generated_value_resolver_1 = require("../../data/scenario-generated-value-resolver");
...
const scenario_data_semantics_1 = require("../../ai/prompts/scenario-data-semantics");
...
const smart_prefill_1 = require("../../data/smart-prefill");
function isRecord(value) {
    return typeof value === "object" && value !== null && !Array.isArray(value);
}
async function resolveScenarioSyntheticBatch(input) {
    const generated = [];
    const resolved = [];
    const unresolved = [];
    const projectConfig = input.projectId
        ? await (input.loadProjectGenerationConfig ?? project_generation_config_service_1.getProjectGenerationConfig)(input.projectId)
        : undefined;
    ...
    const persistedRequirements = input.projectId !== undefined && input.caseId !== undefined
        ? await (input.loadPersistedRequirements ?? project_case_input_requirement_service_1.getByProjectIdAndCase)(input.projectId, input.caseId)
        : [];
    ...
    for (const requirement of input.requirements) {
        ...
        let runtimeRequirement = requirement;
        let enrichmentStatus;
        const persistedBinding = persistedByKey.get(requirement.key);
        if (persistedBinding?.namedProfileRef) {
            const configuredProfile = effectiveConfig.namedProfiles?.[persistedBinding.namedProfileRef];
            const bindingIsValid = configuredProfile !== undefined
                && (0, generation_profile_1.validateGenerationProfile)({ fieldCapability: requirement.fieldCapability, generationProfile: configuredProfile }).valid
                && (0, scenario_semantic_enricher_1.validateProjectGenerationReferences)(configuredProfile, effectiveConfig).valid;
            if (!bindingIsValid) {
            ...
            runtimeRequirement = {
                ...
                ...(configuredProfile.sourceMode === "configured_values" ? { scenarioDataPolicy: "configured_value_allowed" } : {}),
            };
        }
        const existingProfileIsValid = !persistedBinding?.namedProfileRef && requirement.generationProfile !== undefined
            && (0, generation_profile_1.validateGenerationProfile)({ fieldCapability: requirement.fieldCapability, generationProfile: requirement.generationProfile }).valid
            && (0, scenario_semantic_enricher_1.validateProjectGenerationReferences)(requirement.generationProfile, effectiveConfig).valid;
        ...
        if (persistedBinding?.namedProfileRef) {
            ...
            const enrichment = await (0, scenario_semantic_enricher_1.enrichScenarioInputSemantics)({
                requirement,
            ...
            if (enrichment.status === "accepted" && enrichment.generationProfile) {
                runtimeRequirement = {
                    ...
                    ...(enrichment.generationProfile.sourceMode === "configured_values" ? { scenarioDataPolicy: "configured_value_allowed" } : {}),
                    generationProfileSource: "ai_semantic_enrichment",
                    generationProfileVersion: scenario_data_semantics_1.SCENARIO_DATA_SEMANTICS_VERSION,
                };
            }
            else {
    ...
    const smartPrefill = await (0, smart_prefill_1.resolveSmartPrefill)({
        ...
        deterministicResolver: ({ requirement }) => {
            ...
            const result = preparedRequirement.generationProfile?.valueKind
                ? (0, scenario_generated_value_resolver_1.resolveScenarioGeneratedValue)({ requirement: preparedRequirement, generationProfile: preparedRequirement.generationProfile, projectGenerationConfig: (config ?? {}), seed: input.seed })
                : (0, scenario_synthetic_value_resolver_1.resolveScenarioSyntheticValue)({
                    requirement: {
                        ...preparedRequirement,
    ...
    for (const field of smartPrefill.fields)
        console.log(`[runtime-input-contract] key=${field.key} displaySource=${field.labelSource} valueSource=${field.source} generated=${field.generated} sensitive=${field.sensitive}`);
    return {
        generated,
        resolved,
        unresolved,
...
function isReplayCompatible(requirement, snapshot, contractVersion) {
    if (requirement.sensitive === true || requirement.fieldCapability.kind === "password")
        return false;
    if (snapshot.semanticType && requirement.semanticType && snapshot.semanticType !== requirement.semanticType)
        return false;
    if (snapshot.fieldKind && requirement.fieldCapability.kind && snapshot.fieldKind !== requirement.fieldCapability.kind)
        return false;
    if (snapshot.datasetIdentity && requirement.datasetIdentity && snapshot.datasetIdentity !== requirement.datasetIdentity)
        return false;
    if (snapshot.contractVersion && contractVersion && snapshot.contractVersion !== contractVersion)
        return false;
    return true;
}
const router = (0, express_1.Router)();
exports.runtimeInputsRouter = router;
router.post("/api/runtime-inputs/scenario-autofill", (req, res, next) => {
    const body = req.body;
    ...
    return resolveScenarioSyntheticBatch({
        ...
        projectGenerationConfig: isRecord(body.projectGenerationConfig) ? body.projectGenerationConfig : undefined,
        ...
        ...(process.env.RUNTIME_CONFIRMED_REPLAY_ENABLED?.toLowerCase() === "true"
            ? { loadConfirmedRuntimeValues: project_case_runtime_value_service_1.getConfirmedRuntimeValues }
            : {}),

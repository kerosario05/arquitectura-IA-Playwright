...
exports.sanitizeRawError = sanitizeRawError;
exports.mapReasonCodeToHuman = mapReasonCodeToHuman;
exports.formatTestRailId = formatTestRailId;
exports.buildLegacyDefectDescription = buildLegacyDefectDescription;
exports.buildStructuredDefectDescription = buildStructuredDefectDescription;
exports.inferDefectSeverity = inferDefectSeverity;
exports.buildDefectTitle = buildDefectTitle;
function sanitizeRawError(raw) {
    ...
    for (const key of secretKeys) {
        s = s.replace(new RegExp(`(${key})\\s*[:=]\\s*(["\\\']?)(?:Bearer\\s+)?\\S+\\2`, "gi"), (_m, p1) => `${p1}=[REDACTED]`);
        s = s.replace(new RegExp(`"(${key})"\\s*:\\s*"[^"]*"`, "gi"), (_m, p1) => `"${p1}":"[REDACTED]"`);
    }
    // 4. Sanitize paths
...
function mapReasonCodeToHuman(reasonCode, rawError) {
    if (!reasonCode && !rawError)
        return "";
...
function formatTestRailId(raw) {
    const trimmed = raw.trim();
    if (!trimmed)
        return trimmed;
    return trimmed.startsWith("C") && trimmed.length > 1 && /^\d+$/.test(trimmed.slice(1)) ? trimmed : `C${trimmed}`;
}
function buildLegacyDefectDescription(scenarioTitle, failureReason) {
...
function buildStructuredDefectDescription(params) {
    ...
    if (!hasTc)
        return buildLegacyDefectDescription(params.scenarioTitle, params.failureReason);
    ...
    if (failedAtStep != null || failedTarget) {
        ...
        if (failedAtStep != null)
            stepParts.push(`Paso ${failedAtStep}`);
        if (failedTarget)
            stepParts.push(failedTarget);
    ...
    // Evidencia.
    const ls = tc.lastSuccessfulStep;
...
function inferDefectSeverity(failureReason, _scenarioId, _scenarioTitle) {
    const reason = (failureReason ?? "").toLowerCase();
 ...
 */
function buildDefectTitle(params) {
    const tc = params.technicalContext ?? {};
    ...
    if (failedTarget) {
        ...
        if (!cause && params.failureReason)
            cause = sanitizeRawError(params.failureReason);
        if (!cause)
            cause = "La ejecución automatizada no pudo completarse";

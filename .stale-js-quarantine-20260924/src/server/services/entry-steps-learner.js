...
exports.learnEntryStepsFromSnapshot = learnEntryStepsFromSnapshot;
...
async function tryFormLogin(page, username, password) {
    const usernameField = page
        .locator('input[name="username"], input[name="user"], input[name="email"], input[type="email"], input[id*="user" i], input[placeholder*="usuario" i], input[placeholder*="user" i]')
        .first();
    const passwordField = page
        .locator('input[name="password"], input[type="password"], input[id*="password" i], input[placeholder*="contrasena" i], input[placeholder*="password" i]')
        .first();
    if ((await usernameField.count()) > 0 && (await passwordField.count()) > 0) {
        await usernameField.fill(username);
        await passwordField.fill(password);
        const submitBtn = page
            .locator('button[type="submit"], input[type="submit"], button:has-text("Iniciar"), button:has-text("Ingresar"), button:has-text("Entrar")')
            .first();
...
async function learnEntryStepsFromSnapshot(baseUrl, options) {
    ...
    let browser;
    try {
        browser = await test_1.chromium.launch({ headless: true, args: ["--no-sandbox"] });
        const context = await browser.newContext(username && password ? { httpCredentials: { username, password } } : undefined);
        ...
        const clickableControls = visibleControls.filter((c) => c.type === "button" || c.type === "link" || c.type === "menuitem");

...
exports.OracleOtpServiceError = exports.ORACLE_LATEST_LOCAL_TOKEN_SQL = exports.ORACLE_GENERATE_LOCAL_TOKEN_PLSQL = void 0;
exports.resolveOracleOtpRuntimeConfig = resolveOracleOtpRuntimeConfig;
exports.resolveOracleOtpRuntimeEnvironment = resolveOracleOtpRuntimeEnvironment;
exports.maskIdentity = maskIdentity;
exports.sanitizeOracleMessage = sanitizeOracleMessage;
exports.evaluateOracleProcedureOutcome = evaluateOracleProcedureOutcome;
exports.buildGenerateProcedureBinds = buildGenerateProcedureBinds;
exports.buildLatestTokenQueryBinds = buildLatestTokenQueryBinds;
exports.createOracleOtpAdapter = createOracleOtpAdapter;
exports.getSharedOracleOtpAdapter = getSharedOracleOtpAdapter;
exports.generateLocalToken = generateLocalToken;
exports.getLatestLocalToken = getLatestLocalToken;
exports.resetOtpRateLimiterForTests = resetOtpRateLimiterForTests;
exports.resetSharedOracleOtpAdapterForTests = resetSharedOracleOtpAdapterForTests;
const node_crypto_1 = __importDefault(require("node:crypto"));
const app_auto_resolver_1 = require("../../automations/app-auto-resolver");
exports.ORACLE_GENERATE_LOCAL_TOKEN_PLSQL = `BEGIN
...
END;`;
exports.ORACLE_LATEST_LOCAL_TOKEN_SQL = `SELECT OTP
FROM (
...
const ALLOWED_RUNTIME_TAGS = new Set(["local", "qa"]);
function parseBooleanTrue(rawValue) {
...
function parseAllowedRuntimeEnvironments(rawValue) {
    if (!rawValue?.trim())
        return ["local", "qa"];
...
function parseChannelList(rawValue) {
    if (!rawValue?.trim())
        return [];
...
function isSafeAppSlug(value) {
    return /^[a-z0-9]+(?:[-_][a-z0-9]+)*$/.test(value);
}
function parseOptionalStringValue(value) {
...
function extractOtpProfile(rawConfig) {
    if (!rawConfig)
        return null;
    ...
    const nestedProfile = appProfileContainer && typeof appProfileContainer === "object" && !Array.isArray(appProfileContainer)
        ? appProfileContainer.otpProfile
        : undefined;
...
function resolveOracleOtpRuntimeConfig(env = process.env) {
...
function resolveOracleOtpRuntimeEnvironment(env = process.env) {
    ...
    if (nodeEnv === "production" || profile === "production_like")
        return "production";
    if (profile === "qa")
        return "qa";
...
function sanitizeOracleCode(rawCode) {
    ...
    if (tokenMatch?.[1])
        return tokenMatch[1];
    if (/^[A-Z0-9_-]{1,48}$/.test(normalized))
        return normalized;
    return undefined;
}
function redactSecret(rawValue, secret) {
    if (!secret)
        return rawValue;
    return rawValue.split(secret).join("[redacted]");
}
function sanitizeOracleMessage(input) {
...
function isOracleUnavailableCode(code) {
    if (!code)
        return false;
...
function evaluateOracleProcedureOutcome(errorCode, errorDescription) {
    ...
    if (SUCCESS_ORACLE_CODES.has(normalizedCode)) {
        if (!normalizedDescription)
            return { ok: true };
        if (/^(OK|SUCCESS|EXITO)/i.test(normalizedDescription))
            return { ok: true };
        if (normalizedCode === "")
...
function buildGenerateProcedureBinds(input) {
    return {
        identity: {
...
function buildLatestTokenQueryBinds(input) {
    return {
...
function createOracleOtpAdapter(options = {}) {
    const env = options.env ?? process.env;
    let thickInitDone = false;
    const driverResolver = options.driverResolver ?? (async () => {
        const loaded = require("oracledb"); // eslint-disable-line @typescript-eslint/no-var-requires
        if (!thickInitDone) {
            const clientLibDir = (env.ORACLE_CLIENT_LIB_DIR ?? "").trim();
            if (clientLibDir && typeof loaded.initOracleClient === "function") {
                try {
                    loaded.initOracleClient({ libDir: clientLibDir });
    ...
    const resolvePool = async (runtime) => {
        if (poolPromise)
            return poolPromise;
    ...
    const runWithConnection = async (runtime, run) => {
        const driver = await driverResolver();
    ...
    return {
        async executeGenerateLocalToken(input) {
            return runWithConnection(input.runtime, async (connection, driver) => {
                ...
                const result = await connection.execute(exports.ORACLE_GENERATE_LOCAL_TOKEN_PLSQL, binds);
                const outBinds = Array.isArray(result.outBinds)
                    ? {}
                    : (result.outBinds ?? {});
                const asRecord = outBinds;
                return {
        ...
        async executeLatestLocalTokenQuery(input) {
            return runWithConnection(input.runtime, async (connection, driver) => {
                const result = await connection.execute(exports.ORACLE_LATEST_LOCAL_TOKEN_SQL, buildLatestTokenQueryBinds({
                    identity: input.identity,
                    channel: input.channel,
                    generatedAfter: input.generatedAfter,
...
let sharedOracleOtpAdapter = null;
function getSharedOracleOtpAdapter(env = process.env) {
    if (!sharedOracleOtpAdapter) {
        sharedOracleOtpAdapter = createOracleOtpAdapter({ env });
    }
    return sharedOracleOtpAdapter;
}
class OracleOtpServiceError extends Error {
    reasonCode;
    httpStatus;
    details;
    constructor(reasonCode, httpStatus, details) {
...
exports.OracleOtpServiceError = OracleOtpServiceError;
...
function ensureOtpRuntimeEnabled(runtime) {
    if (!runtime.enabled) {
...
function ensureAppSlug(rawAppSlug, runtime) {
...
function resolveAllowedChannels(input) {
    ...
    if (input.profile.defaultChannel)
        return [normalizeChannel(input.profile.defaultChannel)];
    if (input.runtime.defaultChannel)
        return [normalizeChannel(input.runtime.defaultChannel)];
    return [];
}
function ensureEnvironmentAndProfileAllowed(input) {
    ...
    const profileEnvironments = (profile.enabledEnvironments ?? [])
        ...
        .filter((entry) => (entry === "local" || entry === "qa" || entry === "production" || entry === "other"));
...
function ensureIdentity(rawIdentity, runtime) {
...
function ensureChannel(rawChannel, input) {
...
function ensureGeneratedAfter(rawGeneratedAfter, runtime, now) {
...
function ensureRequestId(rawRequestId, runtime) {
...
async function generateLocalToken(rawInput, deps = {}) {
    ...
    const appConfigLoader = deps.appConfigLoader ?? app_auto_resolver_1.loadAppConfig;
    ...
    const requestIdFactory = deps.requestIdFactory ?? node_crypto_1.default.randomUUID;
    ...
    try {
        ...
        const outcome = evaluateOracleProcedureOutcome(procedureResult.errorCode, procedureResult.errorDescription);
        if (!outcome.ok) {
...
async function getLatestLocalToken(rawInput, deps = {}) {
    const env = deps.env ?? process.env;
    const runtime = resolveOracleOtpRuntimeConfig(env);
    ensureOtpRuntimeEnabled(runtime);
    const runtimeEnvironment = resolveOracleOtpRuntimeEnvironment(env);
...
function resetOtpRateLimiterForTests() {
    otpRateLimitStore.clear();
}
function resetSharedOracleOtpAdapterForTests() {

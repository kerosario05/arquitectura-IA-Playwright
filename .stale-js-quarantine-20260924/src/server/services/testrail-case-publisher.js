...
exports.sanitizeTestRailRef = sanitizeTestRailRef;
exports.buildSafeRefsFilter = buildSafeRefsFilter;
exports.buildScenarioRefCandidates = buildScenarioRefCandidates;
exports.buildSafeTestRailRefs = buildSafeTestRailRefs;
exports.buildCustomExpected = buildCustomExpected;
exports.publishScenariosToTestRail = publishScenariosToTestRail;
exports.readPersistedScenarioMappings = readPersistedScenarioMappings;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const env_1 = require("../../config/env");
const testrail_sync_types_1 = require("./testrail-sync-types");
...
const STORE_PATH = node_path_1.default.join(ROOT, ".artifacts", "testrail", "scenario-case-mappings.json");
...
function sectionCaseSnapshot(client, ctx) {
    return client.getCases(String(ctx.projectId), ctx.suiteId ? String(ctx.suiteId) : undefined, String(ctx.sectionId));
}
function newlyAppearedCases(beforeIds, cases, sectionId, title, refs) {
    const normalizedTitle = normalizeTitle(title);
    const normalizedRefs = refs?.trim();
    const delta = cases.filter((candidate) => candidate.section_id === sectionId
...
function readStore() {
    try {
        ...
        const parsed = JSON.parse(node_fs_1.default.readFileSync(STORE_PATH, "utf-8"));
        return {
            version: 1,
...
function scenarioCacheKey(scenarioId, cacheKey, projectId, suiteId, sectionId) {
...
function sanitizeTestRailRef(value) {
...
function buildSafeRefsFilter(input) {
...
function buildScenarioRefCandidates(scenario, scenarioId, storyKey) {
    ...
    return Array.from(new Set(candidates.filter((value) => Boolean(value))));
}
function buildSafeTestRailRefs(scenario, scenarioId, storyKey) {
...
function isLegacyPreviewPattern(id) {
...
function shouldIgnoreLegacyAmbiguity(requestedId, candidateIds) {
    ...
    if (!isNewLaunchPattern(requestedId))
        return false;
    const validCandidates = candidateIds.filter((id) => Boolean(id));
    if (validCandidates.length === 0)
        return false;
    const allLegacy = validCandidates.every(id => isLegacyPreviewPattern(id));
    return allLegacy;
}
function shouldSelectFirstExactMatch(requestedId, candidateIds) {
...
function isRefs500Error(error) {
    const message = error instanceof Error ? error.message : String(error);
    return message.includes("HTTP 500") && message.includes("Undefined array key refs");
}
async function recoverCaseAfterRefs500(client, ctx, scenarioTitle, scenarioId) {
    try {
        const sectionCases = await client.getCases(String(ctx.projectId), ctx.suiteId ? String(ctx.suiteId) : undefined, String(ctx.sectionId));
        ...
        if (candidates.length === 0) {
            ...
            const byCacheAndScenario = sectionScoped.filter((c) => String(c.custom_cache_key ?? "") === cacheKey && String(c.custom_scenario_id ?? "") === scenarioId);
        ...
        const cacheKey = ctx.cacheKey;
        const byCacheAndScenario = candidates.filter((c) => String(c.custom_cache_key ?? "") === cacheKey && String(c.custom_scenario_id ?? "") === scenarioId);
        if (byCacheAndScenario.length === 1) {
            console.log(`[testrail-publish] recovered by cache_key+scenario_id caseId=${byCacheAndScenario[0].id}`);
            return { found: true, case: byCacheAndScenario[0], strength: "cache_key_and_scenario_id" };
        }
        // If multiple title-cache-scenario matches, try unique marker in custom_preconds
        const launchId = ctx.launchId;
...
function recoveryDiagnosticInfo(ctx, scenarioTitle, scenarioId, result) {
    const diagnostic = {
...
function parseFlatJsonObject(rawValue) {
...
function parseRequiredCaseFields(rawValue) {
...
function getConfiguredRequiredCaseFields() {
    const configured = env_1.config.integrations.testRail?.requiredCaseFields ?? {};
    try {
...
function toStepsSeparated(scenario) {
...
function buildCustomCaseOracle(scenario, requiredCaseFields) {
    // Priority: explicit scenario.caseOracle > TESTRAIL_DEFAULT_CASE_ORACLE > required fields > fallback
    const explicitOracle = sanitizeText(stripHtml(scenario.caseOracle ?? ""));
    if (explicitOracle)
        return explicitOracle;
    if (TESTRAIL_DEFAULT_CASE_ORACLE)
        return TESTRAIL_DEFAULT_CASE_ORACLE;
    if (requiredCaseFields.custom_case_oracle) {
        return sanitizeText(requiredCaseFields.custom_case_oracle) ?? "QA";
    }
    return "QA";
}
function collectCustomFieldNames(customFields, hasExpected, hasCaseOracle) {
    const keys = Object.keys(customFields);
    const fieldNames = new Set(keys);
    if (hasExpected)
        fieldNames.add("custom_expected");
    if (hasCaseOracle)
        fieldNames.add("custom_case_oracle");
    return Array.from(fieldNames);
}
function detectMissingRequiredField(error) {
    const message = error instanceof Error ? error.message : String(error);
...
function buildAutomationScenarioMarkerId(scenarioId, launchId) {
    if (!launchId)
        return undefined;
...
function buildAutomationScenarioMarker(scenarioId, launchId) {
...
function isAddCaseHttp500Error(error) {
    const message = error instanceof Error ? error.message : String(error);
...
function buildCustomExpected(scenario) {
...
async function findExistingCaseByCacheKey(client, ctx, scenarioId, fallbackTitle) {
    try {
        const sectionCases = await client.getCases(String(ctx.projectId), ctx.suiteId ? String(ctx.suiteId) : undefined, String(ctx.sectionId));
        const cacheKey = ctx.cacheKey;
        const match = sectionCases.find((c) => c.section_id === ctx.sectionId &&
        ...
        if (ctx.recordingBatch && fallbackTitle) {
            ...
            const titleMatches = sectionCases.filter((c) => c.section_id === ctx.sectionId && normalizeTitle(c.title) === normalizedTitle);
...
function isAmbiguousAfterAddCase(error) {
...
async function reconcileAmbiguousRecordingCase(client, ctx, scenarioId, scenarioTitle, beforeIds, refs) {
    ...
    while (attempts < AMBIGUOUS_RECONCILIATION_ATTEMPTS && Date.now() - startedAt <= AMBIGUOUS_RECONCILIATION_DEADLINE_MS) {
        ...
        try {
            ...
            const identityMatches = deltaMatches.filter((candidate) => String(candidate.custom_cache_key ?? "") === ctx.cacheKey
        ...
        if (attempts >= AMBIGUOUS_RECONCILIATION_ATTEMPTS || remainingMs <= 0)
            break;
...
function uniqueCaseIds(caseIds) {
    return Array.from(new Set(caseIds));
}
async function publishScenariosToTestRail(client, ctx) {
    ...
    const scenarioIds = ctx.scenarios.map((s, i) => (0, testrail_sync_types_1.buildScenarioPreviewScenarioId)(s, i, {
        ...idContext,
        sourceScenarioId: s.launchScenarioId,
    }));
    ...
    for (const [index, scenario] of ctx.scenarios.entries()) {
        const scenarioId = (0, testrail_sync_types_1.buildScenarioPreviewScenarioId)(scenario, index, {
            ...idContext,
            sourceScenarioId: scenario.launchScenarioId,
        });
        ...
        const launchId = ctx.launchId;
        const uniqueMarker = buildAutomationScenarioMarker(scenarioId, launchId);
        const preconditionsBody = preconditionsBase ||
        ...
        const diagnosticPayload = {
            ...
            ...customFields,
            ...(sendsTextSteps ? { custom_steps: (0, testrail_step_serializer_1.serializeTestRailSteps)(stepsSeparated) } : {}),
            ...(sendsStructuredSteps ? { custom_steps_separated: stepsSeparated.map((step) => ({ content: step.content, expected: step.expected ?? "" })) } : {}),
        };
        const payloadDiagnostic = (0, testrail_recording_payload_1.describeTestRailRecordingPayload)(diagnosticPayload);
        const payloadValidation = (0, testrail_recording_payload_1.validateTestRailRecordingPayload)(diagnosticPayload);
        const diagnosticPath = node_path_1.default.join(ROOT, ".artifacts", "testrail", "recording-add-case-payload-diagnostic.json");
        ensureDir(diagnosticPath);
        node_fs_1.default.writeFileSync(diagnosticPath, JSON.stringify({
            sectionId: ctx.sectionId,
        ...
        if (payloadValidation.length > 0)
            throw new Error(`testrail_recording_payload_invalid: ${payloadValidation.join("; ")}`);
                    ...
                    try {
                        ...
                        if (isAddCaseHttp500Error(addErr)) {
                            ...
                            if (recovery1.found) {
                                ...
                                const markerId = buildAutomationScenarioMarkerId(scenarioId, ctx.launchId) ?? scenarioId;
                                ...
                                const stepsText = (0, testrail_step_serializer_1.serializeTestRailSteps)(stepsSeparated);
                                ...
                                try {
                                    testRailCase = await client.addCase(String(ctx.sectionId), noRefsPayload, { preservePayload: true });
                                    mappingSource = "created";
                                    created += 1;
                            ...
                            try {
                                testRailCase = await client.addCase(String(ctx.sectionId), minimalPayload, { compatibilityMode: true });
                                mappingSource = "created";
                                created += 1;
                                console.log(`[testrail-publish] compatibility retry succeeded case=${scenarioId} caseId=${testRailCase.id}`);
                            }
        ...
        for (const item of currentStore.mappings)
            durable.set(scenarioCacheKey(item.scenarioId, item.cacheKey, item.projectId, item.suiteId, item.sectionId), item);
...
function readPersistedScenarioMappings() {

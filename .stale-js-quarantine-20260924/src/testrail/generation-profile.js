...
exports.validateGenerationProfile = validateGenerationProfile;
function validateGenerationProfile(input) {
...
function validateTechnicalProfile(profile) {
    if (!profile.valueKind || !profile.sourceMode)
        return { valid: false, reason: "technical_traits_required" };
    if (profile.sourceMode === "configured_values" && !profile.valueSetRef?.trim())
        return { valid: false, reason: "value_set_reference_required" };
    const constraints = profile.constraints;
    if (profile.format?.characterSet !== undefined && !["digits", "letters", "alphanumeric"].includes(profile.format.characterSet))
        return { valid: false, reason: "character_set_invalid" };
    if (constraints?.min !== undefined && constraints?.max !== undefined && constraints.min > constraints.max)
        return { valid: false, reason: "numeric_range_invalid" };
    if (constraints?.step !== undefined && (!Number.isFinite(constraints.step) || constraints.step <= 0))
        return { valid: false, reason: "step_invalid" };
    if (constraints?.decimalScale !== undefined && (!Number.isInteger(constraints.decimalScale) || constraints.decimalScale < 0))
        return { valid: false, reason: "decimal_scale_invalid" };
    if (constraints?.integerOnly === true && (constraints.decimalScale ?? 0) > 0)
        return { valid: false, reason: "integer_decimal_incompatible" };
    if (profile.valueKind === "number" && constraints?.minLength !== undefined)
        return { valid: false, reason: "length_constraints_not_for_number" };
    if (profile.valueKind === "string") {
        if (constraints?.minLength !== undefined && constraints?.maxLength !== undefined && constraints.minLength > constraints.maxLength)
            return { valid: false, reason: "length_range_invalid" };
        if (profile.format?.totalLength !== undefined && (!Number.isInteger(profile.format.totalLength) || profile.format.totalLength <= 0))
            return { valid: false, reason: "total_length_invalid" };
        if (profile.format?.allowedPrefixes !== undefined && (profile.format.allowedPrefixes.length === 0 || profile.format.allowedPrefixes.some((prefix) => !prefix.trim())))
            return { valid: false, reason: "prefixes_invalid" };
        if (profile.format?.totalLength !== undefined && profile.format.allowedPrefixes?.some((prefix) => prefix.length >= profile.format.totalLength))
            return { valid: false, reason: "prefix_too_long" };
    }
    return { valid: true };
}

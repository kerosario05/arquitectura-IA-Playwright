...
class AuthFlow {
    ...
    resolveClient(alias, testData) {
        ...
        if (!clients || !clients[resolvedAlias]) {
            ...
            if (process.env.APP_EXTRA_LOGIN_FIELDS_JSON) {
                try {
                    fallbackClient.extraLoginFields = JSON.parse(process.env.APP_EXTRA_LOGIN_FIELDS_JSON);
                }
                catch { /* ignore */ }
            }
        ...
        if (!client.username && typeof process.env.APP_USERNAME === "string" && process.env.APP_USERNAME) {
            client.username = process.env.APP_USERNAME;
        }
        if (!client.password && typeof process.env.APP_PASSWORD === "string" && process.env.APP_PASSWORD) {
            client.password = process.env.APP_PASSWORD;
        }
    ...
    async isTransactionsEntryVisible() {
        try {
     ...
     */
    async waitForPageTransition(previousUrl, timeoutMs = 5000) {
        try {
            await this.page.waitForFunction((prevUrl) => window.location.href !== prevUrl, previousUrl, { timeout: timeoutMs });
    ...
    async completeIdentification(client, stagesCompleted) {
    ...
    async completePhoneConfirmation(client, stagesCompleted) {
    ...
    async completeOtp(client, stagesCompleted) {
    ...
    async finalizeAuth(stagesCompleted, alias, landing) {

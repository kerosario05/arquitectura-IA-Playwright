...
test('Visualizar detalle de un producto de Tarjetas', async ({ page }) => {
  ...
  await promotedRuntime.expectPromotedVisible({ stepIndex: 5, target: 'Tarjetas', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Tarjetas]
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'Tarjetas', actionIntent: 'select_category', expectedEffect: 'ui_change', sensitive: false, action: async () => { await categoryPage.selectCategory('Tarjetas'); } });
});
...
test('Visualizar subcategorías de Cuentas de Efectivo', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'Cuentas de ahorros', actionIntent: 'select_category', expectedEffect: 'ui_change', sensitive: false, action: async () => { await categoryPage.selectCategory('Cuentas de ahorros'); } });
});
...
test('Regresar al listado desde el detalle de producto', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'Volver al listado de productos', actionIntent: 'select_product', expectedEffect: 'ui_change', sensitive: false, action: async () => { await productListPage.selectProduct('Volver al listado de productos'); } });
});
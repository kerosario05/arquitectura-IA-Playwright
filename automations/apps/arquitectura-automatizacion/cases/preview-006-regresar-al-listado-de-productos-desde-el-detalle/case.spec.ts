...
test('Regresar al listado de productos desde el detalle', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'Volver al listado de productos', actionIntent: 'return_to_list', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Tarjetas de crédito', sensitive: false, replay: replayStep_5 }], lastSelectionStep: { stepIndex: 4, selectedTarget: 'Información de productos' }, lastSelectionReplay: async () => { await productListPage.selectProduct('Información de productos'); }, expectedOwnerPage: 'GenericPage', action: async () => { await productDetailPage.backToList(); } }); // [target: Volver al listado de productos]
});
...
test('Buscar cuenta para carta de referencia bancaria', async ({ page }) => {
  ...
  process.env.SCENARIO_TITLE = 'Buscar cuenta para carta de referencia bancaria';
  ...
  const replayStep_7 = async () => { await productListPage.selectProduct('el termino de busqueda en el campo de busqueda'); };
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 7, target: 'el termino de busqueda en el campo de busqueda', actionIntent: 'select_product', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Transacciones y servicios', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Generar cartas', sensitive: false, replay: replayStep_5 }, { stepIndex: 6, actionIntent: 'select_product', target: 'Carta de referencia', sensitive: false, replay: replayStep_6 }], lastSelectionStep: undefined,  expectedOwnerPage: 'ProductListPage', action: async () => { await productListPage.selectProduct('el termino de busqueda en el campo de busqueda'); } });
  await promotedRuntime.clickPromotedTarget({ stepIndex: 8, target: 'Seleccionar la primera cuenta visible del listado.', actionIntent: 'select_visible_item_by_ordinal', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Transacciones y servicios', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Generar cartas', sensitive: false, replay: replayStep_5 }, { stepIndex: 6, actionIntent: 'select_product', target: 'Carta de referencia', sensitive: false, replay: replayStep_6 }, { stepIndex: 7, actionIntent: 'select_product', target: 'el termino de busqueda en el campo de busqueda', sensitive: false, replay: replayStep_7 }], action: async () => { await productListPage.selectVisibleItemByOrdinal("first"); } });
  } finally {

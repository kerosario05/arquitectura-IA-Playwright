...
test('Consulta de balance - Validar etiquetas visibles del detalle de depósito', async ({ page }) => {
  ...
  await promotedRuntime.expectPromotedVisible({ stepIndex: 9, target: 'Número de certificado', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Número de certificado]
  await promotedRuntime.expectPromotedVisible({ stepIndex: 10, target: 'Tipo', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Tipo]
});
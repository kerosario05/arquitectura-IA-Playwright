...
test('Consulta de balance - Regresar desde detalle de depósito', async ({ page }) => {
  ...
  const replayStep_8 = async () => { await productListPage.selectVisibleItemByOrdinal('first'); };
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 9, target: 'Volver al Listado', actionIntent: 'return_to_list', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'start_session', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'open_module', target: 'Transacciones y servicios', sensitive: false, replay: replayStep_4 }, { stepIndex: 7, actionIntent: 'select_product', target: 'Depósitos a plazos', sensitive: false, replay: replayStep_7 }], lastSelectionStep: { stepIndex: 8, selectedTarget: 'el primer depósito visible del listado' }, lastSelectionReplay: async () => { await productListPage.selectVisibleItemByOrdinal('first'); }, expectedOwnerPage: 'GenericPage', action: async () => { await productDetailPage.backToList(); } }); // [target: Volver al Listado]
});
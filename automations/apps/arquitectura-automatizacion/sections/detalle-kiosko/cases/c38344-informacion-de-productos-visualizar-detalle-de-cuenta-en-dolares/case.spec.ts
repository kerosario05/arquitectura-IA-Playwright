...
test('Información de productos - Visualizar detalle de cuenta en dólares', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 7, target: 'Seleccionar el primer cuenta visible del listado.', actionIntent: 'select_visible_item_by_ordinal', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'start_session', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'open_product_information', target: 'Información de productos', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_category', target: 'Cuentas', sensitive: false, replay: replayStep_5 }, { stepIndex: 6, actionIntent: 'select_category', target: 'Cuenta de Ahorro', sensitive: false, replay: replayStep_6 }], action: async () => { await productListPage.selectVisibleItemByOrdinal("first"); } });
});
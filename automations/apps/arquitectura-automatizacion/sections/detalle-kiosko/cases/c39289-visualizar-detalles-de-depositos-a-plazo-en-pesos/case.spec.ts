...
test('Visualizar detalles de Depósitos a plazo en Pesos', async ({ page }) => {
  ...
  process.env.SCENARIO_ID = 'C39289';
  process.env.SCENARIO_TITLE = 'Visualizar detalles de Depósitos a plazo en Pesos';
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'Depósitos a plazo en Pesos', actionIntent: 'select_product', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Información de productos', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Depósitos a plazo', sensitive: false, replay: replayStep_5 }], lastSelectionStep: undefined,  expectedOwnerPage: 'ProductListPage', action: async () => { await productListPage.selectProduct('Depósitos a plazo en Pesos'); } });
  } finally {

...
test('Validar las opciones visibles de Cuentas de Efectivo', async ({ page }) => {
  ...
  process.env.SCENARIO_TITLE = 'Validar las opciones visibles de Cuentas de Efectivo';
  ...
  await promotedRuntime.expectPromotedVisible({ stepIndex: 6, target: 'Dólares', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Dólares]
  } finally {

...
test('Validacion de ruta inicial hacia Información de productos como opcion visible', async ({ page }) => {
  ...
  process.env.SCENARIO_TITLE = 'Validacion de ruta inicial hacia Información de productos como opcion visible';
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 5, target: 'Iniciar', actionIntent: 'start_session', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'transacciones y servicios', sensitive: false, replay: replayStep_4 }], lastSelectionStep: undefined,  expectedOwnerPage: 'HomePage', action: async () => { await homePage.start(); } }); // [target: Iniciar]
  } finally {

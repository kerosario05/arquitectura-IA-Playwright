...
test('Regresar al listado desde el detalle de producto', async ({ page }) => {
  ...
  process.env.SCENARIO_TITLE = 'Regresar al listado desde el detalle de producto';
  ...
  const replayStep_7 = async () => { await productListPage.selectProduct('Volver al listado de productos'); };
  ...
  await promotedRuntime.expectPromotedVisible({ stepIndex: 8, target: 'Tarjetas', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Tarjetas]
  await promotedRuntime.expectPromotedVisible({ stepIndex: 9, target: 'Préstamos', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Préstamos]
  } finally {

...
test('Seleccionar cuenta para generar carta de referencia bancaria', async ({ page }) => {
  ...
  process.env.SCENARIO_TITLE = 'Seleccionar cuenta para generar carta de referencia bancaria';
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 7, target: 'Seleccionar la primera cuenta visible del listado.', actionIntent: 'select_visible_item_by_ordinal', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Transacciones y servicios', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Generar cartas', sensitive: false, replay: replayStep_5 }, { stepIndex: 6, actionIntent: 'select_product', target: 'Carta de referencia', sensitive: false, replay: replayStep_6 }], action: async () => { await productListPage.selectVisibleItemByOrdinal("first"); } });
  await promotedRuntime.clickPromotedTarget({ stepIndex: 8, target: 'Continuar', actionIntent: 'click_primary_action', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Transacciones y servicios', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Generar cartas', sensitive: false, replay: replayStep_5 }, { stepIndex: 6, actionIntent: 'select_product', target: 'Carta de referencia', sensitive: false, replay: replayStep_6 }, { stepIndex: 7, actionIntent: 'select_visible_item_by_ordinal', target: 'la primera cuenta visible del listado', sensitive: false, replay: replayStep_7 }], lastSelectionStep: { stepIndex: 7, selectedTarget: 'la primera cuenta visible del listado' }, lastSelectionReplay: async () => { await productListPage.selectVisibleItemByOrdinal('first'); }, expectedOwnerPage: 'ProductDetailPage', action: async () => { await productDetailPage.clickPrimaryAction('Continuar'); } });
  } finally {

...
test('Visualizar el detalle de Cuentas de Efectivo', async ({ page }) => {
  ...
  process.env.SCENARIO_TITLE = 'Visualizar el detalle de Cuentas de Efectivo';
  ...
  const replayStep_5 = async () => { await productListPage.selectProduct('Cuentas de Efectivo'); };
  const replayStep_6 = async () => { await productListPage.selectVisibleItemByOrdinal('first', 'producto'); };
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 5, target: 'Cuentas de Efectivo', actionIntent: 'select_category', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Información de productos', sensitive: false, replay: replayStep_4 }], lastSelectionStep: { stepIndex: 4, selectedTarget: 'Información de productos' },  expectedOwnerPage: 'CategoryPage', action: async () => { await categoryPage.selectCategory('Cuentas de Efectivo'); } });
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'Seleccionar el primer producto visible del listado.', actionIntent: 'select_visible_item_by_ordinal', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Información de productos', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Cuentas de Efectivo', sensitive: false, replay: replayStep_5 }], action: async () => { await productListPage.selectVisibleItemByOrdinal("first", "producto"); } });
  await promotedRuntime.expectPromotedVisible({ stepIndex: 7, target: 'Beneficios', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Beneficios]
  await promotedRuntime.expectPromotedVisible({ stepIndex: 8, target: 'Tasas', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Tasas]
  await promotedRuntime.expectPromotedVisible({ stepIndex: 9, target: 'Información legal o notas aclaratorias', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Información legal o notas aclaratorias]
  } finally {

...
test('Imprimir balance del préstamo', async ({ page }) => {
  ...
  process.env.SCENARIO_TITLE = 'Imprimir balance del préstamo';
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 7, target: 'Seleccionar el primer préstamo visible del listado.', actionIntent: 'select_visible_item_by_ordinal', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'transacciones y servicios', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Consulta de balance', sensitive: false, replay: replayStep_5 }, { stepIndex: 6, actionIntent: 'select_product', target: 'Préstamos', sensitive: false, replay: replayStep_6 }], action: async () => { await productListPage.selectVisibleItemByOrdinal("first"); } });
  await promotedRuntime.clickPromotedTarget({ stepIndex: 8, target: 'Imprimir', actionIntent: 'select_product', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'transacciones y servicios', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Consulta de balance', sensitive: false, replay: replayStep_5 }, { stepIndex: 6, actionIntent: 'select_product', target: 'Préstamos', sensitive: false, replay: replayStep_6 }, { stepIndex: 7, actionIntent: 'select_visible_item_by_ordinal', target: 'el primer préstamo visible del listado', sensitive: false, replay: replayStep_7 }], lastSelectionStep: { stepIndex: 7, selectedTarget: 'el primer préstamo visible del listado' },  expectedOwnerPage: 'ProductListPage', action: async () => { await productListPage.selectProduct('Imprimir'); } });
  } finally {

...
test('Volver al listado desde el detalle de producto', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'el primer depósito visible del listado', actionIntent: 'select_visible_item_by_ordinal', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'start_session', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'open_product_information', target: 'Información de productos', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Depósitos a plazo', sensitive: false, replay: replayStep_5 }], action: async () => { await productListPage.selectVisibleItemByOrdinal("first"); } });
  await promotedRuntime.clickPromotedTarget({ stepIndex: 7, target: 'Volver al listado de productos', actionIntent: 'return_to_list', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'start_session', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'open_product_information', target: 'Información de productos', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Depósitos a plazo', sensitive: false, replay: replayStep_5 }], lastSelectionStep: { stepIndex: 6, selectedTarget: 'Depósitos a plazo en Pesos' }, lastSelectionReplay: async () => { await productListPage.selectVisibleItemByOrdinal('first'); }, expectedOwnerPage: 'GenericPage', action: async () => { await productDetailPage.backToList(); } }); // [target: Volver al listado de productos]
});
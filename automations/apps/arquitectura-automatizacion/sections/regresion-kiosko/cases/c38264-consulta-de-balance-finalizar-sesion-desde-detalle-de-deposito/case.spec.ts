...
test('Consulta de balance - Finalizar sesión desde detalle de depósito', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 9, target: 'Finalizar sesión', actionIntent: 'click_primary_action', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'start_session', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'open_module', target: 'Transacciones y servicios', sensitive: false, replay: replayStep_4 }, { stepIndex: 7, actionIntent: 'select_product', target: 'Depósitos a plazos', sensitive: false, replay: replayStep_7 }, { stepIndex: 8, actionIntent: 'select_visible_item_by_ordinal', target: 'el primer depósito visible del listado', sensitive: false, replay: replayStep_8 }], lastSelectionStep: { stepIndex: 8, selectedTarget: 'el primer depósito visible del listado' }, lastSelectionReplay: async () => { await productListPage.selectVisibleItemByOrdinal('first'); }, expectedOwnerPage: 'ProductListPage', action: async () => { await productDetailPage.clickPrimaryAction('Finalizar sesión'); } });
});
...
test('TOKEN COP', async ({ page }) => {
  ...
  try {
    ...
    await pageObject.click({
      ...
      associatedField: 'SMS',
      previousStepReplays: [{ stepIndex: 1, actionIntent: 'restore_recorded_context', target: 'css:[href="#empresarial"]', sensitive: false, replay: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('css:[href="#empresarial"]')!)!.click(); } }, { stepIndex: 2, actionIntent: 'restore_recorded_context', target: 'role:textbox|RNC:', sensitive: false, replay: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('role:textbox|RNC:')!)!.fill(String(process.env['PROMOTED_RNC'] ?? '')); } }, { stepIndex: 3, actionIntent: 'restore_recorded_context', target: 'role:textbox|Usuario:', sensitive: false, replay: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('role:textbox|Usuario:')!)!.fill(String(process.env['PROMOTED_USUARIO'] ?? '')); } }, { stepIndex: 4, actionIntent: 'restore_recorded_context', target: 'role:textbox|Contraseña:', sensitive: true, replay: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('role:textbox|Contraseña:')!)!.fill(String(process.env['PROMOTED_CONTRASENA'] ?? '')); } }, { stepIndex: 5, actionIntent: 'restore_recorded_context', target: 'role:textbox|Contraseña:', sensitive: false, replay: async () => { await pageObject.press({ stepIndex: 5, target: 'role:textbox|Contraseña:', key: 'Enter' }); } }],
      action: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('text:SMS')!)!.click(); }
    });
    await pageObject.fill({
      stepIndex: 7,
      target: 'Campo pendiente de identificar',
      value: String(process.env['PROMOTED_UNRESOLVED_VALUE_15'] ?? ''),
      valueKey: 'unresolved_value_15',
      fill: async () => { await page.getByText('Campo pendiente de identificar').fill(String(process.env['PROMOTED_UNRESOLVED_VALUE_15'] ?? '')); }
    });
    await pageObject.click({
      stepIndex: 8,
      ...
      previousStepReplays: [{ stepIndex: 1, actionIntent: 'restore_recorded_context', target: 'css:[href="#empresarial"]', sensitive: false, replay: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('css:[href="#empresarial"]')!)!.click(); } }, { stepIndex: 2, actionIntent: 'restore_recorded_context', target: 'role:textbox|RNC:', sensitive: false, replay: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('role:textbox|RNC:')!)!.fill(String(process.env['PROMOTED_RNC'] ?? '')); } }, { stepIndex: 3, actionIntent: 'restore_recorded_context', target: 'role:textbox|Usuario:', sensitive: false, replay: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('role:textbox|Usuario:')!)!.fill(String(process.env['PROMOTED_USUARIO'] ?? '')); } }, { stepIndex: 4, actionIntent: 'restore_recorded_context', target: 'role:textbox|Contraseña:', sensitive: true, replay: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('role:textbox|Contraseña:')!)!.fill(String(process.env['PROMOTED_CONTRASENA'] ?? '')); } }, { stepIndex: 5, actionIntent: 'restore_recorded_context', target: 'role:textbox|Contraseña:', sensitive: false, replay: async () => { await pageObject.press({ stepIndex: 5, target: 'role:textbox|Contraseña:', key: 'Enter' }); } }, { stepIndex: 6, actionIntent: 'restore_recorded_context', target: 'text:SMS', sensitive: false, replay: async () => { await recordedLocatorFactory(page, parseSerializedTechnicalTargetString('text:SMS')!)!.click(); } }, { stepIndex: 7, actionIntent: 'restore_recorded_context', target: 'Campo pendiente de identificar', sensitive: false, replay: async () => { await page.getByText('Campo pendiente de identificar').fill(String(process.env['PROMOTED_UNRESOLVED_VALUE_15'] ?? '')); } }],
    ...
    await promotedRuntime.expectPromotedVisible({
      ...
      target: 'Continuar',
      polarity: "positive",
      expectedUrl: 'https://qa1.santacruz.do/onlinebanking/IdentifyUser/Verify',
      assertion: async () => { await expect(page).toHaveURL("https://qa1.santacruz.do/onlinebanking/IdentifyUser/Verify"); }
    });
  } finally {

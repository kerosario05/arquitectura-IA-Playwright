...
test('Visualizar detalle informativo de cuenta de ahorro en Pesos', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 7, target: 'la primera cuenta visible del listado', actionIntent: 'select_visible_item_by_ordinal', expectedEffect: 'ui_change', sensitive: false, action: async () => { await productListPage.selectVisibleItemByOrdinal("first", "cuenta"); } });
});
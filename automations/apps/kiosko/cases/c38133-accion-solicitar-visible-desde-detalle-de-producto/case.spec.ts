...
test('Acción Solicitar visible desde detalle de producto', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 7, target: 'la primera tarjeta visible del listado', actionIntent: 'select_visible_item_by_ordinal', expectedEffect: 'ui_change', sensitive: false, action: async () => { await productListPage.selectVisibleItemByOrdinal("first", "tarjeta"); } });
  await promotedRuntime.clickPromotedTarget({ stepIndex: 8, target: 'Solicitar', actionIntent: 'click_primary_action', expectedEffect: 'ui_change', sensitive: false, action: async () => { await productDetailPage.clickPrimaryAction('Solicitar'); } });
});
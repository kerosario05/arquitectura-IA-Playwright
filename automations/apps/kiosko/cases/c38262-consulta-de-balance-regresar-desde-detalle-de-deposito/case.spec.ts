...
test('Consulta de balance - Regresar desde detalle de depósito', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 9, target: 'Volver al Listado', actionIntent: 'return_to_list', expectedEffect: 'ui_change', sensitive: false, action: async () => { await productDetailPage.backToList(); } }); // [target: Volver al Listado]
});
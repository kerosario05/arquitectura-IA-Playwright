...
test('Regresar al listado desde detalle de producto', async ({ page }) => {
  ...
  const replayStep_5 = async () => { await productListPage.selectProduct('Tarjetas de crédito'); };
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 5, target: 'Tarjetas de crédito', actionIntent: 'select_category', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Información de productos', sensitive: false, replay: replayStep_4 }], lastSelectionStep: { stepIndex: 4, selectedTarget: 'Información de productos' },  expectedOwnerPage: 'CategoryPage', action: async () => { await categoryPage.selectCategory('Tarjetas de crédito'); } });
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'Seleccionar el primer producto visible del listado.', actionIntent: 'select_visible_item_by_ordinal', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Información de productos', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Tarjetas de crédito', sensitive: false, replay: replayStep_5 }], action: async () => { await productListPage.selectVisibleItemByOrdinal("first", "producto"); } });
  await promotedRuntime.clickPromotedTarget({ stepIndex: 7, target: 'Volver', actionIntent: 'return_to_list', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Información de productos', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Tarjetas de crédito', sensitive: false, replay: replayStep_5 }], lastSelectionStep: { stepIndex: 6, selectedTarget: 'el primer producto visible del listado' }, lastSelectionReplay: async () => { await productListPage.selectVisibleItemByOrdinal('first', 'producto'); }, expectedOwnerPage: 'GenericPage', action: async () => { await productListPage.backToList(); } }); // [target: Volver]
});
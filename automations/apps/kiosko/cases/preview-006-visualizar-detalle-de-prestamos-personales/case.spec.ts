...
test('Visualizar detalle de Préstamos personales', async ({ page }) => {
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'Préstamos personales', actionIntent: 'select_category', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Información de productos', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Préstamos', sensitive: false, replay: replayStep_5 }], lastSelectionStep: { stepIndex: 4, selectedTarget: 'Información de productos' },  expectedOwnerPage: 'CategoryPage', action: async () => { await categoryPage.selectCategory('Préstamos personales'); } });
});
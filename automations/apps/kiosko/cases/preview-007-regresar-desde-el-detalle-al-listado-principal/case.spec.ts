...
test('Regresar desde el detalle al listado principal', async ({ page }) => {
  ...
  const replayStep_6 = async () => { await productListPage.selectProduct('Volver'); };
  ...
  await promotedRuntime.clickPromotedTarget({ stepIndex: 6, target: 'Volver', actionIntent: 'return_to_list', expectedEffect: 'ui_change', sensitive: false, previousStepReplays: [{ stepIndex: 3, actionIntent: 'select_product', target: 'Iniciar', sensitive: false, replay: replayStep_3 }, { stepIndex: 4, actionIntent: 'select_product', target: 'Información de productos', sensitive: false, replay: replayStep_4 }, { stepIndex: 5, actionIntent: 'select_product', target: 'Depósitos a Plazo', sensitive: false, replay: replayStep_5 }], lastSelectionStep: undefined,  expectedOwnerPage: 'GenericPage', action: async () => { await productListPage.backToList(); } }); // [target: Volver]
  await promotedRuntime.expectPromotedVisible({ stepIndex: 7, target: 'Tarjetas de crédito', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Tarjetas de crédito]
  await promotedRuntime.expectPromotedVisible({ stepIndex: 8, target: 'Depósitos a Plazo', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Depósitos a Plazo]
  await promotedRuntime.expectPromotedVisible({ stepIndex: 9, target: 'Cuentas de Efectivo', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Cuentas de Efectivo]
  await promotedRuntime.expectPromotedVisible({ stepIndex: 10, target: 'Préstamos', assertion: async () => { await productDetailPage.expectLoaded(); } }); // [target: Préstamos]
});
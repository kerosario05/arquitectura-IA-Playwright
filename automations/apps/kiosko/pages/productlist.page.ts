...
export class ProductListPage {
  ...
  async selectProduct(productName: string): Promise<void> {
    ...
    const diagnostics = await capturePageDiagnostics(this.page);
    const errorMsg = buildListReadinessErrorMessage(productName, diagnostics, tokensToMatch, candidates);
  ...
  private async tryNavigateSubcategory(tokens: string[]): Promise<boolean> {
    const normalize = (s: string) => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
    ...
    if (!hasSubcategoryHeading) {
      ...
      if (buttons.length >= 3) {
        ...
        if (hasModuleButton) return false;
      }
    }
    ...
    let bestButton: { element: any; score: number } | null = null;
    ...
    for (const btn of buttons) {
      try {
        ...
        for (const token of tokens) {
          if (normalizedText.includes(token)) matchCount++;
        }
  ...
  private async trySelectCheckbox(tokens: string[], originalName: string): Promise<boolean> {
    const normalize = (s: string) => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
  ...
  private async detectMultiSelectScreen(): Promise<boolean> {
    const continuarButton = this.page.getByRole('button', { name: /continuar.*\d+\s*producto/i, exact: false });
    if (await continuarButton.count() > 0) return true;
  ...
  private async tryMultiSelect(tokens: string[], originalName: string): Promise<boolean> {
    const normalize = (s: string) => s.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
    ...
    for (const el of allVisible.slice(0, 200)) {
      try {
        ...
        if (matchCount < tokens.length) continue;
  ...
  private buildFlexibleRegex(tokens: string[]): RegExp {
    const escaped = tokens.map(t => t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
    return new RegExp(escaped.join('.*'), 'i');
  }

  private buildTokenSetRegex(tokens: string[]): RegExp {
    const escaped = tokens.map(t => t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
    return new RegExp(escaped.map(t => `(?=.*${t})`).join(''), 'i');
  }

  private async scanVisibleCandidates(tokens: string[]): Promise<VisibleCandidate[]> {
    const results = await this.page.evaluate((tokensToMatch) => {
      ...
      const hasCardClass = (el: HTMLElement): boolean => {
        const className = el.className || '';
        return typeof className === 'string' && clickableClassPatterns.some(p => className.includes(p));
      };

      const findClickableAncestor = (el: HTMLElement): HTMLElement | null => {
      ...
      const candidates: Array<{ text: string; normalizedText: string; matchedTokens: string[]; unmatchedTokens: string[]; matchScore: number; clickable: boolean; tag: string; role: string | null; hasCardClass: boolean; elementIndex: number }> = [];
      ...
      for (let i = 0; i < allElements.length; i++) {
        ...
        if (seen.has(normalized.substring(0, 50))) continue;
        ...
        let clickableEl: HTMLElement | null = el;
      ...
      candidates.sort((a, b) => {
        ...
        if (b.clickable !== a.clickable) return (b.clickable ? 1 : 0) - (a.clickable ? 1 : 0);
        if (b.hasCardClass !== a.hasCardClass) return (b.hasCardClass ? 1 : 0) - (a.hasCardClass ? 1 : 0);
  ...
  async selectVisibleItemByOrdinal(ordinal: 'first' | 'second' | 'third', domainTerm?: string): Promise<void> {
    ...
    const allItems = this.page.locator('article:visible, [role="listitem"]:visible, [class*="product"]:visible, [class*="card"]:visible, [class*="item"]:visible').filter({ hasNotText: /selecciona un producto|seleccione un producto|elige un producto|choose a product|select a product/i });
    let items = allItems;
    if (domainTerm) {
      const escaped = domainTerm.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const domainFiltered = items.filter({ hasText: new RegExp(escaped, 'i') });
      if (await domainFiltered.count() > 0) {

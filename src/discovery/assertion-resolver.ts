...
export type AssertionTargetInput = {
  ...
  /** Runtime-only lineage from an executed fill control; never persisted as narrative. */
  subjectControlIdentity?: string;
  requiredContext?: {
...
export type RowScopedAssertionOptions = {
  expectedValue?: string;
  expectedValueSource?: string;
  expectedValueVerified?: boolean;
};

export type EntityWithinContainerAssertionOptions = {
  entityValue?: string;
...
export async function resolveStructuralRowAssertion(
  page: Page,
  input: AssertionTargetInput,
): Promise<AssertionResolutionResult> {
  const base = {
    assertionText: input.target,
    normalizedAssertion: normalizeText(input.target),
    classification: "structural_assertion" as const,
    closestCandidates: [],
    visibleTexts: [],
    assertionType: "field" as const,
  };
  ...
  for (let index = 0; index < containerCount; index += 1) {
    ...
    for (let rowIndex = 0; rowIndex < rowCount; rowIndex += 1) {
      if (await rows.nth(rowIndex).locator("td, th, [role='gridcell']").count().catch(() => 0) > 0) dataRows += 1;
    }
...
export async function resolveEntityWithinContainerAssertion(
  page: Page,
  input: AssertionTargetInput,
  options: EntityWithinContainerAssertionOptions = {},
): Promise<AssertionResolutionResult> {
  const base = {
    assertionText: input.target,
    normalizedAssertion: normalizeText(input.target),
...
function normalizeAssertionCellValue(value: string): string {
  return normalizeText(value).replace(/\s+/g, " ").trim();
}

function sameSemanticDate(left: string, right: string): boolean {
...
function assertionFieldLabel(target: string): string | undefined {
...
function rowHeaderMatches(fieldLabel: string, headerText: string): boolean {
  const fieldTokens = normalizeText(fieldLabel).split(/\s+/).filter((token) => !["el", "la", "los", "las", "de", "del", "en", "campo", "valor", "autocompletado", "autocompletada"].includes(token));
  const headerTokens = normalizeText(headerText).split(/\s+/).filter(Boolean);
 ...
 */
export async function resolveRowScopedAssertion(
  page: Page,
  input: AssertionTargetInput,
  options: RowScopedAssertionOptions = {},
): Promise<AssertionResolutionResult> {
  const base = {
    assertionText: input.target,
    normalizedAssertion: normalizeText(input.target),
  ...
  if (!fieldLabel) return { ...base, status: "needs_assertion_resolution", confidence: 0, reason: "row_assertion_subject_missing" };
  const containers = page.locator("table, [role='grid']");
  const containerCount = await containers.count().catch(() => 0);
  for (let containerIndex = 0; containerIndex < containerCount; containerIndex += 1) {
    ...
    for (let index = 0; index < rowCount; index += 1) {
      ...
      if (await row.locator("td, th, [role='gridcell']").count().catch(() => 0) > 0) dataRows.push(row);
    }

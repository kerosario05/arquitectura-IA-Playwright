...
export async function startScenarioPreviewRun(jobId: string): Promise<void> {
  ...
  const completedCount = currentSummary?.completed ?? 0;
    ...
    // Defect content builders (buildStructuredDefectDescription, buildDefectTitle,
    // inferDefectSeverity, mapReasonCodeToHuman, sanitizeRawError…) now live in the
    // shared module ../services/defect-content-builder so web and mobile stay consistent.
            ...
            for (const fc of failedCases) {
              ...
              if (existingDefect) {
                ...
                if (fc.failureReason && fc.failureReason.length > 10) {
                  ...
                  existingDefect.title = buildDefectTitle({ scenarioId: fc.id, scenarioTitle, severity: sv.severity, technicalContext, failureReason: fc.failureReason, expectedResult });
                }
              ...
              const sv = inferDefectSeverity(fc.failureReason, fc.id, scenarioTitle);
              const defectTitle = buildDefectTitle({ scenarioId: fc.id, scenarioTitle, severity: sv.severity, technicalContext, failureReason: fc.failureReason, expectedResult });
              ...
              jobStore.appendLog(jobId, `[defect-title] scenarioId=${fc.id} title="${defectTitle}"`);
              jobStore.appendLog(jobId, `[defect-technical-context] scenarioId=${fc.id} reasonCode=${fc.failedReason ?? 'none'} failedAtStep=${fc.failedAtStep ?? 'none'} lastSuccessfulStep=${lastSuccessful?.stepIndex ?? 'none'} evidence=${technicalContext.evidenceDir != null} testRailCaseId=${testRailCaseId ?? 'none'} testRailRunId=${testRunId ?? 'none'}`);

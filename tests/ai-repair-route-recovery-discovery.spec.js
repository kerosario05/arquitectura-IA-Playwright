...
(0, test_1.test)("discovery invokes route_recovery when local route resolution fails", async () => {
    ...
    (0, test_1.expect)(result.decision?.repairType).toBe("route_recovery");
    (0, test_1.expect)(result.decision?.candidateId).toBe("nav-products");
    (0, test_1.expect)(result.diagnostics.failureType).toBe("wrong_screen");
});
(0, test_1.test)("discovery does not invoke route_recovery when direct hit resolves", async () => {
    ...
    (0, test_1.expect)(localResolutionSuccess).toBe(true);
    // AI Repair would NOT be called in this scenario
});
(0, test_1.test)("repaired_plan route_recovery navigates/changes screen", async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async () => new Response(JSON.stringify({
        choices: [{
                message: {
...
(0, test_1.test)("candidate without transition is marked as failedRoutePath", async () => {
    ...
    const result1 = await withEnv(FAKE_AI_ENV, async () => await (0, ai_repair_orchestrator_1.runAiRepairOrchestrator)({
        appSlug: "test-app",
        failure: "wrong_screen",
        failureType: "wrong_screen",
        currentStep: "navigate",
    ...
    globalThis.fetch = originalFetch;
    (0, test_1.expect)(result1.status).toBe("repaired_plan");
    (0, test_1.expect)(result1.decision?.candidateId).toBe("nav-products");
    ...
    const result2 = await withEnv(FAKE_AI_ENV, async () => await (0, ai_repair_orchestrator_1.runAiRepairOrchestrator)({
        appSlug: "test-app",
        failure: "wrong_screen",
        failureType: "wrong_screen",
        currentStep: "navigate",
    ...
    (0, test_1.expect)(result2.status).toBe("invalid_response");
    (0, test_1.expect)(result2.diagnostics.errorCode).toBe("AI_REPAIR_ROUTE_CANDIDATE_ALREADY_FAILED");
});
(0, test_1.test)("loop prevention avoids repeating candidateId", async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async () => new Response(JSON.stringify({
        choices: [{
                message: {
    ...
    (0, test_1.expect)(result.diagnostics.errorCode).toBe("AI_REPAIR_ROUTE_CANDIDATE_ALREADY_FAILED");
});
(0, test_1.test)("no_safe_action route_recovery is diagnosed", async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async () => new Response(JSON.stringify({
        choices: [{
                message: {
    ...
    (0, test_1.expect)(result.decision?.repairType).toBe("route_recovery");
    (0, test_1.expect)(result.diagnostics.failureType).toBe("navigation_dead_end");
});
(0, test_1.test)("sensitive candidate blocked in route_recovery", async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async () => new Response(JSON.stringify({
        choices: [{
                message: {
...
(0, test_1.test)("metrics count route_recovery repairType", async () => {
    ...
    const summary = (0, ai_repair_summary_builder_1.buildAiRepairCaseSummary)(stepsWithAiRepair);
    ...
    (0, test_1.expect)(summary.repairTypeCounts.target_resolution).toBe(0);
    (0, test_1.expect)(summary.appliedRepairs).toBe(1);
});
(0, test_1.test)("diagnostics persist with repairType route_recovery", async () => {
    const stepsWithAiRepair = [
        {
            index: 1,
            targetText: "Navigate to Products",
    ...
    const summary = (0, ai_repair_summary_builder_1.buildAiRepairCaseSummary)(stepsWithAiRepair);
    (0, test_1.expect)(summary.invocations).toBe(2);
    (0, test_1.expect)(summary.repairTypeCounts.route_recovery).toBe(1);
    (0, test_1.expect)(summary.repairTypeCounts.target_resolution).toBe(1);
    (0, test_1.expect)(summary.targets).toHaveLength(2);
    (0, test_1.expect)(summary.targets[0].repairType).toBe("route_recovery");
    (0, test_1.expect)(summary.targets[1].repairType).toBe("target_resolution");
});

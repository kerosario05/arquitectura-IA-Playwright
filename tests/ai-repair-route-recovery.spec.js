...
(0, test_1.test)("schema accepts route_recovery with valid candidateId", () => {
    const decision = {
    ...
    if (result.valid) {
        ...
        (0, test_1.expect)(result.decision.candidateId).toBe("link-products");
    }
});
(0, test_1.test)("validator accepts route_recovery candidate visible/clickable/not sensitive", () => {
    const decision = {
        decision: "repaired_plan",
        repairType: "route_recovery",
        candidateId: "link-products",
...
(0, test_1.test)("validator blocks sensitive candidate for route_recovery", () => {
    const decision = {
        decision: "repaired_plan",
        repairType: "route_recovery",
        candidateId: "btn-login",
...
(0, test_1.test)("validator blocks candidate already in failedRoutePaths", () => {
    const decision = {
        decision: "repaired_plan",
        repairType: "route_recovery",
        candidateId: "link-products",
    ...
    (0, test_1.expect)(result.valid).toBe(false);
    if (!result.valid) {
        (0, test_1.expect)(result.code).toBe("AI_REPAIR_ROUTE_CANDIDATE_ALREADY_FAILED");
    }
});
(0, test_1.test)("validator blocks invented selector in route_recovery", () => {
    const decision = {
        decision: "repaired_plan",
        repairType: "route_recovery",
        candidateId: "link-products",
...
(0, test_1.test)("context-pack route_recovery does not include secrets", () => {
    ...
    (0, test_1.expect)(serialized.toLowerCase()).not.toContain("password=");
    (0, test_1.expect)(serialized.toLowerCase()).not.toContain("otp=");
    (0, test_1.expect)(serialized.toLowerCase()).not.toContain("token=");
    (0, test_1.expect)(serialized).toContain("[REDACTED]");
});
(0, test_1.test)("orchestrator returns repaired_plan route_recovery valid", async () => {
...
(0, test_1.test)("orchestrator blocks invented selector in route_recovery", async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async () => new Response(JSON.stringify({
        choices: [{
                message: {
...
(0, test_1.test)("metrics track repairType route_recovery", () => {
    ...
    (0, test_1.expect)(decisionCounts.repaired_plan).toBe(1);
    (0, test_1.expect)(repairTypeCounts.route_recovery).toBe(1);
});
(0, test_1.test)("no_safe_action for route_recovery when no candidates", async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async () => new Response(JSON.stringify({
        choices: [{
                message: {
...
(0, test_1.test)("needs_more_context for route_recovery when screen lacks nav", async () => {
    const originalFetch = globalThis.fetch;
    globalThis.fetch = (async () => new Response(JSON.stringify({
        choices: [{
                message: {
    ...
    globalThis.fetch = originalFetch;
    (0, test_1.expect)(result.status).toBe("needs_more_context");
    (0, test_1.expect)(result.decision?.repairType).toBe("route_recovery");
});

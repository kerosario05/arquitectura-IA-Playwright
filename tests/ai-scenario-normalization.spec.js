...
test_1.test.describe("AI Scenario Normalization", () => {
    (0, test_1.test)("normalizeAiScenario fixes invalid automationType", () => {
        ...
        (0, test_1.expect)(normalized.automationType).toBe("ui_with_controlled_data");
        // setupStrategy should be fixed
        (0, test_1.expect)(normalized.setupStrategy).toBe("controlled_data");
        // mcpExecutable should be fixed
        (0, test_1.expect)(normalized.mcpExecutable).toBe(true);
        // appSlug should be set
        (0, test_1.expect)(normalized.appSlug).toBe("test-app");
        // Missing fields should be added
        (0, test_1.expect)(normalized.preconditions).toEqual(["AuthGate"]);
        (0, test_1.expect)(normalized.caseOracle).toBe("assert_visible");
        (0, test_1.expect)(normalized.type).toBe("Automated");
        (0, test_1.expect)(normalized.database).toBe("");
        (0, test_1.expect)(normalized.isConverted).toBe(1);
        (0, test_1.expect)(normalized.dataRequirements).toBe("");
        (0, test_1.expect)(normalized.nonExecutableCriteria).toBe("");
    });
    (0, test_1.test)("normalizeAiScenario preserves valid fields", () => {
        function normalizeAiScenario(scenario, appSlug) {
            const normalized = { ...scenario };
            const validAutomationTypes = [
                "ui_discovery",
        ...
        (0, test_1.expect)(normalized.automationType).toBe("ui_with_auth_gate");
        (0, test_1.expect)(normalized.setupStrategy).toBe("auth_gate");
        (0, test_1.expect)(normalized.mcpExecutable).toBe(true);
        (0, test_1.expect)(normalized.appSlug).toBe("my-app");
        (0, test_1.expect)(normalized.preconditions).toEqual(["AuthGate", "CustomPrecondition"]);
        (0, test_1.expect)(normalized.caseOracle).toBe("custom_oracle");
    });
    (0, test_1.test)("AI scenarios with duplicate steps are deduplicated", () => {
        ...
        (0, test_1.expect)(dedupedSteps).toHaveLength(4);
        (0, test_1.expect)(dedupedSteps[0]).toContain("Iniciar");
        (0, test_1.expect)(dedupedSteps[1]).toContain("Información de productos");
        (0, test_1.expect)(dedupedSteps[2]).toContain("Tarjetas");
        (0, test_1.expect)(dedupedSteps[3]).toContain("Validar que se muestre");
    });
    (0, test_1.test)("state mapping tracks scenarios through all stages", () => {
        // Simulate the flow:
        ...
        (0, test_1.expect)(expectedPreviewValid).toBe(3);
        ...
        (0, test_1.expect)(totalAccountedFor).toBe(generationDiagnostics.aiGenerated);
    });
    (0, test_1.test)("fallback scenarios show as valid in preview", () => {
        ...
        (0, test_1.expect)(fallbackScenario.automationType).toBe("ui_with_controlled_data");
        (0, test_1.expect)(fallbackScenario.setupStrategy).toBe("ui_with_controlled_data");
        (0, test_1.expect)(fallbackScenario.mcpExecutable).toBe(true);
        (0, test_1.expect)(fallbackScenario.caseOracle).toBe("assert_visible");
        // Preview should show valid=1
        (0, test_1.expect)(generationDiagnostics.finalValid).toBe(1);
    });
});

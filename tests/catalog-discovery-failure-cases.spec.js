...
test_1.test.describe("Catalog Discovery Failure Cases", () => {
    (0, test_1.test)("navigation failure returns empty products array", () => {
        ...
        const expectedResult = {
            ...
            capturedAt: test_1.expect.any(String),
            products: [],
            categories: [],
            totalProducts: 0,
        ...
        (0, test_1.expect)(expectedResult.products).toHaveLength(0);
        (0, test_1.expect)(expectedResult.representativeProducts).toHaveLength(0);
        (0, test_1.expect)(expectedResult.categories).toHaveLength(0);
    });
    (0, test_1.test)("false positive greetings are filtered", () => {
        // Mock isValidProductLabel function
        function isValidProductLabel(label) {
        ...
        for (const greeting of greetings) {
            (0, test_1.expect)(isValidProductLabel(greeting)).toBe(false);
        }
        // Valid product labels should pass
        (0, test_1.expect)(isValidProductLabel("Cuenta de Ahorros")).toBe(true);
        (0, test_1.expect)(isValidProductLabel("Tarjeta de Crédito")).toBe(true);
    });
    (0, test_1.test)("false positive generic labels are filtered", () => {
        function isValidProductLabel(label) {
            const trimmed = label.trim();
            if (trimmed.length < 3)
                return false;
        ...
        for (const generic of genericLabels) {
            (0, test_1.expect)(isValidProductLabel(generic)).toBe(false);
        }
        // Specific product names should pass
        (0, test_1.expect)(isValidProductLabel("Producto Premium Plus")).toBe(true);
    });
    (0, test_1.test)("false positive short headings are filtered", () => {
        function isValidProductLabel(label) {
            const trimmed = label.trim();
            if (trimmed.length < 3)
                return false;
        ...
        (0, test_1.expect)(isValidProductLabel("Inicio")).toBe(false);
        (0, test_1.expect)(isValidProductLabel("Tarjetas")).toBe(false); // Changed from true to false - single word headings
        (0, test_1.expect)(isValidProductLabel("Menú")).toBe(false);
        // Multi-word or longer labels should pass
        (0, test_1.expect)(isValidProductLabel("Tarjetas de Crédito")).toBe(true);
        (0, test_1.expect)(isValidProductLabel("Cuenta123")).toBe(true); // Has number
    });
    (0, test_1.test)("catalog validation requires at least 2 products", () => {
        // Mock validateCatalogScreen function
        function validateCatalogScreen(products, containers) {
        ...
        (0, test_1.expect)(validateCatalogScreen(oneProduct, oneContainer)).toBe(false);
        ...
        (0, test_1.expect)(validateCatalogScreen(twoProducts, twoContainers)).toBe(true);
    });
    (0, test_1.test)("entrySteps executed before entry navigation", () => {
        ...
        (0, test_1.expect)(steps[0].target).toBe("Iniciar");
        (0, test_1.expect)(steps[0].description).toContain("Entry step");
        (0, test_1.expect)(steps[1].target).toBe("Información de productos");
        (0, test_1.expect)(steps[1].description).toContain("Navigate to");
    });
    (0, test_1.test)("persistence skips when no products discovered", () => {
        // Mock persistDiscoveredProducts behavior
        function shouldPersist(discoveryResult) {
        ...
        (0, test_1.expect)(shouldPersist(emptyResult)).toBe(false);
        ...
        (0, test_1.expect)(shouldPersist(validResult)).toBe(true);
    });
    (0, test_1.test)("navigation returns false on first failure", () => {
        // Mock navigateToCatalogSection behavior
        async function navigateToCatalogSection(steps) {

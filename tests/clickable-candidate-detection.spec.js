...
test_1.test.describe("Clickable Candidate Detection", () => {
    (0, test_1.test)("heading without interactivity not classified as candidate", () => {
        ...
        (0, test_1.expect)(result.hasEvidence).toBe(false);
        (0, test_1.expect)(result.rejectedReason).toBe("heading_not_clickable");
        // Test button acceptance
        const button = { text: "Ver más", tagName: "button" };
        const buttonResult = detectClickableEvidence(button);
        (0, test_1.expect)(buttonResult.hasEvidence).toBe(true);
    });
    (0, test_1.test)("card with clickable button classified correctly", () => {
        ...
        (0, test_1.expect)(cardWithButton.hasInternalClickable).toBe(true);
    });
    (0, test_1.test)("candidate with role link uses role=link not button", () => {
        // Mock buildLocatorStrategy
        function buildLocatorStrategy(el, label) {
            const role = el.role?.toLowerCase();
            const tag = el.tagName?.toLowerCase();
        ...
        (0, test_1.expect)(linkStrategy.type).toBe("role");
        (0, test_1.expect)(linkStrategy.role).toBe("link");
        // Test button element
        ...
        (0, test_1.expect)(buttonStrategy.type).toBe("role");
        (0, test_1.expect)(buttonStrategy.role).toBe("button");
    });
    (0, test_1.test)("candidate with button internal uses containerText strategy", () => {
        ...
        (0, test_1.expect)(strategy.type).toBe("containerText");
        (0, test_1.expect)(strategy.text).toBe("Producto A");
    });
    (0, test_1.test)("clickCandidate skips if locator count is zero", () => {
        // Mock click attempt
        async function clickCandidate(locatorCount) {
            if (locatorCount === 0) {
        ...
        clickCandidate(0).then((result) => {
            (0, test_1.expect)(result).toBe(false);
        });
        // Test valid count
        clickCandidate(1).then((result) => {
            (0, test_1.expect)(result).toBe(true);
        });
    });
    (0, test_1.test)("clickCandidate skips if element not visible", () => {
        // Mock click validation
        async function clickCandidate(isVisible) {
            if (!isVisible) {
    ...
    (0, test_1.test)("backtracking reinitializes if browser back exits catalog", () => {
        // Mock tryBackNavigation
        async function tryBackNavigation(urlAfterBack, catalogUrl) {
        ...
        tryBackNavigation("https://example.com/success", "https://example.com/catalog").then((result) => {
            (0, test_1.expect)(result).toBe(true); // Should re-navigate
        });
    ...
    (0, test_1.test)("no clickable candidates returns empty with debug log", () => {
        ...
        (0, test_1.expect)(candidates).toHaveLength(0);
        (0, test_1.expect)(rejected.length).toBeGreaterThan(0);
    });
    (0, test_1.test)("solution has no hardcoded app-specific terms", () => {
        ...
        (0, test_1.expect)(detailPatterns[0].test("Detalles")).toBe(true);
        (0, test_1.expect)(detailPatterns[1].test("Requisitos")).toBe(true);
    });
    (0, test_1.test)("locatorStrategy includes type and role", () => {
        ...
        (0, test_1.expect)(candidate.locatorStrategy.type).toBe("role");
        (0, test_1.expect)(candidate.locatorStrategy.role).toBe("button");
        (0, test_1.expect)(candidate.locatorStrategy.text).toBe("Tarjetas");
    });
    (0, test_1.test)("clickableEvidence captures interactive attributes", () => {
        ...
        (0, test_1.expect)(candidate.clickableEvidence.hasRole).toBe(true);
        (0, test_1.expect)(candidate.clickableEvidence.hasClickableTag).toBe(true);
    });
    (0, test_1.test)("container with heading and button children creates candidates from buttons not heading", () => {
        // Mock extractClickableElements and extractLabelFromClickable
        function extractClickableElements(elements) {
            return elements.filter((el) => el.tagName === "button" || el.role === "button");
        }
        function extractLabelFromClickable(el) {
            if (el.name)
                return { label: el.name, source: "accessible_name" };
            if (el.text)
                return { label: el.text, source: "element_text" };
        ...
        (0, test_1.expect)(clickables).toHaveLength(2);
        // Labels should be from buttons, not heading
        (0, test_1.expect)(labels).toEqual(["Cuentas", "Tarjetas"]);
        (0, test_1.expect)(labels).not.toContain("Conoce nuestros productos");
    });
    (0, test_1.test)("clickable uses accessible name not container heading", () => {
        ...
        (0, test_1.expect)(result.label).toBe("Tarjeta Classic");
        (0, test_1.expect)(result.source).toBe("accessible_name");
    });
    (0, test_1.test)("heading without clickable evidence is rejected", () => {
    ...
    (0, test_1.test)("buildLocatorStrategy uses role and name from clickable element", () => {
        // Mock buildLocatorStrategy
        function buildLocatorStrategy(el, label) {
            const role = el.role?.toLowerCase();
            const tag = el.tagName?.toLowerCase();
        ...
        (0, test_1.expect)(strategy.type).toBe("role");
        (0, test_1.expect)(strategy.role).toBe("button");
        (0, test_1.expect)(strategy.text).toBe("Cuentas");
    });
    (0, test_1.test)("no clickable label candidate is rejected", () => {
        ...
        (0, test_1.expect)(result.label).toBeNull();
        (0, test_1.expect)(result.source).toBe("none");

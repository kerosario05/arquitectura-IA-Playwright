...
test_1.test.describe("Detail Evidence Gate Validation", () => {
    (0, test_1.test)("Evidence gate blocks when capturedAfterStep < finalProductClickStepIndex", async () => {
        // Simulate scenario:
        ...
        try {
            ...
            if (detailScreenshotRequired && found && capturedAfterStep < finalProductClickStepIndex) {
                ...
                console.log(`[evidence-gate] scenario=test detailScreenshotRequired=true found=true ` +
        ...
        (0, test_1.expect)(setIndexLog).toBeDefined();
        ...
        const gateBlockedLog = logs.find(l => l.includes("[evidence-gate]") &&
        ...
        (0, test_1.expect)(gateBlockedLog).toBeDefined();
    });
    (0, test_1.test)("Evidence gate passes when capturedAfterStep === finalProductClickStepIndex", async () => {
        const logs = [];
        const originalLog = console.log;
        console.log = (...args) => {
            logs.push(args.join(" "));
        ...
        // Verify logs
        const gatePassedLog = logs.find(l => l.includes("[evidence-gate]") &&
            l.includes("found=true") &&
            l.includes("capturedAfterStep=4") &&
            l.includes("finalProductClickStepIndex=4") &&
            !l.includes("statusOverride=failed"));
        (0, test_1.expect)(gatePassedLog).toBeDefined();
    });
    (0, test_1.test)("Visa Clásica: finalProductClickStepIndex must be 4, not 3", async () => {
        // Mock scenario steps
        const steps = [
        ...
        let detailTarget;
        let finalProductClickStepIndex;
        ...
        (0, test_1.expect)(detailTarget).toBe("Tarjeta Crédito Visa Clásica");
        (0, test_1.expect)(finalProductClickStepIndex).toBe(4); // Must be 4, not 3
        console.log(`✓ Visa Clásica: finalProductClickStepIndex=${finalProductClickStepIndex} (expected 4)`);
    });
    (0, test_1.test)("Cuenta Pesos: finalProductClickStepIndex must be 4, not 3", async () => {
        const steps = [
            { index: 1, target: "Iniciar", type: "action" },
            { index: 2, target: "Información de productos", type: "action" },
        ...
        (0, test_1.expect)(detailTarget).toBe("Cuenta de Ahorros Personal en Pesos");
        (0, test_1.expect)(finalProductClickStepIndex).toBe(4);
        console.log(`✓ Cuenta Pesos: finalProductClickStepIndex=${finalProductClickStepIndex} (expected 4)`);
    });
    (0, test_1.test)("Préstamo ordinal: capturedAfterStep must be 4 (ordinal step)", async () => {
        const steps = [
            { index: 1, target: "Iniciar", type: "action" },
            { index: 2, target: "Información de productos", type: "action" },
        ...
        const capturedAfterStep = 4;
        (0, test_1.expect)(capturedAfterStep).toBe(finalProductClickStepIndex);
        console.log(`✓ Préstamo ordinal: capturedAfterStep=${capturedAfterStep} finalProductClickStepIndex=${finalProductClickStepIndex}`);
    });
    (0, test_1.test)("DOCX should not use detail_loaded from intermediate step", async () => {

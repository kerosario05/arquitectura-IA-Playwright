...
test_1.test.describe("Detail Oracle Validation", () => {
    (0, test_1.test)("Visa no pasa si solo productName=true y detailSections=false", async () => {
        ...
        try {
            ...
            console.log(`[detail-oracle] target="${detailTarget}" ` +
            ...
            if (!detailOpened) {
                ...
                (0, test_1.expect)(reason).toBe("only_product_name_visible");
            }
    ...
    (0, test_1.test)("NUEVO: Detalle acepta con señales exclusivas sin transición URL/DOM (SPA navigation)", async () => {
        const logs = [];
        const originalLog = console.log;
        console.log = (...args) => {
            logs.push(args.join(" "));
        ...
        try {
            ...
            (0, test_1.expect)(strongDetailSignal).toBe(true);
            (0, test_1.expect)(navigationTransitionDetected).toBe(false); // No transition, but still accepted
            if (detailOpened) {
            ...
            const status = detailOpened ? "Exitoso" : "Fallido";
            (0, test_1.expect)(status).toBe("Exitoso");
        ...
        (0, test_1.expect)(capturedLog).toBeDefined();
    });
    (0, test_1.test)("NUEVO: Oracle rechaza solo actionButtons sin heading/sections", async () => {
        const logs = [];
        const originalLog = console.log;
        console.log = (...args) => {
            logs.push(args.join(" "));
        ...
        try {
            ...
            (0, test_1.expect)(detailOpened).toBe(false);
            (0, test_1.expect)(strongDetailSignal).toBe(false); // No strong signal (buttons don't count)
            (0, test_1.expect)(actionButtonsVisible).toBe(true); // Buttons present but not enough
            if (!detailOpened) {
                const reason = !productNameVisible ? "product_name_not_visible" :
                ...
                console.log(`[detail-oracle] opened=false reason=${reason}`);
                console.log(`[detail-screenshot] skipped reason=detail_not_opened`);
                (0, test_1.expect)(reason).toBe("insufficient_detail_signals");
            }
            const status = detailOpened ? "Exitoso" : "Fallido";
            (0, test_1.expect)(status).toBe("Fallido");
    ...
    (0, test_1.test)("Préstamo ordinal pasa solo si selectedCandidate == Préstamo Personal", async () => {
        const logs = [];
        const originalLog = console.log;
        console.log = (...args) => {
            logs.push(args.join(" "));
        ...
        try {
            ...
            console.log(`[ordinal-selection] expectedTarget="${detailTarget}" ` +
                `selectedCandidate="${selectedCandidateText}" match=${candidateMatches}`);
            (0, test_1.expect)(candidateMatches).toBe(true);
            if (candidateMatches) {
        ...
        // Verify logs
        const matchLog = logs.find(l => l.includes("[ordinal-selection]") &&
            l.includes("match=true"));
        (0, test_1.expect)(matchLog).toBeDefined();
        const verifiedLog = logs.find(l => l.includes("candidate match verified ✓"));
        (0, test_1.expect)(verifiedLog).toBeDefined();
    });
    (0, test_1.test)("Depósitos Dólares falla si selectedCandidate == Pesos", async () => {
        const logs = [];
        const originalLog = console.log;
        console.log = (...args) => {
            logs.push(args.join(" "));
        ...
        try {
            ...
            (0, test_1.expect)(candidateMatches).toBe(false);
            if (!candidateMatches) {
        ...
        // Verify logs
        const mismatchLog = logs.find(l => l.includes("[ordinal-selection]") &&
            l.includes("match=false"));
        (0, test_1.expect)(mismatchLog).toBeDefined();
        const detectedLog = logs.find(l => l.includes("[ordinal-selection] MISMATCH detected!"));
        (0, test_1.expect)(detectedLog).toBeDefined();
    });
    (0, test_1.test)("detailEvidence no puede capturarse si detailOpened=false", async () => {
        const logs = [];
        const originalLog = console.log;
        console.log = (...args) => {
            logs.push(args.join(" "));
        ...
        try {
            ...
            if (!detailOpened) {
                ...
                (0, test_1.expect)(detailEvidence.captured).toBe(false);
            }
        ...
        (0, test_1.expect)(setLog).toBeDefined();
    });
    (0, test_1.test)("El DOCX usa displayStepIndex correcto: step-004-detail-loaded", async () => {
        ...
        (0, test_1.expect)(screenshotFilename).toBe("step-004-detail_loaded-visa.png");
        // DOCX should reference display step 4, not runtime index 3
        const docxStepReference = `Paso ${displayIndex}`;
        (0, test_1.expect)(docxStepReference).toBe("Paso 4");
        ...
        (0, test_1.expect)(stepRecord.index).toBe(4);
        (0, test_1.expect)(stepRecord.screenshotPath).toContain("step-004");
        console.log("✓ Step record contains correct display index for DOCX insertion");
    });
    (0, test_1.test)("El caso no puede quedar Exitoso si falta la pantalla final de detalle", async () => {
        const logs = [];
        const originalLog = console.log;
        console.log = (...args) => {
            logs.push(args.join(" "));
        ...
        try {
            ...
            console.log(`[evidence-gate] scenario=test detailScreenshotRequired=true found=${detailFound} ` +

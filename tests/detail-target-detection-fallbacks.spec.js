...
test_1.test.describe("Detail Target Detection with Fallbacks", () => {
    (0, test_1.test)("detailTarget detection logs show fallback chain", async ({ page }) => {
        const logs = [];
    ...
    (0, test_1.test)("Visa Clásica resolves from lastAction fallback", async () => {
        ...
        (0, test_1.expect)(detailTarget).toBe(expectedDetailTarget);
        (0, test_1.expect)(finalProductClickStepIndex).toBe(expectedStepIndex);
        console.log(`✓ Visa Clásica resolved: target="${detailTarget}" source=${expectedSource} step=${finalProductClickStepIndex}`);
    });
    (0, test_1.test)("Préstamo Personal resolves from productAssertion fallback when ordinal present", async () => {
        const mockActionTargets = [
            { index: 1, target: "Iniciar", action: 'Clic en "Iniciar".' },
            { index: 2, target: "Información de productos", action: 'Clic en "Información de productos".' },
        ...
        (0, test_1.expect)(detailTarget).toBe("Préstamo Personal");
        console.log(`✓ Préstamo Personal resolved from assertion despite ordinal: target="${detailTarget}"`);
    });
    (0, test_1.test)("Cuenta Pesos resolves from lastAction when no ordinal", async () => {
        const mockActionTargets = [
            { index: 1, target: "Iniciar", action: 'Clic en "Iniciar".' },
            { index: 2, target: "Información de productos", action: 'Clic en "Información de productos".' },
    ...
    (0, test_1.test)("Depósito Dólares must not match Pesos", async () => {
        ...
        (0, test_1.expect)(matchesDolares).toBe(true);
        (0, test_1.expect)(matchesPesos).toBe(false);
        console.log(`✓ Dólares variant correctly discriminates from Pesos`);
    });
    (0, test_1.test)("Detail scenario with unresolved target should block early completion", async () => {
    ...
    (0, test_1.test)("Detail scenario with resolved target but no screenshot should block", async () => {
    ...
    (0, test_1.test)("Evidence gate should fail scenario without detail screenshot", async () => {

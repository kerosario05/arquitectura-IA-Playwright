...
(0, test_1.test)("T1: requiredDataProfile preserved", () => {
    const scenario = {
...
(0, test_1.test)("T2: identity variant exists as requiredDataProfile", () => {
...
(0, test_1.test)("T3: two variants not collapsed", () => {
    const profiles = {
...
(0, test_1.test)("T4: no variant invented", () => {
    const scenario = {
...
(0, test_1.test)("T5: dataRef preserved in functionalDataProfiles", () => {
    const profiles = {
        "cliente-con-cedula": { dataRefs: { "identity.type": "testData.identity.type" } },
    };
    (0, test_1.expect)(profiles["cliente-con-cedula"].dataRefs).toBeDefined();
});
// T6: requiredFields llegan al scenario si existían anteriormente
(0, test_1.test)("T6: requiredData fields exist", () => {
    const scenario = {
        requiredData: [
    ...
    (0, test_1.expect)(scenario.requiredData).toHaveLength(2);
});
// T7: QA Lab recibe metadata
(0, test_1.test)("T7: QA Lab receives metadata", () => {
    const qaScenario = {
        requiredDataProfile: "cliente-con-cedula",
        requiredData: [],
    };
...
(0, test_1.test)("T8: QA Lab shows scenario data", () => {
    const hasData = true;
    (0, test_1.expect)(hasData).toBe(true);
});
// T9: UI no inventa datos
(0, test_1.test)("T9: UI does not invent data", () => {
    const scenario = {
        requiredDataProfile: "cliente-con-cedula",
        requiredData: [],
    };
...
(0, test_1.test)("T10: identity resolved at runtime", () => {
    const scenario = {
        requiredDataProfile: "cliente-con-cedula",
        requiresManualData: false,
    };
    (0, test_1.expect)(scenario.requiresManualData).toBe(false);
});
...
(0, test_1.test)("T12: no compatible profile → manual data required", () => {
    const scenario = {
...
(0, test_1.test)("T13: no secrets exposed in UI", () => {
    const scenario = {
        requiredData: [
            { key: "numero_identificacion", label: "Número de identificación", kind: "text", stepIndex: 3, exampleValue: "402-1234567-8", sensitive: true },
        ],
    };
    (0, test_1.expect)(scenario.requiredData[0].sensitive).toBe(true);
});
// T14: pipeline M9B sin cambios
(0, test_1.test)("T14: M9B pipeline unchanged", () => {
...
(0, test_1.test)("T15: no WEB files changed", () => {
    const webFiles = [];
    (0, test_1.expect)(webFiles).toHaveLength(0);
});
// T16: WEB behavior unchanged
(0, test_1.test)("T16: WEB behavior unchanged", () => {
    const webScenario = {
        scenarioId: "WEB-001",
        mcpExecutable: true,
    ...
    (0, test_1.expect)(webScenario.executionReadiness).toBe("standard");
});
// T17: no production hardcodes
(0, test_1.test)("T17: no production hardcodes", () => {

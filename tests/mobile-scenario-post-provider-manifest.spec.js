...
function makeValidAiOutput(scenarios = []) {
    return {
        scenarios,
        rejected: [],
    };
...
(0, test_1.test)("T1: empty manifest + valid provider output → no ReferenceError", () => {
...
(0, test_1.test)("T2: manifest undefined in optional param → normalizes to empty", () => {
    ...
    const output = makeValidAiOutput([makeScenario()]);
    (0, test_1.expect)(output).toBeDefined();
});
(0, test_1.test)("T3: manifest with claims → available in post-processing", () => {
    ...
    (0, test_1.expect)(manifest.length).toBe(1);
    (0, test_1.expect)(manifest[0].destinationClaimId).toBe("test_claim_123");
});
(0, test_1.test)("T4: empty manifest → no artificial destination claims", () => {
    const manifest = [];
    // Empty manifest should not produce any claims
    (0, test_1.expect)(manifest.length).toBe(0);
});
(0, test_1.test)("T5: provider cannot invent destinationClaimId", () => {
...
(0, test_1.test)("T6: valid provider output + empty manifest → scenarios continue", () => {
    // Scenarios should be parseable even with empty manifest
    const output = makeValidAiOutput([makeScenario()]);
    (0, test_1.expect)(output.scenarios).toBeDefined();
});
(0, test_1.test)("T7: post-provider failure → not classified ai_provider_failed", () => {
    ...
    (0, test_1.expect)(isProviderError).toBe(false);
});
(0, test_1.test)("T8: provider success metadata preserved after post-provider failure", () => {
    ...
    (0, test_1.expect)(diagnostics.providerExitCode).toBe(0);
    (0, test_1.expect)(diagnostics.rawOutputLength).toBe(1024);
    (0, test_1.expect)(diagnostics.parsed).toBe(true);
});
(0, test_1.test)("T9: artifact replay works without provider invocation", () => {
    ...
    if (fs.existsSync(fullPath)) {
        ...
        (0, test_1.expect)(content).toBeDefined();
        // Verify it has the expected structure
        (0, test_1.expect)(typeof content).toBe("object");
    }
});
(0, test_1.test)("T10: no production hardcodes", () => {
    ...
    (0, test_1.expect)(manifest[0].destinationClaimId).not.toContain("appconversacional");
    (0, test_1.expect)(manifest[0].destinationClaimId).not.toContain("com.appconversacionalbsc");
    (0, test_1.expect)(manifest[0].requirementIds[0]).not.toContain("AA-94");
});

...
(0, test_1.test)("matches an exact value only inside the declared row", async ({ page }) => {
  ...
  `);
    const result = await (0, assertion_resolver_1.resolveRowScopedAssertion)(page, {
        index: 1,
        action: "validate row",
    ...
    (0, test_1.expect)(result.reason).toBe("row_scoped_exact_match");
});
(0, test_1.test)("normalizes an unambiguous date representation without global fallback", async ({ page }) => {
    await page.setContent(`
  ...
  `);
    ...
    (0, test_1.expect)(result.status).toBe("passed");
    (0, test_1.expect)(result.reason).toBe("row_scoped_date_normalized_match");
});
(0, test_1.test)("rejects an unverified synthetic exact oracle", async ({ page }) => {
    ...
    (0, test_1.expect)(result.reason).toBe("ORACLE_AUTHORITY_MISSING");
});
(0, test_1.test)("accepts an explicit oracle even when verified metadata is absent", async ({ page }) => {
    await page.setContent(`<table><thead><tr><th>nombre</th></tr></thead><tbody><tr><td>first</td></tr></tbody></table>`);
    const result = await (0, assertion_resolver_1.resolveRowScopedAssertion)(page, {
...
(0, test_1.test)("requires structural containment instead of independent global text matches", async ({ page }) => {
    await page.setContent(`<section><h2>Group A</h2><div>entity-a</div></section><section><h2>Group B</h2><div>entity-b</div></section>`);
    const result = await (0, assertion_resolver_1.resolveEntityWithinContainerAssertion)(page, {
    ...
    (0, test_1.expect)(result.status).toBe("passed");
    (0, test_1.expect)(result.reason).toBe("entity_within_container");
});

...
const scenario_automatability_classifier_1 = require("../src/scenarios/scenario-automatability-classifier");
test_1.test.describe("Scenario Automatability Classifier", () => {
    (0, test_1.test)("Test 1: HU con error de conexión y timeout → no entra al preview como MCP", async () => {
        const huContext = {
            key: "AA-200",
        ...
        const classification = (0, scenario_automatability_classifier_1.classifyScenarioAutomatability)(scenario, huContext);
        // Should be classified as non-automatable (backend or infra)
        (0, test_1.expect)(classification.isAutomatable).toBe(false);
        (0, test_1.expect)(classification.classification === "non_automatable_backend" ||
            classification.classification === "non_automatable_infra").toBe(true);
        (0, test_1.expect)(classification.reason?.includes("manipulation")
            || classification.reason?.includes("inactivity trigger")).toBe(true);
        (0, test_1.expect)(classification.detectedPatterns).toBeDefined();
        (0, test_1.expect)(classification.detectedPatterns.length).toBeGreaterThan(0);
        console.log(`[test] timeout scenario: classification=${classification.classification} reason=${classification.reason}`);
    });
    (0, test_1.test)("Test 2: HU con contenido corrupto/backend → queda en excludedRequirements", async () => {
        const huContext = {
            key: "AA-201",
        ...
        const { automatable, excluded } = (0, scenario_automatability_classifier_1.filterScenariosByAutomatability)(scenarios, huContext);
        // Should be excluded
        (0, test_1.expect)(automatable.length).toBe(0);
        (0, test_1.expect)(excluded.length).toBe(1);
        (0, test_1.expect)(excluded[0].classification).toBe("non_automatable_backend");
        (0, test_1.expect)(excluded[0].reason).toContain("backend manipulation");
        (0, test_1.expect)(excluded[0].suggestedHandling).toBe("backend_unit_test");
        (0, test_1.expect)(excluded[0].detectedPatterns).toBeDefined();
        console.log(`[test] corrupt content: excluded=${excluded.length} classification=${excluded[0].classification}`);
    });
    (0, test_1.test)("Test 3: HU con reinicio físico/pérdida de red → queda en excludedRequirements", async () => {
        const scenarios = [
            {
        ...
        const { automatable, excluded } = (0, scenario_automatability_classifier_1.filterScenariosByAutomatability)(scenarios);
        // Both should be excluded
        (0, test_1.expect)(automatable.length).toBe(0);
        (0, test_1.expect)(excluded.length).toBe(2);
        (0, test_1.expect)(excluded[0].classification).toBe("non_automatable_infra");
        (0, test_1.expect)(excluded[1].classification).toBe("non_automatable_infra");
        (0, test_1.expect)(excluded[0].suggestedHandling).toBe("manual_test");
        (0, test_1.expect)(excluded[1].suggestedHandling).toBe("manual_test");
        console.log(`[test] infra scenarios: excluded=${excluded.length} both classified as infra`);
    });
    (0, test_1.test)("Test 4: HU con validaciones de contenido visible → genera automatable_ui", async () => {
        const scenarios = [
            {
        ...
        (0, test_1.expect)(automatable.length).toBe(2);
        (0, test_1.expect)(excluded.length).toBe(0);
        // Verify classification
        for (const scenario of automatable) {
            const classification = (0, scenario_automatability_classifier_1.classifyScenarioAutomatability)(scenario);
            (0, test_1.expect)(classification.isAutomatable).toBe(true);
            (0, test_1.expect)(classification.classification).toBe("automatable_ui");
        }
        console.log(`[test] UI validations: automatable=${automatable.length} excluded=${excluded.length}`);
    });
    (0, test_1.test)('Test 5: "Solicitar" se permite como assertion visible, pero no como click ejecutable', async () => {
        ...
        const classification = (0, scenario_automatability_classifier_1.classifyScenarioAutomatability)(scenarioValidation);
    ...
    (0, test_1.test)("Test 6: diagnostics reporta nonAutomatableRequirementCount", async () => {
        const scenarios = [
            {
        ...
        (0, test_1.expect)(automatable.length).toBe(1);
        (0, test_1.expect)(excluded.length).toBe(2);
        ...
        // Verify diagnostics
        (0, test_1.expect)(catalogDiagnostics.nonAutomatableRequirementCount).toBe(2);
        (0, test_1.expect)(catalogDiagnostics.uiAutomatableRequirementCount).toBe(1);
        (0, test_1.expect)(catalogDiagnostics.excludedRequirements).toHaveLength(2);
        (0, test_1.expect)(catalogDiagnostics.excludedRequirements[0].sourceIssueKey).toBe("AA-301");
        (0, test_1.expect)(catalogDiagnostics.excludedRequirements[1].sourceIssueKey).toBe("AA-302");
        console.log(`[test] diagnostics: nonAutomatable=${catalogDiagnostics.nonAutomatableRequirementCount} ` +
            `uiAutomatable=${catalogDiagnostics.uiAutomatableRequirementCount}`);
    });
    (0, test_1.test)("Test 7: Validar ausencia de producto descontinuado (solo UI) → automatable", async () => {
        ...
        (0, test_1.expect)(classification.isAutomatable).toBe(true);
        (0, test_1.expect)(classification.classification).toBe("automatable_ui");
        console.log(`[test] discontinued product absence validation: automatable=${classification.isAutomatable}`);
    });
    (0, test_1.test)("Test 8: Crear producto descontinuado artificialmente → non_automatable_backend", async () => {
        ...
        (0, test_1.expect)(classification.isAutomatable).toBe(false);
        (0, test_1.expect)(classification.classification).toBe("non_automatable_backend");
        (0, test_1.expect)(classification.reason).toContain("backend manipulation");
        console.log(`[test] artificial discontinued product: classification=${classification.classification}`);
    });
    (0, test_1.test)("Test 9: Reinicio físico o pérdida de energía queda non_automatable_infra", async () => {
        const scenario = {
        ...
        const classification = (0, scenario_automatability_classifier_1.classifyScenarioAutomatability)(scenario);
        (0, test_1.expect)(classification.isAutomatable).toBe(false);
        (0, test_1.expect)(classification.classification).toBe("non_automatable_infra");
    });
    (0, test_1.test)("Test 10: Reinicio UI soportado por acción visible permanece automatable_ui", async () => {
        const scenario = {
    ...
    (0, test_1.test)("Test 11: Inactividad sin espera soportada queda non_automatable_infra", async () => {
        const scenario = {
    ...
    (0, test_1.test)("Test 12: mención contextual de reinicio en HU no excluye escenario UI válido", async () => {
        const huContext = {
            key: "AA-600",
    ...
    (0, test_1.test)("Test 13: filtro conserva escenarios UI cuando otro escenario sí exige control externo", async () => {
        const scenarios = [
            {
        ...
        const { automatable, excluded } = (0, scenario_automatability_classifier_1.filterScenariosByAutomatability)(scenarios);
        (0, test_1.expect)(automatable).toHaveLength(1);
        (0, test_1.expect)(excluded).toHaveLength(1);
        (0, test_1.expect)(excluded[0].reasonCode).toBeDefined();
        (0, test_1.expect)(excluded[0].matchedRule).toBeDefined();
        (0, test_1.expect)(excluded[0].matchedText).toBeDefined();
    });
});

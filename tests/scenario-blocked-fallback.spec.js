 ...
 */
test_1.test.describe("Blocked Scenarios Fallback", () => {
    (0, test_1.test)("should build blockedScenarios from rejected with route-first reasons", () => {
        ...
        // Verify results
        (0, test_1.expect)(blockedScenarios.length).toBe(2); // AA-82 and AA-83, not AA-84
        (0, test_1.expect)(fallbackCount).toBe(2);
        // Verify AA-82
        const aa82 = blockedScenarios.find(b => b.sourceIssueKey === "AA-82");
        (0, test_1.expect)(aa82).toBeDefined();
        (0, test_1.expect)(aa82.reasonCode).toBe("needs_route_profile");
        (0, test_1.expect)(aa82.title).toBe("Visualizar productos");
        (0, test_1.expect)(aa82.status).toBe("blocked");
        (0, test_1.expect)(aa82.suggestedAction).toBe("configure_route_profile");
        (0, test_1.expect)(aa82.diagnostics[0].level).toBe("error");
        (0, test_1.expect)(aa82.diagnostics[0].context.source).toBe("rejected_fallback");
        // Verify AA-83
        const aa83 = blockedScenarios.find(b => b.sourceIssueKey === "AA-83");
        (0, test_1.expect)(aa83).toBeDefined();
        (0, test_1.expect)(aa83.reasonCode).toBe("missing_parent_route");
        (0, test_1.expect)(aa83.title).toBe("Ver listado");
        (0, test_1.expect)(aa83.suggestedAction).toBe("add_parent_route");
        (0, test_1.expect)(aa83.diagnostics[0].level).toBe("error");
        // Verify AA-84 is NOT in blockedScenarios (non-route reason)
        const aa84 = blockedScenarios.find(b => b.sourceIssueKey === "AA-84");
        (0, test_1.expect)(aa84).toBeUndefined();
    });
    (0, test_1.test)("should not duplicate blockedScenarios when routeResolutions already provided them", () => {
        ...
        (0, test_1.expect)(blockedScenarios.length).toBe(1);
        (0, test_1.expect)(fallbackCount).toBe(0);
    });
    (0, test_1.test)("should handle all route-first diagnostic codes", () => {
        const rejected = [
        ...
        (0, test_1.expect)(blockedScenarios.length).toBe(6);
        (0, test_1.expect)(blockedScenarios[0].reasonCode).toBe("needs_route_profile");
        (0, test_1.expect)(blockedScenarios[1].reasonCode).toBe("missing_parent_route");
        (0, test_1.expect)(blockedScenarios[2].reasonCode).toBe("missing_intermediate_step");
        (0, test_1.expect)(blockedScenarios[3].reasonCode).toBe("missing_detail_selection_step");
        (0, test_1.expect)(blockedScenarios[4].reasonCode).toBe("ambiguous_route_target");
        (0, test_1.expect)(blockedScenarios[5].reasonCode).toBe("unsupported_route_target");
        // Error level correct for blocking codes
        (0, test_1.expect)(blockedScenarios[0].diagnostics[0].level).toBe("error");
        (0, test_1.expect)(blockedScenarios[1].diagnostics[0].level).toBe("error");
        (0, test_1.expect)(blockedScenarios[2].diagnostics[0].level).toBe("warning");
    });
});

...
test_1.test.describe("Scenario Coverage Corrections (AA-82 fixes)", () => {
    (0, test_1.test)("Test 1: HU with 14 aligned products + variants → dynamicLimit > 4", async () => {
        // Mock HU mentioning multiple categories and explicit variants
        const issues = [
            {
                key: "AA-82",
                summary: "Visualizar categorías oficiales del kiosco",
                description: "Como usuario quiero visualizar todas las categorías de productos: " +
        ...
        const dynamicLimit = (0, mcp_scenario_prompt_builder_1.calculateDynamicScenarioLimit)(issues, routeProfile, alignedCategories);
        ...
        (0, test_1.expect)(dynamicLimit).toBeGreaterThan(4);
        (0, test_1.expect)(dynamicLimit).toBeGreaterThanOrEqual(7);
        console.log(`[test] dynamicLimit=${dynamicLimit} for 4 categories + 3 variants`);
    });
    (0, test_1.test)("Test 2: Representative mode generates seeds per uncovered category", async () => {
        ...
        const seeds = (0, scenario_deterministic_seeds_1.generateDeterministicSeeds)(routeProfile, existingScenarios, issueContext, "kiosko", "representative", { automationType: "ui_with_auth_gate", setupStrategy: "auth_gate" });
        ...
        (0, test_1.expect)(seedTitles.some((t) => t.includes("Tarjeta Visa Gold"))).toBe(true);
        (0, test_1.expect)(seedTitles.some((t) => t.includes("Depósito a Plazo en Pesos"))).toBe(true);
        (0, test_1.expect)(seedTitles.some((t) => t.includes("Préstamo Personal"))).toBe(true);
        console.log(`[test] representative mode: ${seeds.length} seeds generated for uncovered categories`);
    });
    (0, test_1.test)("Test 3: Exhaustive mode generates 1 seed per uncovered product", async () => {
        ...
        const seeds = (0, scenario_deterministic_seeds_1.generateDeterministicSeeds)(routeProfile, existingScenarios, issueContext, "kiosko", "exhaustive");
        ...
        (0, test_1.expect)(seeds.length).toBe(2);
        const seedTitles = seeds.map((s) => s.title);
        (0, test_1.expect)(seedTitles.some((t) => t.includes("Tarjeta Visa Gold"))).toBe(true);
        (0, test_1.expect)(seedTitles.some((t) => t.includes("Tarjeta Mastercard Platinum"))).toBe(true);
        console.log(`[test] exhaustive mode: ${seeds.length} seeds generated for uncovered products`);
    });
    (0, test_1.test)("Test 4: Seeds use sourceIssueKey from HU and generationSource=deterministic_seed", async () => {
        const targetPaths = {
            "Tarjeta Visa Gold": {
                target: "Tarjeta Visa Gold",
                requiredIntermediates: ["Información de productos", "Tarjetas de Crédito"],
        ...
        const existingScenarios = []; // No coverage, seed needed
        const seeds = (0, scenario_deterministic_seeds_1.generateDeterministicSeeds)(routeProfile, existingScenarios, issueContext, "kiosko", "representative", { automationType: "ui_with_auth_gate", setupStrategy: "auth_gate" });
        (0, test_1.expect)(seeds.length).toBe(1);
        (0, test_1.expect)(seeds[0].sourceIssueKey).toBe("AA-82"); // From HU, NOT "SEED"
        (0, test_1.expect)(seeds[0].generationSource).toBe("deterministic_seed"); // Traceability
        (0, test_1.expect)(seeds[0].mcpExecutable).toBe(true);
        (0, test_1.expect)(seeds[0].steps.length).toBeGreaterThan(0);
        (0, test_1.expect)(seeds[0].automationType).toBe("ui_with_auth_gate"); // Valid type
        (0, test_1.expect)(seeds[0].setupStrategy).toBe("auth_gate"); // Valid strategy
        console.log(`[test] seed uses sourceIssueKey=${seeds[0].sourceIssueKey} generationSource=${seeds[0].generationSource}`);
    });
    (0, test_1.test)("Test 5: Validator allows 'Validar que el botón Solicitar esté visible'", async () => {
        const scenario = {
            sourceIssueKey: "AA-82",
        ...
        const validation = (0, scenario_validator_1.validateScenario)(scenario, routeProfile);
        // Should be valid - validation context is allowed for sensitive actions
        (0, test_1.expect)(validation.valid).toBe(true);
        (0, test_1.expect)(validation.errors.length).toBe(0);
        console.log(`[test] validator allows "Validar que el botón Solicitar esté visible"`);
    });
    (0, test_1.test)("Test 6: Validator rejects 'Clic en Solicitar'", async () => {
        const scenario = {
            sourceIssueKey: "AA-82",
        ...
        (0, test_1.expect)(validation.errors.some((e) => e.includes("sensitive_action_not_allowed"))).toBe(true);
        console.log(`[test] validator rejects "Clic en Solicitar"`);
    });
    (0, test_1.test)("Test 7: Validator allows 'Validar que se muestre Solicitar' (validation context)", async () => {
        const scenario = {
            sourceIssueKey: "AA-82",
    ...
    (0, test_1.test)("Test 8: Dynamic limit calculation with explicit variants detection", async () => {
        // HU explicitly mentions Pesos, Dólares, Euros
        const issues = [
            {
                key: "AA-82",
                summary: "Visualizar productos en múltiples monedas",
                description: "Como usuario quiero visualizar productos en Pesos, Dólares y Euros. " +
        ...
        (0, test_1.expect)(dynamicLimit).toBeGreaterThanOrEqual(6);
        console.log(`[test] dynamicLimit=${dynamicLimit} with 3 categories + 3 explicit variants`);
    });
    (0, test_1.test)("Test 9: Product card clickable genera click + validaciones detalle", async () => {
        const targetPaths = {
            "Tarjeta Visa Gold": {
                target: "Tarjeta Visa Gold",
                requiredIntermediates: ["Información de productos", "Tarjetas de Crédito"],
        ...
        const seeds = (0, scenario_deterministic_seeds_1.generateDeterministicSeeds)(routeProfile, [], issueContext, "kiosko", "representative", { automationType: "ui_with_auth_gate", setupStrategy: "auth_gate" });
        (0, test_1.expect)(seeds.length).toBe(1);
        const seed = seeds[0];
        // Debe tener click en target
        (0, test_1.expect)(seed.steps.some((s) => s.includes('Clic en "Tarjeta Visa Gold"'))).toBe(true);
        // Debe validar target después del click
        (0, test_1.expect)(seed.steps.some((s) => s.includes('Validar que se muestre "Tarjeta Visa Gold"'))).toBe(true);
        // Debe validar secciones
        (0, test_1.expect)(seed.steps.some((s) => s.includes('"Beneficios"'))).toBe(true);
        (0, test_1.expect)(seed.steps.some((s) => s.includes('"Detalles"'))).toBe(true);
        (0, test_1.expect)(seed.steps.some((s) => s.includes('"Requisitos"'))).toBe(true);
        // Debe validar botones (como visible, no click)
        (0, test_1.expect)(seed.steps.some((s) => s.includes('Validar que el botón "Solicitar" esté visible'))).toBe(true);
        (0, test_1.expect)(seed.steps.some((s) => s.includes('Validar que el botón "Volver" esté visible'))).toBe(true);
        // NO debe hacer click en Solicitar
        (0, test_1.expect)(seed.steps.some((s) => s.includes('Clic en "Solicitar"'))).toBe(false);
        console.log(`[test] product_card clickable generates click + detail validations`);
    });
    (0, test_1.test)("Test 10: Representative mode con variantes genera múltiples seeds", async () => {
        const targetPaths = {
            "Cuenta en Pesos": {
        ...
        (0, test_1.expect)(seeds.length).toBe(3);
        const seedTitles = seeds.map((s) => s.title);
        (0, test_1.expect)(seedTitles.some((t) => t.includes("Pesos"))).toBe(true);
        (0, test_1.expect)(seedTitles.some((t) => t.includes("Dólares"))).toBe(true);
        (0, test_1.expect)(seedTitles.some((t) => t.includes("Euros"))).toBe(true);
        console.log(`[test] representative mode with variants generates ${seeds.length} seeds for 3 variants`);
    });
    (0, test_1.test)("Test 11: Validator acepta secciones genéricas", async () => {
        const scenario = {
            sourceIssueKey: "AA-82",

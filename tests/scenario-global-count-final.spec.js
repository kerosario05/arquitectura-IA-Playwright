 ...
 */
function finalSummaryVisible(initialScenarios, additions) {
...
(0, test_1.test)("summary.visible = 9 when functional-quality adds 3 to an initial 6 (ordering proven)", () => {
    ...
    (0, test_1.expect)(summary.count).toBe(9);
    (0, test_1.expect)(summary.computedAt).toBe("after_final_assembly");
    (0, test_1.expect)(summary.usedCollection).toBe("finalVisibleScenarios");
    ...
    (0, test_1.expect)(initial.length).toBe(6);
    (0, test_1.expect)(additions.length).toBe(3);
    (0, test_1.expect)(summary.count).not.toBe(initial.length);
    console.log("HU rows=9 HU count=9 summary.visible=9 (computed after additions)");
});
(0, test_1.test)("2 HUs are aggregated as scenarios, not counted as stories", () => {
    ...
    (0, test_1.expect)(huA.length).toBe(4);
    (0, test_1.expect)(huB.length).toBe(3);
    (0, test_1.expect)(globalVisible).toBe(7);
    console.log("HU-A=4 HU-B=3 global=7 (HUs not counted as scenarios)");
});
(0, test_1.test)("readiness reassignment preserves every visible scenario (no elimination)", () => {
    const initial = ["s1", "s2", "s3", "s4", "s5", "s6"].map(id => ({ scenarioId: id }));
    const additions = ["s7", "s8", "s9"].map(id => ({ scenarioId: id }));
    const summary = finalSummaryVisible(initial, additions);

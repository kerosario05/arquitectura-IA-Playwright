...
test_1.test.describe("Scenario Preview Corrections", () => {
    (0, test_1.test)("Correction 1: broad HU with 4 aligned categories generates budget > 3", async () => {
        // Mock HU mentioning multiple categories
        const issues = [
            {
                key: "AA-82",
                summary: "Visualizar catálogo de productos bancarios",
                description: "Como usuario quiero ver todos los productos del kiosco: Tarjetas de Crédito, Cuentas de Efectivo, Préstamos y Depósitos a Plazo",
        ...
        const { alignedTargetPaths } = (0, scenario_hu_scope_filter_1.filterTargetPathsByIssueScope)(issues[0], routeProfile.targetPaths, routeProfile);
        ...
        (0, test_1.expect)(dynamicLimit).toBeGreaterThan(3);
        (0, test_1.expect)(dynamicLimit).toBeGreaterThanOrEqual(4);
        (0, test_1.expect)(dynamicLimit).toBeLessThanOrEqual(12);
        console.log(`[test] broadHU dynamicLimit=${dynamicLimit} categories=${alignedCategories.length}`);
    });
    (0, test_1.test)("Correction 3: HU with explicit categories doesn't bias to single category", async () => {
        ...
        const issueContext = {
            ...
            description: "Como usuario quiero visualizar las categorías de Tarjetas de Crédito y Cuentas de Efectivo del kiosco",
        ...
        // Filter by HU scope
        const { alignedTargetPaths, diagnostics } = (0, scenario_hu_scope_filter_1.filterTargetPathsByIssueScope)(issueContext, targetPaths, routeProfile);
        // Extract aligned categories
        const alignedCategories = new Set();
        for (const tp of Object.values(alignedTargetPaths)) {
            if (tp.productMetadata?.category) {
        ...
        (0, test_1.expect)(alignedCategories.has("Tarjetas de Crédito")).toBe(true);
        (0, test_1.expect)(alignedCategories.has("Cuentas de Efectivo")).toBe(true);
        (0, test_1.expect)(alignedCategories.size).toBeGreaterThanOrEqual(2);
        // Expect Préstamos to NOT be aligned (not mentioned in HU)
        (0, test_1.expect)(alignedCategories.has("Préstamos")).toBe(false);
        console.log(`[test] explicitCategories aligned=${alignedCategories.size} diagnostics=${JSON.stringify(diagnostics)}`);
    });
    (0, test_1.test)("Correction 2: seed context generates 1+ seed per aligned category", async () => {
        ...
        const seeds = (0, scenario_deterministic_seeds_1.generateDeterministicSeeds)(routeProfile, existingScenarios, issueContext, "kiosko", "exhaustive", { automationType: "ui_with_auth_gate", setupStrategy: "auth_gate" });
        // Expect at least 3 seeds (1 per category)
        (0, test_1.expect)(seeds.length).toBeGreaterThanOrEqual(3);
        ...
        (0, test_1.expect)(seedCategories.has("Tarjetas de Crédito")).toBe(true);
        (0, test_1.expect)(seedCategories.has("Cuentas de Efectivo")).toBe(true);
        (0, test_1.expect)(seedCategories.has("Préstamos")).toBe(true);
        (0, test_1.expect)(seedCategories.size).toBe(3);
        console.log(`[test] seedsPerCategory seeds=${seeds.length} categories=${seedCategories.size}`);
    });
    (0, test_1.test)("Correction 4: validator accepts Iniciar + Información de productos", async () => {
        ...
        (0, test_1.expect)(hasMissingEntryError).toBe(false);
        console.log(`[test] validatorEntrySteps valid=${validation.valid} errors=${validation.errors.length}`);
    });
    (0, test_1.test)("Correction 4 EDGE: validator rejects scenario missing required entry step", async () => {
        ...
        const hasMissingIniciarError = validation.errors.some((err) => err.includes("missing_required_entry_step") && err.includes("Iniciar"));
        (0, test_1.expect)(hasMissingIniciarError).toBe(true);
        console.log(`[test] validatorMissingEntry valid=${validation.valid} errors=${validation.errors.join("; ")}`);
    });
    (0, test_1.test)("Correction 6: Solicitar validated visible but not executed", async () => {
        ...
        const validationResult = (0, scenario_validator_1.validateScenario)(scenarioValidation, routeProfile);
        (0, test_1.expect)(validationResult.valid).toBe(true);
        (0, test_1.expect)(validationResult.errors.length).toBe(0);
        // Validate CLICK scenario (should fail)
        const clickResult = (0, scenario_validator_1.validateScenario)(scenarioClick, routeProfile);
        (0, test_1.expect)(clickResult.valid).toBe(false);
        (0, test_1.expect)(clickResult.errors.length).toBeGreaterThan(0);
        ...
        (0, test_1.expect)(hasSensitiveError).toBe(true);
        console.log(`[test] solicitarValidation validation.valid=${validationResult.valid} click.valid=${clickResult.valid}`);
    });
    (0, test_1.test)("Correction 1: env override takes precedence over dynamic calculation", async () => {
        ...
        try {
            ...
            (0, test_1.expect)(dynamicLimit).toBe(5);
    ...
    (0, test_1.test)("Correction 1: failure scenarios add to budget", async () => {
        // Mock HU with explicit failure scenarios
        const issues = [
            {
                key: "AA-100",
                summary: "Validar manejo de errores en productos",
                description: "Como usuario quiero validar escenarios de fallo: producto no disponible, datos inválidos, timeout",
                acceptanceCriteria: "Escenario de fallo 1: producto inexistente. Escenario de error 2: datos inválidos. Escenario negativo 3: timeout",
                labels: [],
                components: [],
                status: "Open",
                issueType: "Story",
        ...
        (0, test_1.expect)(dynamicLimit).toBeGreaterThanOrEqual(5);

...
test_1.test.describe("Scenario Seed Fixes", () => {
    const mockIssue = {
        key: "AA-82",
    ...
    (0, test_1.test)("Fix 1: No duplica 'Información de productos' en steps", () => {
        const routeProfile = {
            name: "test-profile",
        ...
        const informacionCount = seed.steps.filter((s) => s.toLowerCase().includes("información de productos")).length;
    ...
    (0, test_1.test)("Fix 2: Préstamo Personal no queda agrupado en General", () => {
        const routeProfile = {
            name: "test-profile",
            entry: [{ visibleLabel: "Información de productos", businessLabel: "Products" }],
            aliases: {},
        ...
        (0, test_1.expect)(seeds.length).toBeGreaterThanOrEqual(2);
        ...
        (0, test_1.expect)(prestamoSeed).toBeDefined();
        (0, test_1.expect)(tarjetaSeed).toBeDefined();
    ...
    (0, test_1.test)("Fix 3: Representative incluye Préstamo Personal", () => {
        const routeProfile = {
            name: "test-profile",
            entry: [{ visibleLabel: "Información de productos", businessLabel: "Products" }],
            aliases: {},
        ...
        (0, test_1.expect)(hasPrestamoPersonal).toBe(true);
        (0, test_1.expect)(hasTarjeta).toBe(true);
        (0, test_1.expect)(hasCuenta).toBe(true);
        console.log(`Representative seeds (${seeds.length}):`);
        for (const seed of seeds) {
            console.log(`  - ${seed.title}`);
        }
    });
    (0, test_1.test)("Fix 4: Validator acepta botones visibles", () => {
        const scenario = {
    ...
    (0, test_1.test)("Fix 5: Validator acepta botones con mojibake", () => {
    ...
    (0, test_1.test)("Fix 6: Representative coverage completo para HU amplia", () => {
        const routeProfile = {
            name: "test-profile",
            entry: [{ visibleLabel: "Información de productos", businessLabel: "Products" }],
            aliases: {},
        ...
        const coveredProducts = expectedProducts.filter((product) => seeds.some((s) => s.title.includes(product)));
        ...
        (0, test_1.expect)(coveredProducts.length).toBe(expectedProducts.length);
    });
});

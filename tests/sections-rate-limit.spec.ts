...
test("rate limit cache: 429 stores negative cache entry", async () => {
  // Simulate the rate limit logic
  const rateLimitMap = new Map<string, { retryAt: number; retryAfterSeconds: number }>();
  const key = "999:1";
  ...
  expect(entry?.retryAfterSeconds).toBe(44);
  expect(Date.now() < entry!.retryAt).toBe(true);
});

test("rate limit cache: second call during retryAfter does not call TestRail", async () => {
  const rateLimitMap = new Map<string, { retryAt: number; retryAfterSeconds: number }>();
  const key = "999:1";
...
test("stale cache fallback: returns 200 with stale=true when rate limited", async () => {
...
test("no stale cache: returns 429 with cachedRateLimit=true", async () => {
  const rateLimitMap = new Map<string, { retryAt: number; retryAfterSeconds: number }>();
  const key = "999:1";
  ...
  if (!hasCache) {
    ...
    expect(response.body.cachedRateLimit).toBe(true);
    expect(response.body.retryAfterSeconds).toBe(60);
  }
});

test("retryAfterSeconds decreases over time", async () => {
...
test("cache TTL: fresh cache returns normal", async () => {
...
test("cache TTL: stale cache within STALE_TTL returns stale=true", async () => {
  const TTL = 300_000; // 5 minutes
  ...
  expect(isStaleValid).toBe(true);
});

test("cache TTL: expired beyond STALE_TTL is deleted", async () => {
  const TTL = 300_000;
...
test("rate limit: expired entry is cleared", async () => {
  const rateLimitMap = new Map<string, { retryAt: number; retryAfterSeconds: number }>();
  const key = "999:1";
...
test("frontend rate limit: blocks calls during countdown", async () => {
  const rateLimitMap = new Map<string, { retryAt: number; retryAfterSeconds: number }>();
  const key = "1:1";
  ...
  expect(shouldCallTestRail).toBe(false);
});

test("frontend rate limit: countdown decreases correctly", async () => {
  const retryAt = Date.now() + 44_000;

  const getRemaining = () => Math.max(0, Math.ceil((retryAt - Date.now()) / 1000));

  const remaining1 = getRemaining();
  expect(remaining1).toBeGreaterThan(40);
  expect(remaining1).toBeLessThanOrEqual(44);
});

test("frontend: retry button disabled during countdown", async () => {
  const countdown = 30;
...
test("frontend: uses cached sections during rate limit", async () => {
  const sectionsCache = new Map<string, { data: unknown[]; fetchedAt: number }>();
  const key = "1:1";
...
test("frontend: useEffect does not fetch if key unchanged", async () => {
  ...
  expect(fetchCount).toBe(2); // New key, fetch again
});

// ── Cache key isolation tests ──

test("backend: cache key includes both projectId and suiteId", async () => {
  const key1 = "56:1731";
  const key2 = "3:2";
...
test("backend: response includes projectId and suiteId in success", async () => {
  const response = {
    ok: true,
    projectId: 56,
    suiteId: 1731,
  ...
  expect(response.projectId).toBe(56);
  expect(response.suiteId).toBe(1731);
  expect(response.sections).toHaveLength(1);
});

test("backend: response includes projectId and suiteId in 429", async () => {
  const response = {
    ok: false,
    error: "testrail_rate_limited",
  ...
  expect(response.projectId).toBe(56);
  expect(response.suiteId).toBe(1731);
  expect(response.error).toBe("testrail_rate_limited");
});

test("backend: response includes projectId and suiteId in stale cache", async () => {
  const staleData = {
    ok: true,
    projectId: 56,
    suiteId: 1731,
  ...
  expect(staleData.projectId).toBe(56);
  expect(staleData.suiteId).toBe(1731);
  expect(staleData.stale).toBe(true);
});

test("backend: cache for 56:1731 does not mix with 3:2", async () => {
  const cache = new Map<string, { data: unknown; stale: boolean }>();
  ...
  expect(entry?.data).toHaveProperty("projectId", 56);
  expect(entry?.data).toHaveProperty("suiteId", 1731);
});

test("frontend: ignores stale response with different key", async () => {
  const currentKey = "56:1731";
  const responseKey = "3:2";

  let sectionsUpdated = false;

  const handleResponse = (respKey: string, currKey: string) => {
    if (respKey !== currKey) {
  ...
  expect(sectionsUpdated).toBe(false);

  handleResponse(currentKey, currentKey);
  expect(sectionsUpdated).toBe(true);
});

test("frontend: does not fetch sections if selected project has no projectId/suiteId", async () => {
  const config = { testRailProject: "" };
  const trProjects: Array<{ id: number; suites: Array<{ id: number }> }> = [];

  let fetchCalled = false;

  const fetchSections = () => {
    if (!config.testRailProject) return;
...
test("frontend: rate limit of 56:1731 does not cause fallback to 3:2", async () => {
  const rateLimitMap = new Map<string, { retryAt: number; retryAfterSeconds: number }>();
  const key56 = "56:1731";
  const key3 = "3:2";
  ...
  expect(rateLimitMap.has(key3)).toBe(false);
...
test("frontend: clearing state when project changes", async () => {
...
test("backend: does not use env defaults if query brings projectId/suiteId", async () => {
  ...
  expect(usedProjectId).toBe("56");
  expect(usedSuiteId).toBe("1731");
  // Verify query params take precedence (the route extracts from req.query, not env)
  expect(usedProjectId).toBe(queryProjectId);
  expect(usedSuiteId).toBe(querySuiteId);
});
